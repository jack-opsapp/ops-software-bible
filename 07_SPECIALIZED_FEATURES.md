...
The standalone OPS Decks FULL builder (`appSurface == .opsDecks`, `DeckCapabilities.full`) was rebuilt ground-up across P11-1…P11-8 (spec: `ops-decks-ios/docs/superpowers/specs/2026-07-05-ops-decks-p11-builder-redesign-design.md`, rev 3). The prior stacked-toolbars editor (`DeckDrawingEditorView`, `FramingControlsView`) is DELETED. Engines, models, `deck_designs` schema, sync, entitlements, and the LIGHT surfaces consumed by main OPS are untouched. The main OPS app's own DeckBuilder (`ops-ios/OPS/DeckBuilder/`) is also untouched — P11 copied its proven interaction core into DeckKit (provenance recorded per-commit as `ported-from: … @ 28440097`); converging ops-ios onto DeckKit is a separate, later initiative.
