...
**Prepared diagnostics (local, production approval pending, 2026-09-04).** `ops-web/src/lib/social/instagram-failure-diagnostics.ts`, `instagram-oauth-client.ts`, and `instagram-connection-service.ts` add source-defined stage labels, allowlisted local error codes, numeric HTTP/provider codes, and fixed response-shape labels. No tokens, authorization codes, state, emails, URLs, provider messages, stacks, or response bodies are logged. OAuth requests, scope checks, one-time-state semantics, encryption, account storage, public callback results, and publishing behavior remain unchanged. Local code commit: `25b752f0` in the Instagram release worktree. Focused verification passed 149 social/API tests and a targeted TypeScript check. After approved deployment, one fresh login is required to identify the cause; successful connection requires independent encrypted-row and displayed-username proof. Runbook: `ops-web/docs/social/instagram-operations.md`, OAuth completion incident section.
