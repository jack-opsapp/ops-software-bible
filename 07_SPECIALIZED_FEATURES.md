...
### Cloud Instagram editorial notifications (2026-09-05; local, not deployed)

New type `social_editorial` is written by service-only `notify_social_editorial(text,text)` in `20260905185527_create_social_editorial.sql`. A `prepared` run produces standard `INSTAGRAM DRAFT READY`; a `failed` run produces persistent `INSTAGRAM PREPARATION STOPPED`. Both link to `/admin/social#cloud-production` with `VIEW SOCIAL`. Recipient IDs use `SOCIAL_OPERATOR_*`, falling back to `PMF_OPERATOR_*`. The run's `notified_at` is acknowledged in the same transaction as insertion, under row locks; notification failures stay replayable and successful replays insert zero duplicates. Skipped editorial ideas remain in the run history without a failure alert. These new notifications are not live yet. See §22 and the cloud editorial runbook.
...
The fixed model is `gpt-5.6-sol`, with two bounded calls, no tools and no SDK retries. Each claim reserves US$0.50 from a monthly estimate allowance capped at US$20; hosting/storage are additional. One successful local canary produced four actual JPEGs for the published article “WORD OF MOUTH ISN'T A MARKETING PLAN,” with estimated model usage US$0.052068. A previous canary failed, so this is not the full testing bill. Local evidence is in `ops-web/docs/artifacts/social-editorial/`; the actual admin component was inspected as a static browser fixture. Exact migration behavior was exercised in disposable PostgreSQL, including concurrent claims, mode changes, recovery, grants and zero-row notification replay.

New migration: `ops-web/supabase/migrations/20260905185527_create_social_editorial.sql`; its bible copy is under `migrations/pending/` because it is not applied. Production migration, deployment and paid prepare activation still require approval. First publication remains separately unauthorized and unverified. This local feature does not reopen the completed OAuth repair or inherit its prior deployment permission. Canonical runbook: `ops-web/docs/social/cloud-editorial-operations.md`.
