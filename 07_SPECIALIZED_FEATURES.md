...
The sample is phone-first, with 44px minimum controls and a 52px primary action. Actions follow the content so they cannot mask the note/photo or receipt. Back preserves sample task truth; Restart resets it; versioned, exact-type session storage supports reload recovery. A neutral initialization and a recoverable error screen preserve native Exit/trial links.

**Visual refinement (2026-09-14):** `DemoExperience.tsx` and `demo.module.css` compose three distinct views around persistent job/task elements: the owner's scheduled task and assigned crew, the crew's photo-led field plan with address/time/note, and the same owner task with its completed status. The before-work photo remains explicitly labelled as a site reference. Public-surface Mohave display type, an Owner → Crew → Owner progress rail, and a stable desktop stage establish hierarchy; compact phone and short-desktop layouts keep the primary action accessible without overlaying useful content. A check appears only for completed status; the pending task has no inert checkbox affordance.
...
**Review readiness and viewport boundary (OPS-Web Phase C, 2026-07-25).** A review blueprint may use only live company data plus evidence confirmed in the current setup session; company knowledge can influence review only after the operator confirms it. Its commercial values must come from confirmed facts tied to the operator's questions—not from model-authored action payloads or general model knowledge. Customer prices, labor costs, minimum charges, and product tax treatment remain required inputs; taxable products use the company's existing default tax rate. If a required fact is missing, the session stays in the interview and asks only for that fact before producing a review. In the dashboard's fixed-height application frame, the transcript is the conversation scroll owner so the newest exchange and every review/approval control remain reachable without repositioning an ancestor container.

**Executable capability registry boundary (OPS-Web Phase C, 2026-08-06).** `ops-web/src/lib/ops-capabilities/registry.ts` is the runtime registry seam for released OPS tools and the capabilities exposed to Guided Catalog Setup. Every tool entry names its released runtime consumers, evidence paths, verified abilities, and one Phase C access state: `configure`, `discover_only`, or `unavailable`. `catalog-capability-manifest.ts` projects the registry into the model context with both the catalog-manifest revision and registry revision pinned to the durable session. `discover_only` tools are context that prevents false promises; they cannot drive a question, action, or claim of Phase C execution. A future OPS tool or OPS Spec buildout is not available to Phase C until its registry entry identifies an actual released consumer and explicitly grants `configure` access.

Deck Designer is registered as released but `discover_only`. Verified DeckKit behavior includes deck material quantities, vinyl cut plans, ordered area and cut waste, compatible offcut reuse, and vinyl order notes (`ops-decks-ios/Packages/DeckKit/Sources/DeckKit/Engine/EstimateGeneratorService.swift`; `ops-decks-ios/Packages/DeckKit/Sources/DeckKit/Engine/VinylCutListEngine.swift`). Phase C cannot configure or execute Deck Designer catalog bindings, geometry, roll/offcut inventory, or purchasing automation. The registry therefore gives Phase C accurate system awareness without presenting those abilities as available setup outcomes.

**Question narrowing boundary (OPS-Web Phase C, 2026-08-29).** A guided question persists only fact keys that remain unresolved after the turn's facts merge. `turn-service` re-resolves the model's question decision through `narrowGuidedQuestionToUnresolvedFacts` (question-policy) before writing `unresolved_questions` and the transcript message, so a partially answered decision renders the narrowed template sentence instead of repeating the answered one under a fresh model-minted id (bug 986009b0). A re-ask that confirms nothing repeats verbatim by design. The transcript identity/dedupe contract (`questionId + content`, version-blind) is unchanged.

Every available blueprint action has a strict typed payload contract that is supplied to the model as JSON Schema and revalidated after generation and again before commit begins. Cross-action references use unique logical client IDs; duplicate IDs and direct database UUIDs fail closed except for the existing-variant join that the compiler explicitly resolves by UUID. Unknown fields, wrong types, unresolved task-type/catalog-family references, and unavailable actions fail closed before any live write. Guided Setup configures only whether a product is taxable; it uses the company's existing default tax rate and cannot create or change tax rates. Quote unit-display behavior is not a Guided Setup capability. Pricing questions ask only for the commercial facts still missing.

An explicitly stale, non-committing active session is refreshed on resume. Confirmed facts and the transcript are preserved; a still-supported question is regenerated from the current server-owned template. Removed questions receive a safe free-text continuation. If the refresh changes the question contract, queued input for the old question is marked removed and its processed revision is advanced; Phase C never interprets it as an answer to the replacement question. Stale review/approval state is cleared before the session can generate or commit a new plan. This prevents sessions created under an older registry from bypassing the current capability boundary.
...
**Designer interaction performance contract (2026-08-07).** Expensive geometry
derivation now crosses one exact snapshot boundary instead of running from
SwiftUI body/readout access. `DeckDrawingData` and each `DeckLevel` retain a
runtime-only cache keyed only by ordered vertex id/position and edge endpoint
ids. A geometry change recomputes vertex lookup, edge segments/adjacency,
closure/order, detected faces, canonical surface ids, area, and perimeter once;
label, material, assignment, dimension, and other metadata changes reuse that
snapshot. Multi-level drawings retain each unchanged level's snapshot when a
different level moves. These caches are excluded from Codable, so
`drawing_data` and sync payload shape are unchanged.

Direct vertex and selection moves publish an ephemeral overlay at the native
display cadence through a one-shot `CADisplayLink`. Raw touch callbacks collapse
to the newest frame input; snapping resolves once for that frame against indexed
committed geometry. The committed drawing, undo snapshot, topology, persistence,
sync, and 3D scene stay unchanged while the finger is down. Lift applies all
positions and scale/manual-dimension rules in one indexed O(V+E) pass, then
defers reconciliation, encoding, and the SwiftData save outside the
gesture callback. Active-session saves are local-only. Actual exit calls
`flushBeforeExit()`; background interruption calls `flushLocallyForInterruption()`
and does not queue cloud work. One canonical JSON encoding is shared by the
`DeckDesign` cache and the exit-boundary outbound payload. The 2D canvas renders asynchronously from the live overlay, while the
SceneKit bridge rebuilds only for a new committed drawing revision and never
JSON-serializes to detect a change.

Sources: `ops-ios/OPS/DeckBuilder/Models/DeckPerformanceCore.swift`,
`DeckGeometrySnapshot.swift`, `DeckGeometry.swift`, `DeckLevel.swift`,
`DeckBuilderViewModel.swift`, `Views/DeckCanvasView.swift`, and
`3D/DeckScene3DView.swift`. Deterministic coverage uses the 29-vertex/29-edge
1121 Oscar scale plus 29/64/96-vertex multi-level geometry, repeated snapshot
reads, callback-burst collapse, one-pass mutation counters, deferred one-encode
save, and 3D revision gating in
`OPSTests/DeckBuilder/DeckDesignerPerformanceTests.swift`. Local verification
for `ops-ios` commit `435cec12b2edae8150727f14487f77df76d0e04f` includes pure core execution,
whole-change Swift parsing, direct focused-test type-checking, and an iOS-SDK
type-check of the final native display scheduler. The bounded Xcode run compiled
the production target before that scheduler refinement, but the selected XCTest
bundle did not launch because the pre-existing test target fails to compile at
`SiteVisitServerMergeTests.swift:398`; physical-device
Instruments/FPS and hitch proof remains required before claiming device-level
performance.
...
**Camera follow during speed draw (2026-09-15, bug `5f285f64`; ops-ios main `cda89663`, merge `cda89663`; supersedes the follow added in `abebedcc`).** Four causes kept the camera parked on the point the operator had already placed: the bottom-chrome remeasure handler stopped the camera snap on every frame while the length picker animated in (killing the 200 ms pan at ~0%); a committed length lands in `.choosingDirection`, where no draft preview exists, so nothing followed; the direction-drag release recentred on the anchor; and the follow clamp recentred whenever the workspace fit the viewport, discarding the pan. Now `ViewportSnapAnimator.translateTarget(by:)` retargets an in-flight ramp instead of cancelling it (the chrome correction shifts start and target together); the pure `DeckCanvasCameraPlan` (`OPS/DeckBuilder/Models/`) decides what to follow — the new vertex after a commit, the draft's far end after a direction or length change, the ghost-ray tip when no length exists yet, nothing when a walk starts (that case keeps its recentre); `DeckCanvasView.followPoint` pans the minimum distance that brings the point into the unobstructed work area with `centerWhenWorkspaceFits: false`; the reorient release follows the draft end. Reduce Motion places the camera instantly. Tests: `DeckCanvasCameraPlanTests`, `ViewportSnapAnimatorTests`, `DeckCanvasFollowPolicyTests`, `DeckCanvasWorkspaceTests`.
...
**Surface label sizing (2026-09-15, bug `f7dd3673`; ops-ios main `cda89663`, merge `f3bc570e`).** The label used to be drawn at a fixed 11pt in CANVAS units at the naive vertex mean, so the fit transform (0.25–0.5 on a real deck) showed 3–6pt on screen and a concave surface could put the anchor outside the shape. `DeckSurfaceLabelPlacement` (`OPS/DeckBuilder/Rendering/`) now rasterizes the surface on a 64×64 grid (per-row scanline, even/odd, bit-identical to `PolygonMath.pointInPolygon`), takes the largest inscribed axis-aligned rectangle, and fits the name inside it: font size clamped to 11–28pt ON SCREEN at the current zoom (the clamp is computed so `size × scale` never exceeds the bound by an ulp), ellipsis truncation when even the floor cannot fit, degenerate surfaces fall back to the area-weighted centroid at the floor. The pill (glass-dense fill, `line` hairline, `chipRadius`) is exactly the fitted rectangle, so it can never cross an edge. Custom edge captions scale to 60% of the edge's on-screen length, clamped 11–20pt. The builder's own label shares the anchor, the truncation helper and the tokens (JetBrains Mono on `Colors.text`; the old `.system` monospaced face in accent is gone). Tests: `DeckSurfaceLabelPlacementTests`, `DeckViewerEdgeLabelRenderingTests.testSurfaceLabelFillsItsSurfaceAndStaysInside`.
...
**Partial-width stair measurement boundaries (fixed 2026-08-09).** `DeckStairRenderPlanner` emits presentation-only boundary markers at every internal stair corner plus dimension chips for the stair width and each remaining host-edge span. These markers deliberately do not split the persisted edge: a topology mutation would change the stair's attachment edge and risk losing edge materials, railings, surface membership, and connection identity. Measurement text uses the edge's authoritative `dimension` when present, respects the drawing's imperial/metric setting, and occupies fixed screen-space lanes so near-full or offset stairs remain readable at every zoom. `DeckCanvasView`, `DeckTab2DView`, and `DeckRenderer` share the plan; the full host-edge label remains visible on the side opposite the stair across interactive and exported 2D surfaces.
...
**Order-layout camera (2026-09-15, bug `1a8e48af`; ops-ios main `cda89663`, merge `503dbe06`).** The fullscreen `VinylOrderWorkspace` used to scale the fitted render with an outer `scaleEffect` (a bitmap "snapshot" that only zoomed about the centre) and blocked panning at fit. `VinylCutPreview` now carries a `viewport` (`VinylOrderViewportState`) applied INSIDE the `Canvas` (`translateBy` then `scaleBy`), so geometry stays vector-crisp at every zoom while callouts inverse-scale to hold their screen size (`drawScreenSized`). Pinch zooms about the fingers (fit…8× fit), one-finger pan works at every zoom, and a content-bounds clamp keeps a quarter-viewport band of drawing on screen. One `CanvasGestureView` recognizer (shared with the deck builder; it gained `scaleRange` and `allowsSingleFingerPan`, both defaulted to the editor's behaviour) owns pan and pinch. Double-tap and FIT glide through `VinylOrderDrawingCamera` (an `Animatable` wrapper, since a `Canvas` only redraws from values SwiftUI owns) on `OPSStyle.Animation.panel`, instant under Reduce Motion. Tests: `VinylOrderViewportStateTests`, `VinylOrderWorkspaceGeometryTests`, `VinylOrderWorkspaceSnapshotTests` (`vinyl-workspace-zoomed-4x.png`).
...
**In-app rail events (iOS-originated)** are created through narrow SECURITY DEFINER RPCs bound in `NotificationRepository` (see § "Server-authoritative creation boundary" below and `04_API_AND_INTEGRATION.md` § "iOS notification-surface RPCs") — the 2026-07-15 hardening revoked app-role INSERT on `notifications`, so the legacy `NotificationRepository.createNotification(_:)` direct-INSERT path fails `42501` for app roles and survives only as a documented dead shape (no production call sites since the 2026-08-17 legacy call-site wave; bug `e302355c` resolution notes hold the full history). Rail refresh after a create remains `NotificationCenter.default.post(name: .notificationReceived, object: nil)`. Example of a rewired rail event: **Stock system built (Guided Stock Setup, 2026-06-01)** — on a fully-successful guided build, `notify_guided_setup_completed('stock_setup', …counts)` writes a **standard** notification (title `STOCK SYSTEM BUILT`, body the summary counts e.g. `2 families · 6 variants · 7 rolls · 1 offcut`, `action_url` `/catalog?segment=stock`, `action_label` `VIEW STOCK`, `deep_link_type` `catalog_stock`); partial commits post **no** success notification (the in-flow RETRY governs). See § 13b.
...
New type `journal_editorial`, delivered by the hourly journal worker (`ops-web/src/lib/journal/editorial/worker.ts`) through service-only `deliver_journal_editorial_notification(...)`, which resolves superseded items, inserts against the open-notification dedupe indexes and marks the slot's `notified_state` in one transaction. `JOURNAL POST READY` — persistent; the body names the Vancouver launch (`… goes live Mon Sep 14 · 06:00 unless you stop it.`) or says the post waits for a go; action `/admin/blog?journal=<id>` `PREVIEW`. `JOURNAL POST LIVE` — standard. `JOURNAL POST BLOCKED` — persistent, reason copy per `last_code`. `JOURNAL WRITER STALLED` — persistent, at most once per Vancouver day while a queued slot is inside 12 hours of its launch; `resolve_journal_editorial_stall` clears it once nothing is at risk. `JOURNAL PHOTO FAILED` (2026-09-15) — standard, inserted by `fail_journal_editorial_image` when a requested replacement photograph fails three times; the current photograph stays; key `journal:<identity>:image-failed:<n>`. `JOURNAL RADAR DEGRADED` (topic funnel, live 2026-09-16) — standard, one open item keyed `journal:radar-degraded`, raised by `notify_journal_radar` after a scan in which fewer than half the watchlist feeds answered, resolved by the next healthy scan. Recipient: `SOCIAL_OPERATOR_*`, falling back to `PMF_OPERATOR_*`, always through the trimmed `getEditorialOperator`. Dedupe keys `journal:<identity>:ready|live|blocked` and `journal:writer-stalled:<date>`. See §21.

### Google Ads engine notifications (2026-09-10, live; guardrail alerts revised 2026-09-11, live 2026-09-15 via PR #123)

New type `ads_engine`, recipient `PMF_OPERATOR_USER_ID` / `PMF_OPERATOR_COMPANY_ID` (trimmed, `ops-web/src/lib/ads/engine/operator.ts`), all `action_label` `VIEW ADS`, written by the service-only RPCs in `20260910120000_ads_engine.sql` and delivered by the daily `/api/cron/ads-engine` tick. Since 2026-09-11 (`20260911180000_ads_guardrail_pauses.sql`, applied) an alert can be resolved — `ads_engine_alerts.resolved_at` via `resolve_ads_engine_alerts`, which also sets `is_read=true, resolved_at` on its rail row — and a resolved alert is never delivered. The guardrail rows below describe the 2026-09-11 build; the deployed build still raises `AD DISAPPROVED` keyed by ad resource with the old copy.
...
| `AD DISAPPROVED` | yes | `ads-engine:disapproved:<guardrail episode id>` (before 2026-09-11: `<ad resource>`) | `/admin/google-ads#engine` | the guardrail paused an engine ad Google disapproved (a landing-page-only verdict only after it stood 20 h). The body names Google's reason and says OPS turns the ad back on once approved; it promises a replacement only when the routine is active and the brief names the group. Resolved when the episode closes (restored or let go) |
| `AD DISAPPROVED` (could not pause) | yes | `ads-engine:pause-failed:<episode id>` | `/admin/google-ads#engine` | Google refused the pause; OPS retries next tick. Resolved once the pause lands or the episode closes |
| `AD STILL PAUSED` | yes | `ads-engine:restore-failed:<episode id>` | `/admin/google-ads#engine` | Google approved an ad the guardrail paused but refused the restore; OPS retries next tick. Resolved when restored or let go |
| `AD BACK ON` / `ADS BACK ON` | no | `ads-engine:restored:<hash8 of the episode ids>` | `/admin/google-ads#engine` | the guardrail switched approved ads back on; one per tick, naming up to three ad groups |
...
**Learning from mailbox sends: recognition + signature matching (2026-08-06, shipped `73afdd11`).** The learn-from-edits loop was dead for every draft the operator sent out of their own mail client — two independent failures, both fixed.

*Recognition (commit `cca1120e`).* `classifyDraftOutcome` (`src/lib/api/services/draft-reconciliation.ts`) treated "API draft still in the Drafts folder" as proof the operator ignored our draft and wrote fresh (`from_scratch`). Operators routinely lift the draft's wording into a **new compose** and send that, leaving the API draft object orphaned — so five real sends were filed as rewrites. A still-present draft is now absence of proof, not proof: only the sent body settles it. `outboundBodyDerivedFromDraft` scores the longest verbatim run (normalized, salutation + everything from the sign-off onward stripped, since both are stock scaffolding this operator repeats) and `>= DRAFT_DERIVATION_MIN_VERBATIM_RUN` (50 chars) counts as `used`. Threshold calibrated on production, not guessed: 5 true draft→send pairs scored 54/64/86/151/174; 245 negative pairs (each draft vs. 45 unrelated real sends by the same operator, same voice, same stock opener, same signature) topped out at 45.

*Signature matching (commit `5576d2f3`).* Even a recognized send could not train, because learning authority is gated on provably removing the known signature (`authoredBodyWithoutKnownSignature` → `signatureRemoved` → `learningAuthority` `operator_approved`/`operator_authored` vs `autonomous` → `apply_learning`; fail-closed by design so contact details are never learned as writing style). `stripRenderedEmailSignature` only knew our **stored plain-text mirror**, which gives every fact its own line. A real send carries the **provider composer's own rendering of the signature HTML** — Gmail (verified against real Canpro sends): `[image: {alt}]` line for the logo, block boundaries as single line breaks, anchors reduced to their label, inline runs left joined, so phone and website share one `·` line. It never suffix-matched → `signatureRemoved:false` → autonomous → nothing ever learned.

Fixed in the **matcher, not the template** — mimicking one provider in the stored mirror would be fragile, and other providers render differently. `emailSignatureHtmlToProviderRenderedText` derives the provider rendering from the stored `content_html`, and `stripRenderedEmailSignature` now tries every candidate rendering (mirror + derived), followed by a whitespace-collapsed pass for re-wrapped lines. **Safety property preserved:** every candidate is derived solely from the known signature (never from the body being matched), and each is removed only as an anchored suffix sitting on its own lines behind a blank line — so nothing beyond the known signature can ever be stripped, and a near-miss (one changed digit) or any authored text following the block leaves the body untouched.
...
*Recovery executed 2026-08-06/07.* All five durable draft/sent pairs were re-armed after the fix was customer-live and reached terminal `sent_from_mailbox` history plus `operator_approved` learning jobs. The first bounded worker pass exposed a separate failure: three `gpt-5.4-mini` edit analyses hit the 200-token completion ceiling, returned truncated JSON, and silently completed without model-derived corrections. The analyzer now uses strict structured output, a 2,500-token completion budget, and required preparation semantics for the durable worker; any refusal, non-`stop` finish, malformed JSON, or missing field leaves the job retryable. Mark, Jim, and Karan were then replayed against the same queue receipts. Final production proof: all five jobs completed with no error; all five history rows retain final bodies and nonzero edit distances; Mark's savings arithmetic, Karan's deck-permit/new-build questions, and Steve's four-word punctuation-light reply are durable correction/edit evidence. Existing 3/5-example promotion gates were preserved; this batch promoted only the repeated `shorter` structure and `more_direct` tone patterns.
...
*Terminal sweep.* The per-thread reconciler selects on `status='auto_drafted'`, so it stops looking at a row the instant it goes terminal — orphans there are unreachable, including the pre-`cca1120e` backlog. `sweepOrphanedMailboxDrafts` runs once per connection sync (bounded: 200 candidate rows → 25 distinct draft objects) over `sent_from_mailbox`/`superseded` rows with `mailbox_draft_cleanup_at IS NULL`, and never throws.

Two guards carry the safety of the sweep. **Ownership:** a draft id outlives the row that placed it (the composer patches the same object; `reassign_phase_c_mailbox_draft` re-points it), so only the *newest* row describes what the object holds now — if that row is `auto_drafted`, the object is live work and the sweep does not touch it. **Admissible proof:** deletion needs either the durable queue's receipt (`sent_from_mailbox` **and** `sent_provider_message_id` non-null) or a fresh `outboundBodyDerivedFromDraft` pass against the send. `sent_from_mailbox` alone is deliberately not enough — six production rows reached that state back when absence from a bounded `listDrafts()` page was still treated as proof of a send, and their drafts may be genuinely unsent work. A bare `superseded` is never deleted: the operator wrote their own reply and this is their draft.

`ai_draft_history.mailbox_draft_cleanup_at` (migration `20260806120000_phase_c_mailbox_draft_cleanup_marker.sql`, nullable, iOS-invisible) is the settled marker: stamped when the object is deleted, confirmed already gone, or present-and-inadmissible. A terminal row's verdict cannot change, so "leave it alone" is as final as "deleted" and the object is read from the provider at most once. Transient failures stay NULL so the next sweep retries.
...
The blast radius comes from **where that trigger sits** — on the transition into `reconciliation_required`:

1. `mark_email_provider_mutation_reconciliation_required` always threw, so `EmailProviderMutationAttemptService.execute` could never quarantine a stalled attempt. The row stayed `attempting` — and `execute` quarantines *any* prior `attempting` row for the same `(connection, operation_kind, operation_key)`, so **one stalled attempt permanently blocks every later placement for that operation.**
2. That same RPC is what persists `last_error`, so the underlying provider error was destroyed on the way out — every stranded row carries an empty `last_error`.
3. That trigger **is** the operator's `Draft placement needs review` notification. The one alert designed to surface a stranded draft was suppressed by the same defect, leaving the failure invisible in the UI *and* the logs. The operator found it by opening Gmail.

**Recovery pattern (reusable).** Resolve a stranded attempt with `mark_email_provider_mutation_rejected` → `provider_rejected`, which `claim_email_provider_mutation_attempt` accepts (`prepared|provider_rejected`), unjamming the operation key. Only safe once the provider side is *proven* empty — here the live Gmail Drafts folder was listed and confirmed to hold no draft from any stranded attempt, so re-attempt could not double-create. These RPCs require `service_role`, and `service_role` has **no** direct SELECT on `email_provider_mutation_attempts` (definer-RPC-only access), so drive them through the app's service-role client rather than raw SQL.

**Diagnostic note.** `PhaseCAutonomyRouter.route()` swallows placement failures and returns the message in `result.detail`, so running the real router against a stranded thread surfaces the exact exception without instrumenting or deploying anything. Placement invoked *outside* a sync defers with `mailbox_sync_continuation_pending`, and `deferPhaseCThread` re-queues the thread by nulling `category_classified_at` — so recovery of stranded threads rides the next in-sync classification rather than the manual call.

**Self-healing recovery path (2026-08-06, built after the outage).** The two defects above were each a one-afternoon fix; what made them a five-day outage is that neither had a recovery path. Both gaps are now closed, in two lanes on `/api/cron/email-sync`, running **after every sync lease is released** (the router takes the mailbox lease itself, and its sync-terminal fence must be able to see a *foreign* sync in order to stand down for it). The lanes run over every subscription-active, sync-enabled connection the cycle enumerated — deliberately **not** only the ones that were due for a sync. Recovery needs no sync to have happened, and inheriting the sync-interval gate puts it straight back to waiting on an unrelated schedule: a 60-minute mailbox (Canpro's setting) would get one attempt an hour, and a connection skipped for any other reason would get none at all. Caught in production on 2026-08-07 — the first post-deploy invocation returned `mailboxDraftRecovery.connections: 0` with 11 stranded drafts sitting untouched, because nothing was due that tick. Neither lane can fail a sync — both swallow everything and return counters. The subscription gate still applies in full: no provider work of any kind for a lapsed tenant.

*Lane 1 — reconciliation resolver* (`email-provider-mutation-reconciliation-resolver.ts`, bounded 25 attempts/connection, 30-day window). Reads `status='reconciliation_required'` `draft_create` attempts and asks the provider what actually happened. Migration `20260806223005_email_provider_mutation_reconciliation_recovery` adds the exit that did not previously exist: `resolve_email_provider_mutation_reconciliation(attempt, verdict, resource_id, secondary, result, evidence)`, service-role only, evidence text mandatory for either verdict.
- `resource_exists` → `provider_accepted` (**not** `completed`: OPS-side binding still has to happen, and `complete_email_provider_mutation_attempt` remains the only thing that closes the row). `mark_email_provider_mutation_accepted` cannot be used here — it raises `identity_conflict` when binding an id onto a row whose `provider_resource_id` is NULL.
- `resource_absent` → `provider_rejected` **and clears the identity** (`provider_resource_id`/`secondary`/`provider_result`/`provider_accepted_at`). A rejected row that kept its resource id would be read straight back by `EmailProviderMutationAttemptService.execute` as a durable acceptance and reconciled against a resource proven gone. The caller must *name* the identity it disproves or the RPC raises `email_provider_mutation_absence_contradicted`.

**Evidence standard — the load-bearing rule.** Nothing resolves on absence of evidence. Two attempt shapes admit different proof: (a) the ledger recorded a `provider_resource_id` → `getDraft` on that immutable id settles it both ways, because the identity is ours; (b) the ledger recorded nothing (**the outage shape** — the sync-terminal fence threw inside `executeProvider` before Gmail was ever called) → only the new per-thread probe can speak, and it may only ever prove ABSENCE. A draft sitting on the thread proves *a* draft exists, not that it is ours; the operator composes on these threads too, and adopting one would let reconciliation later overwrite or delete their unsent reply. That case stays quarantined for the operator on purpose. A failed provider read is likewise not a finding.

*The probe* — `EmailProviderInterface.findDraftsOnThread(threadId)` → `{present, draftIds}`. `listDrafts()` could not do this job: it is a bounded newest-first page (Gmail caps at 15) whose truncation is invisible, so a miss there proves nothing; `fetchThread()` is also unusable because it strips exactly the DRAFT-labelled messages in question (`NON_DELIVERY_MESSAGE_LABELS`). **Gmail:** `threads.get?format=minimal` → any message carrying the `DRAFT` label (complete for that thread; a 404 thread is also a proven absence), then the metadata-only drafts index (`maxResults=500`) purely to *name* what was already proven to exist. **M365:** `/me/mailFolders/drafts/messages?$filter=conversationId eq '…'` (server-side, uncapped). `present:true` with empty `draftIds` means "exists but unnameable" → the caller declines to act.

*Lane 2 — stranded placement sweep* (`phase-c-draft-placement-recovery.ts`; 7-day window, 200 candidate rows → 10 distinct threads, 2-minute deadline). Finds `origin='phase_c'`, `status='drafted'`, `mailbox_draft_id IS NULL`, **`thread_id IS NOT NULL`** and re-drives placement through `PhaseCAutonomyRouter.retryStrandedMailboxDraft` (= `route(thread, {placementOnly:true})`), so every fence stays in one place: `assertPhaseCSyncTerminal` outside the lease, the mailbox lease around the provider mutation, the `ownsMailboxLease` waiver inside it, current autonomy level, and live actor authorization. `thread_id IS NOT NULL` is an ownership boundary, not a convenience — the contact-form draft worker writes `origin='phase_c'` rows too, its drafts open a *new* conversation (no thread), and its own durable queue retries them.

Placement-only routing differs from the live path in exactly two ways, both deliberate: it **never calls the draft model** (recovery places what already exists; minting a new draft is the classification path's decision and cost — outcome `noop_no_stranded_draft`), and it **never marks the thread dirty on a deferral** (`deferPhaseCThread` is skipped, because a recurring sweep nulling `category_classified_at` would queue classification work nobody asked for). Only effective level `auto_draft` places — anything send-capable returns `noop_placement_not_applicable` and is left to the pipeline that owns it. Lane order matters: the ledger is unjammed *before* the sweep, so a quarantined row does not refuse the placement retry that follows it in the same cycle.

*Three defects the unit suite could not have caught, all found by probing the live cron after deploy (2026-08-07).* Every one passed locally because the Supabase double answers what production refuses, and because mocked schedules have no cadence. **(1) The sync-interval gate** — the lanes originally iterated only connections due for a sync, so a 60-minute mailbox got one recovery attempt an hour and a skipped connection got none; first production run returned `connections: 0` with 11 stranded drafts sitting there. They now cover every subscription-active, sync-enabled connection the cycle enumerated. **(2) The ledger read** — `email_provider_mutation_attempts` grants `service_role` nothing (definer-RPC-only, reads included), so the resolver's direct PostgREST select raised `permission denied` before scanning a row, standing as `reconciliation.failed: 1` every cycle; reads now go through `list_email_provider_mutation_reconciliation_candidates` (migration `20260807152233`), and the test double now throws on any direct read of that table so the boundary cannot be bypassed silently again. **(3) A lying counter** — a thread already covered by a placed draft returns `auto_drafted`, which the sweep counted as a placement; production reported `placed: 3` on consecutive cycles with the database unchanged at 33 placed / 6 stranded. Placement-only now answers before that branch. The lesson worth keeping: these counters are the outage tripwire, so a permanent false positive is worse than no counter — a real collapse to zero placements would hide inside it.

*Visibility.* Outstanding placements and unresolved quarantines now reach the cron result (`mailboxDraftRecovery`, `mailboxDraftRecoveryError`) and flip its status, so a repeat of this outage shows up as a persistently failing cron instead of five days of silence. The `reconciliation_required` notification also stops being a dead end: it now retires on **any** exit from quarantine (previously only `completed`, which a rejected attempt never reaches) and deep-links a Phase C reply draft to its conversation — `/inbox/{thread.id}?opportunityid={id}`, resolved in the trigger from the `phase-c-reply-draft:{ai_draft_history.id}` operation key, falling back to `/pipeline?opportunityid=` and then to `/settings?tab=integrations` for a company mailbox with no resolvable thread.
...
Narrow self-service paths preserve legitimate client-triggered behavior without restoring generic creation authority:
...
- `sync_review_stack_notification(p_stack, p_count)` (2026-08-17, migration `20260818014340`-adjacent `20260818014657_review_stack_notification_rpc.sql`) — granted to app roles; self-notification only. Owns the entire iOS review-stack rail semantic: fixed copy per stack kind, threshold (5), at-most-one-unread dedupe under an advisory lock, below-threshold auto-clear. Contract detail in `04_API_AND_INTEGRATION.md` § "Review-stack rail RPC". iOS caller: `NotificationRepository.syncReviewStack` from `ReviewThresholdService`.
- The eleven iOS surface RPCs of migrations `20260818023254_ios_notification_surface_rpcs.sql` + `20260818023657_measurement_notification_dedupe_keys.sql` (2026-08-17) — granted to app roles; one narrow RPC per remaining dead iOS surface (bug `e302355c`). Same doctrine throughout: actor from `private.get_current_user_id()`, recipients derived from note/photo/batch/event/project rows or `users_with_permission`, fixed server-side copy (caller data limited to clamped counts / validated dimension integers), server-literal `type`, internal-path-or-NULL `action_url`, dedupe via at-most-one-unread advisory-lock sync shapes or `dedupe_key` + the platform's partial unique indexes. Full contracts in `04_API_AND_INTEGRATION.md` § "iOS notification-surface RPCs". iOS bindings: `NotificationRepository` § "Narrow creation RPCs".
- The twenty-one legacy call-site RPCs of migration `20260818043043_ios_legacy_notification_surface_rpcs.sql` (2026-08-17) — granted to app roles; one narrow RPC per surface shape for the post-audit legacy `createNotification` call sites (bug `e302355c` ADDENDUM): task/project lifecycle fan-outs (completion, reschedule, dependency-ready, assignment, pair spawn, bulk-schedule summaries), vinyl order confirmations, guided-setup completions, role assignment, team invites, the time-off REQUEST side, inventory threshold alerts, the photo-storage cap banner, the billable-week nudge, and the cashflow dip/cleared rail. Extends the doctrine with two constrained shapes: caller-supplied id lists may only SUBSET an anchor row's recorded membership (delta assignment notifications), and one sanitized self-display string (the photo-storage device name, on a row only its author receives). Full contracts in `04_API_AND_INTEGRATION.md` § "iOS notification-surface RPCs — legacy call-site wave". iOS bindings: `NotificationRepository` § "Legacy call-site RPCs".
...
**iOS rewire (audited + closed 2026-08-17, bug `e302355c`).** The hardening rewired every web surface (the lockout request button calls `request_lockout_admin_notification`; all other web writers are server-side under service role) but never rewired iOS: thirteen audited iOS surfaces inserted via `NotificationRepository.createNotification` (direct PostgREST INSERT) and were silently dead from 2026-07-17 — push still fired where OneSignal was wired; only the rail rows were missing. The review stacks (surface #1) were fixed via `sync_review_stack_notification`; the remaining twelve were rewired the same day through the eleven narrow RPCs of `20260818023254_ios_notification_surface_rpcs.sql` (+ dedupe-key follow-up `20260818023657`). The two `ops://…` action URLs that additionally violated `notification_action_url_internal` are gone — those surfaces navigate by `deep_link_type` (iOS) and carry the live web click-through (`/dashboard?openProject=<id>&mode=view`) or NULL. Where the server now owns recipient selection (`invoice_overdue`, note fan-out, crew `photo_uploaded`), the RPC returns the recipient ids that received NEW rows and the iOS OneSignal push targets exactly that list, so push can no longer diverge from the rail's dedupe. Beyond the audited thirteen, the twenty-nine legacy `createNotification` call sites added by later feature work (DataController flows, vinyl orders, catalog setup completions, ManageTeamView, ProjectFormSheet, UserEventSheet/TimeOffRequestSheet request-side, QuantityAdjustmentSheet, PhotoPrefetchService, HomeBillableThisWeek + Forecast dispatchers) were rewired 2026-08-17 through the twenty-one narrow RPCs of `20260818043043_ios_legacy_notification_surface_rpcs.sql` under the same doctrine — no iOS call site remains on the dead direct-INSERT path.
...
| `action_url` | Internal web app path only — the `notification_action_url_internal` CHECK (2026-07-15 hardening) accepts NULL or a trimmed single-leading-slash path (no protocol, no `//` prefix, no backslash/control chars). **`ops://` deep links are no longer storable** — iOS routes on `deep_link_type` (first-class) and uses `action_url` only as supplementary info (e.g. `?tab=...` query strings). Phone-only surfaces set it NULL. |
...
### Weekly journal post on the cloud routine (2026-09-10; live since 2026-09-10; generated photographs live 2026-09-15)
...
- **Image (2026-09-15):** every post carries a generated photograph. The writer supplies art direction (`article.image_prompt`, 120–1,500 characters) under the brief's rules — composed, never posed; a real residential job with crews dressed as they really dress (jeans, t-shirt or hoodie, ball cap, sneakers or plain boots; never hard hats or high-vis); wide and far with generous negative space; warm, refined, gently desaturated; sometimes a warm-toned monochrome; never the stock trades cliché — and OPS appends its fixed house style (`ops-web/src/lib/journal/editorial/image.ts` `JOURNAL_IMAGE_STYLE`) and generates one frame with OpenAI `gpt-image-2.5-flare` (1536 × 1024, high, ≈US$0.05) on OPS's own `OPENAI_API_KEY`, crops it to 1600 × 900 and stores `blog/weekly/<slot>-<sha16>.jpg`; a public HEAD must answer 200 before a preview is promised. The Blog hub's NEW PHOTO regenerates (a preview, or a post live under 8 days; 8 per slot); a failed request retries hourly and gives up after three with `JOURNAL PHOTO FAILED`. Jackson approved the standard after four proof rounds (`ops-web/docs/artifacts/journal-editorial/photo-proof-2026-09-15/round-4`). The earlier typographic plate (`blog/journal/`, 2026-09-10 to 15) is retired; Instagram still treats those plate URLs as imageless and uses generated photographs as covers. The routine never calls an image service.
...
- **Topic funnel (2026-09-16; live: ledger `20260916232832`, ops-web `2421f1358` deployed `dpl_AYmCWDsLy1R8CJMKnVxV1r4zmCMb`, routine prompt v3 installed, all on Jackson's go; first production radar scan 2026-09-17 00:09Z: 26/26 feeds, 389 signals, no degraded item).** Jackson (2026-09-16): the weekly genre was missing because the cloud routine picked evergreen backlog topics; the old Cowork funnel scraped what was trending (YouTube, construction and leadership blogs, Simon Sinek, Echelon Front, Jocko, adjacent spheres, Twitter), picked the best topic, had an agent work out a strong angle and hook, then wrote. Restored as: (1) **radar** — the hourly tick reads a watchlist of 26 verified public feeds once a day, after 04:00 Vancouver (Jackson asked why not once a week: trade news feeds list only about a day of stories, so daily reads build the week) (`ops-web/src/lib/journal/editorial/radar/`): 16 YouTube channels (Jocko Podcast, Echelon Front, Simon Sinek, Daily Stoic, Alex Hormozi, My First Million, Codie Sanchez, The Diary Of A CEO, Tommy Mello, Breakthrough Academy, Contractor Fight TV, Roofing Insights, Mike Rowe, Essential Craftsman, Build Show Network, The Honest Carpenter) with views → momentum against each channel's typical video; Seth Godin and Farnam Street; Construction Dive, NAHB Now, Eye on Housing, Canadian Contractor, On-Site, Daily Commercial News, Fine Homebuilding; ContractorTalk with reply counts and board, standing megathreads dropped. First live scan from a Mac: 26/26 feeds, 390 signals, 2.3 s. Publishers that refuse automated readers are excluded; Reddit (commercial API contract) and X (paid pay-per-use API) are reached through the routine's search. (2) **topic** — the routine (prompt v3) reads the radar, widens it with ≤12 searches, clusters five to eight themes, scores heat/fit/pain/ground/fresh (brief v3 § The topic funnel) and grounds the pick. (3) **headline, Sam Parr's way** — Jackson rejected the first rehearsal headline ("THE LAST PAYMENT IS DECIDED BEFORE THE FIRST POST HOLE") as not making sense and made Sam Parr's copywriting guide the journal's writing rule (2026-09-16; `ops-web/docs/journal/voice/blog-voice-sam-parr.md`, rebuilt from *Write Like The Hustle* and his My First Million writing masterclass: plain-English headlines of 3–10 words, grade 7–8, sentences of about 25 words, paragraphs of 2–4 sentences). The routine writes a description, a headline writer produces 25 headlines with first sentences, three independent reader subagents each pick the three they would click and say what they expect, and a headline editor takes the most-clicked headline that makes sense and matches the post. (4) **pitch** — OPS validates it against observed signals and keeps it (`…/pitch`), renewing the lease. (5) research, writing and the editor (who now judges `hooked`) run as before. The Blog hub shows WHY THIS POST (topic, why now, angle and hook; the signals and search results, and the headlines weighed and topics passed over, each one click deeper) above the controls. Rehearsal on live data 2026-09-16 (`ops-web/docs/artifacts/journal-editorial/topic-funnel-2026-09-16/`): the radar surfaced a 450× Roofing Insights video of a fence crew tearing out an unpaid fence; the routine picked "getting paid is decided at signing", the hook room wrote 12 headlines, the critic chose and sharpened THE LAST PAYMENT IS DECIDED BEFORE THE FIRST POST HOLE, and OPS accepted the pitch on the first try.
- **Alerts:** `journal_editorial` rail items (§14): READY, LIVE, BLOCKED, and a once-a-day `JOURNAL WRITER STALLED` when a queued slot is within 12 hours of launch, cleared by the tick; `JOURNAL RADAR DEGRADED` when fewer than half the radar feeds answer.
...
- **Proof:** SQL harness (now 20 functions), focused unit/integration tests, and a local rehearsal on a disposable stack (`ops-web/docs/artifacts/journal-editorial/local-e2e-2026-09-10/`). Live: ledger migration applied 2026-09-10, ops-web `c217c3bc4` deployed 2026-09-10, `JOURNAL_AUTHORING_TOKEN` and the `OPS Journal` environment set by Jackson, routine `trig_01NbGDkXXigXTR9K2FYPjZCk` enabled in `prepare` mode. First cloud-written post: `weekly:2026-09-14` "COUNT THE HOURS OR COUNT THE LOSSES", drafted Sunday 2026-09-13 06:21 Vancouver by run `cse_01YT8yyp4HLBTAPe3uEFuuDN` (editor rejected pass 1 for an overstated claim, approved pass 2; one 422 fixed), published by PUBLISH NOW 2026-09-14 20:39. Generated photographs: migration `20260915051626` applied and ops-web `81f55e56c` pushed 2026-09-15 with routine prompt v2.
...
### Cloud editorial replacement (2026-09-05; production preparation active)

**Immediate run completed; reliability fixes pending deployment (2026-09-06 00:12 UTC).** Jackson requested a draft immediately. A session-started run using production services prepared “A clean report can still be wrong” / “Run the closed-job test” from the live Fable article, with five public 1080 × 1350 JPEGs, complete Sam Parr reference, editor approval, one exact Jackson notification and zero social posts. All image hashes matched independent public readbacks. The two model calls cost an estimated US$0.112664; saved copy was reused through recovery. Three reservations remain US$2.25. Replay produced no new claim or notification. Assets use the pre-existing Supabase `social-media` bucket via the supported renderer backend; no production storage setting, IAM policy, scheduler or database schema was changed.

The run exposed two code bugs, both reproduced and fixed locally: Node's all-address lookup callback must return an array containing only the already-validated address; saved source comparison must compare values because JSONB reorders keys. Fix commits `003566c8a` and `931723891`, plus manual draft trigger `d85c956d4`, passed 291 focused tests and targeted TypeScript. Automatic approval review blocked the production push for lack of explicit deployment authorization; the safer one-time run completed without deploying. An approval question is pending for all three changes. The currently deployed scheduled worker still needs these fixes, and its S3 upload path is not yet live-proven. Preparation mode remains active; first Instagram publication remains unauthorized. Full evidence: `ops-web/docs/artifacts/social-editorial/manual-run-2026-09-05/README.md`.
...
The fixed model is `gpt-5.6-sol`, with two bounded calls, no tools and no SDK retries. Each claim reserves US$0.75 from a monthly estimate allowance capped at US$20; hosting/storage are additional. The full-guide local canary produced five JPEGs (“Your best work is hiding in your camera roll”) from the published article “WORD OF MOUTH ISN'T A MARKETING PLAN,” with estimated model usage US$0.094948. The earlier condensed-guide four-slide canary (US$0.052068) is preserved separately. A previous canary failed, so neither is the full testing bill. Local evidence is in `ops-web/docs/artifacts/social-editorial/`; the actual admin component was inspected as a static browser fixture. Exact migration behavior was exercised in disposable PostgreSQL, including concurrent claims, mode changes, recovery, grants and zero-row notification replay.

New migration: `ops-web/supabase/migrations/20260905185527_create_social_editorial.sql`; it was applied as production migration `20260905233314` after Jackson approved migration, deployment and paid preparation-only activation. Its exact archive is `migrations/20260905233314_create_social_editorial.sql`. Initial readback verified service-only tables/RPCs, zero runs and mode `off` with US$20 allowance. Final deployment `dpl_7QzcFZb7nh8uTXwWne7kvCsaDv51` passed its complete build and is READY at `app.opsapp.co`. Authenticated cron invocations returned 200 on that deployment before and after activation; both unauthenticated endpoints returned 401/no-store. Vercel reports the enabled 15-minute schedule on the final deployment. The exact singleton was changed to `prepare` at 23:51:50 UTC; independent readback after invocation at 23:52:46 confirmed `prepare`, US$20, zero editorial runs and zero social posts. The full guide loads successfully in the cloud. Final focused regression passed 277 tests across 28 files and targeted TypeScript. First scheduled generation is Monday 2026-09-07 10:00 Vancouver (17:00 UTC); production model generation, storage, authenticated draft inspection and notification delivery remain unobserved until that eligible slot. First publication remains separately unauthorized and unverified. This approved release does not reopen the completed OAuth repair. Canonical runbook: `ops-web/docs/social/cloud-editorial-operations.md`.
...
| Vercel worker | `GET /api/cron/social-publish` | 15-minute bounded queue processing (`13-59/15 * * * *`, the last full-day slot inside the three-lane cron budget) behind `CRON_SECRET`, run inside the shared durable cron workload lease `social-publish` (fails closed on `circuit_open` / `control_unavailable`) |
...
## Financial rule enrollment notifications (Phase 16, released and dormant)
...
Native Codex plus signed-in MAVERICK browser acceptance passed for a seven-record catalog setup, a separately approved stock count, and a price-only update. The production fault/retry test preserved both old prices and no receipt on failure, then saved the exact proposal once on retry. No other company's data or existing financial draft was touched. Acceptance also found a queue request-isolation defect (fixed in `1c319af79`, live verified on `c3e9f141d`) and two saved-review readability defects: untranslated catalog source and a generic raw sealed-JSON fallback. The latter are fixed in `27ee6d8ad` with pending/executed/rejected rendering tests; release verification is tracked in the Phase17 API section. The human-readable proposal remains the review/history presentation; hashes and UUIDs are not a substitute for it.
...
Only the proposal's named operator can review and save it. Catalog actions are excluded from generic creation/status execution, autonomous execution, bulk approval and generic expiration cleanup. Current queue visibility is checked separately before exposing cost-bearing payloads or receipts. Approval submits only the exact change-set ID and preview seal. A malformed or expired preview disables approval. The service verifies the transaction receipt and independently reads the executed action back before reporting success; an uncertain response retries the identical commit key. Rejection and successful save resolve the persistent review notification.
...
## Phase 19 site-visit review and phone coexistence (2026-09-10; deployed, activation disabled)

**Restricted-trial update, 2026-09-14:** the exact trial path keeps every business save in the OPS approval queue. Queue visibility refreshes current permissions while retaining the original MCP grant/client/scopes; it does not manufacture an internal write actor. Review testing reproduced and fixed a regression that would otherwise hide valid approvals. Expired/revoked trial authority denies saving and receipt replay; internal/API channels cannot bypass trial write authorization. Focused proof is 802 application tests plus 54 real synthetic PostgreSQL assertions, including both queue visibility helpers and subject foreign-key indexes. The current choice-compatible phone is installed as recorded in [the phone release receipt](docs/artifacts/2026-09-14-site-visit-choice-release.md); installation does not prove physical-device pending-work recovery or native-host prepare/review/save acceptance. Shared-write activation remains off pending those checks. [Trial release evidence](docs/artifacts/2026-09-14-site-visit-trial-release.md).

**Release update, 2026-09-12 UTC / September 11 local:** the web review code and six database migrations are production-installed. No company shared-write compatibility, effect policy, V22 client or V22 grant is activated. Source `3f5eaffe` produced a signed Release/arm64 iPhone test application with a verified deep/strict signature, but it has not been installed, uploaded or released. Signed-device offline/reconnect/conflict/media/recovery and native-host prepare→OPS review→save→readback remain unperformed; customer-live site-visit writes are not claimed. [Release record](docs/artifacts/2026-09-12-site-visits-release.md).
...
The final trial SQL suite adds a foreign-key index coverage assertion, bringing it to **54 passing checks**. The already-live application passed native Canpro read compatibility; this does not establish MAVERICK trial acceptance.
...
See [Phase 19 implementation and release boundaries](specs/2026-09-10-ops-mcp-site-visits-p19-implementation.md), the [30-persona scenario mapping](specs/2026-09-10-ops-mcp-site-visits-p19-personas.md), and the [accepted handoff](specs/2026-09-10-ops-mcp-site-visits-p19-handoff.md). SwiftData V28 preserves all27 released fingerprints and populated-store custody; exact hosted results and skipped private-device fixtures are recorded there. The subsequent production installation is recorded above; public candidates remain disabled.

Parent integration was verified locally on 2026-09-11: 671 distinct web tests, focused TypeScript and independent merge-seam review, plus one combined phone run with 138 passes, two optional private-store-copy skips and no failures. The phone run includes upgrade/reopen, originating-actor and pending-work recovery, calendar mirror/lead resolution, and site-visit settings input. Existing main's calendar, receipt and toast changes are retained. The subsequent release adds 721 exact-release web checks, installed migrations and a signed phone build; signed-device coexistence and exact host/company activation remain separate gates.

## Single-choice site-visit checklist (2026-09-11; local implementation)

**Approved database release, reverified 2026-09-14 UTC:** both the phone choice contract and its MCP coexistence repair are now production-installed. The local build/verification checkpoints below are historical. Exact company/host MCP write activation remains disabled, and physical-device acceptance is not implied by server installation. [Current production and phone-install status](docs/artifacts/2026-09-14-site-visit-choice-release.md).

**MCP compatibility follow-up, 2026-09-12:** the local SQL repair in OPS-Web `babe8ec1d` closes invalid-choice approvals and ordinary-default changes failing against the phone's choice guards. All 31 new coexistence assertions pass, including approved create/edit defaults, exact answer/clear, replay, stale-row rollback and forged metadata. Independent review is clear. The phone choice and MCP coexistence migrations remain unapplied, and company/host activation remains disabled. Do not install the earlier `3f5eaffe` test build over choice-used local data: unchanged SwiftData V28 does not prove queued-command/Codable compatibility. [Current verification and phone-build boundary](docs/artifacts/2026-09-12-site-visit-choice-coexistence.md).

Company admins can add a Multiple choice field in Site Visit Types and define, reorder or remove 2–20 answer options. The crew chooses one answer, with no automatic first selection, or explicitly clears it. Each visit retains the choices captured when its checklist was created. An unrecognized legacy free-text answer remains visible and retained but does not satisfy a required choice.

**Signed test build, 2026-09-12:** exact choice-compatible source `2e46d728` built successfully for Release/arm64 and passed normal macOS deep/strict signature verification. Locally integrated evidence commit `c709139a` records the application and hashes. This supersedes the older test artifact for a choice-compatible trial; it is not installed/distributed and physical-device/native-host acceptance remains open. The two new site-visit migrations are still unapplied. See the linked compatibility record above.

The editor and capture controls use existing OPSStyle form, typography, surface and touch-target tokens. Template options are additive short-text metadata, allowing older readers to display the selected answer. Choice snapshots survive setters, server hydration, disk reopen, pending work and explicit conflict review. The write protocol and unreleased server migration are documented in chapters 03 and 04. Source: `SiteVisitSingleChoice.swift`, `SiteVisitChoiceOptionsEditor.swift`, `SiteVisitTypeSettingsView.swift` and `SiteVisitCaptureView.swift`.

iOS verification passes **150 distinct tests, zero failures or skips**: 102 focused tests on production source `1168058f`, followed by 48 packet/recovery tests on `8b290d48`. Only reviewed test fixtures changed between these runs. All declared schema checksums passed. Root inspected four exported screenshots and accessibility geometry: settings option editing, legacy text retention, selected answer and explicit clear. Full questions wrap at accessibility sizes, and new clear/add/move/remove actions expose their existing 44-point tokenized frames. Evidence commit `6b2a8ea3` and all 27 changed source/test hashes are recorded in iOS `docs/artifacts/ios-bugs-p4-checklist-20260911/`. Initial production integration is local iOS main `b44492aa`; local main `2e46d7285170b7520ee73a7a12be0751b0713c9e` adds the fixture corrections and complete verification record. This is local implementation and hosted-control proof, not an applied server contract, physical-device acceptance or distributed iOS build.

## Site-visit capture saves locally until the operator saves (2026-09-15; ops-ios local, unreleased)

Direction from Jackson, 2026-09-15: "It should only be uploading when the user saves the site visit. Otherwise, just saving locally on the phone." Motivated by a visibly laggy General Notes field and by the self-conflict class documented in `04_API_AND_INTEGRATION.md` § "Phone base revision and self-conflict settlement" — every keystroke used to run a full store transaction, a queue search over ~3,000 `SyncOperation` rows, a checklist re-render and a network push.

**Behavior (iOS capture screen):**

- Typing and every other capture-screen edit — checklist answers, notes, measurements, identity fields, deck attachment, address — persist to the phone only, through `SiteVisitPersistenceCoordinator.commit(queueing: false)`. Rows carry `needsSync`; no `SyncOperation` is created. Checklist keystrokes are debounced (`SiteVisitCaptureViewModel.checklistFlushDelayNanoseconds`, 400 ms; focus loss flushes at once) so the store sees one transaction per pause; the published buffer drives the screen meanwhile.
- **Saving the visit** — DONE, SAVE DRAFT & CLOSE, the review sheet's draft-save path, and completion — runs `saveDraft()`: flush buffers → `queueDirtyWork(siteVisitId:)` (queues every dirty row of that visit, refreshing unattempted commands in place and re-anchoring a pending completion behind the new tail) → `onWorkQueued` → `SyncEngine.notifyDurableOperationQueued()`.
- Going to the background, closing without confirming, and opening the deck editor only flush buffers (`preserveDraft()`); nothing is queued.
- Unchanged: the visit row itself is created server-side when the visit starts (children need the parent); photo bytes keep uploading as captured; lead bind/reassign/clear, discard and completion keep their immediate server semantics; the launch orphan sweep (`recoverOrphanedWrites`) still delivers any dirty rows it finds.

Tests: `SiteVisitPersistenceCoordinatorTests.test_localOnlyCommitQueuesNothingUntilDirtyWorkIsQueuedForSave`, `SiteVisitFieldWorkflowTests.testTypingSavesLocallyAndUploadsOnlyWhenTheVisitIsSaved`, `SiteVisitFieldWorkflowTests.testKeystrokesWaitBeforeTouchingTheStore`.

**Saving never restarts stopped work on its own (2026-09-17; ops-ios local, unreleased).** `queueDirtyWork` re-sends what changed on the phone and nothing else — the outcome the per-edit queueing had before typing went local-only. As first built it re-derived a send for every dirty row and `enqueue` revived any stopped send it matched, so every DONE restarted a permanent rejection held for review ("only user Retry / Discard moves it out"), brought back a send the operator had declined in PENDING WORK (06 § `declined`), queued a duplicate behind an answer command already in flight or held, and restarted or duplicated photo uploads that capture already owned. `SiteVisitContinuityTests.test_unchangedIdentityAndAnswerNeverRestartStoppedWork` was red from 09-15 until this fix. The save now decides per record (`SiteVisitPersistenceCoordinator.saveShouldQueue`):

- **Photo uploads** belong to capture and markup (`commit(revisedMediaArtifactIds:)`). The save only derives an upload for a photo with no unresolved upload at all — the orphan sweep's rule.
- **Checklist answers** carry their values in the queued command. When this operator's newest attempted, stopped or failed command for the answer already asks for exactly these values — same protocol, row values and explicit clear — the save adds no successor. An unattempted pending command is still refreshed in place; a genuine edit is still sequenced behind an attempted one.
- **Visit, artifact and identity-draft sends** that are parked or declined restart only if the record was edited on the phone after the stop. Typing never queues, so the local-only commit records the edit on the stopped send itself, in the same transaction as the row: `SiteVisitSyncOperation.Payload.restartMark` (JSON `restart_mark` — the send's `status`, `last_attempted_at` and `retry_count` at the moment of the edit; phone-local, never sent). The save honours a mark only while the stop it records still stands: every attempt stamps `lastAttemptedAt`, and RETRY or a decline changes the status, so an edit the operator has since overruled, or one a send has since carried, restarts nothing. Restarting rewrites the payload, which drops the mark. Failed sends are unchanged: a save revives them, as the launch recovery would.
- **Nothing is sequenced behind a declined send.** A dependency is satisfied only by `completed`, and a declined send never completes, so `dependencyRoot` skips declined sends when it picks the send new work waits behind — otherwise an answer saved after a decline would never upload.

The launch orphan sweep is unchanged apart from that last rule: it still queues only records with no unresolved send. Tests (2026-09-17): `SiteVisitContinuityTests.test_unchangedIdentityAndAnswerNeverRestartStoppedWork`, `…test_editAfterAStopRestartsThatSendWhenTheVisitIsSaved`, `…test_savingTheVisitNeverRestartsAStoppedPhotoUpload`; `SiteVisitPersistenceCoordinatorTests.test_saveLeavesStoppedSendsForUneditedRecordsAlone`, `…test_phoneEditAfterAStopRestartsThatSendAtTheNextSave`, `…test_anEditMadeBeforeTheOperatorDeclinedNeverOverridesTheDecline`, `…test_anEditTheSendHasAlreadyCarriedDoesNotRestartItAgain`, `…test_saveAddsNoSuccessorWhenTheQueuedAnswerAlreadySaysTheSame`, `…test_workSavedAfterADeclineIsNeverSequencedBehindTheDeclinedSend`, `…test_saveNeverRestartsOrDuplicatesAQueuedPhotoUpload`. The capture screen persists through its own isolated context, so the capture-level tests stop sends from a separate context (as the sender and PENDING WORK do) and read results back through a fresh one.
