...
**Sync serialization (production 2026-07-21):** Cron, webhook, and manual sync all claim the same ten-minute connection lease through the service-role-only `acquire_email_connection_sync_lock_as_system` RPC (migration `20260721071828_email_sync_lock_rpc.sql`). PostgreSQL performs the empty-or-expired comparison and owner assignment in one conditional update; a competing runner receives `NULL` and skips. The active runner renews and releases only with its returned owner UUID, while a crashed runner self-recovers after the lease expires. This avoids PostgREST's unreliable `OR`-filtered update path without weakening the single-runner guarantee.
