...
**Release state (2026-09-01): locally implemented, not customer-live.** The code and migration exist on the `feat/instagram-auto-publishing` worktree. The migration has not been applied, Vercel/Meta secrets have not been configured, OPS Web has not been deployed, and no real Instagram post has been authorized. The older Slack/Python/edge-function pipeline below is retained as historical context until production cutover is explicitly approved and verified.
