...
**iOS rewire debt (audited 2026-08-17, bug `e302355c` — status `triaged`).** The hardening rewired every web surface (the lockout request button calls `request_lockout_admin_notification`; all other web writers are server-side under service role) but never rewired iOS. Thirteen iOS surfaces insert via `NotificationRepository.createNotification` (direct PostgREST INSERT) and have been silently dead since 2026-07-17 — push still fires where OneSignal is wired; only the rail rows are missing. The review stacks (surface #1) were fixed 2026-08-17 via `sync_review_stack_notification`; the remaining twelve (AppState review reminders + `invoice_overdue`, share-extension `photo_uploaded`, photo-comment `mention`/`photo_comment`, project-note `mention`/`project_note`, expense `approved`/`rejected`/`paid`, `time_off_approved`/`denied`, crew `photo_uploaded`, inventory `threshold_alert`, measurement trio) carry a full inventory + remediation blueprint in the bug row's fix notes and await their own narrow RPCs (initiative `IOS NOTIFICATIONS RPC`). Two of the dead payloads (`ops://…` action URLs) would additionally violate `notification_action_url_internal` today — their replacement RPCs must route via `deep_link_type`.
