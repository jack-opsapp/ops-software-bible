...
Stage and summary evaluation is identity-safe and fail-closed. Each in-memory batch maps its real provider/message-scoped thread identities to short server-owned aliases before model input, and strict structured output requires exactly one result for each allowed alias with constrained stage and terminal-flag values plus a non-empty summary. The model never receives or echoes the canonical OPS/provider identity used for persistence. OPS still verifies the complete alias set, duplicates, values, refusal, truncation, and summary content after parsing. A malformed contract retries once from the already-fetched messages; a repeatedly malformed multi-thread batch is isolated into bounded singleton evaluations without positional recovery. Any unresolved contract, provider, or model failure aborts the cycle before cursor advancement, so no partial stage or summary projection is published.
