...
**In-app rail events (iOS-originated)** are created through narrow SECURITY DEFINER RPCs bound in `NotificationRepository` (see § "Server-authoritative creation boundary" below and `04_API_AND_INTEGRATION.md` § "iOS notification-surface RPCs") — the 2026-07-15 hardening revoked app-role INSERT on `notifications`, so the legacy `NotificationRepository.createNotification(_:)` direct-INSERT path fails `42501` for app roles and survives only as the shape used by a handful of not-yet-rewired legacy call sites (dead until rewired; tracked in bug `e302355c` resolution notes). Rail refresh after a create remains `NotificationCenter.default.post(name: .notificationReceived, object: nil)`. Legacy iOS-originated rail events still on the dead path:
