...
`PhotoThumbnail(url:project:)` is the shared thumbnail used by the activity-feed carousel (`ProjectPhotosCarousel`), the full-screen photo grid (`ProjectPhotosGrid`), the activity-feed annotation cards (`AnnotationEntryView`), and the bio/picker sheets. It resolves its image by normalized URL: in-memory `ImageCache` first (which is where the composited markup lands), then the on-disk original via `ImageFileManager`, then network. It listens for `.annotationsComposited` and re-reads the cache (`reloadFromCache`) so a composite that finishes after first paint swaps in.
