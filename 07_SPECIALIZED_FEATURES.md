...
defers reconciliation, encoding, SwiftData save, and sync enqueue outside the
gesture callback. Exit/background still calls `flushBeforeExit()` synchronously.
One canonical JSON encoding is shared by the `DeckDesign` cache and outbound
payload. The 2D canvas renders asynchronously from the live overlay, while the
