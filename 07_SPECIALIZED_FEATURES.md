...
**Web parity (partial; 2026-07-23):** `/api/uploads/share-photo` now creates a deduped, uploader-facing `photo_uploaded` completion notification for the iOS Share Extension. Ordinary photos/comments posted from the desktop app still do not fan out these collaboration notifications to iOS crew, and `photo_uploaded` / `photo_comment` are not yet in the web `NOTIF_TYPE_META` registry; those web-rail rows continue to use fallback rendering until the remaining parity work lands.
...
**One idempotent upload operation.** Each manifest job owns a stable UUID and capture timestamp. Both transfer paths POST the raw JPEG to `/api/uploads/share-photo` with the same `projectId`, `jobId`, and `takenAt`: the extension starts a best-effort background `URLSession` request with the usable bridged token, while `ShareUploadCoordinator` is the durable app-side backstop. The app drains after the Darwin nudge, on launch, on foreground, and when connectivity returns; it obtains a fresh Firebase token and force-refreshes once after a `401`/`403`. The server re-checks company ownership and `projects.edit`, stores at deterministic key `projects/{companyId}/{projectId}/share-{jobId}.jpg`, attaches the URL to `projects.project_images` and `project_photos` (`source = in_progress`), posts a deduped uploader-facing completion notification, and returns `{ success, url }`. Extension/app overlap is therefore a replay of the same server operation, never a second random-key upload.

**Retry and retention.** Connectivity, authentication, throttling, and server failures remain queued without consuming the parking budget. Deterministic failures increment the job attempt count; at 10 failures the job is parked so it cannot loop forever, but both the manifest row and JPEG bytes remain intact. Offline, account-change, and temporary permission states also retain the queue.
