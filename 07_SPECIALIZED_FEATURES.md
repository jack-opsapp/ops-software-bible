...
*Backfill.* The five affected `ai_draft_history` rows were re-armed (`status='auto_drafted'`, `discarded_at=null`) so the connection-level reconciliation sweep — which selects on `status='auto_drafted'` regardless of new mail — reprocesses them. Known follow-ups, not built: orphan draft objects left in the Drafts folder after a recognized send (duplicate-send hazard, also applies to the ~112 historical superseded rows), and a historical learning audit of those rows.
