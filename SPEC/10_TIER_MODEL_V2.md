...
**Status:** Approved design (Jackson, 2026-07-14 session). **Implemented 2026-07-14:** database re-seeded (spec01/02/03 live in `spec_capacity` + v2 engagement columns), `/spec` rebuilt per § 8 on ops-site `feat/spec-tier-model-v2` (EN + ES, guide remapped per § 7), ops-web admin renamed on `worktree-spec-launch-consolidation` (`514d4bbf`, never merged — re-applied 2026-09-05 on `fix/spec-admin-tier-v2-20260905` against current `main`, with per-tier milestone shapes in the console and the retired multiplier removed from the capacity editor; **SPEC-03 locked-total control landed 2026-09-06 on the same branch** — Scope Doc tab, see 05 Tab 4; awaiting push GO), ToS v2 prose shipped to `/legal?page=spec-terms` (hash rotated). Deposits remain OFF; the § 9-gated flip checklist lives in 07_ROLLOUT § Phase 0.
**Owner:** Jackson Sweet.
...
- **SPEC-01 payment shape** (50/50) and **SPEC-03 variable-total** (P1 fixed $6,250; total locked at P2 scope sign-off) have first-class support in the milestone helpers (`specMilestoneSchedule` / `composeMilestonesTab` carry per-tier shapes) and the scope-doc flow: the operator locks the SPEC-03 total on the current scope-doc draft from the console's Scope Doc tab (`lock-total` action — written into `spec_scope_documents.content_json.locked_total_cents` with `content_hash` recomputed, and copied to `spec_projects.locked_total_cents`) **before the doc goes out**; the customer's `scope_signoff` countersign (`payload_hash` = that `content_hash`) makes it binding and closes re-locks, after which changes travel as change orders. Until locked, the floor governs and P2–P4 refuse to fire; P2 additionally keeps its sign-off prerequisite, so money never moves ahead of the signature.
...
1. **Hero** — 3D phone scene slot preserved as-is. **Phone rebuilt 2026-07-15** (ops-site `feat/spec-tier-model-v2`, `src/components/spec/phone-scene/`): newest approved hardware vendored under `phone-scene/hardware/` (powder-coat solid + OPS mark + shaped dark-studio rig + smudged hero glass, from the platform's 2026-07-10 morph direction; two-channel MorphRig — `drawing` resolves the body into its contour drawing on the build zone, `glass` sheds screen glass for content phases). Screens key natively on `spec01/02/03` (the v1 bridge in `SpecPageContent` is removed) and narrate the ladder with one persona (Ray, Cascade Deck & Rail): OPS home → SPEC-01 workflows console → SPEC-02 backbone dashboard → SPEC-03 Deckset build console (hardware goes to drawing) → per-tier delivered screens on card DETAILS, flagship the Deckset deck designer (plan drafts → takeoff prices → client render + the scene's single accent CTA). The phone is hero-resident (v2 sections are full-width; the phase state persists, so DETAILS + scroll-up shows the tier screen in the hero). Headline direction shipped: `WE BUILD THE MACHINE THAT RUNS YOUR BUSINESS.` Founder line rewritten — **"contractor" is banned**; approved: owner-operator / the trades / subtrades.
...
Scope for the ToS/rider pass (06A/06B/06C update): per-tier scope-of-services and acceptance definitions; per-tier guarantee levers (§ 4) including the SPEC-01 money-back posture; SPEC-03 variable-total mechanics (deposit against floor, total locked at scope sign-off); care-plan terms (required-while-hosted, billing start, cancellation/off-boarding, reasonable-efforts uptime posture — **no SLA at these price points, stated plainly**); white-label rider (customer brand warranty + publishing license to OPS, OPS publisher-of-record disclosure incl. the seller line, App Transfer off-boarding at billable hours); **SPEC-03 exclusivity clause, scoped precisely (Jackson, 2026-07-14):** OPS will not sell or transfer the customer's app itself to another company. OPS retains **unrestricted code-reuse rights** — up to and including building a functionally equivalent proprietary tool for another customer, and folding components into OPS's own products. Exclusive to the customer: their instance, their brand, their configuration, their data, their license. Not exclusive: the code, the patterns, the concept. The ToS states the reuse right plainly; page copy sells the app's exclusivity ("built only for you") and never promises concept or code exclusivity. IP otherwise unchanged (OPS owns code, customer licensed; brand assets remain the customer's). Quebec exclusion, prohibited workflows, refund matrix skeleton, LIL cap all carry from the fourth pass.
