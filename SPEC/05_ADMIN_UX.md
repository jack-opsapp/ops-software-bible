...
- Slots consumed per tier (precise — admin RLS, not the sanitized public view) — e.g. `SPEC-01: 2/6 · SPEC-02: 3/3 · SPEC-03: 1/1` (tier designations per 10 § 2; every tier string in the console renders from the single designation map in `src/lib/admin/spec-tiers.ts`)
- Hold breakdown per tier — e.g. `SPEC-02: 1 ops_blocked · 2 customer_requested`
...
- **`// LOCKED TOTAL` panel — SPEC-03 only** (fixed-total tiers never render it). The quoted total's home is the scope doc the customer signs, so the operator locks it here, on the current draft, before the doc goes out:
  - Always shows the state chip (`NOT LOCKED` / `LOCKED`), the figure (`—` until locked), `[SPEC-03 · FLOOR · $25,000]`, and once locked the split `[P1 $6,250 · P2 … · P3 … · P4 …]` (residual cents on P4).
  - **Open** (current doc unsent + unsigned, no P2 invoice, engagement not closed): dollar input + `LOCK TOTAL` (or `RE-LOCK` once locked). One action (`lock-total`) writes `content_json.locked_total_cents` + a recomputed `content_hash` on the current doc and `spec_projects.locked_total_cents`, logs a `spec_communications` system row (`Total locked at $31,000 on scope doc v2 (SPEC-03 · floor $25,000)` / `Total re-locked $31,000 → $32,500 …`) and an operator notification (`spec_total_locked`, `VIEW MILESTONES`).
  - **Closed** — the form is gone and the blocker is named: `[V2 SENT — CUT A NEW REVISION TO CHANGE THE TOTAL]` · `[SIGNED · SEP 05, 2026 — CHANGES GO THROUGH A CHANGE ORDER]` · `[P2 INVOICED — TOTAL IS BINDING]` · `[DRAFT V1 FIRST — THE TOTAL LIVES ON THE SCOPE DOC]` · `[ENGAGEMENT CLOSED]`. A doc whose content carries a different figure than the project shows `[V3 CARRIES $30,000 — RE-LOCK TO SYNC]`.
  - Validation (server + shared pure module): integer cents, `≥ $25,000` floor (`SPEC_TIER_TOTAL_CENTS.spec03`), `≤ $21,474,836.47` (int4 ceiling); the `NEW SCOPE REVISION` control carries the figure forward in `content_json`. Money never moves early: P2 keeps its `scope_signoff` prerequisite on the Milestones tab.

### Tab 5: Milestones

- Checkpoint table from `spec_payments`, shaped per tier (SPEC-01 P1 + P4; SPEC-02 P1–P4; SPEC-03 P1 against the floor, P2–P4 from `spec_projects.locked_total_cents`). While a SPEC-03 total is unlocked the summary line reads `FLOOR · FROM $25,000` and links `LOCK TOTAL ON SCOPE DOC →` to Tab 4; once locked it reads `LOCKED TOTAL · $31,000`.
...
- ~~Subscription multiplier estimate~~ — retired with Tier Model v2 (10 § 5); the column persists unread and the editor no longer shows it (2026-09-05). Care plan monthly ($, stored as `retainer_monthly_cents`) takes its place in the PRICING section
