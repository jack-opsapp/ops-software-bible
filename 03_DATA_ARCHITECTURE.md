...
The schema is live but dormant. Active MCP v2 remains read-only; no production client or grant selects v3, no routine row exists, and the worker flag is absent. OPS-Web production-readiness release `1945c60b` includes the authenticated Connected agents configuration control and offset cron registration, but the control is invisible without an eligible v3 grant and the cron route exits through its disabled path while the flag is absent.
