...
## Phase17 conversational catalog authoring — dormant candidate (2026-09-08)

The Phase17 web implementation adds `private.agent_catalog_proposals` and an empty `private.agent_catalog_effect_policy` through unapplied migration `20260908221635_agent_catalog_authoring.sql`. This is candidate source, not an applied production migration. It does not reconstruct the unshipped August catalog-authoring kernel.
...
Disposable PostgreSQL concurrency tests verify NOWAIT contention, other-company writer release after a stalled save, rollback, exact retry and receipt replay, shorter statement/transaction timers, and elapsed-time preservation. This is direct PostgreSQL evidence. A pooled/PostgREST disconnect-and-retry canary is required before activation; browser visual and signed-in native-host proof also remain unverified.
