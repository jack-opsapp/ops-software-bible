...
### Complete MCP read catalogue P2 (production-applied 2026-08-29; externally dark)

Production has applied all thirty-nine ordered migration files for manifest-v8 compatibility, versioned OAuth consent, strict Codex DCR callback registration, durable MCP limiting/evidence redemption, shared attention projections, and twenty-three new purpose-built reads. OPS-Web production HEAD `a567902f86946436385244acc1fd0154afe78a1e` is served by READY Vercel deployment `dpl_FGXsf3DT5rc3n1coooSyq3yuFLFc` at `app.opsapp.co`; it includes both live post-release auth repairs, preserving the sanitized MCP authorization return path after sign-in and using an on-page Firebase popup for MCP-specific Google/Apple login after the live full-page redirect returned no session. The Bible's migration archive mirrors the applied ordered wave.
...
The active production policy identity is `2026-08-22.capability-manifest.v8`. The compatibility migration preserves v6/v7 output values and recursively re-proves them only when v8 is requested. New P2 RPCs accept v8 only. The frozen v7 manifest remains byte-identical, and exposure remains independently versioned at `2026-08-22.mcp-exposure.v1` with exactly eleven tools/seven grantable scopes. The other twenty-three reads are deployed dark, no thirty-four-read exposure revision has been built, and no external write is available.
...
## MCP OAuth Authorization Server Schema (base applied 2026-08-18; P2 consent/Codex extension applied 2026-08-29 UTC)

Base ledger `20260818155813_mcp_oauth_authorization_server`, consent-catalog ledger `20260823072837_mcp_oauth_consent_catalog_versioning`, and Codex callback ledger `20260829192448_mcp_oauth_codex_dcr_callbacks` are mirrored in `migrations/` and production-applied. This is storage for the remote MCP mount — see `04_API_AND_INTEGRATION.md` § "OPS Remote MCP Server — P1 Mount". The consuming routes are deployed at `https://app.opsapp.co/api/mcp`; a dynamically registered Claude client and one unrevoked grant were verified in production on 2026-08-20. Codex completed DCR and OAuth in production through the system browser on 2026-08-29 after the in-app browser returned Firebase `auth/network-request-failed`; the enabled `ops_-_maverick_projects` connector and one exact read-only `search_jobs` canary establish host acceptance for the verified read path.
...
### Codex DCR callback policy (production-applied 2026-08-29)

Migration `20260829192448_mcp_oauth_codex_dcr_callbacks.sql` replaces only the current eight-argument `register_mcp_oauth_client_as_system` body. It adds no table, column, index, trigger, data backfill, grant type, scope, or external capability. It is production-applied as part of the thirty-nine-file P2 wave.

The registration RPC now mirrors the application callback boundary: either the two exact Claude HTTPS callbacks, or exactly one Codex callback with raw shape `http://127.0.0.1:<1-65535>/callback/<8-128 base64url characters>`. Mixed Claude/Codex arrays and multiple Codex callbacks are rejected. Existing client rows and grants are unchanged. Every later OAuth function continues using exact array membership or equality, so consent, code creation, and token exchange never substitute ports, callback identifiers, hosts, or paths. A failed wrong-port code exchange leaves the code unconsumed; an exact retry remains valid. OAuth metadata and the MCP bearer challenge passed live checks after deployment. MCP-specific Google/Apple login uses an on-page popup because the live full-page Firebase return produced no session. Codex then completed DCR/OAuth through the system browser after its in-app browser returned Firebase `auth/network-request-failed`; `codex mcp list --json` reported the production connector enabled with OAuth, and `search_jobs` with `{"lifecycle_states":["active"],"limit":1}` succeeded without printing business data. The constrained Codex canary report did not surface an explicit contract-version label.
