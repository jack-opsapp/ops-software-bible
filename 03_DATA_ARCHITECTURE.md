...
Production has applied all thirty-nine ordered migration files for manifest-v8 compatibility, versioned OAuth consent, strict Codex DCR callback registration, durable MCP limiting/evidence redemption, shared attention projections, and twenty-three new purpose-built reads. OPS-Web production HEAD `87abc02ac3205254c1446bb50f7eda3607000140` is served by READY Vercel deployment `dpl_6oUh3jTry9DWRAuTD1pnPSqKLRaA` at `app.opsapp.co`; it includes the live post-release auth continuation repair that preserves the sanitized MCP authorization return path after sign-in. The Bible's migration archive mirrors the applied ordered wave.
...
The registration RPC now mirrors the application callback boundary: either the two exact Claude HTTPS callbacks, or exactly one Codex callback with raw shape `http://127.0.0.1:<1-65535>/callback/<8-128 base64url characters>`. Mixed Claude/Codex arrays and multiple Codex callbacks are rejected. Existing client rows and grants are unchanged. Every later OAuth function continues using exact array membership or equality, so consent, code creation, and token exchange never substitute ports, callback identifiers, hosts, or paths. A failed wrong-port code exchange leaves the code unconsumed; an exact retry remains valid. OAuth metadata and the MCP bearer challenge passed live checks after deployment, and Codex DCR discovery and registration work in production. The full host canary is awaiting the user's final browser consent, so host acceptance is not yet claimed.
