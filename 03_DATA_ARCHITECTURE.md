...
The approved scope-order, conversation, all-day task and read-input repairs are live on OPS-Web `2a349d8182e449af8891332f5b9636f3121df652`, with production migration `20260905191357_agent_maverick_read_repairs`. Authenticated reads pass and task detail agrees with project summary. The unchanged-title probe exposed lost timestamp precision and misleading no-change handling; the additional repair is now live with production migration `20260905235844_agent_customer_update_source_precision` and web integration commit `e7c530015`. Its final signed-in check is blocked by this task’s OPS connector startup timeout; local and production database verification pass. Production readback confirms unchanged business/grant state and zero proposals. Exact release evidence, local verification and the separate unresolved B.C. timezone-data mismatch are recorded in [Maverick MCP repairs](specs/2026-09-05-maverick-mcp-read-repairs.md).
