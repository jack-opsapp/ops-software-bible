...
**`line_items.configured_options` shape** — keys are `product_options.id`; values are the `product_option_values.id` for `select`, a JSON number for `integer`, a JSON boolean for `boolean`. Both editors materialize every option's `default_value` when a product line is created (web: `product-configuration-resolver.ts`; iOS: `LineItemEditSheet.seedDefaultsForOptions`). Before 2026-09-15 the web resolver dropped `integer` and `boolean` options entirely, so every web-authored line hit the step 3 fallthrough above.
