...
The repair's `OPS/Utilities/OPSStorageBootstrap.swift` owns a single asynchronous container-open/migration task away from the main actor. `OPS/OPSApp.swift` installs `OPSInitializedRoot` and its context-dependent `DataController` only after successful opening. Failures retain the same store and display `OPS/Views/Storage/StorageRecoveryView.swift` with same-location retry; no deletion, metadata rewriting, replacement empty database, or silent unsent-data reset is permitted. This gates context-dependent startup; normal process-level Firebase/notification initialization is not claimed to stop.

**Verified locally:** 68 focused tests passed, none failed, and the optional external V15 private-store fixture test was skipped. Executed coverage includes all declared V1–V26 fingerprints, synthetic populated V16/V25 upgrades, reopening a store with the widened debug-V25 graph, stopped-outbox/photo-byte preservation, unknown dirty merge bases, and storage-failure/retry/concurrent-open behavior. Sources: `OPSTests/DataModels/AppUpdateMigrationTests.swift`, `DeckMergeBaseMigrationTests.swift`, and `StorageBootstrapTests.swift`; integration artifact `docs/artifacts/ios-performance-p1-1/storage-test-summary.json`. An additional opt-in test executed against a private disposable copy of the exported released V25 phone store:1passed,0failed,0skipped. It migrated to V26 and independently reopened, preserving counts and per-run-keyed content/custody digests across16groups including all967 ProjectPhoto records and2636 outbox records. Dirty deck merge bases remained unknown/unsent. The exported original stayed byte-identical during the test; private export/test copies were removed afterward. Referenced external asset bytes were not read by this migration proof. Summary: `ops-ios/docs/artifacts/ios-performance-audit-20260906/private-store-upgrade-summary.json`. Actual phone installation, optimized device performance, rendered storage-recovery-screen proof, and release remain separate unverified steps.
...
### 22C-1. Guarded visit stage storage (applied 2026-09-07 UTC)

Source: `ops-web/supabase/migrations/20260906171832_site_visit_stage_commands.sql`, isolated branch `codex/ios-performance-p1-5`, final commit `26816f355f55807232c8c9f8cb3b406d4314d948` (following `caec2dfa` and `e95d12a0`). Applied to `ops-app` (`ijeekuhbatykdomumfjx`) with Jackson's explicit approval at ledger version `20260907001000`, name `site_visit_stage_commands`. The exact ledger SQL is archived at `migrations/20260907001000_site_visit_stage_commands.sql`; SHA256 `2f642640d3be1fc273e6bef82ccdc9f0f149d9dac3bbf1fc950dd41b9df696a6`. Independent catalog readback exactly matches the tested function definitions, ACLs, RLS and trigger; legacy move MD5 remains `34ce4987e8411de6bfbe8d93dcec7afa`. No public opportunity column or SwiftData field is added by this server contract.

- `private.site_visit_stage_revisions`: `opportunity_id uuid PRIMARY KEY REFERENCES public.opportunities(id) ON DELETE CASCADE`, `revision uuid NOT NULL`. An AFTER INSERT/UPDATE trigger rotates the token only for stage, assignment, manual-boundary or lifecycle changes; ordinary notes do not rotate it. Existing rows expose a reserved zero UUID until their first relevant write; no data backfill. One MVCC snapshot returns matching lead state/token. ABA stage changes rotate the token even when timestamps coincide.
- `private.site_visit_stage_command_receipts`: `command_id uuid PRIMARY KEY`, `actor_user_id uuid NOT NULL`, `company_id uuid NOT NULL`, exact `request jsonb` and immutable `result jsonb` object envelopes, `created_at timestamptz NOT NULL DEFAULT clock_timestamp()`. Contains bindings, finite guard/stage outcomes, tokens and timestamps, not photos/notes. No cascading FK or expiry: a removed/recreated entity cannot make an old command executable again. Do not prune/reset receipts during rollback or reenablement.
- Both tables enable RLS and revoke all table access from PUBLIC, anon, authenticated and service_role. Only the guarded private helpers behind authenticated public invoker RPCs access them; every new function pins an empty search path. Trusted actor/company resolve through the existing Firebase-compatible private identity helpers. Request identity fields are equality assertions, never authority.

The no-write `already_satisfied` outcome stores a private receipt with null transition_id when an eligible completed visit's current lead already equals its requested target. It makes no stage, transition, manual-attribution or notification write. Exact replay preserves that historical receipt even after later movement. Existing conflict receipts stay conflicts. API and lifecycle details are in chapters04/10.

Verified locally:34/34 synthetic PostgreSQL17 tests passed in5.091seconds, including the captured real activity auto-advance and legacy/manual-boundary function bodies, replay/ABA, permission/account changes, row-lock inversions, immutable refusals and transaction rollback. The original legacy move function MD5 stays `34ce4987e8411de6bfbe8d93dcec7afa`. External delivery sinks are synthetic; production migration, live authenticated readback, provider fan-out and iOS release remain unverified separate gates. Proof: worktree `docs/artifacts/ios-performance-p1-5/test-results.txt` and `HANDOFF.md`.

### 22C-2. Local capture journal and photo custody (2026-09-06; integration source, unreleased)

The IOS PERFORMANCE integration branch adds a file journal independent of frozen SwiftData models. Source: `OPS/Utilities/DurableCaptureStore.swift`, `StagedPhotoDestinations.swift`, `ProjectPhotoFormDraftStore.swift`, `LeadImageStager.swift`, and the camera/lead/project adapters. Final source amendments: `ops-ios` worker commits `7690fb0c`, `ce1def1b`, `ce233114` following the core media/cache commits; the source is locally integrated. All eight media suites (35 cases) passed in the combined core03 run. No backend column, endpoint or SwiftData model is added by this slice.

`Documents/CaptureBatches` holds a versioned exact `(companyID,userID,contextID)` batch manifest; unchanged source bytes and normalized JPEGs live under `Documents/ProjectImages`. Stable batch/item UUIDs survive retry, with item ID used as destination artifact/photo ID. Journal intent precedes the atomic original-byte write; accepted capture state retains bounded thumbnails instead of full image batches. Higher manifest versions are not rewritten or deleted. New custody semantics/required fields require a version increment.

Custody order: durable original → typed local destination receipt → canonical remote delivery → durable local model healing → journaled retirement intent → original cleanup. Camera `onStagedUpload` returns true only after destination models/outbox or the project-form draft receipt are durable. Failed host save/acknowledgement reuses the same IDs. No age/cache policy prunes unsent originals. Direct recovery and account indexing throw on unreadable/incompatible journals; missing/corrupt custody is never reported as empty. Prepared siblings remain recoverable when another image is invalid. Retained form recovery returns successful items plus explicit failed-item metadata.

Lead destinations adopt existing staged URLs without reencoding. Existing project destinations commit stable ProjectPhoto rows and the local URL projection in an owned ModelContext; upload recovery uses existing pending-row reconciliation. Canonical readback checks photo ID/project/company/uploader/live URL. Local healing must save successfully before upload queue/journal retirement. Original actor/company is frozen before delivery and revalidated after network/manifest awaits, before healing and inside retirement; an account switch retains original-account custody.

`Documents/ProjectPhotoDrafts` reserves an exact project UUID before camera entry and stores photo batch IDs plus identifying/basic form fields. Resume/discard is explicit and account-scoped. The eventual project and its create SyncOperation save atomically using that reserved ID before photo transfer. Closed markers prevent delayed draft-field saves from resurrecting a completed draft. Partial transfer leaves damaged originals and the draft available while valid siblings preview/transfer; discard preserves photos already owned by ProjectPhoto rows. This is a photo draft, not a promise to persist every field in the general project form.

Photo performance source: `PhotoCacheLedger` serializes byte reservations and incremental writes/overwrites/deletions; prefetch reconciles once on a background pass rather than walking all directories per image. PhotoProcessor and ImageFileManager participate in accounting. Protected local originals/composites remain excluded from eviction. `StorageProfiler.budgetSnapshot()` reads its thread-safe persisted scalar without the main-actor singleton. `PhotoThumbnailLoader` uses bounded asynchronous ImageIO downsampling with orientation, local composite/source precedence, real remote fallback, per-waiter cancellation and cache-arrival invalidation (`sourceURL=*` denotes clear).

Verified proof is in eight `OPSTests/Media` classes:35/35 cases passed in integration core03, covering interrupted writes/cleanup, identity changes, partial originals, exact-owner reopen, stable replay, parent-create custody, byte accounting and downsampling. The larger core03 run had287passes and5visit test failures under repair; media success is not a claim that the full repair is complete. Optimized device latency and customer distribution remain unverified.

### 22C-3. Capture transaction ownership (2026-09-06; integration source, unreleased)

`SiteVisitCaptureViewModel` retains its own autosave-disabled ModelContext. `SiteVisitPersistenceCoordinator` commits the exact visit/entity scope and durable operation changes together; ordinary edits do not perform a global orphan sweep. Caller-context pending work is neither saved nor rolled back. Shared-context consumers use isolated adapters: capture entry/console/deck, dimensioned capture, Pending Work, local queued-lead delivery and `SiteVisitTypeSeedStore`. A no-op template seed does not save. Never-synced blank/scanned decks are explicitly marked for first delivery before the capture screen opens them; a failed host save retains the drawing for retry. Detached Opportunity snapshots use the complete canonical `apply(_:)` mapping, including assignmentVersion and summary/geometry fields.

`SiteVisitIdentityClientStore` runs synchronously inside the capture-owned coordinator transaction. New/updated Client, deduplicated extra SubClient contacts, standard client/contact SyncOperations and draft.clientId commit or roll back together. The generated client ID is persisted before lead delivery and reused on retry. A post-commit `notifyDurableOperationQueued()` wakes transport without saving DataController's shared context. Payload keys use the server convention, while changedFields also includes the local camelCase names used by inbound dirty-field protection. In-progress/stopped owners retain their payload and dependency ordering. Local existence does not prove the server has the parent: pending creates require visibility, and rejected creates route to durable review without a direct lead RPC bypass.

Contact clears retain the existing explicit JSON-null contract. Blank capture notes preserve existing client notes, including old resumed client-bound drafts; only nonblank capture notes update them. Draft saves remain available before completion; completion validates only applicable required answers. Stage intent retains the original reviewed snapshot as described in22C-1. Source includes worker commits344c3c7b,fad0e453,93eac1ec,ea5d8798,2ef96911. Host/seed/client/queue/continuity and typed-delivery tests passed across core04 and the final core05 rerun. Core05 executed127cases with0failures/0skips after correcting stale-object/network-race fixtures; the real queue-to-open-visit observer path is covered. All source is on local main622010a0. The optimized unsigned generic iPhone build and two real contact-picker UI cases passed; physical-device latency remains unmeasured. No new persistent model or server schema is introduced by this ownership amendment.
...
| `public.project_server_state(p_project_id uuid) → text` | `active` \| `deleted` \| `absent` \| `unknown`. Company-scoped (another tenant's row reads `absent`), SECURITY DEFINER, **granted to `anon`** — load-bearing under the Firebase JWT bridge, where the app executes as `anon`. Deliberately reveals deleted-ness to company members: RLS SELECT hides soft-deleted rows from everyone, so this RPC is the only way a device can learn a deletion happened. Unresolved JWT/company ⇒ `unknown`: retain pending media and retry; it is not evidence of a missing project. Original migration `cluster_j_03`; corrected by `20260904223347_project_server_state_unknown_verdict.sql`. Production definition independently verified 2026-09-14 (MD5 `79c400ac93cecf1dea2e15b451341f4e`). |
...
#### Note attachment edits (2026-09-14; applied 2026-09-15 UTC)

Applied `20260915021754_project_note_mentions_attachments.sql` adds nullable `requested_attachments jsonb` and `attachments_snapshot jsonb` to the immutable mention-edit event table. It is byte-identical to the prepared `20260914224026` artifact (SHA256 `863b2db89142bf2cc3728fb389786489ac71bd6e5d4ff82794baa3fd876e1d1d`); filenames now match the observed production ledger. Historical events remain untouched. One defaulted five-argument RPC replaces the existing four-argument signature without overload ambiguity. Explicit arrays are removal-only ordered subsequences; blank legacy entries cannot keep an otherwise empty note alive. Omitted/SQL-NULL attachment input preserves the existing text/mention response and UPDATE-column semantics. The project gallery and `photo_url` subject are untouched.

Function/helper/schema/ACL/dependency and proof-storage guards passed before the explicitly approved replacement. Existing author/company/mention authority and exact replay are preserved; the recreated function grants EXECUTE only to postgres, anon and authenticated. OPS-Web local main `c7098c283` contains the migration under its applied ledger version. Independent disposable PostgreSQL verification passed 62 SQL assertions plus 24 concurrency/migration-adversarial checks. Live readback at `2026-09-15T02:18:43.07205Z` confirmed the single five-argument function, expected MD5 `2bbb8d6782b140843ab050fabe72431b`, new nullable columns and unchanged security/immutability protections. Three anonymous no-change API probes reached the expected legacy validation, new attachment validation and actor-denial guards. No customer note was edited for verification.

A related mixed-media/text discard repair is integrated on local iOS main as `b1cdc644` after 118 focused app-hosted simulator tests passed, including all 13 new regressions. `OPS/Network/Sync/ProjectNoteMentionEditSync.swift` projects media independently of the newest text edit and rebases only the next surviving media operation's rollback baseline. `SyncEngine.swift` guards every rebased/rewired row against active execution. Failed cancellation restores only its owned attachment-baseline key, preserving unrelated concurrently committed payload metadata and absent/null legacy fields. Entire app, test and project source trees match the passing checkout `7ef35203`; independent review found no actionable issues. This is local iPhone 17 / iOS 26.5 simulator proof, not a phone installation or release. The server migration was subsequently applied independently; original-phone acceptance and release of the local iOS correction remain pending. Evidence: `ops-ios/docs/artifacts/ios-bugs-release-20260914/photo-attachments.md`.
...
- `PUT /api/users/[id]/permission-overrides` — body is exactly `{ expectedOverrides, set, clear, assignmentResolutions }`: `expectedOverrides` is the member's whole canonical override snapshot as the editor loaded it; `set` rows are `{permission, scope, granted}` (registered permission, an offered scope when granted, `scope: null` when not); `clear` is registered permission ids; `assignmentResolutions` is `[{opportunity_id, expected_assigned_to, expected_assignment_version, new_assigned_to}]`; every list strictly ascending. Answers (verified end to end 2026-09-18, route file `src/app/api/users/[id]/permission-overrides/route.ts`): **200** `{ ok, userId, overrides, resolvedAssignments }`; **400** `invalid_request` (route shape/registry) or the save's own code (`invalid_permission_dependencies`, `invalid_override_set_clear`, `stale_company_override`, `protected_permission_override_invalid`, `missing_resolution`, `extra_resolution`, `duplicate_resolution`, `no_op_resolution`, `assignment_target_ineligible`, …); **401** `unauthorized`; **403** `access_denied` (actor lacks `team.assign_roles` at `all`, or another company); **404** `target_user_not_found` (missing, deleted or deactivated); **409** `target_is_admin`, `permission_snapshot_mismatch` (+ `currentOverrides`), `assignment_resolution_required` (+ `strandedCount`, `stranded`, `eligibleAssignees`), `assignment_resolution_conflict` (+ `opportunity_id`, `assigned_to`, `assignment_version`); **500** `permission_update_failed` (anything else; logged as `Guarded RPC failed` with the Postgres code). A committed change writes one `user_permission_change_deliveries` row per member and transaction (`change_kind user_override`) and re-queues the member's email-signature check for each company mailbox. Powers the Team member access editor (web) and iOS `PermissionAdminService`.
  - **Write path (since 2026-07-15):** the route calls `public.apply_user_permission_overrides_as_system(actor, target, expected_overrides, set, clear, assignment_resolutions)` (service-role only). **2026-09-18 outage fix:** from `20260715161000_lead_assignment_permission_migration` (re-issued by `20260715180900_internal_spec_permission_guard`) until ledger `20260918022722_permission_overrides_clear_alias`, the function's `p_clear` validation aliased `unnest(p_clear) permission` and joined `private.lead_permission_editor_registry` (which also has a `permission` column) — Postgres raised 42702 *column reference "permission" is ambiguous* on **every** call, so every individual permission save from web and iOS answered 500 `permission_update_failed` and wrote nothing (last successful `user_permission_overrides` write: 2026-05-26). The fix replaced the function from its md5-guarded live definition, changing only that validation to the qualified alias `cleared(permission)`. Regression guard: `tests/unit/supabase/permission-overrides-clear-alias-migration.test.ts` (ops-web). Rule: never alias a set-returning function with the same name as a column it is later compared against.
  - **End-to-end proof and regression guard (2026-09-18):** `scripts/test-permission-overrides-postgres.sh` (ops-web) builds a disposable socket-only PostgreSQL 17 cluster (ICU `en-US` collation and UTC, as production) carrying the save's whole execution closure as production runs it — 76 functions, 31 tables, 19 triggers, grants, PostgREST identities and reference data, proved by md5 in `tests/sql/permission-overrides-fidelity.sql` — puts a real PostgREST in front of it, and drives the real route through supabase-js with payloads built by the editor's own helpers (`tests/integration/permission-overrides-postgres-runtime.test.ts`, 20 scenarios; skipped in a plain `vitest` run). It reproduces the outage (500, 42702, nothing written), installs `20260918022722` under the running API (the save becomes md5 `8e1cb41e52232d3217024a68baed4e27`, production's live definition), then proves each outcome with a real COMMIT: grant, clear, set and clear together, switching off a role grant, a no-op, a stale screen (409 with the current snapshot), no authority / another company / an admin / a missing person, the dependency refusal, the database's own clear-list validation for callers that skip the route, the internal SPEC override kept untouched, corrupt overrides refused, the lead-handoff prompt and every refused handoff, and a handoff that unassigns one lead and transfers a website contact-form lead (the departing holder's pending first-reply draft goes `stale`; a new one queues for the new holder). Every save-path function that can run must then have run (59 of 76; the 17 that cannot run during a save are listed with reasons in `tests/sql/permission-overrides-coverage.sql`). Without the repair, 12 of the 20 scenarios fail. `scripts/capture-permission-overrides-fixture.py` regenerates the fixture from read-only production metadata. **When a migration changes anything on this path, append it after the repair in the script and the runtime test, and extend the scenarios.**
  - **Why the outage shipped:** `tests/sql/lead-assignment-permission-contract.sql` exercises exactly this save (set, clear, resolutions), but nothing executes it — unit tests only read it as text, and the repair's first guard reads its migration as text too. A proof that rolls back never runs `trg_user_permission_overrides_final_state` (`DEFERRABLE INITIALLY DEFERRED`); it first ran on this path in the harness. As of 2026-09-18 production held zero `opportunity_assignment_events` with source `permission_change`: the permission-change lead handoff had never succeeded for any mutation kind. The editor-registry test (`tests/unit/supabase/lead-assignment-permission-migration.test.ts`) now folds every registry write in migration order instead of comparing against the July migration alone; the fold reproduces production's 104 rows.
...
**iOS source correction (2026-09-10).** Local iOS commit `5b254437` routes `TaskRepository.softDelete` through `public.soft_delete_project_task` and `restore` through `public.restore_project_task`; both outbound drivers split lifecycle fields through `TombstoneFieldSplit`. The earlier statement that the current iOS repository still directly PATCHes `deleted_at` is obsolete. Read-only production inspection on September 10 confirmed the task read policy still hides tombstones and the restore RPC exists. An installed app version alone does not identify the source commit in that binary.

**Invisible task rows are not deletion proof (2026-09-10).** The authenticated task read policy can hide a row because it is deleted, out of scope, or behind an inaccessible/deleted parent. Consequently a zero-row UPDATE or empty follow-up SELECT must not complete a pending schedule, clear local changes, or infer a tombstone. iOS `SyncOperationReconcilers.reconcileTaskUpdate` accepts a tombstone only from an explicit matching task/company row, protects unresolved creates/restores, and retains schedule-bearing refusals for recovery. No RLS change or production data repair accompanied this local client fix. See Chapter 07, Schedule Entry Contracts and terminal-failure reconciliation.
...
| `project_photos_update_grants_and_write_guard` | Column-scoped `GRANT UPDATE (deleted_at, is_client_visible, caption)` to `anon, authenticated` + `BEFORE UPDATE` guard `trg_project_photos_00_write_guard`. Full contract and rationale in `07_SPECIALIZED_FEATURES.md` § Synced `project_photos` gallery store. Bug `1154fe67`. **Verify this grant with `information_schema.column_privileges`, never `role_table_grants`** — the latter reports only TABLE-level grants and shows `anon`/`authenticated` holding `INSERT, SELECT` alone, which misled the 2026-08-28 sweep into re-reporting this as unfixed. A widening to `(… , taken_at, thumbnail_url, rendered_url)` is authored and **awaiting apply** as `migrations/20260829021000_project_photos_client_update_grant.sql`. |
...
**Override level rule (2026-09-16).** `price_override`, `unit_cost_override`, `warning_threshold`, `critical_threshold` and `unit_id` are two-level fields: NULL inherits the family default (thresholds then the category default), so the next family edit reaches the variant; a set value pins it. The MCP catalogue setup writes never change that level as a side effect — a family write sets only the family default, and a variant write whose value equals what the variant inherits leaves it inheriting (clearing any override it carried). `public.commit_catalog_setup_write_as_actor` enforces it: any write that changed one of the five to a non-NULL value equal to what the variant inherits is refused with `CATALOG_SETUP_OVERRIDE_LEVEL_CHANGED` (SQLSTATE `55000`) and rolled back. Redundant overrides a write did not touch never trip it — production has 31 variants whose `unit_id` equals the family `default_unit_id`, created outside MCP. The guard lives in that MCP commit only; no trigger or constraint enforces the rule on other writers. Chapter 04 § Catalogue setup write tools, *The override level rule*.
...
Web viewer rebuilt 2026-09-15 (bug `b130d23f`, also closes `acc0d021`; branch `fix/web-deck-viewer-parity-20260915`, on main via PR #126 → `b3634b2c0`, 2026-09-16): the read-only surface above is superseded. `DeckDesignService.fetchForOpportunity` / `fetchForProject` / `fetchDesignWithDrawing` now select the FULL `drawing_data` blob (multi-level decks keep their geometry under `levels[]`, which the old root-only `vertices`/`edges` selection never saw — a multi-level design rendered as an empty glyph). A client-safe port of the server geometry calculator (`src/lib/deck/drawing-data.ts`, pinned to the `__fixtures__/ops-ios` and `__fixtures__/ops-decks-ios` golden files) turns the blob into faces, edges, dimensions, stairs and levels in the browser; `deck-plan-svg.tsx` draws it as live SVG and `deck-viewer.tsx` hosts it fullscreen (`z-modal` portal) with pan, anchored wheel/pinch zoom, double-click fit, a measure tool (`src/lib/deck/measure.ts`, ported from the iOS `DeckViewerToolState` snap/readout rules), a labels toggle, a level isolator, and a lazily loaded React Three Fiber 3D massing (`deck-scene-3d.tsx`, `src/lib/deck/scene-plan.ts`). Surface names use the same inscribed-rectangle placement as iOS (`src/lib/deck/label-placement.ts`). The pipeline deal panel's `// DECK DESIGN` section opens it, and the project workspace Details tab gained the same section (`project-deck-section.tsx`) — the first place the web has ever rendered a project's deck. Authoring is still iOS-only.
...
Preview placement basis updated 2026-09-05 (bug `311ccfe5`): `DeckFramingPreviewPlanner` no longer places members from unsourced constants. The 12 in beam setback, the 8 ft beam-multiplication cap, the 72 in post cap and the 16 in joist spacing are gone. Placement now comes from `DeckSpanTables` — the Hem-Fir No.2 **incised** rows of the Canadian Wood Council *Prescriptive Residential Exterior Wood Deck Span Guide*, Rev.1 (2016), Tables 3b / 5b / 7b plus Note 5 — solved by `DeckFramingSolver` for the fewest posts every published limit allows. The outermost beam sits a full published cantilever inboard of the outer edge (16 in on 2x8, 24 in on 2x10 and 2x12), post spacing is whatever the beam table permits (up to 8 ft), and the overhang is emitted as a `.cantilever` member. A 16 ft x 12 ft ledger-attached deck goes from two beams and eight posts to one beam and three.

**The transient contract above is unchanged.** Preview sets are still never saved, synced, sized, or emitted as authored framing; `DeckDrawingData.framing` remains get-only; `FramingMember.sizing` stays nil; `DesignToEstimateAdapter` still reads persisted `components[]` and never touches the plan. The change is rendering, not authoring.

Standing of the numbers: the CWC guide is an industry guide based on NBC 2015 and CSA O86-14, reproduced for British Columbia use by BC Housing's *Illustrated Guide: Building Safe and Durable Wood Decks and Balconies* (p.12). It is **not** BCBC text — BCBC Part 9's floor-joist tables are interior-floor only (9.23.4.1.(1)), carry no wet-service or incising reduction, and Part 9 has no deck-joist cantilever article. The incised tables already include the treatment and wet-service factors, so no further reduction is applied. Posts default to 6x6 per BCBC 9.17.4.1. The tables assume a 1.9 kPa live load and OPS has no snow-load input, so the output is optimistic in high-snow locations. Output is labelled `ILLUSTRATION ONLY` in-product on all three surfaces that render it (project deck tab, fullscreen viewer, deck builder), with an extra line whenever the layout falls outside the tables. The full citation record — documents, editions, table numbers, verbatim design assumptions, and what is deliberately NOT encoded — is vendored at `ops-ios/docs/reference/deck-span-tables/`.

Cross-repo status: DeckKit (`ops-decks-ios`) has the sizing structs (`BeamSpanSizingRow`, `PostHeightSizingRow`, `StructuralSizingEngine`) but its `CodePackages/CA-BC-2024.subset.json` is still empty — `beamSpanTable: []`, `postHeightTable: []` — and the two repos share no code. `DeckSpanTables` mirrors DeckKit's field naming (`maxSpanFeet`, `codeSection`, `limitingCheck`, `maxSpacingInchesOC`) so a later merge is mechanical, but no SPM link was added.
...
**The live engine (verified against `origin/main` a06f6f5e5 and prod, 2026-09-15).** One consumer runs today: `private.resolve_estimate_material_demand_plan(p_estimate_id, p_project_id)`, reached only through `public.accept_estimate_to_job` → `private.persist_estimate_material_booking_projection` (and the QuickBooks twin `accept_estimate_to_job_from_quickbooks`). Per selected line with a product, per recipe row:

1. `catalog_variant_id` pinned → use it. Else resolve the family through `private.resolve_catalog_variant_for_material_demand` with `variant_selector`. Zero candidates → warning `catalog_variant_not_resolved`; several → lowest uuid plus `catalog_variant_resolution_ambiguous`. iOS `RecipeResolver.swift` implements the same rule. A family axis the selector does not name is not filtered on.
2. `scaled_by_option_id` null → `required = quantity_per_unit × line_quantity`.
3. `scaled_by_option_id` set → `required = quantity_per_unit × the line's configured count` for that option (the line quantity drops out). **The count must be present**: a jsonb number, or a numeric string from a legacy writer. Missing, non-numeric, boolean or null → `required = 0` plus warning **`scaled_option_value_missing`** naming the line, the recipe row and the product option — on the run's warnings and on that demand row's `warning_payload`. Until 2026-09-15 this case silently fell back to step 2, so a 20 ft railing line with no end count booked 20 end posts, 20 corner posts and 380 lag screws with no warning at all (source migration `20260915220000_recipe_scaled_option_missing_zero.sql`). **LIVE ON PRODUCTION 2026-09-15**: applied as ledger version `20260916050249 recipe_scaled_option_zero_patch`, which patches the live `pg_get_functiondef` in place and refuses unless the result hashes to the locally proved definition (`70bc716bd98576551498fe40f91c9c94`) — confirmed post-apply. The capability-key CHECK went on as `20260916045924 capability_binding_key_format`. Web ops-web `main` 88517442d deployed to production (Vercel `dpl_36s1wRfSwHECfdXjh9HsjrWLphcq`, app.opsapp.co); iOS merged to `main` 91ab8eaa, not yet in an App Store build.
4. A negative count clamps to 0 without a warning.

**`variant_selector` shape** — keys are catalog option *names* on the family, values are `"$option.<product option name>"`; both sides compare lowercased and trimmed, e.g. `{"Color":"$option.color","Mount Type":"$option.mount type","Height":"$option.height"}`. A family-pinned row with no selector (or `{}`) is silently dropped from the cut list, and `catalog-setup` payload building refuses to emit one.

**`line_items.configured_options` shape** — keys are `product_options.id`; values are the `product_option_values.id` for `select`, a JSON number for `integer`, a JSON boolean for `boolean`. A blank count has **no key** (never `0`); an entered `0` is the JSON number `0`. Both editors materialize `select` and `boolean` defaults when a product line is created, and **never an `integer` count** — not from `default_value`, not as a fallback `0` (web: `product-configuration-resolver.ts`, ops-web `a2048ce54` on branch `fix/estimate-accept-missing-counts`; iOS: `LineItemConfigurationForm.seeded` on branch `fix/estimate-counts-start-blank`; both 2026-09-17, both unmerged at time of writing — the iOS change is in no App Store build yet). A count is job geometry: acceptance refuses a blank count but cannot tell an injected `0` from a real one, so a count stays blank until the estimator enters it, and a required option left blank blocks saving the line (web: "Complete the required product options."; iOS: `// NOT ENTERED` prompt naming the options, `LineItemConfigurationForm.saveGate`). Before 2026-09-17 both editors filled counts from `default_value` (iOS fell back to `0`), which booked zero end/corner posts silently. Before 2026-09-15 the web resolver dropped `integer` and `boolean` options entirely, so every web-authored line hit the step 3 fallthrough above.

**iOS editor detail (2026-09-17)**: `LineItemEditSheet` count rows show `—` until entered; minus from blank enters `0`, plus enters `1`; `0` is the floor and a count never returns to blank. Opening an existing line resolves its product from `line_item.product_id` (the estimate form hands none in), hydrates the snapshot through `LineItemConfigurationForm.hydrated` (stored counts kept, a legacy integer string read as a count, anything else blank), and an edit saves the full snapshot + `resolved_unit_price` + `resolved_options_label` through `UpdateLineItemDTO`. `DesignToEstimateAdapter` sets a count only from a drawing measurement (`$design.<key>` or the name-matched takeoff key) that is a whole number — measured `0` stays `0`, unmeasured stays absent, `1.5`/booleans are not counts. The option authoring sheet (`ProductOptionAuthoringSheet`) no longer offers a default for `integer` options and saving one sends `default_value: null` (`ProductOptionDefaultPolicy`), mirroring ops-web `3ff27023f`. QA harness: launch argument `-OPS_LINE_ITEM_COUNTS_QA` (add `-OPS_LINE_ITEM_COUNTS_QA_EDIT` for an existing line), driven by `OPSUITests/LineItemCountsUITests`.

**Not running**: `product_material_quantity_rules` and `catalog_product_capability_bindings`. Both have a writer (the Phase C commit adapter) and readback, and zero rows in production. No web, iOS, OPS Decks or database code reads either table to compute a quantity, so `edge_length` and `cut_plan` rules do nothing; the capability registry marks `dynamic-material-quantity/v1` `unavailable` and `deck-geometry/v1` `discover_only`, and the commit path rejects both actions as `capability_unavailable`. `capability_key` is an OPS capability **registry ref** — hyphenated and versioned (`deck-geometry/v1`); the Phase C fixtures wrote `deck_geometry/v1`, which no consumer could match. Fixed 2026-09-15, with the payload contracts and a table CHECK now enforcing the format.

**How a drawing reaches the recipe (2026-09-15)**: `RailingTakeoff` (iOS, `OPS/DeckBuilder/Engine`) walks a Deck Designer drawing's railed deck edges, groups them into runs per railing system, and reports linear feet, left and right end posts, 90° corners, off-angle (45°) corners and house returns. `ComponentEmitter` emits one `railing` component per system carrying those totals, and `DesignToEstimateAdapter` writes them onto the estimate line's `configured_options` for any integer product option named after the measurement ("Left ends", "Right ends", "Corners", "45° corners", "House returns"). The line quantity is the total linear feet. The recipe does the rest through step 3 above — no rules engine involved. Verified 2026-09-15: 50 iOS tests green (22 takeoff cases incl. reflex corners, 178° near-straight joints, closed loops, T-junctions, stair/gate openings, winding independence, multi-level aggregation). **Not yet reachable in the app**: the bridge only fires for a component type with a `company_default_products` row, and production has none for `railing` — plus zero `product_options` set `option_default_source` anywhere, which is why the adapter matches count options by NAME ("Left ends", "Right ends", "Corners", "45° corners", "House returns") rather than requiring per-product wiring.
...
- **Reconciliation recovery:** claim/renew/complete/fail/release RPC family over `approved_action_email_intents` plus the `approved_action_email_reconciliation_alert_outbox` table. An optional claim with no eligible intent can arrive through PostgREST as a fully null table composite. `ApprovedActionEmailIntentService` treats only a complete all-null payload—with the identity, company, connection, status, and reconciliation lease fields present and null—as no work; an absent ID in any partial payload is an invalid response and fails closed. Source: OPS-Web `c7f0bae4`.
...
### Customer and job discovery reads (production-live 2026-08-22 UTC)

Production ledger `20260822015049_agent_discovery_reads_20260820220000`, mirrored as `migrations/20260822015049_agent_discovery_reads_20260820220000.sql` at SHA-256 `2f91ba90b01e3249180e30db0477dde1acffcc1beff1b74c6f71b9bd850570e8`, adds two service-role-only, `SECURITY DEFINER` JSONB read boundaries: `read_agent_customer_discovery_as_system` and `read_agent_job_discovery_as_system`. It advances the active read proof boundary to capability-manifest v7 by re-proving the nine existing reads and the Phase C wrapper in the same database statement; it never relabels a v6 proof in TypeScript. Customer discovery supports canonical name matching plus exact email or NANP phone selection without projecting either contact value. Job discovery searches only safe title/address/status/date identity fields and retains the Task 13 reciprocal opportunity/project identity rule.

The database boundary freezes Unicode 15.0 interchange text, excluding the 66 reserved noncharacters; rejects malformed dates, controls, bidi controls, unpaired surrogates, partial contacts, and noncanonical cursor inputs; and emits ordered child plus collection proofs under the operational read-revision fence. Exact, prefix, token, contact, assigned-scope, and filter-only branches use bounded expression/keyset indexes. Reaching the 501st inspected source row returns terminal `SOURCE_QUERY_BOUND` rather than scanning farther or claiming an incomplete empty result.
...
### Complete MCP read catalogue P2 and immutable v2 exposure (2026-08-29/30 UTC)

Production has applied all thirty-nine ordered P2 migrations for manifest-v8 compatibility, versioned OAuth consent, strict Codex DCR callback registration, durable MCP limiting/evidence redemption, shared attention projections, and twenty-three new purpose-built reads. It has also applied callback-policy source migration `20260830113800_mcp_oauth_chatgpt_rfc9207_callback.sql` under ledger version `20260830004843` and name `20260830113800_mcp_oauth_chatgpt_rfc9207_callback`. The Bible's migration archive mirrors the P2 wave and the callback source migration byte-for-byte. OPS-Web release commit `d5befc466c7dbf3d67b76cde698c9a9aa4df719c` is served by READY Git-backed production deployment `dpl_3DWhhcueWxeFW2AJnEmxT1bFhfNu` with `app.opsapp.co` attached and no alias error. Canonical and cache-bypass authorization/protected-resource metadata return the exact twenty scopes and RFC 9207 support flag; unauthenticated `POST /api/mcp` returns the matching 401 resource metadata and scope challenge. This proves the production discovery/authentication boundary, not an authenticated v2 business-data call.

The untouched v1 Codex connector `ops_-_maverick_projects` authenticated and listed exactly its eleven expected v1 tools without making a business tool call. Two fresh v2 Codex DCR clients each requested the exact twenty scopes but their callbacks expired before approval or token exchange; both had zero grants and were guarded-disabled. A separate ChatGPT DCR canary returned HTTP 201 for the exact stable callback and the exact public-client, authorization-code/refresh-token, response-code, and twenty-scope registration contract; it also received zero grants and was immediately disabled. Production readback after cleanup is v1 seven active clients/two active grants unchanged, and v2 zero active clients/three disabled clients/zero grants. This proves v1 catalogue pinning plus Codex/ChatGPT registration paths, not v2 host acceptance.
...
The production policy identity is `2026-08-22.capability-manifest.v8`. The compatibility migration preserves v6/v7 output values and recursively re-proves them only when v8 is requested. New P2 RPCs accept v8 only. Release commit `d5befc466c7dbf3d67b76cde698c9a9aa4df719c` adds immutable exposure `2026-08-29.mcp-exposure.v2` with exactly thirty-four read tools and twenty grantable read scopes while retaining byte-identical `2026-08-22.mcp-exposure.v1` at eleven/seven. The OAuth client and grant persist an exposure revision, and bearer resolution registers the exact stored catalogue; existing v1 clients/tokens remain v1, unknown revisions fail closed, and v2 requires fresh DCR plus consent. No external business write is available.
...
**That push-failure family no longer goes terminal (bug `77113c23`, ops-web `be46c2a2e`, live on main `3c6344efd` 2026-09-05).** The task worker persists the in-app rail row first and only then calls OneSignal. A provider rejection after the rail write (typically HTTP 200 with an `errors` body — no subscribed device) used to be flattened into a generic retryable error, burning all 10 attempts and leaving a `failed/failed` row even though the crew member already had the notification in their rail. The worker now completes the event as `processed` with result evidence `{ deliveryState: "degraded", inAppDelivery: "persisted", pushDelivery: "failed", pushRecipients, pushFailure: { status, detail } }` (`detail` is the provider body, bounded to 1000 chars). `TaskAutomationBatchResult` gained `degraded` and `warnings`; `GET /api/cron/lead-assignment-deliveries` reports `taskAutomation.degraded` / `taskAutomation.warnings` and returns 503 for that run when `degraded > 0`, so the outcome is visible without being retried. If the `complete_task_schedule_automation_event` RPC itself fails while the lease is current, the event is requeued (`Task automation completion failed: …`, retryable) — the degraded evidence is never dropped silently.


## MCP OAuth Authorization Server Schema (base applied 2026-08-18; connector callback policy extended 2026-08-29/30 UTC)

Base ledger `20260818155813_mcp_oauth_authorization_server`, consent-catalog ledger `20260823072837_mcp_oauth_consent_catalog_versioning`, Codex callback ledger `20260829192448_mcp_oauth_codex_dcr_callbacks`, and ChatGPT/RFC 9207 callback-policy ledger version `20260830004843` (name `20260830113800_mcp_oauth_chatgpt_rfc9207_callback`) are production-applied and mirrored in `migrations/`. This is storage for the remote MCP mount — see `04_API_AND_INTEGRATION.md` § "OPS Remote MCP Server — P1 Mount". The v2 application is READY in production. Preserved-v1 authenticated listing and exact Codex/ChatGPT v2 DCR registration behavior are proven. OAuth approval, token exchange, and authenticated v2 tool acceptance remain pending; every v2 canary client is disabled and grant-free.

**Every table lives in `private` with ALL privileges revoked from `public`, `anon`, `authenticated`, and `service_role`.** The authorization/audit subsystem now contains six private tables and thirteen owner-executed public `SECURITY DEFINER` access RPCs: twelve OAuth RPCs plus `append_mcp_request_audit_as_system`. Each public RPC is gated on `auth.role() = 'service_role'` (raising `42501 access_denied` otherwise) with a pinned `search_path`; five private OAuth helper/trigger functions are owner-only and are not PostgREST access paths. PostgREST never sees the tables. Production shape after the P2 wave: 6 tables, 13 public access RPCs, 5 private helpers, zero table grants to any role, and service_role-only EXECUTE on every public access RPC.
...
| `private.mcp_oauth_consent_previews` | Single-use, five-minute exact-visible-consent snapshots binding client/user/company, redirect, PKCE, resource, requested scopes and labels, consent-catalog revision, and exposure revision before approval can mint a code. |
...
Key behaviors implemented in the RPCs rather than the application: the consent decision consumes one exact preview snapshot whose redirect, PKCE, resource, scopes, labels, consent-catalog revision, and exposure revision cannot be widened; presenting a consumed authorization code revokes the grant it minted and all of that grant's tokens; presenting an already-rotated refresh token revokes the entire token family and its grant; minting a grant revokes any prior live grant for the same binding; and `revoke_mcp_oauth_token_as_system` follows RFC 7009 by returning true even for an unknown token. The original replay/revocation behaviors were exercised live during the mount's E2E; the P2 migration wave carries closed-world catalogue, ACL, and replay postflights for the extended shape.

### Claude, ChatGPT, and Codex callback policy (production-applied 2026-08-29/30 UTC)

Migration `20260829192448_mcp_oauth_codex_dcr_callbacks.sql` first added the strict Codex family. Source migration `20260830113800_mcp_oauth_chatgpt_rfc9207_callback.sql`, applied as ledger version `20260830004843`, replaces only that same eight-argument `register_mcp_oauth_client_as_system` body to add ChatGPT's exact stable callback. It adds no table, column, index, trigger, data backfill, grant type, scope, external capability, or change to an existing client/grant.

Registration now accepts exactly one pure callback family: Claude's two exact hosted HTTPS values, exactly one ChatGPT callback `https://chatgpt.com/connector_platform_oauth_redirect`, or exactly one Codex callback with raw shape `http://127.0.0.1:<1-65535>/callback/<8-128 base64url characters>`. Mixed families, multiple ChatGPT/Codex callbacks, query/fragment variants, callback aliases, and wildcard or equivalent ports are rejected. Every later OAuth function continues using exact array membership or equality, so consent, code creation, and token exchange never substitute bytes.

The application release at commit `d5befc466c7dbf3d67b76cde698c9a9aa4df719c` also advertises `authorization_response_iss_parameter_supported: true` and emits the exact metadata issuer `iss=https://app.opsapp.co` on successful code redirects and explicit authorization denials. READY deployment `dpl_3DWhhcueWxeFW2AJnEmxT1bFhfNu` serves the support flag in canonical and cache-bypass metadata. The redirect response remains application-tested but is not yet host-accepted through the pending v2 OAuth approval. Rotating refresh-token grants remain the persistent-access mechanism; `offline_access` is not a business-data scope.
...
- `public.ads_tests` — `id uuid` PK; `campaign_id`, `ad_group_id`, `ad_group_name`; `control_ad_id`, `challenger_ad_id` (distinct); `proposal_id` (the challenger creation; null for a test the worker opened for a pair nobody proposed — the blueprint's, an engine-built ad group's, a hand edit's, since 2026-09-14), `concluded_proposal_id` (the follow-up); `label` (null on those worker-opened tests); `started_at` (the apply instant for an engine challenger; Vancouver midnight of the first account day both ads recorded impressions for a worker-opened one); `min_days` 14, `min_impressions` 2000, `max_days` 56; `state` running/control_won/challenger_won/no_verdict/cancelled; `stats jsonb` (both arms' impressions/clicks/ctr/trials, days, z, p, veto, reason, window, computed_at — updated every tick while running; a cancellation keeps them and adds `reason` and `cancelled_at`); `verdict_at` (also the cancellation instant); timestamps. Unique partial index: one running test per ad group — the worker's pair tracking relies on it to turn a lost race into "nothing opened". `cancelled` is written when an arm leaves the pair (gone, paused outside the guardrail, or the challenger held by the guardrail for its copy); a cancelled pair may be tested again from the day after, a pair with a verdict never is (`04_API_AND_INTEGRATION.md`, Google Ads engine → ad pair tests).
- `public.ads_engine_alerts` — the durable outbox for the rail: `id`, `kind` ad_disapproved/ad_restored/budget_pacing/apply_failed (`ad_restored` since 2026-09-11), `dedupe_key text` unique (`ads-engine:…`), `title`, `body`, `persistent`, `action_url` (default `/admin/google-ads#engine`), `created_at`, `notified_at`, `resolved_at` (2026-09-11: a resolved alert is never delivered, and resolving it closes its rail row). Partial index `ads_engine_alerts_pending_idx (created_at) where notified_at is null and resolved_at is null`.
- `public.ads_guardrail_pauses` (2026-09-11, `ops-web/supabase/migrations/20260911180000_ads_guardrail_pauses.sql`) — one row per episode of the disapproved-ad guardrail. `id uuid` PK; `ad_resource_name` (checked `customers/<id>/adGroupAds/<group>~<ad>`), `ad_id` (numeric), `ad_group_resource_name`, `ad_group_name`, `campaign_resource_name`; `state` holding (disapproved, pause deferred — a `DESTINATION_NOT_WORKING`-only verdict waits 20 h) / paused / restored / released; `policy_topics text[]` and `policy_entries jsonb` (Google's `policy_topic_entries` at the last sighting); `observed_at`; `pause_requested_at`, `paused_at` (the window Google's commit of the pause falls in — how the guardrail later proves the pause is its own), `pause_request_id`, `pause_error` (≤4000); `restore_request_id`; `close_reason` restored/cleared_before_pause/ad_gone/changed_elsewhere/engine_decision/retired_by_blueprint/unverifiable; `closed_at`; `created_at`, `updated_at`. Checks: an open episode carries neither close reason nor close time and a closed one carries both (`ads_guardrail_pauses_open_closed`); paused and restored carry the window with `paused_at >= pause_requested_at`; only a restore closes with `restored`. Unique partial index `ads_guardrail_pauses_one_open_per_ad (ad_resource_name) where state in ('holding','paused')`. RLS on, no policy, service_role CRUD only. Harness: `ops-web/tests/sql/ads-guardrail-pauses-runtime.mjs`. Applied to production 2026-09-11 (ledger `20260911172121 ads_guardrail_pauses`; `md5(statements[1])` `458e71c776b406db96d4de224737403e`, byte-identical to the file), mirrored at `migrations/20260911172121_ads_guardrail_pauses.sql`. Its backfill joins `ads_entities` (phase 1) and resolved the 22 `ad_disapproved` alerts of 2026-09-10 — every one's ad `APPROVED` in the snapshot — and their 20 rail rows.
- Service-only RPC (2026-09-11): `resolve_ads_engine_alerts(text[])` → count of open alerts it closed; sets `resolved_at` on the alerts and `is_read=true, resolved_at` on their `ads_engine` rail rows; keys never raised are ignored. The same file replaced `notify_ads_engine(text,text)` to skip resolved alerts.
...
- Atomic claim serializes through settings, reserves US$0.75, grants a six-minute token lease and caps attempts at three. Checkpoint/finalization/rejection audit require the live owner. Recovery terminalizes expired slots. Mode changes never promote a prepared row. Stable key `cloud-editorial-v1:YYYY-MM-DD` binds any downstream post.
...
Production ledger migrations `20260831042518_agent_day_closeout_foundation_zero.sql`, `20260831042631_agent_day_closeout_routine_worker.sql`, `20260831042924_agent_day_closeout_fk_indexes.sql`, and `20260831061700_agent_day_closeout_routine_configuration.sql` define the private persistence, dormant-worker, foreign-key index, and operator-owned configuration boundary for the first Invisible Office vertical. All four are applied to `ops-app` and mirrored byte-exact in this Bible. The schema owns six concerns instead of leaving them in an assistant host:

| Table | Purpose |
|-------|---------|
| `private.agent_day_closeout_routines` | Host-neutral local wall-clock schedule, IANA timezone, ISO weekdays, named actor, OAuth client/grant binding, required scope ceiling, v9/v3 pin, next occurrence, claim token/expiry, attempt count, retry-not-before, cursor, schedule revision, and last outcome. Rows default disabled. |
| `private.agent_day_closeout_runs` | Immutable, tenant-bound reactive or routine `DayCloseoutResult`, authority snapshot, source revisions, coverage, idempotency binding, and metric-definition revision. Only schema-valid `clear`, `attention`, or `partial` results with `prepared` or `filed` status belong here. |
| `private.agent_day_closeout_routine_failures` | Separate typed history for terminal `blocked` or `failed` occurrences, including exact tenant/actor/grant/client, schedule revision, occurrence, attempt, failure code, and idempotency binding. It is not a `DayCloseoutResult`. |
...
RLS is enabled on all six tables and all direct privileges are revoked from `public`, `anon`, `authenticated`, and `service_role`. Narrow service-role-only functions perform preparation, filing, durable prepare-rate-limit consumption, due claiming, exact current-claim authorization, routine-bound persistence, finalization, and current-actor routine configuration. The configuration list/upsert boundary binds the Firebase-resolved actor and company to one exact live v3 grant, requires `settings.integrations: all`, rechecks the seven granular closeout permissions before create/enable, owns timezone and next-run calculation, invalidates leases on material changes, and disables/de-leases a routine in the same transaction as grant/token revocation. The worker RPCs are `claim_agent_day_closeout_routines_as_system`, `assert_agent_day_closeout_routine_claim_as_system`, `persist_agent_day_closeout_routine_as_system`, and `finalize_agent_day_closeout_routine_as_system`. The due index is ordered by `coalesce(retry_not_before, next_run_at), id`; claims use `FOR UPDATE SKIP LOCKED` with 60–900 second leases. Application code requests one row at a time, immediately before work begins, and only while at least 60 seconds remain in its 240-second total budget. Routine work is cancelled at the 210-second boundary, reserving the final 30 seconds for truthful finalization.

Both reactive and routine preparation re-resolve the current actor/company grant and exact seven owner permissions; tool input never supplies tenant identity. The routine auth adapter mints an actor only from the full exact binding returned by the current-claim RPC. Persistence repeats authority and exact routine/company/actor/grant/client/schedule/claim checks. The deterministic occurrence key is `routine:{routine_id}:{schedule_revision}:{scheduled_for ISO}`. Finalization searches for that exact already-prepared routine run before scheduling a retry or writing failure, covering the case where persistence committed but its response was lost. Transient execution failures and work-budget expiry retry after 5 and 15 minutes; terminal failures use the separate failure table. Successful or terminal occurrences advance from the stored local wall-clock schedule via `private.next_agent_day_closeout_run`, preserving the chosen local time across daylight-saving changes.
...
The schema is live but dormant. Active MCP v2 remains read-only; no production client or grant selects v3 or v4, no routine row exists, and the worker flag is absent. OPS-Web exact release `f0dd81be` is contained by Ready production descendant `d3d51884` / `dpl_H7ncZ6jA7dyN8k4xcGMx6XUGczPU`; it includes the authenticated Connected agents configuration control, offset cron registration, and private canary runner, but the control is invisible without an eligible v3 grant and the cron route exits through its disabled path while the flag is absent.

Production ledger version `20260901012208_mcp_v3_synthetic_canary`, sourced from `20260831190000_mcp_v3_synthetic_canary.sql` at OPS-Web release `f0dd81be`, is applied and mirrored byte-exact in the production migration archive. It adds one fail-closed `private.mcp_oauth_canary_bindings` table plus service-role-only provision/resolve/disable, aggregate acceptance-inspection, and aggregate cleanup-verification functions. A binding is exact to one client, user, company, v3 exposure, consent revision, active membership, reviewed scope/permission ceiling, and maximum 24-hour expiry. Durable OAuth writes and bearer resolution recheck that binding. Provision, refresh, routine configuration, durable v3 OAuth writes, and disablement serialize on one transaction-scoped client advisory lock, making shutdown the final authority decision without incompatible row-lock order. The migration preserves a non-v3-only one-argument resolver for the old application during migration-first rollout, while the new two-argument resolver refuses any stored exposure that differs from the application-selected active exposure. Spent-refresh reuse revokes the family/grant and disables/de-leases the bound routine, including a stale lease on an already-disabled row. The acceptance inspector returns booleans only and requires the exact host-prepared run, action, change set, preview hash, Firebase-reviewed receipt (`filed_inside_ops`, zero messages, no money movement), and enabled unclaimed routine to agree across the actor, tenant, grant, client, confirmation, receipt, and action records. The cleanup verifier independently proves the binding/client/grants/tokens are inactive and no bound routine is enabled or leased. Post-apply readback proves RLS enabled, zero policies, zero app-role table privileges, all five service-role functions present and search-path pinned, four enabled enforcement triggers, zero bindings, zero v3 clients/grants, zero v4 clients/grants, and zero enabled closeout routines.

## Invisible Office Collections State (production-released, dormant)

Source migrations `supabase/migrations/20260831140000_agent_collections_vertical.sql` and `supabase/migrations/20260831220500_agent_collections_fk_indexes.sql` define the private persistence, approval, and indexing boundary for the second Invisible Office vertical. Production ledger versions `20260831215642_agent_collections_vertical` and `20260831222256_agent_collections_fk_indexes` are applied and mirrored byte-exact under `migrations/`. The schema is live but dormant: active MCP remains read-only v2, with zero enabled v4 clients and zero live v4 grants.
...
The read binds current actor, company, grant, client, grant revision, exact scope ceiling, permission snapshot, capability manifest v11, and dormant exposure v5 before reading. It returns one strict 13-complete-week source snapshot: exact canonical role membership, work-window capacity less time off assigned only through the event's canonical `user_id`, merged task/site-visit intervals, project-linked payment cash, company-currency expense allocations, role/project/week attribution, completeness, source revisions, and at most 100 supporting records. The contract reconstructs `business_date` from the exact requested `observed_at` and company timezone, derives the current ISO-week Monday, and rejects a mismatched clock or shifted window. Null expense currency or allocation evidence is invalid and forces insufficiency. Every source limit uses an extra row to prove overflow and returns insufficient data instead of a truncated answer.
...
The read binds current actor, company, OAuth client/grant/revision, exact scope ceiling, permission snapshot, manifest v13, exposure v7, and `analyze_sales_truth:2026-09-01.v1` before touching business data. It returns one strict 180-company-local-day snapshot capped at 5,000 opportunities, 20,000 transitions, 5,000 dispositions, and 20,000 linked activities. Each source query uses one extra row to prove overflow. The application rejects any bound before computing a metric and returns no free-text activity body, note, title, customer identity, or raw disposition note.

`opportunities_agent_sales_truth_cohort_v1_idx` keys `(company_id, created_at, id)` for non-deleted, non-merged opportunities. `activities_agent_sales_truth_history_v1_idx` keys `(company_id, opportunity_id, created_at, id)` for exact opportunity-linked email/text activity history. Migration postflight verifies exact keys, predicates, validity, readiness, and uniqueness, so a drifted same-named index aborts replay. The SHA-256-pinned PostgreSQL 17.11 harness proves migration replay, ACL/search-path shape, exact authority, tenant and revoked-grant denial, source revisions, bounded golden metrics, both index paths, source overflow, and wrong-index rejection in disposable databases.
...
It adds owner-only `private.assert_agent_payroll_readiness_authority(...)`, service-role-only `public.read_agent_payroll_readiness_as_system(...)`, private `payroll_readiness` read-domain revisioning, and source triggers on settings, recurring expenses, reimbursement batches and lines, invoices, and payments. Both functions are stable security-definer functions with empty pinned search paths. The public RPC revokes execution from browser roles; the private helper has no app-role grant.

The read returns one fixed company-local as-of snapshot: signed operator-maintained cash balance and freshness, scheduled recurring obligations and coverage confirmation, approved unpaid reimbursement batches, reconciled open invoices, and actual per-payer net-settlement history. Malformed active recurrence rows remain in the source snapshot; invalid cadence/currency, `NaN`/infinite/over-64-character numerics, and noncanonical temporal values become bounded invalid sentinels so the application reports the exact evidence gap instead of silently filtering a row or rejecting the whole snapshot. Reimbursement amounts match the canonical product rule for partial/full/automatic approval and null/zero/positive approved amounts. Historical settlement aggregates every non-void payment dated no later than the business date, including negative adjustments, and recognizes only the first date from which cumulative net payments remain fully settled; invalid payment/invoice amounts remain visible but never enter a payer distribution. Six-digit timestamp/time precision is preserved. Bounds are 40 recurring obligations, 50 reimbursement batches, 100 open receivables, and 500 historical settlements; every query reads one extra row and overflow fails closed. The deterministic maximum source snapshot is 154,555 characters.
...
Owner-only `private.assert_agent_recurring_service_price_change_authority(...)` and service-role-only `public.read_agent_recurring_service_price_change_as_system(...)` are stable security-definer functions with empty pinned search paths. Each read phase independently binds the current actor, tenant, OAuth client/grant/revision, manifest v15, exposure v9, exact scopes, current company-wide permissions, capability id/revision, and a five-minute observation window before touching business data. The application reauthorizes before the source phases and after it has compared them, then performs the exact SQL authority assertion immediately before returning the preview.

The migration also installs one owner-only additive authority bridge behind the four inherited v8 tools. Each accepts only its historical manifest/exposure pair or v15/v9. A v15/v9 call must also prove the exact 16-scope client ceiling and serialization, v4 consent, and exact labels; the historical branches retain their previous authority rules. All five bridge functions are stable security-definer functions with empty search paths and no direct app-role execution.

The first source phase returns a bounded recurrence-driving catalog. TypeScript removes only histories it can prove ended before the requested month under exact, conservative recurrence semantics, within a 100,000-unit aggregate classification budget. The detail phase accepts that exact recurrence selection and returns both a freshly recomputed catalog and all selected facts under one PostgreSQL statement snapshot. Canonical catalog equality, selection equality, and exact recurrence-to-account evidence are mandatory; drift is stale context. Invalid or overflowing target-month exceptions remain conservatively in scope, and conflicting RRULE/database termination fails closed.

The selected detail snapshot resolves one exact active task type and at most 100 unique client/service account identities through active clients and same-company accepted/in-progress projects. Ended historical, future-only, and inactive-project rows do not spend the identity bound; a 101st unique identity is the overflow sentinel. Multiple relevant recurrence rows for one identity return one `duplicate_account_service` exclusion with the first two recurrence source references/hashes instead of consuming multiple slots. A 10,001-row catalog sentinel, a 3,500,000-byte conservative construction threshold, and exact 4,000,000-byte UTF-8 ceilings on the catalog and complete detail wrapper stop extreme duplicate/history sets before transport becomes unbounded. RRULE input must use the non-expanding canonical uppercase recurrence alphabet before the construction estimate. Each account carries the recurrence and bounded exceptions, explicit policy, current accepted estimate or delivered-invoice price line and hash, non-null optional-selection flags, zero line-item/document discount proof, tax treatment, one verified contact identity, provider-normalized correspondence counts and latest-state fixed risk signals, coherent collectible late-payment evidence, and stable supporting-record hashes. A missing, inactive, unsafe, or out-of-range tax source preserves the verified price facts so the application can return `tax_unavailable`. Raw provider subjects, bodies, snippets, and normalized text never enter the snapshot. Any ambiguous service or recipient, unreadable correspondence, stale price source, invalid recurrence, missing tax/terms, notice failure, unsupported currency, or source/result bound fails closed or excludes the affected account with an exact reason.

Three month-relevance indexes bound open, ended, and rescheduled recurrence branches before the identity sentinel; provider lookup composes tenant/time ordering with sender and GIN recipient/CC indexes, while contact, invoice, policy, and exception paths have separately verified bounded indexes. The PostgreSQL proof includes companies and every other in-scope business source in its before/after digests. The migration defines no price update, contract edit, invoice write, service mutation, notice delivery, provider draft, queue item, notification, confirmation, receipt, routine, or commit RPC. The application result is ephemeral, expires after 24 hours, persists no preview/notice business content, and is identified by context/source-bound per-account preview hashes plus one canonical package hash over every evaluated account revision. Canonical behavior and release boundaries: `specs/2026-09-01-ops-mcp-recurring-price-preview-vertical.md`.

## Invisible Office Weather Reschedule Preview State (production-released, dormant)

Production ledger entries `20260903194613_agent_weather_reschedule_preview` and `20260903194749_agent_weather_reschedule_preview` define the read-only source boundary for the ninth Invisible Office vertical, `prepare_weather_reschedule`. A concurrent approved apply recorded the exact replay twice; both rows contain the same one-statement, 47,596-byte source at SHA-256 `6809661f9c7a22665786e975773ce38455dffafc59410c8099efa6ce95311e45` and are mirrored byte-for-byte. The migration is replay-safe and adds no business row. Active production MCP remains read-only v2; v11 has no client, grant, activation, or customer-live authority.

The migration adds no table or index. It extends the immutable OAuth label catalogue with v6 labels for client schedule-update drafting and weather schedule preparation, then exposes two service-role-only stable security-definer RPCs with empty search paths: one exact source read and one final same-observation source/authority assertion. Browser roles cannot execute them. Every call binds manifest v17, exposure v11, capability revision, exact grant/client/consent/labels, all seven required scopes, the full server-owned 103-key permission registry used for actor-revision parity, all nine required company-wide permissions, and entity authority for tasks, projects, conflicts, and recipients.

The source is bounded to 100 target tasks, 25 projects, 500 relevant conflicts, 100 explicit outdoor task types, 50 assignees per task, and a 14-day candidate window. It reads typed current `companies.schedule_settings`, active single-day `project_tasks`, projects, task types, active same-company users, exact client/sub-client recipients with non-merged parent-client lineage, global suppression state, and complete project/day `weather_forecasts`. Outdoor classification comes only from `outdoor_task_type_ids`. UUID assignment identities are canonicalized before overlap selection. Weather policy consumes only probability and precipitation numbers from exact `open-meteo` rows retrieved within 12 hours; condition text is display-only untrusted data. Missing or ambiguous recipients, shared email ownership, dependencies, recurrence/pairing/locks, stale forecasts, malformed intervals, stale authority, and reached sentinels fail closed.

Every settings, task, recipient, parent-client lineage, forecast, and conflict record is SHA-256 bound. One canonical source revision covers the complete snapshot; the final assertion rebuilds it from current rows using the original observation instant. The PostgreSQL 17 harness compiles Phases 7–9 in production order, including the exact ledger replay, and proves identical replay, full-registry authority parity, UUID-case collision identity, typed-settings enforcement, stale-forecast/shared-recipient/merged-parent/locked-task rejection, exact ACL and function shape, and zero business mutation. Live readback confirms owner and ACL shape, exact v6 labels, zero v11 clients/grants, zero fresh 12-hour forecast rows, and unchanged business control totals. Canonical behavior and release gates: `specs/2026-09-03-ops-mcp-weather-reschedule-vertical.md`.
### Supplier Bills / Accounts Payable Vertical (2026-09-03)

Supplier invoices are first-class liabilities, not expenses waiting for a payment date. The canonical model is `suppliers`, `supplier_bills`, `supplier_bill_line_items`, `supplier_bill_project_allocations`, `supplier_bill_payments`, `supplier_bill_documents`, `supplier_bill_events`, and provider mapping/link records. Bills retain supplier and normalized invoice identity, invoice/due dates, itemized subtotal/tax/total, remaining balance, `open` / `partial` / `paid` / `void` status, exact multi-job allocations, payment settlements, and the immutable original PDF custody record.

Unpaid captures become supplier bills. Already-paid purchases continue through the existing expense and approval path, with the supplier PDF linked to that expense. Active duplicate supplier invoice numbers are unique per company and supplier; original PDFs are unique per company SHA-256. All money is canonical two-decimal currency text at the application boundary and constrained non-negative numeric data in PostgreSQL. Line, tax, total, allocation, and settlement equality are revalidated inside the database before any consequential write.

Prepare/commit intents are company-, actor-, action-, request-, payload-hash-, and expiry-bound. Commit rechecks current company membership plus `expenses.approve` and `accounting.view`, requires the exact server-issued confirmation, and returns a fresh database receipt. RLS grants authenticated users tenant-scoped reads only; service-role writes are allowed only through the guarded application contract and database RPCs. Source documents and event records are service-role insert-only. No MCP, Invisible Office, or vinyl-order schema or authority is introduced by this vertical.
...
#### Pre-AP intake and clearance layer (database and web released, 2026-09-04)

Production ledger migrations `20260904171301_supplier_bill_intake_clearance.sql` and `20260904171632_supplier_bill_intake_fk_indexes.sql` add a durable review boundary before the production AP model and cover all 13 foreign keys introduced by it. They are byte-exact mirrors of the OPS-Web source migrations authored as `20260904040358_supplier_bill_intake_clearance.sql` and `20260904171534_supplier_bill_intake_fk_indexes.sql`. Production ledger `20260904184303_supplier_bill_company_data_lifecycle` integrates supplier bills with account closure without weakening their immutable ACLs: the service-role-only purge helper covers all 42 delete-blocked company tables, supplier event-ledger purges transactionally erase the matching private prepared-write intents, and optional site-visit handoff provenance clears rather than blocking the purge. The 267-table company-data scope and 42-table privilege snapshot match production exactly. OPS-Web source is published on production main at `f901c6d9c0e1bd63abddcb616a697f7fb9115ad6` through `READY` Vercel deployment `dpl_7BCJY52J5SB6KwmY7qdCrLSzCUH3`, which owns `app.opsapp.co`. The OPS iOS implementation is local commit `c6269763` and remains unreleased.

The normalized public model is `supplier_bill_intakes`, `supplier_bill_intake_line_items`, `supplier_bill_intake_allocations`, `supplier_bill_intake_checks`, `supplier_bill_intake_documents`, and `supplier_bill_intake_events`. The private `supplier_bill_intake_write_intents` table holds 15-minute, actor-, company-, action-, request-, content-hash-, expected-revision-, and confirmation-bound mutations. Lifecycle is `review`, `held`, `to_pay`, `paid`, or `payroll`; source kind is `material`, `subcontractor`, or `employee`. Due date remains nullable and is distinct from the operator-owned planned payment date.

Line items preserve SKU, description, ordered and invoiced quantities, unit of measure, price/tax/total, job hint, match basis, and confirmation state. Address and purchase-order matches remain suggestions until an operator confirms a same-company project. Allocations are exact currency amounts and distinguish proportional suggestions, confirmed suggestions, and manual overrides. Checks preserve observed value, policy limit, structured evidence, outcome, disposition, note, and actor/time evidence. Source PDFs are unique per company SHA-256, limited to 20 MB, and immutable after insert; event snapshots are also append-only.

`accounting.view` owns tenant-scoped reads. `accounting.bills.capture`, `accounting.bills.approve`, and `accounting.bills.pay` independently own capture/review, approval, and payment lanes. The two public mutation functions are service-role-only security-definer RPCs with an empty `search_path`; browser roles have no direct write or RPC execution. Approval revalidates required checks, exact allocations, payment owner, and planned date before it promotes a material or subcontractor intake exactly once into the production `suppliers` / `supplier_bills` model and creates provider queue work. Held or payroll-routed intakes cannot create canonical AP or provider rows. Release readback proved RLS and ACL shape, all 37 supplier-bill foreign-key repair indexes, zero rows in all 17 public supplier/intake tables and both private intent tables, and no supplier-related security-advisor finding. Canonical contract: `specs/2026-09-03-canpro-supplier-bill-clearance.md`.
...
The migration extends the immutable OAuth label catalogue with v7 labels that disclose both inherited weather scheduling and crew-recovery preparation. It exposes two public service-role-only stable security-definer RPCs with empty search paths: one exact source read and one same-observation authority/source replay assertion. Browser roles cannot execute them. Every call binds manifest v18, exposure v12, capability revision, exact client/grant/revision/ceiling/labels, ten required scopes, the full server-owned actor permission registry, all eleven required company-wide permissions, and current project/task/client entity authority.

The source resolves exactly one active same-company crew identity by exact full name or a unique exact first/last name. It uses the company-local civil date window and returns every authorized active task and booked site visit assigned to that member on the requested date, plus active replacement candidates. Candidate evidence is limited to current role membership, same-task completion history, existing project continuity, company working hours, approved or neutral time off, and task/site-visit/personal-event collisions. Same-task history proves experience only; OPS has no authoritative crew licence or certificate source, so the contract cannot claim licensure.
...
### Deck geometry successor custody (production-applied 2026-09-10)

Production ledger `20260910233314_agent_deck_geometry_v23_exposure` contains one 30,386-byte statement, SHA-256 `657578123aeb0196c91abff5004553af524aed5afb1146cd17029c6ca0222989`. The exact SQL is archived in `migrations/20260910233314_agent_deck_geometry_v23_exposure.sql` and mirrored in OPS-Web. It adds no tables or business records. V23 retains V14's 35 tools, 21 scopes, customer-update manifest v20 and consent v9; only the deck result representation changes.

Service-only `prepare_agent_customer_update_for_grant_as_system` derives the exposure from the exact immutable actor/company/client/grant/revision/scope binding and delegates to the original preparation routine. Access resolution, customer authority, queued reauthorization and rate limiting accept the exact V14/V23 pins. Original preparation, refresh and consent-immutability functions remain unchanged. All changed routines retain postgres ownership and pinned search paths; private helpers remain postgres-only, and public entry points remain service-role-only.

The migration checks captured function definitions, ACLs and exact immutable triggers before DDL. SHARE locks on both trial binding tables fence concurrent enrollment and refuse any enabled unexpired financial or catalog trial. Production pre/post readback confirmed unchanged original connection records, all four expired trial bindings, the customer-update effect fingerprint and its existing policy rows. The pre-existing customer policy mismatch remains fail-closed. Global financial/catalog effect fingerprints change because they include all public/private functions; no policy is resealed and subsequent trial enrollment requires its separately authorized review. Security advisor categories/counts were unchanged, with no findings on the modified routines. Production application/host acceptance is tracked in the API chapter's dated deck geometry section.

### September 5 Maverick repairs — both repairs live; signed-in precision check blocked
...
## Invisible Office Phase 13 customer-message state

Source migration `20260906050000_agent_customer_message_follow_up.sql` defines the dormant exact-reply persistence boundary. `private.agent_customer_messages` is a force-RLS actor/grant/source/preview ledger with no direct application-role access. `private.agent_customer_message_policy` seals the installed transport and authorization effects. Service-only prepare, exact approval, and pre-send rejection RPCs own every write. The selected source must be the latest normalized inbound provider delivery in the job's canonical thread, with no attachments, one customer sender, and an active personal mailbox owned by the named operator. Source, recipient, mailbox, thread, subject, body, approval, and current authority are rechecked at provider claim. Active production remains v20/v14/v9; v21/v15/v10 have no selectable exposure, consent installation, client, grant, or business row. Full contract and release state: [Phase 13](specs/2026-09-06-ops-mcp-customer-message-follow-up.md).

**End of Data Architecture Documentation**

## Invisible Office Phase 14 schedule/crew state (2026-09-06; production-applied 2026-09-07, dormant)

The locally verified dormant vertical adds force-RLS `private.agent_schedule_changes` (exact actor/grant/request/source/timezone/preview/expiry/receipt ledger), `agent_schedule_capacity_fences` (current task-version capacity protection), `agent_schedule_write_tokens` (privileged exact transaction-owned canonical writes) and `agent_schedule_change_policy` (installed trigger/helper fingerprint). Application roles have no direct relation privileges. Restrictive action RLS protects named-actor schedule previews. All task mutations, project rollups, reminder readback, history, notification intents, action completion and receipt persistence are atomic.

Sources are `20260906234703_task_mutation_time_validation.sql` and `20260906235016_agent_schedule_crew_approval.sql`, production-applied on 2026-09-07 and archived under actual ledger names `20260907055324_task_mutation_time_validation.sql` and `20260907055344_agent_schedule_crew_approval.sql` in `migrations/`. The first repairs native-time format validation; the second includes the canonical all-day reminder date repair. Neither rewrites existing schedules, installs consent v11 nor creates customer/grant/policy business seeds. Existing company/actor/grant/client foreign-key retention restrictions follow the prior approved-update ledgers; hard deletion of a task cascades only its ephemeral capacity fences. See [Phase 14 contract](specs/2026-09-06-ops-mcp-schedule-crew-approval.md) for limits, locks, authority and hosted timezone blocker.


## MCP private financial drafts (Phase 15, dormant)

Production additive estimate metadata separates private distribution custody, document kind, accepted baseline, currency and scope from predecessor revisions. Private versioned policies, sealed proposals, write tokens and effect policy have forced RLS and no direct app-role table access. Reviewed source mirror: `supabase/migrations/20260907062351_financial_document_private_drafts.sql`; actual production ledger: `20260907173708`. Existing financial records and OAuth authority are unchanged. See [Phase 15 contract](specs/2026-09-07-ops-mcp-financial-document-approval.md) for authority, arithmetic, locks, verification and release evidence and separate activation gates.

## Financial policy owner enrollment (Phase 16, released and dormant)

The migration `20260908024426_financial_policy_readiness.sql`, applied as production ledger `20260908054740` on 2026-09-08, adds force-RLS `private.financial_policy_previews`, an exact owner/source/tax/authority preview and receipt ledger with no direct application-role table privileges. Existing `private.financial_document_policies` gains approved owner/time/preview linkage. Policy content is immutable; supersession and revocation retire a revision without deleting evidence. Current company ownership, unchanged source note, exact approved default tax and durable enrollment receipt join the existing financial source gate. The migration deliberately does not renew the financial effect policy, enroll business data or change OAuth grants. See [Phase16 contract](specs/2026-09-08-ops-mcp-financial-readiness.md).

The second migration `20260908033425_financial_trial_oauth.sql`, applied as production ledger `20260908054759` on 2026-09-08, extends the existing private canary binding ledger with `financial_policy_id`, `financial_policy_sha256` and `financial_effect_revision`. Paired constraints preserve v3/v2 and permit only exact v17/v12 financial bindings with nonnull SHA256 evidence and at most two hours. Financial binding identity, hashes and expiry are immutable; disable is irreversible. The table is force-RLS and has no direct application-role privileges. Exact reviewed baseline function MD5 checks abort on migration drift. Token issuance/refresh/revocation and financial actions share the existing client advisory lock; financial approval uses a nonblocking attempt to preserve the company lock ordering. Neither migration seeds bindings, grants, company records or financial effect approval. The shared canary predicate pins UTC for timestamp-bearing policy/source/tax hashes. Installed financial effect approval remains unchanged and mismatches the new graph, intentionally closing financial authority.

## Phase17 conversational catalog authoring — production schema, restricted trial (2026-09-09)

The Phase17 implementation adds `private.agent_catalog_proposals` and an initially empty `private.agent_catalog_effect_policy`. Source migration `20260908221635_agent_catalog_authoring.sql` was approved and applied as production ledger `20260909021307`; source `20260909015000_catalog_trial_oauth.sql` was applied as `20260909021401`. Byte-identical ledger mirrors are in `migrations/`. Neither migration seeds business data or authority. This does not reconstruct the unshipped August catalog-authoring kernel.

The second migration adds force-RLS `private.agent_catalog_trial_bindings`, with no direct application-role privileges. A fresh immutable OAuth client is bound to one actor/company, exact V19/V14 seven-scope ceiling, reviewed catalog effect hash and expiry of at most two hours. Identity/hash/expiry cannot be changed; disable is irreversible. Current actor permissions, membership, binding, scope ceiling and installed effect hash are checked at consent, code/grant/token creation, bearer, refresh, prepare, named OPS save and replay. During the restricted rollout the original channel must be MCP; internal/API contexts cannot bypass the binding. Catalog state is separate from Phase16 financial policies and bindings, which are not renewed or resealed. Chapter04 records current release and host-acceptance evidence.

Existing `products`, `catalog_items`, `catalog_options`, `catalog_option_values`, `catalog_variants`, option joins, `catalog_units`, `catalog_categories`, and fixed `product_materials` remain canonical. A proposal stores its actor/company/origin, exact input and source row identities, canonical before/after values, current source/relationship fingerprints, authority snapshot, effect seal, 30-minute expiry, action identity, commit key and durable receipt. `agent_actions` and persistent notifications remain the review surface. Private proposal reads and writes are revoked from ordinary runtime roles.

Prepare and commit fence canonical catalog writers with bounded `SHARE ROW EXCLUSIVE NOWAIT` table locks. This includes inserts from existing web/iOS writers, not only pre-existing rows. The transaction revalidates the named actor, company, permission snapshot, current OAuth grant/client/scopes/consent, exact action payload, expiry and schema effect revision before recompiling and comparing the proposal. Apply, canonical triggers, revision bumps, inventory adjustment audit where explicitly requested, receipt and notification completion share one transaction. A late failure rolls all of them back. Identical commit replay returns the original receipt under current authority; a changed commit key fails.

The effect revision includes public/private functions, noninternal triggers, column types/defaults/generated expressions, constraints and rewrite rules. The policy table is empty by default. Any schema drift requires fresh effect review and explicit activation, rather than accepting a changed default or trigger silently. Production migration coordination must account for other initiatives' effect seals without changing those seals implicitly.

Before catalog or authority locks, PostgreSQL 17 `transaction_timeout` is armed transaction-locally with a maximum two-second budget. Existing shorter configured timers remain active; shortening a longer timer conservatively subtracts elapsed transaction age; repeated checks do not restart the timer. There is no function-level `statement_timeout` override, so PostgREST cannot widen a shorter role limit by hoisting one. Whole-table phantom fences can briefly block writers from other companies; the deadline bounds that interval. A transaction timeout terminates the database session. The client treats contention/deadline errors as retryable and does not infer a save or rollback from a transport error. Immutable same-key replay plus independently read action receipts establish the result.

Disposable PostgreSQL concurrency tests verify NOWAIT contention, other-company writer release after a stalled save, rollback, exact retry and receipt replay, shorter statement/transaction timers, and elapsed-time preservation. Forty additional SQL assertions cover the exact OAuth trial and authority lifecycle. A real local PostgREST16.2 two-connection pool returns HTTP503/PGRST001 when the transaction deadline terminates a delayed save; independent SQL proves rollback, and identical-key retry/replay proves one save with the original receipt. This closes the local pooled-transport gate only. Production pooled transport, browser visual and native authoring acceptance remain separately tracked in chapter04.

### Catalogue setup write state (2026-09-16; applied to production and activated)

Six migrations add the catalogue-setup write vertical: `20260916010000_agent_catalog_setup_write_variant.sql` (the shared spine plus the `create_variant` kind), `…020000_…thresholds.sql`, `…030000_…pricing.sql`, `…040000_…supplier_cost.sql`, `…050000_…option.sql`, and the data-only `20260916060000_repair_double_encoded_catalog_text.sql`. They depend on `20260915223000_agent_catalog_recipe_read_v24.sql`, which must be applied first. None of them seeds a client, grant, consent row, effect-policy seal or business row.

**Applied to production 2026-09-16** (Supabase project `ijeekuhbatykdomumfjx`), together with a money-precision follow-on, as eight ledger versions in this order: `20260916165439 agent_catalog_recipe_read_v24`, `20260916172541 agent_catalog_setup_write_variant`, `20260916173055 agent_catalog_setup_write_thresholds`, `agent_catalog_setup_write_pricing`, `agent_catalog_setup_write_supplier_cost`, `agent_catalog_setup_write_option`, `20260916182941 repair_double_encoded_catalog_text`, `agent_catalog_setup_write_money_precision` (roughly 18:00–18:45 UTC). Verified by object fingerprint: 47 of 47 expected function md5s match the release runbook's table (web `docs/artifacts/mcp-catalog-setup-writes/release-runbook.md`). The three private tables below — `private.agent_catalog_setup_writes`, `private.agent_catalog_setup_writer_requests` and `private.catalog_text_repairs_20260916` — are present in production as of 2026-09-16. The effect-policy seal was activated by hand after the production code deploy (see chapter 04), so the vertical is live rather than dormant. Ships from PR #127 (merge commit `8d0603b6a`).

`private.agent_catalog_setup_writes` is the one proposal ledger for all five write kinds, modelled on `private.agent_customer_updates`. Columns: `id`, unique `run_id`, unique `action_id`, `company_id`, `actor_user_id`, `oauth_grant_id`, `oauth_client_id`, a `kind` discriminator checked against `create_variant | set_thresholds | set_pricing | set_supplier_cost | create_option`, `family_id`, `authority` and `request` jsonb, `idempotency_key`, `input_hash`, `pre_image_hash`, `evidence_hash`, `payload`, `policy_revision`, `proposal`, `preview_hash`, `effect_sha256`, `expires_at`, `created_at`, `rejected_at`, `committed_at`, unique `confirmation_id`, `commit_key` and `receipt`. Invariants are table checks: one proposal per `(company, actor, client, idempotency_key)`; `authority`, `request`, `payload` and `proposal` are objects; `request` ≤ 32 KiB, `proposal` ≤ 64 KiB, `payload` ≤ 1 MiB; expiry is after creation and at most 31 minutes past it (the prepare sets 30); a row cannot be both rejected and committed; and the four commit columns are all null or all set together. RLS is enabled and forced and every privilege is revoked from `public`, `anon`, `authenticated` and `service_role`. Five indexes cover grant, company, actor, client and `(company_id, family_id)`.

The review surface is `public.agent_actions` with `action_type = 'approve_catalog_setup_write'` and a persistent notification keyed `catalog-setup-write:<action_id>`. Four restrictive policies on `agent_actions` refuse every application-role insert, update and delete of that action type outright and gate select on `public.can_read_catalog_setup_write_action(action, company)` (granted to `anon` and `authenticated`), which delegates to `private.agent_catalog_setup_write_can_read(actor, company, action)`: the row is readable only by the operator it was staged for, only while the family still exists, and only while that operator holds `agent.review`, `catalog.view`, `catalog.products.view` and `catalog.manage` — plus `finances.view` for a `set_supplier_cost` row and `catalog.stock.adjust` for a request carrying an opening quantity. The service queue batches through `public.filter_catalog_setup_write_actions_as_actor(actor, company, actions[])`, service-role only and capped at 200 ids.

`private.agent_catalog_effect_policy` gains one expected revision key, `2026-09-15.catalog-setup-write.v1`. `private.agent_catalog_setup_write_effect_revision()` hashes the non-internal triggers on `catalog_categories`, `catalog_items`, `catalog_options`, `catalog_option_values`, `catalog_variants`, `catalog_variant_option_values`, `catalog_stock_units`, `catalog_stock_unit_events`, `catalog_supplier_cost_profiles`, `catalog_setup_save_requests`, `agent_actions` and `notifications`, together with `public.catalog_setup_save`, both narrow writers, the vertical's prepare / commit / reject and its compile, projection and payload helpers, transitively through their own call graph. **No migration seeds the row**, and the spine's postflight raises `agent_catalog_setup_write_must_not_activate` if one exists. Until an operator inserts a reviewed hash, `private.agent_catalog_setup_write_assert_seal(false)` refuses every prepare with `CATALOG_SETUP_WRITE_ACTIVATION_REQUIRED` and `assert_seal(true)` refuses every commit with `CATALOG_SETUP_WRITE_EFFECT_POLICY_CHANGED`. Each later kind changes the hash on purpose, so a seal reviewed against a one-kind install cannot cover a five-kind one.

Two narrow writers exist because `public.catalog_setup_save` cannot reach their fields — its family section never writes `catalog_items.default_price`, and it has no supplier-cost section at all; no other Postgres function writes either. `private.catalog_family_default_price_save(uuid, uuid, uuid, numeric, text, jsonb)` sets a family's default price and returns the row's exact stored text. `private.catalog_supplier_cost_profile_save(uuid, uuid, uuid, jsonb, text)` upserts one profile on `(company_id, catalog_variant_id, profile_key)` — a key that is unique *without* a `deleted_at` predicate, so a removed row is revived rather than re-inserted — keeps exactly one default per variant by demoting the old default before promoting the new one so the partial unique index `catalog_supplier_cost_profiles_one_default` never trips, refuses with `CATALOG_SETUP_DEFAULT_REQUIRED` a write that would leave a variant carrying profiles with no default, stamps server provenance under the reserved `ops` key, and mirrors the resulting default's `unit_cost` into the catalogue in the same transaction (reported as `variant_unit_cost_mirrored`) at the level the family already uses — `unit_cost_override := private.agent_catalog_setup_override_for(cost, catalog_items.default_unit_cost)`, so a family default equal to the cost leaves the variant inheriting, and `default_unit_cost` itself is never written (since migration `20260916090000_agent_catalog_setup_write_override_level.sql`, PR #130, applied 2026-09-16 as ledger version `20260916233332`; before it the mirror set the override unconditionally). Both are `private`, `SECURITY DEFINER`, `search_path = ''`, revoked from `public`, `anon`, `authenticated` **and** `service_role`, reachable only from `public.commit_catalog_setup_write_as_actor` through `private.agent_catalog_setup_write_apply`, and idempotent through `private.agent_catalog_setup_writer_requests` — a force-RLS ledger keyed unique on `(company_id, writer, idempotency_key)` holding the request hash, the caller's `source` object and the response, so a replayed key carrying different arguments is a conflict rather than a silent no-op. It too is revoked from every application role.

`private.agent_mcp_rate_limit_buckets`'s closed `policy_id` check gains `mcp-catalog-setup-write-prepare:2026-09-15.v1`, consumed by service-role-only `public.consume_catalog_setup_write_prepare_rate_limit_as_system` at 6 actor / 6 grant / 30 company units per minute for the five prepare capability ids. The existing `mcp-catalog-prepare:2026-09-08.v1` policy is hard-pinned to exposure V19 and a live subject-bound trial, and was deliberately not widened.

`private.catalog_text_repairs_20260916` is the audit ledger for the double-encoded text repair: `table_name`, `column_name`, `row_id`, `before_value`, `after_value`, `repaired_at`, indexed on the target triple, RLS enabled and forced, revoked from every application role, so reversing the repair is an `UPDATE` from `before_value`. The repair migration creates no function that survives it (both helpers are `pg_temp` and dropped before commit), touches no trigger, and neither reads nor writes `private.agent_catalog_effect_policy`. **Applied to production 2026-09-16 as ledger version `20260916182941`, where its text pass found zero double-encoded rows** — production never carried the corruption. Jackson wrote those em dashes himself on 2026-09-15 (one item name and 59 cost-profile labels; the 133 profiles stamped 19:00–20:00 UTC that day are that edit), and no row changed between then and the apply, so nothing outside this branch touched the text. The mojibake existed only in the local proving copy, whose seed export came through the Supabase MCP `execute_sql` JSON path and rendered each UTF-8 byte of an em dash as a separate escaped Latin-1 character. The text repair's apply therefore left the production ledger holding **4 rows, all `column_name = unit_cost`**: the four Canpro Glass Panel `vitrum-2026` supplier cost profiles, rounded half-up from 4.1992, 4.2804, 9.2684 and 9.7440 CAD to two decimals, so no live supplier cost is finer than a cent. The "172 ledger rows" figure in the release runbook was a rehearsal expectation, not a production one. The ledger is reused by one further migration, **applied to production 2026-09-16**: `20260916080000_align_variant_unit_cost_override_to_default_profile.sql` (PR #128, merge commit `5050e127d`, ledger version `20260916203707`) re-aligned the four `catalog_variants.unit_cost_override` values the rounding left a fraction of a cent off their canonical default supplier cost profile — 4.1992 → 4.2, 4.2804 → 4.28, 9.2684 → 9.27, 9.7440 → 9.74 — and added **4 rows** under `column_name = 'unit_cost_override'`, with the exact stored text on both sides, beside the text repair's 4 `unit_cost` rows. **0** of those rows came from NULL. Variants whose override is NULL are costed at family level through `catalog_items.default_unit_cost` and were deliberately untouched — the 35 item-level-costed variants were left inheriting; the migration refuses to run if any such variant's family default disagrees with its profile, and its postflight refuses any ledger row that came from NULL. See chapter 04 § Cost precedence. Chapter 04 records the tool contracts, refusal codes, proofs and the exposure deploy-ordering hazard.

## Weekly journal ledger (2026-09-10; applied to production 2026-09-10)

Source: `ops-web/supabase/migrations/20260910170000_create_journal_editorial.sql` (branch `feat/journal-cloud-editorial`). Applied through MCP `apply_migration` as ledger version `20260910191741` (`create_journal_editorial`); mirror `migrations/20260910191741_create_journal_editorial.sql`, byte-identical to the ledger (`md5 4b37f46c9f63b165d1808ffe0164d91a`). Verified after apply with `ops-web/docs/journal/release/verify-journal-ledger.sql`: three tables with RLS on, no `anon`/`authenticated` grants, seventeen security-invoker functions executable by `service_role` only with `search_path` pinned, settings `off | 1 | 6 | 72 | 360 | OPS Team`, zero assignments. Additive: three tables and seventeen functions; no existing object changes. Harness: `ops-web/tests/sql/journal-editorial-runtime.mjs` (disposable PostgreSQL 17; carries production's `blog_posts` shape, `source` check, unique slug and the open-notification dedupe indexes). Feature: `07_SPECIALIZED_FEATURES.md` §21; routes: `04_API_AND_INTEGRATION.md` § Weekly journal authoring.

- `public.journal_editorial_settings`: singleton boolean `id`; `mode` off/prepare/publish (default `off`); `publish_weekday` 1 and `publish_hour` 6 (Vancouver wall clock, fixed zone `Etc/GMT+7`); `draft_open_hours` 72; `min_veto_minutes` 360; `slot_grace_hours` 72; `authoring_lease_minutes` 60; `max_sources` 24; `byline` `OPS Team`; `authoring_heartbeat_at`; `authoring_stall_notified_on`.
- `public.journal_editorial_assignments`: `identity` unique `weekly:<YYYY-MM-DD>` (checked equal to `'weekly:' || slot_date`); `slot_at` (launch instant) and `publish_at`; `mode` stamped at claim; `state` queued/authoring/drafted/scheduled/published/cancelled/blocked; `attempts` 0–3; `submissions` 0–3 per claim; `claim_token`, `lease_until`, `next_attempt_at`, `claimed_by`; `slug` (partial unique index over drafted/scheduled/published) and `title`; `package jsonb` (article fields, OPS-rendered `html`, `word_count`, `category_id`, citations, evidence, worked examples, internal links, editor review, usage, three reference fingerprints, `brief_version`); `preview jsonb` (hero asset); `blog_id → blog_posts(id) on delete set null`; `attempt_log` (≤40 entries); `last_code`; `notified_state`; `newsletter_state` sending/sent/skipped, `newsletter_token`, `newsletter_at`; lifecycle timestamps. Checks: drafted/scheduled/published carry a package and a slug; scheduled carries a preview and `publish_at`; published carries `published_at`.
- `public.journal_editorial_sources`: every page OPS fetched for an assignment — `claim_token`, `url` (HTTPS, unique per assignment), `final_url`, `http_status` 2xx, `content_type` text/html, text/plain or application/pdf, `bytes`, `sha256`, `title`, `site_name`, `published_hint`, `modified_hint`, `text` (≤150,000 characters), `truncated`, `fetched_at`. Evidence quotes are verified against these snapshots only.
- RLS on all three; privileges revoked from public/anon/authenticated; service_role CRUD. Every function is `SECURITY INVOKER` with an empty search path and service_role-only execution: `discover_journal_editorial_assignment(timestamptz)` (next slot ≤72 h ahead unless `off`; blocks a queued slot 72 h past launch as `SLOT_MISSED`), `recover_…`, `claim_…` (settings lock, heartbeat, skip-locked), `record_journal_editorial_attempt`, `store_journal_editorial_source` (lease-checked, per-URL dedupe, per-claim cap), `finish_…` (drafted reserves the slug; `SLUG_TAKEN` against any `blog_posts` row or reserved slot; editor rejection retries in 6 h, errors in 2 h, third attempt blocks), `schedule_…`, `annotate_…`, `block_…`, `publish_journal_editorial_assignment(uuid,boolean,text)` (the only writer of `blog_posts` in the pipeline: settings row lock; `HELD`/`NOT_DUE` unless manual; `STALE_DRAFT` after 8 days; `SLUG_TAKEN`; `WEEKLY_ALREADY_LIVE` when another `source='weekly'` post went live within 5 days, overridable only manually; inserts `is_live=true`, `published_at=now()`, `source='weekly'`, `author=byline`; marks the backlog topic used), `cancel_…`, `requeue_…`, `deliver_journal_editorial_notification`, `check_journal_editorial_authoring`, `resolve_journal_editorial_stall`, `claim_journal_editorial_newsletter` / `finish_…` (at most once).
- Discovery note (2026-09-10): `public.blog_posts` has no audit trail and no `updated_at` trigger; writers set `updated_at` themselves. This ledger's `attempt_log` and `published_at` are the only audit of weekly publication.

**Generated header photographs (2026-09-15; applied to production 2026-09-15).** Source `ops-web/supabase/migrations/20260915040000_journal_generated_images.sql`, applied through MCP `apply_migration` as ledger version `20260915051626` (`journal_generated_images`); mirror [`migrations/20260915051626_journal_generated_images.sql`](migrations/20260915051626_journal_generated_images.sql), byte-identical to the ledger (`md5 89167b234d2d83e3ae21d168d385e7c5`). Additive: `journal_editorial_assignments` gains `image_generations` (0–8, per-slot cap), `image_requested_at`, `image_requested_by` (≤200) and `image_failures` (0–3). `schedule_journal_editorial_assignment` now counts the first photograph; `publish_journal_editorial_assignment` (same signature and guards) also writes `blog_posts.image_prompt` from `package.article.image_prompt`. New service-only, security-invoker functions: `request_journal_editorial_image(uuid, text)` (returns `requested`, or `NOT_FOUND` / `NOT_ELIGIBLE` — only `scheduled`, or `published` under 8 days / `NO_IMAGE_PROMPT` / `IMAGE_LIMIT`; idempotent while a request is open), `replace_journal_editorial_image(uuid, jsonb)` (requires an `https://` preview URL; swaps `preview` and, for a published slot, `blog_posts.thumbnail_url` + `image_prompt` in one transaction; returns the state), `fail_journal_editorial_image(uuid, text, text, text, text, text)` (`retry` on the first two failures, `dropped` on the third, closing the request, keeping the current photograph and inserting one standard `journal_editorial` rail item keyed `journal:<identity>:image-failed:<n>`). Twenty journal functions in production after apply, all service_role-only with pinned search path; SQL harness `ops-web/tests/sql/journal-editorial-runtime.mjs` covers the lifecycle.

**Topic funnel (2026-09-16; applied to production 2026-09-16).** Source `ops-web/supabase/migrations/20260916190000_journal_topic_funnel.sql`, applied through MCP `apply_migration` as ledger version `20260916232832` (`journal_topic_funnel`) on Jackson's go; mirror [`migrations/20260916232832_journal_topic_funnel.sql`](migrations/20260916232832_journal_topic_funnel.sql), byte-identical to the ledger (`md5 898051989d530cd106ec8d4558449646`). Verified after apply: RLS on, no browser grants, seven new columns, four security-invoker service-only functions with `search_path=""`, settings `prepare` with no scan yet. Additive. New `public.journal_trend_signals` (RLS on, service_role only): `source_key` (watchlist key), `sphere` leadership/business/trades/industry/forum, `kind` video/article/thread, `item_key` (unique with `source_key`), `url` (https), `title` (≤300), `summary` (≤600), `published_at`, `views`, `baseline_views` (the channel's typical video), `momentum numeric(10,2)` (views ÷ baseline), `comments` (forum replies), `first_seen_at`, `last_seen_at`; index on `published_at desc`. `journal_editorial_settings` gains `radar_scan_started_at`, `radar_scanned_at`, `radar_sources jsonb` (array of each feed's last outcome `{key,name,sphere,ok,items,code}`). `journal_editorial_assignments` gains `pitch jsonb` (object), `pitch_claim_token uuid`, `pitches` 0–3, `pitched_at`. New service-only security-invoker functions: `begin_journal_radar_scan(timestamptz)` (the caller passes the moment the day's read became due, 04:00 Vancouver; raises for a future or null moment; false while `mode = off`, when a scan was recorded since that moment, or while a scan started under 10 minutes ago is unfinished; otherwise marks the start), `record_journal_radar_scan(jsonb, jsonb)` (≤1500 signals, ≤80 feeds; upsert keeping each item's first copy per scan and its `first_seen_at`; prunes `published_at` older than 45 days; stores the feed outcomes and `radar_scanned_at`; returns `{stored, pruned, ok, total}`), `notify_journal_radar(text,text,boolean,text,text,text,text)` (`raised`/`open` one standard `journal_editorial` item keyed `journal:radar-degraded`, or `resolved`/`clear`; `skipped` without a recipient), `pitch_journal_editorial_assignment(uuid, uuid, jsonb)` (object ≤60,000 bytes; lease-checked `CLAIM_NOT_OWNED`; three pitches per claim `PITCH_LIMIT`; the first pitch of a claim renews `lease_until` to now + `authoring_lease_minutes`; logs `pitched`; returns `{state:'pitched', lease_until}`). Harness `ops-web/tests/sql/journal-editorial-runtime.mjs` covers scans, upserts, pruning, the degraded item, pitch ownership, limits, lease renewal and grants (PASS 2026-09-16).

## Phase 19 site-visit shared writes and proposals (2026-09-10; production-installed, activation disabled)

**Restricted-trial update, 2026-09-14:** production migration [`20260914203418_site_visit_oauth_trial.sql`](migrations/20260914203418_site_visit_oauth_trial.sql) adds owner-only, RLS-forced `private.agent_site_visit_trial_bindings`. Columns are UUID `id` / unique `oauth_client_id` / `actor_user_id` / `company_id`, immutable `workflow_effect_sha256` and `authorization_sha256`, and `created_at` / `expires_at` / monotonic `disabled_at` timestamps. Expiry is bounded to two hours and cannot be extended or revived. The company must already have reviewed workflow-effect and compatible-phone enrollment; the migration seeds neither. The separate authorization seal covers OAuth/security definitions, ACLs, schema, constraints, triggers and policies. Nine existing lifecycle/review function definitions are fingerprint-guarded before narrow changes. Source alias `20260914200524` is not another applied migration; source/archive SHA-256 is `849d7b420c530d78f9092bf333aee893e50d007e788203cb92b61a0a103d764a`. Independent post-install readback confirmed zero bindings, company enrollments, effects, V22 clients and grants; direct service-role table writes and authenticated provisioning remain denied. No financial/catalog reseal or business mutation occurred. [Trial release and acceptance evidence](docs/artifacts/2026-09-14-site-visit-trial-release.md).

**Release update, 2026-09-12 UTC / September 11 local:** all six reviewed migrations are installed in production, with byte-identical ledger statements and source hashes. Actual ledger versions are `20260912003437`, `20260912003509`, `20260912003525`, `20260912003615`, `20260912003639`, and `20260912003656`. Independent readback verified 52 installed function bodies/ACLs, 37 unchanged dependencies, 33 unchanged existing RLS policies, six nullable no-default columns, 17 enabled triggers and ten private RLS-protected tables. Compatibility enrollment, effect policies, proposals and V22 clients/grants remain zero. Existing customer-update, financial and catalog stored effect seals were already stale before installation; they were preserved unchanged and remain fail-closed. Exact source-to-ledger mapping: [release record](docs/artifacts/2026-09-12-site-visits-release.md).

The Phase 19 candidate adds a shared revision/base write protocol for `site_visit_types` and `site_visit_checklist_answers`, preserving original phone edit bases and both versions of a conflict. Answer state/evidence are explicit; historical visit snapshots remain independent of reusable template edits. Company compatibility activation defaults off. Current and delayed legacy writers must pass the enrolled-company guards; MCP concurrency checks alone are insufficient.

Trial binding FK indexes are installed by [`20260914204329_site_visit_trial_subject_indexes.sql`](migrations/20260914204329_site_visit_trial_subject_indexes.sql): `actor_user_id` and `company_id` each have a leading index; the unique `oauth_client_id` index covers the client FK. No activation or authority definition is changed by the index migration.

Private tables hold company graph revisions, exact proposals/corrections, effect seals, command receipts and packet-discard receipts. Proposal preparation writes only review state and its declared notification. Exact business mutation and successful receipt are one database transaction; source graphs, current authority and the full proposal are rechecked before mutation. The receipt includes exact business rows and, for booking/reschedule, civil time, timezone, UTC and offset. PostgreSQL owns timezone interpretation; Node 22 checks arithmetic and binding without using ICU to reinterpret those rules.

The canonical booking discriminator remains `site_visits.booked_at`. Existing public app booking wrappers delegate to explicit-actor private implementations of the captured business bodies. Historical cancellation preserves inactive/null/empty crew and existing duration. Atomic phone discard tombstones an eligible capture packet and closes never-uploaded parent identity; it is not booking cancellation or physical completion.

The local phone update reserves SwiftData V28, independently of the MCP candidate revision numbers. Released V1–V27 use frozen pre-phase `SyncOperation`, `SiteVisitType` and `SiteVisitChecklistAnswer` stored shapes. Only the new current schema includes the seven nullable write-state/outbox metadata properties. The adjacent migration must preserve all released fingerprint entries and populated template, answer and pending-operation data through an independent reopen; exact proof belongs to the phase handoff.

Parent integration on 2026-09-11 verified all six SQL source/mirror hashes unchanged and all27 historical fingerprint entries intact. Phone merge `187a36d6`, with evidence `35d40210`, is on local iOS main after a single app-hosted run: 138 passed, two optional private-store-copy tests skipped, zero failures. Runtime checks include every released fingerprint and the populated V27→V28 independent reopen preserving original pending-work custody and nil new metadata. Subsequent source `3f5eaffe` built successfully as an Apple Development-signed Release/arm64 application and passed deep/strict signature verification. It is not installed or distributed; company compatibility/effect policies remain unactivated.

Reviewed source migrations: `20260910190020_site_visit_concurrency_protocol.sql`, `20260910191043_site_visit_durable_writes.sql`, `20260910204942_site_visit_canonical_actor_booking.sql`, `20260910205717_agent_site_visit_workflow.sql`, `20260911010000_site_visit_workflow_rate_limit.sql`, `20260911011000_site_visit_packet_discard.sql`. These preparation timestamps differ from the applied ledger versions above; the archive retains both source aliases and exact ledger-named copies, not twelve separate migrations. See the [implementation contract](specs/2026-09-10-ops-mcp-site-visits-p19-implementation.md) and phase handoff for compatibility requirements and the still-pending host/device activation gates.

## Exact Canpro cloud OAuth registration (2026-09-10; applied to production)

`20260910180330_mcp_oauth_canpro_cloud_callback.sql` replaces only `public.register_mcp_oauth_client_as_system(text,text[],text,text[],text,text,text,text)`. It adds one exact callback family for `https://bpgayztkcuencdzinfxv.supabase.co/functions/v1/source-oauth` and a frozen twenty-scope v2 read ceiling for that family. Mixed families, duplicate URIs, aliases, blank/missing scopes, and prepare/write scopes are rejected. Public-client registration continues storing exact scope/ceiling, consent revision and exposure revision in `private.mcp_oauth_clients` through the existing RPC. No table, column, RLS policy, scope catalogue, existing client/grant, financial grant or business row is changed by applying the migration. EXECUTE remains service-role-only and the function repeats its service-role check.

The production function definition was read through the OPS Supabase metadata API before release on 2026-09-10: baseline `md5(pg_get_functiondef(...)) = 96075ae7e99497c1119d2cf2f7721ce9`; ACL `{postgres=X/postgres,service_role=X/postgres}`. The approved migration was applied under actual history version **`20260910230913`**, name `mcp_oauth_canpro_cloud_callback`; independent MCP history and function readback confirmed `md5(pg_get_functiondef(...)) = 4944c9d30c3a0b8d2d1cda6899b6bf2b`, owner `postgres`, and the unchanged owner/service-role-only ACL. The release owner's before/after comparison proved all 21 pre-existing client rows and 12 grants hash-identical. Fresh public-client registration subsequently occurred through the normal application route and is documented in chapter04; registration alone does not create owner consent or a grant.

Source filename in OPS-Web: `supabase/migrations/20260910180330_mcp_oauth_canpro_cloud_callback.sql`. Actual applied-history archive here: `migrations/20260910230913_mcp_oauth_canpro_cloud_callback.sql`; its SQL bytes match the prepared source (`sha256 173826404cb4f6df5d31d221fabe8ad4611fa0099f18b601a2a01f57a920b4c4`). **`20260910180330` is the checked-in preparation timestamp, not the production migration-history version.** The migration is idempotent and preserves existing registration rows; local PostgreSQL tests exercise exact registration, consent/code binding, stale inputs, rotation and replay revocation using synthetic rows only. The companion bearer identity route is documented in `04_API_AND_INTEGRATION.md`.

## Site-visit single-choice snapshots (2026-09-11; local, unapplied)

**Approved production release, independently reverified 2026-09-14 UTC:** the phone extension and MCP coexistence repair are now installed as ledger versions `20260912213224` and `20260912213347`, respectively. Twelve installed function fingerprints, ACLs and security settings match the reviewed sources. The only new public column is nullable/no-default `choice_snapshot jsonb`; the private token table is RLS-enabled and owner-only, and both choice guards are enabled. Existing checklist RLS and stored catalog/financial effect policies are unchanged. Site-visit company compatibility and effect-policy rows remain zero. Earlier local/unapplied statements below describe the preparation checkpoint. [Release proof](docs/artifacts/2026-09-14-site-visit-choice-release.md).

**MCP coexistence follow-up, 2026-09-12 (local, unapplied):** `20260912203552_site_visit_mcp_choice_coexistence.sql` requires the phone choice extension and replaces only the two existing owner-only private workflow compiler/apply bodies. It validates answer choices before approval and permits only an exact current-record collateral default toggle through the v2 writer. Advisory-before-row lock order, revisions, full-set validation, approved proposal recompile, receipts and tenant/actor checks remain intact. No table, public API, grant or enrollment is added. A new effect fingerprint still requires separate reviewed activation. Source OPS-Web `babe8ec1da3a15b417399eb872f979057278d755`; [verification and release boundary](docs/artifacts/2026-09-12-site-visit-choice-coexistence.md).

The additive migration [`20260912004121_site_visit_single_choice_v2.sql`](migrations/pending/20260912004121_site_visit_single_choice_v2.sql) adds nullable `site_visit_checklist_answers.choice_snapshot` JSONB. It does not add a ninth field kind: choice fields remain `short_text`, their template definitions add optional `singleChoice`, and their answer values remain `{}` or `{text: exact label}`. Both metadata objects use `{version: 1, options: [{id, label}]}`. Options have 2–20 entries, unique lowercase UUIDs, nonblank trimmed labels of at most 120 Unicode codepoints, and labels unique after NFC normalization and Unicode root-locale lowercase. Stored labels and selections preserve exact original bytes.

Each answer keeps its original immutable options; later template rename/removal/reordering cannot change a captured visit. V2 writes may create structurally valid historical snapshots without requiring equality with the current template. An authorized v2 historical plain answer may retain a null snapshot after its template gains choices. Older clients cannot silently drop option definitions or create unconstrained answers against a current choice field. Existing valid plain-text selections and explicit clears remain compatible.

A revoked, RLS-enabled private token table distinguishes authorized v2 field changes from legacy metadata stripping. Choice triggers protect snapshots even before company concurrency activation. Existing actor/company, parent access, row revision, provenance and receipt rules remain in effect. The original six Phase 19 migration files remain unchanged; the additive migration includes one minimal v1 collision-resolution projection correction to omit the new column from the old closed write-value object.
...
Exact-company tables `expense_accounting_settings`, `expense_accounting_category_mappings`, `expense_accounting_payee_mappings`, `expense_accounting_project_mappings` and `expense_accounting_tax_mappings` are tied to a specific connection. Expense project/tax settings do not reuse or mutate supplier-bill/AP mappings. Settings replacement is atomic, with independent SQL checks of actor permissions and OPS category/crew ownership. Unchanged archived crew/project mappings may be retained or cleared; new archived assignments are rejected. A provider business/environment relink invalidates current expense settings and mappings while retaining immutable event history. Only explicit pre-write recovery may adopt corrected current mappings; reversals retain the original posting graph.
...
September15 release preparation strengthens this still-pending migration: the RPC resolves current login identity and company again after authority/row-lock waits, before both a new reopen and historical receipt replay. The installer requires 14 captured function definitions/owners, four enabled trigger definitions, required column types/nullability and no existing target objects; it verifies exact RPC/table grants and receipt RLS before commit. Final source SHA256 is `18845d0b67e5b6116ec4e41c165e5a85e4472e22e8f99a60580a462e004b2657`. Its identical prerequisite block passed in a production read-only transaction at `2026-09-15T03:24:50.560841Z`. Local PostgreSQL proof passes 30 behavior assertions, 14 specific migration rejections, four real identity-unlink races and an untouched install/rollback control. A negative control reproduced the original stale-identity acceptance. No production project migration or phone run occurred in this preparation.

**Applied to production 2026-09-15 at 06:26:03Z** as ledger version `20260915062603 project_task_reopen_receipts` (stored bytes md5 `b9472e0fd4867fe627796963a8dd9183`, identical to the prepared `20260914210950` file, which is now archived). Readback: `public.reopen_project_for_task(uuid,uuid,timestamptz,text)` grants exactly anon, authenticated and postgres; `private.project_task_reopen_receipts` has RLS enabled and no rows. The web build serving production carries the server side; iOS `5d0e6da1` is not released, so no client sends the reopen command yet.
...
### Expense release preparation — September15

The pending authority helper now re-resolves both current user and company after its actor/company row-lock wait. Decisions, accounting requests and exact correction replay cannot retain an identity that was unlinked or transferred while waiting. The pending lifecycle migration also revokes all independent grants on the `expense_accounting_events.sequence` identity sequence from PUBLIC, anon, authenticated and service_role; owner-executed event allocation remains available. Live default sequence privileges were reproduced in the fixture, so the regression calls actual `nextval` and `setval` under all three client roles.

The final four expense source files are preserved in an atomic release bundle: one initial captured-baseline/target-absence guard, one transaction, exact original bodies with only their outer transaction statements removed. The approved migration mechanism must eventually record one actual bundle version, then supersede the four prepared constituent paths; they must not be reapplied individually. Project reopening remains independent. This bundle is prepared only. Local proof passes nine original and nine repaired identity cases with a negative control, six allowed-original/six denied-repaired sequence operations plus owner allocation, 69 correction assertions and seven contention checks, 123 accounting assertions, two correction installation guards, and 17 payroll projection assertions with the existing payroll contract. Runtime/source evidence is retained in the iOS release record under `docs/artifacts/ios-bugs-release-20260914/`.

The bundle itself was then proved in disposable PostgreSQL 17 on the exact captured baseline: the read-only preflight and standalone guard pass on a clean baseline; the whole bundle installs in one transaction ending in COMMIT with all postconditions met; a deliberate failure injected before the final COMMIT ends in ROLLBACK with a byte-identical catalog/data snapshot and a still-passing baseline guard; blind reapplication is rejected without change; seven drift fixtures (changed function definition, wrong function owner, renamed function, partial target table, partial target function under a different signature, pre-existing `reimbursement_amount` column, non-postgres migration owner) are each rejected before any migration body runs; and the 123 accounting, 69 correction and 7 contention assertions pass on the bundle-installed database. The identical guard also passed live read-only against production at `2026-09-15T03:37:42Z`. Harness and logs: iOS `docs/artifacts/ios-bugs-release-20260914/claude-handoff/atomic-fixture/` and `expense-atomic-proof-p9/`. No production install occurred.

**Applied to production 2026-09-15 at 06:24:03Z** as ledger version `20260915062403 expense_release_atomic` (stored bytes md5 `4d1122b53e9e0218df4d243149fcae0d`, identical to the frozen bundle), after the compatibility hold build and the queue-only edge adapter were in service and both drains had passed. Production postconditions passed at 06:25:01Z; all 63 existing batches carry `reimbursement_amount`; the legacy-interval query found no approval between the adapter deploy and the bundle. The four prepared constituent versions are superseded and archived; only the bundle version exists in the ledger. `EXPENSE_ACCOUNTING_WRITE_ENABLED` is absent in production, so provider delivery remains paused and every new expense queue row is held. The release web build `3fc2f6d76` has served since 06:28:37Z. iOS clients remain unreleased.


## Try OPS sample-demo ledger (2026-09-14; schema released 2026-09-15)

Source migration `20260915000120_tryops_demo_funnel.sql` adds four service-only tables and was applied after Jackson's deployment approval under production ledger `20260915032402_tryops_demo_funnel` in project `ijeekuhbatykdomumfjx`. Its byte-identical released mirror is `migrations/20260915032402_tryops_demo_funnel.sql` (SHA-256 `1dddda94e7e3383ba6f9a5ed1402834acf56320d54d2d0b52929ce3a402a88b2`). It depends on the released trustworthy TryOps experiment schema and existing canonical users/companies. It does not activate an experiment, create a company, stamp a trial, or claim/send a welcome email.
