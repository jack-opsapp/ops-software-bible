...
### Customer and job discovery reads (local, unapplied 2026-08-21)

OPS-Web migration `supabase/migrations/20260820220000_agent_discovery_reads.sql` is complete on isolated branch `feat/mcp-discovery-reads` but is **not applied to any OPS catalog**. Its frozen SHA-256 is `29d984d6467e7ae65159b66b74e659726ed81ad612ba00d305d4f12690fd0f05`.

The migration adds two service-role-only, `SECURITY DEFINER` JSONB read boundaries: `read_agent_customer_discovery_as_system` and `read_agent_job_discovery_as_system`. It also advances the active read proof boundary to capability-manifest v7 by re-proving the nine existing reads and the Phase C wrapper in the same database statement; it never relabels a v6 proof in TypeScript. Customer discovery supports canonical name matching plus exact email or NANP phone selection without projecting either contact value. Job discovery searches only safe title/address/status/date identity fields and retains the Task 13 reciprocal opportunity/project identity rule.
...
Local PostgreSQL 17 evidence: the exact migration compiles; 143/143 statements parse through ECPG; both RPC signatures, fixed search paths, service-role-only grants, v7 reproof bridges, and 40 indexes match the frozen catalog contract; and 38/38 custom plus forced-generic plans execute with no source-table sequential scan. Broad prefix/contact and assigned-noise paths stop at 501 source rows. True-empty and all-filtered reads return zero matches with no false gap. A separate 11-boundary cutover matrix proves the migration may land before the v7 application: exact v6 calls return byte-identical v6 JSON/rows, exact v7 calls recursively re-prove the same frozen v6 core result, and NULL/unknown/mixed revisions fail. Discovery itself remains v7-only. This evidence does not replace the separately authorized target-catalog apply, readback, rollback, and real-data canary gates.
