...
## Invisible Office Weather Reschedule Preview State (locally built, unreleased)

Candidate migration `supabase/migrations/20260903123200_agent_weather_reschedule_preview.sql` defines the read-only source boundary for the ninth Invisible Office vertical, `prepare_weather_reschedule`. It is committed in an isolated OPS-Web worktree but is not production-applied or archived. Active production MCP remains read-only v2; v11 has no client, grant, activation, or customer-live authority.
...
Every settings, task, recipient, parent-client lineage, forecast, and conflict record is SHA-256 bound. One canonical source revision covers the complete snapshot; the final assertion rebuilds it from current rows using the original observation instant. The PostgreSQL 17 harness compiles Phases 7–9 in order and proves identical replay, full-registry authority parity, UUID-case collision identity, typed-settings enforcement, stale-forecast/shared-recipient/merged-parent/locked-task rejection, exact ACL and function shape, and zero business mutation. Canonical behavior and release gates: `specs/2026-09-03-ops-mcp-weather-reschedule-vertical.md`.
