...
**Images contract (2026-07-14; delete-route hardening prepared locally 2026-07-19, not deployed)**: `opportunities.images text[]` stores **full public S3 URLs**, never storage keys. Three producers: the OPS-Web email-extract pipeline (`src/app/api/integrations/email/extract-images/route.ts`, server-side puts under `email-imports/{companyId}/{opportunityId}/…`, capped 10/lead), iOS lead photos (`OPS/Services/LeadImageService.swift`, presign flow under `opportunities/{companyId}/{opportunityId}/…`), and the OPS-Web pipeline detail PHOTOS tab (added same day: `pipeline-detail-photos-tab.tsx` → `lead-photo-upload.ts` → the `/api/uploads/presign` **multipart direct-upload mode** into the same `opportunities/{companyId}/{opportunityId}/…` folder, gated on `pipeline.manage`). The web uploader deliberately does NOT use browser-direct presigned PUTs: `ops-app-files-prod` has no CORS configuration for browser origins, so a cross-origin PUT dies in preflight (verified against the live bucket 2026-07-14; iOS native URLSession and server-side producers never needed CORS). If bucket CORS is ever added, only `uploadOne` in `lead-photo-upload.ts` changes. iOS adopted `images` + `latitude`/`longitude` onto the `Opportunity` model 2026-07-14 (previously "Phase 1 defers AI/location/images"). All writers use the same server-state read-modify-write (iOS `OpportunityRepository.appendImages`/`removeImage`, web `OpportunityService.appendImages`/`removeImage` over pure `mergeImageUrls`/`removeImageUrl` in `src/lib/utils/opportunity-images.ts`: fetch → merge → update) so concurrent producers never clobber each other — race-verified 2026-07-14 by SQL-appending a sentinel URL behind a stale web cache and adding through the UI (both survived). On the web, `images` is deliberately excluded from `mapOpportunityToDb` and from `CreateOpportunity` — the array is only writable through the RMW methods. iOS offline adds ride a durable local queue (bytes via ImageFileManager, records in UserDefaults, drained on connectivity/retry/appear). At conversion the images copy into `project_photos` (`source='other'`, deduped by URL — see `09_FINANCIAL_SYSTEM.md` § Conversion transaction).

Photo deletion is an array PATCH on every surface; object deletion remains best-effort through `/api/uploads/delete`. The prepared local route authenticates the OPS user, normalizes supported S3/Supabase object references, and authorizes recognized namespaces against the underlying resource: deterministic receipts require the exact authenticated company/user path; legacy receipts resolve the referenced expense; profile/logo objects resolve the owning user/company; and lead/project photos resolve the referenced opportunity/project plus canonical edit/admin authority. Unknown, malformed, or unresolvable namespaces are denied. There is no generic same-company delete fallback. The hardened route is not live for customers until OPS-Web `main` is explicitly pushed.
