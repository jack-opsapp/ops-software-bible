...
## MCP private financial drafts (Phase 15, local)

Pending additive estimate metadata separates private distribution custody, document kind, accepted baseline, currency and scope from predecessor revisions. Private versioned policies, sealed proposals, write tokens and effect policy have forced RLS and no direct app-role table access. The pending source mirror is `supabase/migrations/20260907062351_financial_document_private_drafts.sql`; production is unchanged. See [Phase 15 contract](specs/2026-09-07-ops-mcp-financial-document-approval.md) for authority, arithmetic, locks, verification and separate migration/activation gates.
