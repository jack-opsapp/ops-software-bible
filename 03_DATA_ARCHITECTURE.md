...
The registration RPC now mirrors the application callback boundary: either the two exact Claude HTTPS callbacks, or exactly one Codex callback with raw shape `http://127.0.0.1:<1-65535>/callback/<8-128 base64url characters>`. Mixed Claude/Codex arrays and multiple Codex callbacks are rejected. Existing client rows and grants are unchanged. Every later OAuth function continues using exact array membership or equality, so consent, code creation, and token exchange never substitute ports, callback identifiers, hosts, or paths. A failed wrong-port code exchange leaves the code unconsumed; an exact retry remains valid. OAuth metadata and the MCP bearer challenge passed live checks after deployment, and Codex DCR discovery and registration work in production. MCP-specific Google/Apple login uses an on-page popup because the live full-page Firebase return produced no session. The full host canary is awaiting the user's final browser completion, so host acceptance is not yet claimed.
