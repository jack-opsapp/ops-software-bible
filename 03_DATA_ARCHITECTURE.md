...
**Images contract (2026-07-14)**: `opportunities.images text[]` stores **full public S3 URLs**, never storage keys. Two producers: the OPS-Web email-extract pipeline (`src/app/api/integrations/email/extract-images/route.ts`, server-side puts under `email-imports/{companyId}/{opportunityId}/…`, capped 10/lead) and iOS lead photos (`OPS/Services/LeadImageService.swift`, presign flow under `opportunities/{companyId}/{opportunityId}/…`). iOS adopted `images` + `latitude`/`longitude` onto the `Opportunity` model 2026-07-14 (previously "Phase 1 defers AI/location/images"). iOS writes the array via a server-state read-modify-write (`OpportunityRepository.appendImages`/`removeImage`: fetch → merge → update) so concurrent producers never clobber each other; offline adds ride a durable local queue (bytes via ImageFileManager, records in UserDefaults, drained on connectivity/retry/appear). At conversion the images copy into `project_photos` (`source='other'`, deduped by URL — see `09_FINANCIAL_SYSTEM.md` § Conversion transaction). Photo deletion is an array PATCH; the S3 object delete is best-effort only (**`/api/uploads/delete` does not exist on ops-web** — iOS's call 404s silently by design). The lead form's SITE ADDRESS uses the shared `AddressAutocompleteField`; a selection (or use-my-location) writes `latitude`/`longitude`, and any manual re-edit of the address text nulls them — coordinates never outlive the string they were resolved for.

**Client-alongside-lead (bug 1d5ab9aa, fixed on iOS 2026-07-14)**: a manually saved iOS lead match-or-creates its `clients` row before the opportunity insert (`OPS/Services/LeadClientMatcher.swift`: phone-suffix ≥7 digits → email → exact name, soft-deleted rows never match) and passes `client_id` into the create, matching the web email/lead-engine paths (`ClientService.createClient → createOpportunity(clientId)`). Client-creation failure never blocks the lead (falls back to an unlinked lead). **Known gap: the OPS-Web manual create-lead modal still creates no client** (`src/components/ops/create-lead-modal.tsx` only links pre-existing clients) — tracked as a follow-up web task.
