...
A related mixed-media/text discard repair is integrated on local iOS main as `b1cdc644` after 118 focused app-hosted simulator tests passed, including all 13 new regressions. `OPS/Network/Sync/ProjectNoteMentionEditSync.swift` projects media independently of the newest text edit and rebases only the next surviving media operation's rollback baseline. `SyncEngine.swift` guards every rebased/rewired row against active execution. Failed cancellation restores only its owned attachment-baseline key, preserving unrelated concurrently committed payload metadata and absent/null legacy fields. Entire app, test and project source trees match the passing checkout `7ef35203`; independent review found no actionable issues. This is local iPhone 17 / iOS 26.5 simulator proof, not a phone installation or release. Production still exposed the four-argument RPC at `2026-09-15T00:15:05.686583Z`; the migration remains unapplied. Evidence: `ops-ios/docs/artifacts/ios-bugs-release-20260914/photo-attachments.md`.
