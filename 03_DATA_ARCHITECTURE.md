...
#### Pre-AP intake and clearance layer (local/unreleased, 2026-09-04)

Migration `20260904040358_supplier_bill_intake_clearance.sql` adds a durable review boundary before the production AP model. It is mirrored in `migrations/20260904040358_supplier_bill_intake_clearance.sql` but has not been applied to production. The local OPS-Web implementation is committed through `bf3610e3e`; the local OPS iOS implementation is `c6269763`. Neither client commit has been pushed, deployed, or released.
