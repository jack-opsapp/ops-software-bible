...
## Exact Canpro cloud OAuth registration (2026-09-10; migration prepared, unapplied)
...
The production function definition was read through the OPS Supabase metadata API on 2026-09-10: baseline `md5(pg_get_functiondef(...)) = 96075ae7e99497c1119d2cf2f7721ce9`; ACL `{postgres=X/postgres,service_role=X/postgres}`. The prepared function fingerprint from PostgreSQL 17 is `4944c9d30c3a0b8d2d1cda6899b6bf2b`. Release must stop and reconcile if the baseline definition or ACL has changed. The migration is idempotent and preserves all existing registration rows; local PostgreSQL tests exercise exact registration, consent/code binding, stale inputs, rotation and replay revocation using synthetic rows only. The companion bearer identity route is documented in `04_API_AND_INTEGRATION.md`.
