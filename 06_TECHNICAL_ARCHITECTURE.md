...
**Suspension-safe sync admission (2026-09-11 local repair).** The recovered 3.0.5 incident `7D7758E9-D888-4C35-BE9B-3C7E4B3E639E` terminated with RUNNINGBOARD `0xDEAD10CC` while the queued delta follow-up saved relationships through DataActor. `SyncExecutionCoordinator` now gates ordinary sync admission on foreground state and acquires a UIKit execution assertion before work begins. Nested phases inherit a task-local scope. `DataActor.currentModelTransaction` accounts for the actual synchronous transaction; relationship wiring commits at most 32 parent records per chunk and propagates interruption so incomplete pulls do not advance cursors or announce success. Background entry reserves two seconds of the reported remaining time before the next transaction can start. UIKit expiration closes admission/cancels the root and ends the identifier promptly; transaction drainage is tracked separately until SQLite actually returns. No synchronous wait on the actor is introduced on MainActor, and no guarantee is made for an arbitrarily stalled SQLite write.

`SyncFollowUpRequest` retains a single pending retry across background/denied/interrupted admission and gives the next pass independent ownership. Background scheduler callbacks install their expiration scope before MainActor admission and report actual completion. A cold background startup may borrow a live OS grant; interrupted preparation waits for a newer foreground or system grant. Each grant keeps its own admission generation, and startup selects the newest eligible live permission; an older grant expiring cannot strand startup when a newer grant is already active. Individual expired readiness waiters return without cancelling shared storage preparation. BG callbacks take their account/context/actor session after readiness and revalidate it after each phase. `PhotoProcessor` retains its container, is invalidated before configuration/logout replacement, suppresses late model access after invalidation, and resumes durable `uploading` rows without charging a second retry attempt. The App Group store, V28 schema and installed data are unchanged. This is bounded coverage of the identified actor sync/startup/BG path; independent realtime, legacy flag-off and other app writers are not claimed globally suspension-safe. Optimized physical-device proof remains a separate verification requirement.

**Queued photo-prefetch ownership.** `PhotoPrefetchService` captures the originating `ModelContainer` synchronously before starting its queued task, passes that owner through the profiler wait and snapshot reader, and uses a request UUID so cancellation or completion cannot clear a replacement task. Cancellation is checked after the profiler wait and before publishing completion. Two tests exercise the real queued worker with a held snapshot and caller-released container, plus cancellation/replacement ownership.

**Local verification:** candidate `354ff585` contains repair `d78887ac` and prefetch correction `158e4af1`. Both repairs are integrated on local iOS main `3f5eaffe`, retaining the accepted P19 sources and prior repairs. All 74 selected sync tests passed with zero failures/skips on an iPhone 17 / iOS 26.5 simulator (35 new and 39 existing lifecycle/cursor checks). The combined result also contains two keyboard screenshot capability failures and is not a wholly passing suite. Evidence and per-class counts are retained at `ops-ios/docs/artifacts/ios-bugs-p3-suspension-20260911/`. The initial verification run's signal trap had no recovered stack; association with the independently confirmed prefetch lifetime defect remains unproven. No optimized physical-device proof, push, deployment or iOS release occurred.
...
*Scope narrowed 2026-09-10:* checklist answers left this boundary when they moved
onto the versioned write path (`SiteVisitVersionedSync`, protocol
`site-visit-writes:2026-09-10.v1`, commit `32d67ce9`). That command carries no
`created_by` at all — `apply_site_visit_write` stamps a new answer's author from
...
The iOS app carries an XCTest suite in ops-ios: `OPSTests` (unit, SwiftData, sync and hosted SwiftUI/snapshot tests — 5,869 cases on 2026-09-18) and `OPSUITests` (a small set of UI flows). Both run on an iOS 26.5 simulator from a worktree with its own DerivedData and `-clonedSourcePackagesDirPath .spm-local`, with `OPS/Utilities/Secrets.xcconfig` copied in (ops-ios `CLAUDE.md`, Build Guidelines). A serial full `OPSTests` run on ops-ios main `af8b452b` (2026-09-18) had 5,869 cases: 5,863 passed, 0 failed, 6 skipped by design (listed below).
...
- **A stuck keyboard.** Late in a full `OPSTests` run (after ~5,000 tests) an earlier test leaves the process unable to dismiss any keyboard: with nothing focused anywhere and the scene foreground-active, UIKit never announces the keyboard hiding, even for a fresh plain text field. Every `SiteVisitTypeSettingsKeyboardTests` case first runs a control probe — a plain field raises and dismisses the keyboard — and skips with the command to run the suite alone when it cannot (seen for `testDoneFollowsFocusBetweenAllThreeSettingsInputsWithoutLosingDrafts`, which had failed late in every full run while passing on its own). The polluting test is not yet identified.
...
**Dashboard basemap (2026-09-02).** `/dashboard` keeps its fixed, non-interactive Leaflet background and existing OPS pin/filter/popup layers, but retrieves the basemap from Mapbox Static Tiles using the `mapbox/dark-v11` style and the public `NEXT_PUBLIC_MAPBOX_TOKEN`. The Leaflet layer uses Mapbox's required 512px tile alignment (`tileSize: 512`, `zoomOffset: -1`) and visible Mapbox/OpenStreetMap attribution. If the token is absent, the app creates no external tile request and leaves the existing dark canvas in place. Source: `ops-web` commit `4bba1357`; `src/components/dashboard/map/dashboard-map-background.tsx`; `src/components/dashboard/map/dashboard-map-tiles.ts`.
