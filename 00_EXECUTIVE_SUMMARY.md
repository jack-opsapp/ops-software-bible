...
- **Remote MCP connector:** production-live at `https://app.opsapp.co/api/mcp`, with immutable exposure v1 limited to eleven actor-, company-, permission-, and source-scoped read tools and seven grantable scopes. Claude is accepted. Codex DCR discovery and registration work in production, but the full host canary is awaiting the user's final browser consent and Codex is not yet accepted. Manifest v8's additional twenty-three reads remain externally dark, and no write or site-visit capability is exposed. OAuth consent and revocation remain OPS-owned. See `04_API_AND_INTEGRATION.md` § “OPS Remote MCP Server — P1 Mount, Claude First.”
