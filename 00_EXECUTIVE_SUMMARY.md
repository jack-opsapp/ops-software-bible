...
OPS-Web commit `d5befc466c7dbf3d67b76cde698c9a9aa4df719c` defines immutable MCP exposure `2026-08-29.mcp-exposure.v2`: the complete thirty-four-tool read-only business catalogue and twenty grantable read scopes under capability manifest `2026-08-22.capability-manifest.v8`. It includes site visits, deck-design geometry and measurements, tasks, artifacts, sales, payments, expenses, supply, company/team context, integration health, work queue, and operational overview. Existing clients and grants remain pinned to immutable exposure v1 at eleven tools/seven scopes; moving a host to v2 requires fresh dynamic registration and operator consent, so no connection widens silently. The database callback policy for Claude, ChatGPT, and Codex is production-applied as ledger version `20260830004843` (`20260830113800_mcp_oauth_chatgpt_rfc9207_callback`), including ChatGPT's exact stable callback. Production Vercel deployment `dpl_3DWhhcueWxeFW2AJnEmxT1bFhfNu` is `READY` on that exact Git SHA with `app.opsapp.co` attached and no alias error. Canonical and cache-bypass authorization/protected-resource metadata advertise the exact twenty scopes and RFC 9207 issuer-response support; unauthenticated `POST /api/mcp` returns the matching 401 bearer challenge. The untouched `ops_-_maverick_projects` connector authenticated and listed exactly its eleven v1 tools without making a business call, proving v1 pinning. Two fresh v2 Codex registrations requested the exact twenty scopes but expired before approval/token exchange. A production ChatGPT DCR canary returned HTTP 201 for the exact stable callback, public-client method, authorization-code/refresh-token grants, response code, and twenty-scope string. All three v2 clients remained grant-free and were guarded-disabled. Production now has v1 seven active clients/two active grants unchanged and v2 zero active clients/three disabled clients/zero grants. The local v2 config remains unauthenticated, so no v2 host acceptance is claimed. All externally callable business tools are reads; there are no MCP create, update, delete, prepare, commit, or send tools. See `specs/2026-08-29-mcp-read-catalogue-p2.md`.
...
- **Remote MCP connector:** `https://app.opsapp.co/api/mcp` serves release commit `d5befc466c7dbf3d67b76cde698c9a9aa4df719c` through READY production deployment `dpl_3DWhhcueWxeFW2AJnEmxT1bFhfNu`. Immutable v2 contains thirty-four actor-, company-, permission-, and source-scoped read tools and twenty scopes while preserving every v1 grant at eleven/seven. Fresh DCR and consent are mandatory for v2. Claude's two exact hosted callbacks, ChatGPT's exact stable callback, and one exact Codex loopback callback are mutually exclusive registration families; successful and denied authorization responses carry the exact RFC 9207 `iss`. Rotating refresh tokens provide persistent access without `offline_access`. Both canonical and cache-bypass discovery return the exact twenty-scope v2 surface, and the unauthenticated endpoint returns the matching 401 challenge. Authenticated v1 listing is preserved and proven. Two Codex v2 callbacks expired before approval/token exchange; the ChatGPT DCR path separately returned its exact 201 registration contract. All three v2 clients are disabled with zero grants, and the local v2 entry remains unauthenticated. No v2 host acceptance or business write tool is claimed. See `04_API_AND_INTEGRATION.md` § “OPS Remote MCP Server — P1 Mount, Claude First.”
