...
OPS production now runs capability manifest `2026-08-22.capability-manifest.v8` and the complete thirty-four-read business catalogue at the database/application layer, including site visits, deck-design geometry and measurements, tasks, artifacts, sales, payments, expenses, supply, company/team context, integration health, work queue, and operational overview. All thirty-nine ordered P2 migrations are production-applied, and OPS-Web production HEAD `a567902f86946436385244acc1fd0154afe78a1e` is served by READY Vercel deployment `dpl_FGXsf3DT5rc3n1coooSyq3yuFLFc` at `app.opsapp.co`. The final deployment includes both live post-release auth repairs: it preserves the sanitized MCP authorization return path after sign-in and uses an on-page Firebase popup for MCP-specific Google/Apple login after the live full-page redirect returned no session. External MCP exposure remains immutable `2026-08-22.mcp-exposure.v1`: exactly eleven read-only tools and seven grantable read scopes. The additional twenty-three reads are deployed dark, no thirty-four-read exposure revision has been built, and all external writes remain unavailable. OAuth metadata and the MCP bearer challenge passed live checks. Claude and Codex are accepted production hosts for the verified read-only path. On 2026-08-29 Codex completed DCR and OAuth in the system browser after the in-app browser returned Firebase `auth/network-request-failed`; `codex mcp list --json` reported `ops_-_maverick_projects` enabled with `auth_status` `o_auth` at `https://app.opsapp.co/api/mcp`, and one exact `search_jobs` canary with `{"lifecycle_states":["active"],"limit":1}` succeeded without printing business data. The constrained Codex canary report did not surface an explicit contract-version label. See `specs/2026-08-29-mcp-read-catalogue-p2.md`.
...
- **Remote MCP connector:** production-live at `https://app.opsapp.co/api/mcp`, with immutable exposure v1 limited to eleven actor-, company-, permission-, and source-scoped read tools and seven grantable scopes. Claude and Codex are accepted for the verified read-only path. Codex completed DCR/OAuth through the system browser and an exact `search_jobs` canary on 2026-08-29; the in-app browser had returned Firebase `auth/network-request-failed`, and the constrained Codex canary report did not surface an explicit contract-version label. Manifest v8's additional twenty-three reads remain externally dark, and no write or site-visit capability is exposed. OAuth consent and revocation remain OPS-owned. See `04_API_AND_INTEGRATION.md` § “OPS Remote MCP Server — P1 Mount, Claude First.”
