...
Status (2026-09-10): routine **not yet created** — creation happens at the end of phase 3 (plan Task 10) with the `schedule` skill, DISABLED, and the id is recorded here. Enable only after the phase 1 warehouse and phase 2 campaigns are live, the ops-web release with the engine routes is deployed, `ADS_ENGINE_TOKEN` is set in Vercel Production, the environment API credential exists, and Jackson gives the go.

| Field | Value |
|---|---|
| Routine id | _pending creation_ |
...
| Prompt | canonical copy in `ops-web/docs/ads/engine-routine.md`, version `ads-routine-2026-09-10-v1`; edit there first, then `/schedule update` |
