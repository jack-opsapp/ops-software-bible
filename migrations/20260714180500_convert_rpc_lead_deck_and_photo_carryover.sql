-- Conversion carries the lead's own evidence onto the project:
--   1. deck designs drawn on the lead (deck_designs.opportunity_id) re-parent
--      to the new/linked project (never stealing a deck already parented);
--   2. the lead's photo set (opportunities.images, full public S3 URLs) copies
--      into project_photos (source 'other'), deduped by URL.
-- Both platforms funnel here: iOS calls this RPC directly; web's
-- convert_lead_to_project is a shim into it. Result gains relinked_decks +
-- attached_lead_photos (additive keys; existing readers unaffected).
--
-- Full function body as applied 2026-07-14 (project ijeekuhbatykdomumfjx).
-- Deltas vs the 2026-06-04 version: declares v_decks/v_lead_photos; two new
-- blocks after the site-visit photo copy; two new result keys.
CREATE OR REPLACE FUNCTION public.convert_opportunity_to_project(p_company_id uuid, p_opportunity_id uuid, p_actual_value numeric DEFAULT NULL::numeric, p_expected_stage text DEFAULT NULL::text, p_decided_by uuid DEFAULT NULL::uuid, p_notes text DEFAULT NULL::text, p_title_override text DEFAULT NULL::text, p_link_to_project_id uuid DEFAULT NULL::uuid, p_source_path text DEFAULT NULL::text, p_win_opportunity boolean DEFAULT true, p_project_status text DEFAULT NULL::text, p_evidence jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
declare
  ...
  v_created_by uuid;
  v_decks bigint := 0; v_lead_photos bigint := 0;
begin
  ...
  -- Lead-owned photos (opportunities.images) follow the deal onto the project.
  -- source 'other' (no site visit); dedup by URL against anything already
  -- copied (a URL that arrived via a site visit is not duplicated).
  insert into public.project_photos (
    id, project_id, company_id, url, source, uploaded_by, taken_at, created_at)
  select gen_random_uuid(), v_project_id::text, p_company_id::text, img, 'other',
         coalesce(p_decided_by::text, ''), null, now()
    from unnest(coalesce(v_opp.images, '{}'::text[])) as img
   where img is not null and img <> ''
     and not exists (select 1 from public.project_photos pp
                      where pp.project_id = v_project_id::text and pp.url = img);
  get diagnostics v_lead_photos = row_count;

  -- Deck designs drawn on the lead re-parent to the project. Only unparented
  -- decks move — a deck already attached to some project is never stolen.
  -- opportunity_id stays for provenance (the lead still shows its deck).
  update public.deck_designs
   ...
   where opportunity_id = p_opportunity_id and company_id = p_company_id
     and deleted_at is null and project_id is null;
  ...
  return jsonb_build_object(
    ...
    'attached_lead_photos', v_lead_photos, 'relinked_decks', v_decks,
