-- Onboarding rebuild spec 6.2 (Task 1.2): amend join_user_to_company so the
-- join is fully atomic server-side.
--
-- Changes vs the previous definition (everything else byte-equivalent):
--   1. The existing users UPDATE (company_id assignment) now also sets
--      user_type = 'employee' and is_company_admin = false. Previously the iOS
--      client wrote these AFTER the RPC via a fire-and-forget PostgREST update
--      that silently swallowed failures; that client write is being deleted —
--      the RPC is now the only writer on the join path.
--   2. Per-admin "team member joined" rail notifications are inserted via the
--      existing create_notification_if_new dedup helper, mirroring the ops-web
--      /api/auth/join-company fan-out (buildMemberJoinedCopy) byte-for-byte:
--      same type ('role_needed'), titles/bodies, persistence, action
--      url/label, and dedupe surface (dedupe_key NULL -> partial unique
--      indexes on (user_id, company_id, type, title) while unread + the
--      helper's ON CONFLICT DO NOTHING). Until the web route's own fan-out is
--      removed (Task 1.3) both writers fire; identical payloads make the
--      second insert a no-op.
--   3. Push delivery (OneSignal) intentionally stays client/route-side; the
--      RPC performs no HTTP.
--
-- Notification inserts are best-effort: each is wrapped so a failure logs a
-- WARNING and never aborts the join. The user_type/is_company_admin write is
-- NOT optional — it is part of the atomic join.
