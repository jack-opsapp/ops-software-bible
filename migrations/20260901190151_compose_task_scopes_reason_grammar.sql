-- compose_task_scopes — reason grammar for visits that repeat a task type.
-- Replaces the function applied in 20260901185510_compose_task_scopes_rpc.sql. Composition is
-- unchanged; only the human-readable reason changes: a visit whose scopes are all one task type
-- now reads "<type> (N scopes)" instead of pluralising a single name ("Glass Rail Install share a
-- crew and none depends on another"). Estimate conversion routinely produces several line items of
-- the same type, so that visit shape is common.
--
...
begin
  ...
  foreach v_type_id in array v_pred_order loop
    ...
    if jsonb_array_length(v_scopes) > 1 then
      v_names := v_names || ' (' || jsonb_array_length(v_scopes) || ' scopes)';
    end if;
  ...
  for v_g in 0 .. v_leaf_groups - 1 loop
    ...
    select string_agg(y.display, ', ' order by y.first_idx), count(*)
      into v_names, v_name_count
    ...
    if v_name_count = 1 and jsonb_array_length(v_scopes) > 1 then
      v_names := v_names || ' (' || jsonb_array_length(v_scopes) || ' scopes)';
    end if;
