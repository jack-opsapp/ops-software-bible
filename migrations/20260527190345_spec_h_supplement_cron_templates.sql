-- SPEC Phase 1 — Stage H supplement (round 2): register the three
-- email templates that C.5's owner-approval-expiry + customer_requested
-- hold-expiry crons queue to spec_email_outbox but Stage H's initial
-- batch of 20 (per SPEC/07_ROLLOUT.md § 11) and the P1-2-19 batch (entitlement
-- toggles) did not include. Closes the gap flagged by Stage I verification
-- report § P1-2-18-3 before the SPEC_LIVE_DEPOSITS_ENABLED flag flip.
--
...
-- the renderer rejects unregistered template_ids. The buyer never learns
-- their owner-approval request timed out; the owner never learns the buyer
-- request was cancelled; the customer never learns their 90-day pause hit
-- the stalled state.
--
-- Hashes are sha256 over each template TSX source byte stream at supplement
-- commit time. Same byte-stability contract as
-- 2026-05-26-02-spec-phase1-email-templates.sql and
-- 2026-05-26-07-spec-h-supplement-entitlement-templates.sql — the OPS-Web
...
--
-- All three templates carry no-emoji, no-exclamation OPS tactical voice. The
-- hold-expiry template stays neutral between resume and Guarantee Refund
-- (operator-side action remains manual). The owner-expired template tells
-- the account_holder they can self-initiate at /spec without waiting for a
