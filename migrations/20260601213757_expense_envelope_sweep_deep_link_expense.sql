-- Migration: expense_envelope_sweep_deep_link_expense
-- Applied to prod (ijeekuhbatykdomumfjx) 2026-06-01 via Supabase MCP apply_migration (version 20260601213757).
-- Part of: Expense Auto-Batching — Phase 1 (Server Brain).
--
-- Corrects the auto-send notification's deep_link_type from 'invoice_detail' (NOT in the documented
-- routable set in 07_SPECIALIZED_FEATURES.md §14.3.1) to 'expense' (documented + routable, matches
-- the /expenses?batch= action_url). Only change vs 20260601211633_expense_envelope_sweep_notify_dedupe.
-- The function had not yet fired (cron scheduled afterwards), so no notification ever used the old value.
...
begin
  ...
  loop
    ...
    -- one notification per envelope per approver; per-envelope dedupe_key + ON CONFLICT DO NOTHING.
