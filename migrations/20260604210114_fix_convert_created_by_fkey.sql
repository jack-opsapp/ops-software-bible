...
-- auth.users, so storing p_decided_by (a public.users id) violated
-- projects_created_by_fkey and rolled back every win. Store created_by only when
-- it is a genuine auth.users id, else NULL (the existing-project invariant).
-- Operator attribution stays on the disposition + stage transition.
