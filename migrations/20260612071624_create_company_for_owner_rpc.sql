-- Single shared company-creation path for iOS + OPS-Web (ONBOARDING REBUILD spec §6.1).
-- Caller identity derived strictly from the JWT sub claim (Firebase UID) -> users.firebase_uid.
-- Idempotent: an existing owner gets their real stored company_code back.
-- Typed errors (match on message): NO_JWT (P0001), NO_USER_ROW (P0002),
--   ALREADY_IN_COMPANY (P0003), OWNER_ROLE_MISSING (P0004), INVALID_NAME (P0005),
--   CODE_GENERATION_EXHAUSTED (P0006).
...
-- The draft''s REVOKE FROM anon alone is ineffective: functions grant EXECUTE to PUBLIC
-- by default and anon inherits it. Revoke PUBLIC explicitly.
REVOKE ALL ON FUNCTION public.create_company_for_owner(text, text[], text, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_company_for_owner(text, text[], text, text, text) FROM anon;
GRANT EXECUTE ON FUNCTION public.create_company_for_owner(text, text[], text, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_company_for_owner(text, text[], text, text, text) TO service_role;
