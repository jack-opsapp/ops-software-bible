...
--
-- ops-web compatibility layer: legacy inventory_* table NAMES become VIEWS
-- over the new catalog_* tables, with INSTEAD OF triggers so ops-web's
-- existing CRUD against `inventory_*` keeps working until the follow-up
-- ops-web rewrite session lands. Also installs the base_price ↔ default_price
-- mirror trigger on `products`.
--
-- ORDERING: this runs AFTER Phase 2's data migration. Legacy inventory_*
-- tables for Canpro and Maverick have been soft-deleted; other companies
-- never had real data. We can safely drop the legacy tables and recreate
-- the names as views.

BEGIN;

-- =====================================================
-- 0. SAFETY: refuse to drop if any non-soft-deleted legacy row exists.
--    (Defensive; phase 2 already soft-deleted Canpro/Maverick rows, and
--    no other company has data, but cheap to verify.)
-- =====================================================
DO $$
DECLARE
...
-- =====================================================
-- 1. Drop legacy inventory_* tables
-- =====================================================
...
-- =====================================================
-- Each catalog_variant maps to one row in the legacy inventory_items shape.
-- Family-level fields (name, description, image_url) come from catalog_items;
-- variant-level fields (qty, sku, thresholds, unit_id) come from catalog_variants.
-- ops-web sees what it expects: rows with name + qty + tags.
...
-- =====================================================
-- 6. inventory_snapshots / inventory_snapshot_items VIEWS (read-only — ops-web doesn't write today)
-- =====================================================
...
-- =====================================================
-- When a writer updates one, mirror the other. This keeps ops-web (which
-- writes default_price) and iOS (which writes base_price) coherent until
-- ops-web cuts over in a follow-up session.
