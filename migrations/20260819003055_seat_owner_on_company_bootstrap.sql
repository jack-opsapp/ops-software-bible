-- Seat the owner on company bootstrap, and backfill every company that missed it.
--
-- `create_company_for_owner` (iOS) has always written the owner into
-- `seated_employee_ids`; the web signup path never did, so web-created
-- companies carried a roster missing the one person guaranteed to be using the
-- product. This brings `create_company_for_owner_by_id` to parity and repairs
-- the existing rows.
--
-- The owner legitimately appears in BOTH `admin_ids` and `seated_employee_ids`.
-- ops-web's seat COUNT dedupes across the two arrays (it previously summed
-- them, double-counting the owner on every iOS-created company), so seating the
-- owner does not consume a second seat.
  ...
  if v_company_id is not null then
    ...
    update public.companies
       set name              = btrim(p_name),
           ...
           seated_employee_ids = case
                                 when p_user_id::text = any(coalesce(seated_employee_ids, array[]::text[]))
                                   then seated_employee_ids
                                 else coalesce(seated_employee_ids, array[]::text[]) || p_user_id::text
                               end,
           updated_at        = now()
      ...
      begin
        if v_company_id is null then
          insert into public.companies (
            ...
            referral_method, company_code, admin_ids, seated_employee_ids,
            account_holder_id, created_at, updated_at
          )
          values (
            btrim(p_name),
...
comment on function public.create_company_for_owner_by_id(
  ...
  'explicitly named owner in ONE transaction (company + join code + owner seat '
  '+ Owner user_roles row + owner labels + initialize_company_defaults). Adopts '
  'an existing unlinked company for the same account_holder_id so a retry after '
  'a partial failure completes it instead of orphaning it. NOT granted to '
  'authenticated -- the caller names the owner.';

-- Backfill: seat the account holder on every live company that already exists.
-- Web-created companies never seated the owner (iOS always did), so the roster
-- was missing the one person guaranteed to be using the product. No company can
-- exceed max_seats from this: the owner is already counted through admin_ids,
-- and ops-web now counts DISTINCT people across admin_ids + seated_employee_ids.
--
-- The synthetic `TOCTOU RACE ...` fixtures (holder b0000000-...dead) carry no
-- admin_ids and are not real companies; they are left untouched.
do $seat_owners$
declare
  v_n integer;
begin
  update public.companies
     set seated_employee_ids =
           coalesce(seated_employee_ids, array[]::text[]) || account_holder_id,
         updated_at = now()
   where deleted_at is null
     and account_holder_id is not null
     and account_holder_id <> 'b0000000-0000-4000-8000-00000000dead'
     and not (account_holder_id = any(coalesce(seated_employee_ids, array[]::text[])));

  get diagnostics v_n = row_count;
  raise notice 'seated % owner(s)', v_n;

  -- Post-condition: no live company may exceed its seat cap.
  if exists (
    select 1 from public.companies
     where deleted_at is null
       and coalesce(array_length(seated_employee_ids, 1), 0) > coalesce(max_seats, 10)
  ) then
    raise exception 'POSTCONDITION_FAILED: a company exceeds max_seats after backfill';
  end if;
end
$seat_owners$;
