-- 2026-05-30-04-ios-catalog-p6-inventory-off-release-allocations.sql
--
-- DATA-INTEGRITY FIX (P6 inventory mode toggle). When a company switches inventory
-- mode to 'off', public.set_company_inventory_mode released its open
-- project_material_demands (status -> 'released') and wrote an 'inventory_mode_released'
-- snapshot, but did NOT release the task_material_allocations attached to those
-- demands. The allocations were left dangling in 'projected'/'overrun', diverging from
-- the demand state. private.persist_estimate_material_booking_projection's own off-branch
-- already does this correctly via a `released_allocations` CTE; this grafts the same
-- release into the toggle path so both routes behave identically.
--
-- The body below is the live definition with three surgical additions (everything else
-- is verbatim): (1) the `released` CTE now also returns demand_row.id; (2) a new
-- `released_allocations` CTE flips allocation_status 'projected'/'overrun' -> 'released'
-- for those demands (consumed/released/superseded are protected); (3) the released
-- allocation count is returned as 'released_allocations'. Additive / idempotent;
-- CREATE OR REPLACE preserves ownership, SECURITY INVOKER, and EXECUTE grants.
