...
-- Allow anon + authenticated to SELECT (this is sanitized aggregate-only data; safe to expose).
...
-- Schedule pg_cron job to refresh every 5 minutes.
-- cron.schedule is idempotent on the job name in current pg_cron versions.
