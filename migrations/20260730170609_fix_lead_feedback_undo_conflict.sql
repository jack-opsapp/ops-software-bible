...
--
-- ROOT CAUSE: the apply RPCs write opportunities.updated_at = clock_timestamp()
-- and record that same value as the optimistic-concurrency snapshot Undo checks.
-- The BEFORE UPDATE trigger trg_opp_timestamp -> public.update_timestamp()
-- (NEW.updated_at = now()) overwrites it, landing the row ~3ms behind the
-- snapshot, so undo_lead_disposition_feedback's guard always tripped.
--
-- FIX: read the stored updated_at (and archived_at) back with RETURNING instead
-- of predicting it. Undo's guard is deliberately unchanged. No signature, grant,
-- RLS, constraint or table change; no backfill needed.
...
declare
  ...
  -- The value the row ACTUALLY lands with, after BEFORE triggers.
...
declare
  ...
  v_source_thread_candidate_count integer := 0;
  v_now timestamptz := clock_timestamp();
  -- Undo compares BOTH of these against the stored row.
...
begin
  ...
  -- RETURNING, not v_now: see the disposition path above.
