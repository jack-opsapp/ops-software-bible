...
--
-- Bespoke per-company data migration: Canpro Deck and Rail (a612edc0-...)
-- and Maverick Projects Ltd (ddee107c-...). Other companies have no real
-- inventory data — they remain empty in the new tables and onboard via
-- the new UI.
--
-- Strategy: build TEMP tables that project legacy inventory_items/tags
-- into the new shape, then INSERT into catalog_* tables in dependency
-- order. Verification asserts pre/post variant counts and quantity sums.
--
-- Apply order: 01 schema → 03 data (this) → 02 views/triggers → 04 perms.

BEGIN;

-- =====================================================
-- BASELINE CAPTURE for verification at end
-- =====================================================
...
-- =====================================================
-- 1. CATEGORIES — deterministic UUIDs per company
-- =====================================================
-- Canpro
...
-- Maverick (parallel set; some categories empty for now but available)
INSERT INTO public.catalog_categories (id, company_id, name, parent_id, sort_order) VALUES
...
-- =====================================================
-- 2. UNITS — copy from inventory_units verbatim
-- =====================================================
...
SELECT
    ...
    -- Stable family id derived from the family_key for repeatability
...
UPDATE __canpro_families SET category_id = CASE
    WHEN klasses IS NOT NULL AND 'Rail'   = ANY(klasses) THEN '11111111-1111-0001-1111-000000000040'::uuid  -- Rail
    WHEN klasses IS NOT NULL AND 'Screws' = ANY(klasses) THEN '11111111-1111-0001-1111-000000000051'::uuid  -- Screws (under Fasteners)
    WHEN klasses IS NOT NULL AND 'Gate'   = ANY(klasses) THEN '11111111-1111-0001-1111-000000000060'::uuid  -- Gates
    WHEN mounts IS NOT NULL AND array_length(mounts, 1) > 1 THEN '11111111-1111-0001-1111-000000000020'::uuid  -- Posts (multi-mount)
    ...
    ELSE '11111111-1111-0001-1111-000000000020'::uuid  -- fallback Posts
END;

-- =====================================================
-- 4. CANPRO — INSERT catalog_items
-- =====================================================
...
SELECT
    ...
    f.category_id,
    f.canonical_name,
    NULL,  -- no unit on Canpro inventory (every inventory_item.unit_id was null)
    true,
    now(),
    now()
...
-- =====================================================
-- 5. CANPRO — INSERT catalog_options (Color always; Mount Type only multi-mount)
-- =====================================================

-- Color option for every family that has at least one color
...
-- Mount Type option only for multi-mount families
INSERT INTO public.catalog_options (id, catalog_item_id, name, sort_order)
SELECT
...
-- =====================================================
-- 6. CANPRO — INSERT catalog_option_values
-- =====================================================
-- Color values
...
-- Mount values (only for multi-mount families)
INSERT INTO public.catalog_option_values (id, option_id, value, sort_order)
SELECT DISTINCT
...
-- =====================================================
-- 7. CANPRO — INSERT catalog_variants (preserve inventory_item_id as variant.id)
-- =====================================================
...
SELECT
    ...
    ci.company_id,
    f.family_id,
    NULL,  -- Canpro variants have no SKU set today
    ci.quantity,
...
-- =====================================================
-- 8. CANPRO — INSERT catalog_variant_option_values
-- =====================================================
-- Color links: every variant with a color_tag links to its family's Color/<color>
...
-- Mount links: only for variants whose family is multi-mount
INSERT INTO public.catalog_variant_option_values (variant_id, option_value_id)
SELECT
    ci.inventory_item_id,
...
SELECT
    ...
    i.name AS original_name,
    lower(trim(i.name)) AS family_key,
    -- Maverick uses "Black Qty" / "White Qty" — normalize to "Black"/"White"
...
-- Maverick category by family-name prefix
UPDATE __maverick_families SET category_id = CASE
    WHEN family_key LIKE 'sidemount %'  THEN '22222222-2222-0002-2222-000000000030'::uuid  -- Side mount
    WHEN family_key LIKE 'topmount %'   THEN '22222222-2222-0002-2222-000000000031'::uuid  -- Topmount
    WHEN family_key LIKE 'stair %'      THEN '22222222-2222-0002-2222-000000000012'::uuid  -- Hardware Stair
    WHEN family_key LIKE 'level %'      THEN '22222222-2222-0002-2222-000000000011'::uuid  -- Hardware Level
    WHEN family_key LIKE 'picket %'     THEN '22222222-2222-0002-2222-000000000040'::uuid  -- Rail
    WHEN family_key LIKE 'glass rail%'  THEN '22222222-2222-0002-2222-000000000040'::uuid  -- Rail
    WHEN family_key = 'windwall sleeve in' THEN '22222222-2222-0002-2222-000000000011'::uuid  -- Hardware Level
    WHEN family_key = 'blank adapter'   THEN '22222222-2222-0002-2222-000000000011'::uuid  -- Hardware Level
    WHEN family_key = '45 degree bracket' THEN '22222222-2222-0002-2222-000000000011'::uuid -- Hardware Level
    ELSE '22222222-2222-0002-2222-000000000020'::uuid  -- fallback Posts
END;

-- =====================================================
-- 10. MAVERICK — INSERT catalog_items
-- =====================================================
INSERT INTO public.catalog_items
    (id, company_id, category_id, name, default_unit_id, is_active, created_at, updated_at)
SELECT
...
-- =====================================================
-- 11. MAVERICK — INSERT catalog_options (Color only)
-- =====================================================
INSERT INTO public.catalog_options (id, catalog_item_id, name, sort_order)
SELECT
...
-- =====================================================
-- 12. MAVERICK — INSERT catalog_option_values (Color values)
-- =====================================================
INSERT INTO public.catalog_option_values (id, option_id, value, sort_order)
SELECT DISTINCT
...
-- =====================================================
-- 13. MAVERICK — INSERT catalog_variants (preserve original ids)
-- =====================================================
INSERT INTO public.catalog_variants
    (id, company_id, catalog_item_id, sku, quantity, warning_threshold, critical_threshold, unit_id, is_active, created_at, updated_at)
SELECT
...
-- =====================================================
-- 14. MAVERICK — INSERT catalog_variant_option_values
-- =====================================================
INSERT INTO public.catalog_variant_option_values (variant_id, option_value_id)
SELECT
    mi.inventory_item_id,
...
-- =====================================================
-- 16. SOFT-DELETE LEGACY ROWS (so duplicate visibility doesn't confuse readers
--     between this migration and the 02-views-triggers migration that drops them)
-- =====================================================
...
UPDATE public.inventory_tags SET deleted_at = now()
 WHERE company_id IN ('a612edc0-5c18-4c4d-af97-55b9410dd077','ddee107c-33cd-483e-8278-0f8d8a180181')
   AND deleted_at IS NULL;
-- inventory_units rows were copied to catalog_units verbatim above with same ids;
-- we soft-delete the legacy rows so reads see only the new world.
UPDATE public.inventory_units SET deleted_at = now()
 WHERE company_id IN ('a612edc0-5c18-4c4d-af97-55b9410dd077','ddee107c-33cd-483e-8278-0f8d8a180181')
   AND deleted_at IS NULL;
-- inventory_item_tags has no deleted_at column — when 02-views-triggers replaces
-- inventory_item_tags with a view, it drops the table entirely and rebuilds the view.
