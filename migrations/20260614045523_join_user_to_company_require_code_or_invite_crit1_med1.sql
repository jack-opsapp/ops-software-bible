-- CRIT-1: join_user_to_company only enforced the company-code proof WHEN a
-- non-empty code was supplied. Passing p_company_code = NULL skipped the check
-- entirely and the team_invitations lookup only set the role (never gated the
-- join), so any authenticated user could join ANY company with no code and no
-- invite. Fix: for non-service callers who are not already the company's
-- owner/admin and not already a member of THIS company, REQUIRE either a
-- matching company_code OR a valid pending invitation (matched by the caller's
-- email/phone). A supplied-but-wrong code is still rejected as
-- company_code_mismatch; absent both proofs the join is rejected as
-- join_not_authorized. service_role is unchanged (the API route enforces the
-- code). The invitation lookup is moved above the gate so it can authorize the
-- join, and is still reused below for role assignment + invite-accept.
--
-- MED-1: the joining member controls their own first_name/last_name, which flow
-- verbatim into the admin rail-notification title/body (and the title is the
-- dedupe key). Strip control characters/newlines and length-cap the
-- member-supplied name used in the notification copy to prevent
-- notification-content injection and keep the dedupe title bounded.
...
BEGIN
  ...
  -- Cheap unlocked short-circuit; the authoritative re-check happens below
  -- under the users-row lock.
  ...
  -- TOCTOU guard: re-read company_id under a users-row lock and re-evaluate
  -- the reassignment guard against the locked value. Closes the race where
  -- create_company_for_owner commits a new company_id between the unlocked
  -- read above and the users UPDATE below (which would otherwise overwrite
  -- the fresh company and demote the new owner via the guard below).
  -- Lock acquisition order is companies-row -> users-row; the sibling RPC
  -- takes advisory -> users-row and never locks an existing companies row,
  -- so the ordering is deadlock-free.
  SELECT company_id INTO v_user.company_id FROM users WHERE id = p_user_id FOR UPDATE;
  ...
  v_is_owner_or_admin := COALESCE(v_user_id_text = v_company.account_holder_id, false)
    OR v_user_id_text = ANY(COALESCE(v_company.admin_ids, ARRAY[]::text[]));

  -- Invitation lookup (matched by the caller's email then phone). Moved ahead of
  -- the join-authorization gate (CRIT-1) so a valid pending invite can authorize
  -- a codeless join; the same record is reused below for role assignment and the
  -- invite-accept write.
  v_invitation := NULL;
  ...
  -- CRIT-1 join-authorization gate: a non-service caller must PROVE the join.
  -- Exemptions: the company's own owner/admin (re-join must not be demoted) and
  -- a user already in THIS company (idempotent re-join). Otherwise require a
  -- matching company_code OR a valid pending invitation. A supplied-but-wrong
  -- code is rejected (preserves the prior explicit company_code_mismatch). With
  -- no matching code AND no invite the join is refused. service_role bypasses
  -- this gate: the API route is responsible for the code there.
  IF NOT v_is_service_role
     AND NOT v_is_owner_or_admin
     AND NOT (v_user.company_id IS NOT NULL AND v_user.company_id = p_company_id)
  THEN
    v_expected_company_code := lower(trim(COALESCE(v_company.company_code, '')));
    v_code_supplied := (p_company_code IS NOT NULL AND trim(p_company_code) <> '');
    v_code_matches  := v_code_supplied
                       AND v_expected_company_code <> ''
                       AND lower(trim(p_company_code)) = v_expected_company_code;

    IF v_code_supplied AND NOT v_code_matches THEN
      RETURN jsonb_build_object(
    ...
    IF NOT v_code_matches AND v_invitation.id IS NULL THEN
      RETURN jsonb_build_object(
        'error', 'A valid company code or invitation is required to join.',
        'code', 'join_not_authorized'
      );
    END IF;
  END IF;
  ...
  -- MED-1: sanitize the member-controlled name before it flows into rail copy.
  -- Strip control chars/newlines (collapse to a single space) and length-cap so
  -- a crafted first_name cannot inject layout-breaking content into the admin
  -- notification, and so the dedupe title (which embeds first_name) stays bounded.
  v_new_member_first_name := COALESCE(NULLIF(TRIM(v_user.first_name), ''), 'A new member');
  v_new_member_first_name := left(regexp_replace(v_new_member_first_name, '[[:cntrl:]]+', ' ', 'g'), 40);
  v_new_member_first_name := COALESCE(NULLIF(TRIM(v_new_member_first_name), ''), 'A new member');

  v_new_member_name := TRIM(CONCAT_WS(' ', v_user.first_name, v_user.last_name));
  v_new_member_name := TRIM(left(regexp_replace(v_new_member_name, '[[:cntrl:]]+', ' ', 'g'), 80));
  ...
  -- web joins. Mirrors ops-web buildMemberJoinedCopy byte-for-byte: same type
  -- ('role_needed'), titles/bodies, persistence, action url/label, and the
  -- same dedupe surface (create_notification_if_new inserts with dedupe_key
  -- NULL; the partial unique indexes on (user_id, company_id, type,
  -- title-while-unread) plus ON CONFLICT DO NOTHING dedupe).
  -- The web route's own rail fan-out was removed (Task 1.3, ops-web commit
  -- bc61f062) — this RPC is the sole rail writer for joins. The copy strings
  -- are still mirrored in ops-web notification-copy.ts for push bodies; the
  -- TITLE is the structural dedupe key, so change both or neither.
