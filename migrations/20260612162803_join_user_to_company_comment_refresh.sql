-- Doc-only refresh: Task 1.3 landed (ops-web commit bc61f062), so the interim
-- dual-writer note inside the fan-out block is stale. No behavior change —
-- function body identical except the two comment lines, plus a catalog COMMENT.
...
COMMENT ON FUNCTION public.join_user_to_company(uuid, uuid, text) IS
'Atomic employee join: validates caller (JWT email/firebase_uid/auth_id or service_role), proves the company code, grants a seat, accepts a matching pending team_invitation, maps the role (user_roles upsert), writes the employee record (user_type=employee, is_company_admin=false; owners/admins exempt from demotion), and fans out per-admin rail notifications (sole rail writer since Task 1.3; push stays client-side). Returns 13-key jsonb: success, user_id, company_id, role_id, role_name, seat_granted, invitation_found, admin_ids, invited_by, new_member_id, new_member_name, new_member_first_name, company_name. Business errors return {error, code}: caller_user_mismatch, user_not_found, company_reassignment_denied, company_not_found, company_code_mismatch, seat_full.';
