-- SPEC Phase 1 email templates — initial registration in email_template_versions.
-- Hashes computed at Stage H commit time (sha256 of each template TSX source byte stream).
-- The `email:sync-versions` build script will treat these as 'unchanged' on its next
-- run as long as the source file content matches; any source-content change without
-- a @template-version bump triggers a hash-mismatch failure (CI gate).
...
values
  ...
  ('spec.p4_invoice',                        '1.0.0', '337f7b5f59b6165da2f2f3ac958ebbd7ebf259921bdb7647daf9b94997f21776', 'Phase 1 initial registration — Stage H. P4 milestone invoice (post-walkthrough, includes 30-day Guarantee anchor).'),
