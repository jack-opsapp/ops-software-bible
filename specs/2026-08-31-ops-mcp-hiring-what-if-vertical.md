...
The schema and application are production-released in OPS-Web `a763f1a0`, with production ledger `20260902194603_agent_hiring_what_if_read`. The live function remains service-role-only, and v5 has zero clients and zero grants. Active production exposure remains read-only v2; this vertical is deployed but dormant, not host-accepted or customer-live. The release record in `specs/2026-09-02-ops-mcp-phases-3-7-production-release.md` supersedes the earlier local-only release boundary.
