...
- zero → create client (person as client; organization name → client + sub-client per intake rule); membership `active_full` with `evidence_kind=created_by_identity` (it is their own new record);
...
- Intake worktree tables (`external_api_*`, `lead_intake_*`) are **not** production-live (verified 2026-09-01); P4 lands them together with custom fields.
