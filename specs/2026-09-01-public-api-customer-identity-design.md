...
- more than one distinct parent client → **depends on the caller, on the same rule as the zero branch (I18, extended 2026-09-03 when P1-7 shipped the split).** At a genuine intent: create a fresh client flagged possible-duplicate (D6), membership `active_full` on the fresh client only, staff notification via `create_notification_if_new_with_identity` (persistent, dedupe key `customer_identity:possible_duplicate:<client_id>`). At sign-in or any read: **create nothing** — guessing one of the matches hands the customer someone else's history, and minting a third record makes the company's duplicates worse. Resolve to `none`, append a `membership_match_ambiguous` event against the identity, and let it resolve itself: once staff merge the duplicates the next sign-in matches exactly one. I18's rule is categorical — a client record is born of intent, not of sign-in — and the zero branch is only its most obvious case.
