...
**Status:** Production Foundation Zero, operator-owned configuration, and the exact-subject v3 canary guardrails are live but dormant. The active customer contract remains read-only v2; production has no canary binding, v3 client, v3 grant, or routine, and the worker flag is absent. Authenticated v3 live-host proof, v3 activation, and worker activation have not been performed.
**Parent vision:** `specs/2026-08-30-ops-mcp-vision-handoff.md`
**Release:** OPS-Web Foundation Zero `1742861a`; production-readiness release `1945c60b`; exact canary release `f0dd81be`, contained by live production head `d3d51884`; Vercel `dpl_H7ncZ6jA7dyN8k4xcGMx6XUGczPU` (`Ready`, production, `app.opsapp.co`).
...
Production ledger versions `20260831042518_agent_day_closeout_foundation_zero`, `20260831042631_agent_day_closeout_routine_worker`, `20260831042924_agent_day_closeout_fk_indexes`, `20260831061700_agent_day_closeout_routine_configuration`, and `20260901012208_mcp_v3_synthetic_canary` are applied to `ops-app` and mirrored byte-exact in this Bible. Live readback proves the six closeout tables and canary-binding table have RLS enabled, zero policies, zero direct `anon`/`authenticated`/`service_role` table grants, and zero rows; every public closeout/canary RPC is search-path pinned and executable only by `service_role`. The canary's four OAuth enforcement triggers are enabled. Production has zero bindings, zero v3 clients/grants, zero v4 clients/grants, and zero enabled routines. Supabase's scoped security and performance advisors report no warning or error for this slice. Their remaining informational notices are expected on empty dormant tables: fail-closed RLS with no policies and indexes not yet used.

OPS-Web merge commit `1945c60b` reached production in Vercel deployment `dpl_omGcXfSXUiEX8bHH2aT86vTsCby3`; exact canary release `f0dd81be` reached `Ready` in deployment `dpl_xVHop7mZ5pCxJs6ceosauonbrLMb`. Current live head `d3d51884` is a verified descendant containing both releases and is `Ready` in deployment `dpl_H7ncZ6jA7dyN8k4xcGMx6XUGczPU`. The original release merge passed 723/723 focused closeout, authority, MCP, OAuth, settings, and cron tests. The canary release-day rerun passed 434/434 exact tests, the real concurrency contract, and a complete 356-route production build. The later concurrent analytics merge was confirmed to contain `f0dd81be` and passed its own production build before taking the live alias.

Post-alias live readback at `app.opsapp.co` proves that MCP metadata still advertises exactly the twenty read-only v2 scopes and excludes `ops.operations.prepare`. Unauthenticated probes of MCP transport, routine configuration, and routine cron routes each return 401. Vercel has a production `CRON_SECRET`, but `OPS_DAY_CLOSEOUT_ROUTINES_ENABLED` remains absent. Database readback after the final descendant alias switch found zero canary bindings, zero v3 or v4 enabled clients, zero v3 or v4 live grants, zero enabled routines, zero closeout runs/failures, and zero collections runs/receipts. The exact deployment and scoped routes showed no error or fatal runtime entries after release.
...
### Exact-subject synthetic-canary production release (dormant, 2026-09-01)

OPS-Web commits `9df17e32`, `7b2acd91`, `429f36e9`, `e6794bc6`, and review-hardening commit `6915f9be` form the first authenticated v3 acceptance release at production main `f0dd81be`. Source migration `20260831190000_mcp_v3_synthetic_canary.sql` is applied as ledger version `20260901012208_mcp_v3_synthetic_canary` and mirrored byte-exact into this Bible's production migration archive. Release is not activation: no synthetic tenant/operator exists and live readback remains at zero authority.
...
Candidate proof included 41 focused MCP/day-closeout/collections-coherence files and 874 tests passing, ten loopback acceptance cases, ten migration-source cases, the rollback-isolated PostgreSQL runtime contract, a disposable real two-session concurrency contract for provision/refresh/routine/durable-write races against shutdown, 8 GB TypeScript typecheck, touched-file ESLint, diff hygiene, and an optimized production build. Release-day rerun passed 434/434 exact canary/OAuth/closeout tests, the real four-path two-session concurrency contract, and the complete 356-route production build with type validity. The repository-wide sweep passed 15,539 tests; its 44 failures were outside this release, including ten copies of the sandbox's loopback-bind denial that passed when the exact release suite was rerun with local socket access. The database fixtures prove exact replay/conflict behavior, unrelated-run rejection, cross-tenant denial, old/new application rolling compatibility, v3 rejection by the legacy resolver, current-authority loss, refresh expiry, spent-refresh theft, routine de-leasing, bearer rejection, ACLs, independent cleanup, common-lock waiting, and final zero-authority readback. The build completed with inherited warnings for `libheif-js`, the existing client-side `async_hooks` fallback, stale Browserslist data, and metadata defaults; it produced the optimized route manifest successfully. This release adds no dependency, persistent worker, model/provider call, or new paid service. Its short-lived acceptance polling runs only during the one synthetic canary and adds negligible database/function usage within the existing Supabase/Vercel plans; any future always-on worker cost remains a separate measured activation gate.

The safe release order is fixed. Steps 1 through 4 are complete; steps 5 through 7 remain deliberately gated:
...
3. **Complete:** apply `20260831190000_mcp_v3_synthetic_canary.sql` first as ledger version `20260901012208_mcp_v3_synthetic_canary` and independently verify its functions, ACLs, triggers, and zero canary bindings;
4. **Complete:** deploy exact OPS-Web release `f0dd81be` while v2 remains active and the worker remains off;
