...
**Codex host update (2026-08-29):** Strict Codex desktop DCR callback support is production-deployed after the first real registration attempt exposed the Claude-only callback gate. The server admits only one complete literal `127.0.0.1` loopback callback with an explicit valid port and bounded callback identifier, then retains exact redirect binding throughout OAuth. Application and disposable PostgreSQL 17 lifecycle tests passed, live OAuth metadata plus MCP bearer-challenge checks passed, and Codex DCR discovery and registration now work in production. The first live full-page Firebase provider return produced no session, so MCP-specific Google/Apple login now remains on the authorization page and uses a popup. The full host canary is awaiting the user's final browser completion, so no completed token/tool-call proof or accepted Codex host is claimed. This does not rewrite the historical Claude-first P1 scope, change external exposure, or prove ChatGPT compatibility.
