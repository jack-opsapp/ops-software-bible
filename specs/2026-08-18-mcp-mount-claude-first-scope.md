...
**Dark-v8 production update (2026-08-29):** All thirty-nine ordered P2 migrations are production-applied, and OPS-Web production HEAD `a567902f86946436385244acc1fd0154afe78a1e` is served by READY Vercel deployment `dpl_FGXsf3DT5rc3n1coooSyq3yuFLFc` at `app.opsapp.co`. The final deployment includes both live post-release auth repairs: preserving the sanitized MCP authorization return path after sign-in and using an on-page Firebase popup for MCP-specific Google/Apple login after the live full-page redirect returned no session. Manifest v8 and the complete thirty-four-read business catalogue, including the site-visit and deck-geometry flow, now exist at the database/application layer. This does not widen the production-live P1 connector: immutable exposure v1 still lists exactly eleven reads and seven scopes, the additional twenty-three reads are externally dark, no thirty-four-read exposure revision has been built, and no external write is available. See `specs/2026-08-29-mcp-read-catalogue-p2.md`.

**Codex host update (2026-08-29):** Strict Codex desktop DCR callback support is production-deployed after the first real registration attempt exposed the Claude-only callback gate. The server admits only one complete literal `127.0.0.1` loopback callback with an explicit valid port and bounded callback identifier, then retains exact redirect binding throughout OAuth. Application and disposable PostgreSQL 17 lifecycle tests passed, live OAuth metadata plus MCP bearer-challenge checks passed, and Codex is accepted for the verified production read-only path. The first live full-page Firebase provider return produced no session, so MCP-specific Google/Apple login now remains on the authorization page and uses a popup. During final acceptance the in-app browser returned Firebase `auth/network-request-failed`; the system browser then completed DCR/OAuth successfully. `codex mcp list --json` showed `ops_-_maverick_projects` enabled with `auth_status` `o_auth` at `https://app.opsapp.co/api/mcp`, and one exact `search_jobs` canary with `{"lifecycle_states":["active"],"limit":1}` succeeded without printing business data. The constrained Codex canary report did not surface an explicit contract-version label. This does not rewrite the historical Claude-first P1 scope, change the eleven-tool/seven-scope exposure, expose the twenty-three dark P2 reads, or prove ChatGPT compatibility.
