...
### Undo defect — found AND fixed on prod 2026-07-30
...
Fix: `ops-web/supabase/migrations/20260730000000_fix_lead_feedback_undo_conflict.sql` replaces both apply RPCs so they read the stored `updated_at` (and `archived_at`) back with `RETURNING` instead of predicting it. Undo's guard is deliberately unchanged. No backfill needed — the table held only verification rows when the defect was found, so no customer row carries a stale snapshot.

**Status: APPLIED to prod 2026-07-30** (operator-approved; ledger entry `fix_lead_feedback_undo_conflict`). Verified end-to-end through the web UI afterwards: discard → reason `spam` → UNDO produced `learning_state=retracted`, `resolution_status=undone`, `retracted_by` set, the opportunity restored `discarded` → `new_lead`, two `stage_transitions` rows (forward + reverse), and zero active dispositions. Before the fix the same click hung ~125s and then failed; after it, it resolves in seconds.

Both `apply_*` RPCs now diverge from their original branch migrations — when reconciling migration drift, `20260730000000` is the current truth for those two function bodies.
