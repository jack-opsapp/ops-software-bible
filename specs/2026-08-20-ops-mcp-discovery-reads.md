...
**Local implementation checkpoint:** OPS-Web branch `feat/mcp-discovery-reads`, code commits `133c33e8` through `d8485cb7`. Migration `20260820220000_agent_discovery_reads.sql` is frozen at SHA-256 `29d984d6467e7ae65159b66b74e659726ed81ad612ba00d305d4f12690fd0f05`. Fresh local proof: 78 files / 1,946 tests green, Node 22 TypeScript green, all changed files Prettier-clean, PostgreSQL 17 compile green, 143/143 ECPG statements parsed, and 38/38 custom plus forced-generic query-plan cases green with no executed source sequential scan. A separate 11-boundary cutover matrix proves exact v6 results remain unchanged across the migration while v7 callers receive complete recursive reproof; NULL, unknown, mixed, and bad revisions fail closed. Independent review found no remaining P0/P1 issue. This is build evidence, not a production apply or runtime acceptance claim.

**Production rollout checkpoint:** The ledger-aligned discovery migration is SHA-256 `2f91ba90b01e3249180e30db0477dde1acffcc1beff1b74c6f71b9bd850570e8`; the ACL repair is SHA-256 `e27fff38ad2ad22f3a47b6cf5223be585750e7c85ba0b0b2696179e0a459ac1f`. Production readback proves both discovery RPCs are service-role-only, all 40 indexes are valid and ready, and all prior reads retain the v6/v7 bridge. A live authenticated endpoint canary listed exactly eleven tools: nine calls returned successful results and the two selectors without matching conversation/evidence returned their expected privacy-safe `NOT_FOUND` outcomes. Revoking the disposable grant made the next bearer call return `401`; all disposable OAuth client, code, grant, and token rows were then deleted with zero-row readback while the immutable request audit remained.
