...
**Closed 2026-07-31 — code complete, production data-plane proof complete, web delivery still gated.** The shared manifest is version `2026-07-31`: 220 classified tables, 199 ordered purge steps, and 96 export tables. `delete-account` preserves its Firebase/admin, Stripe, and audit boundaries but now delegates the entire tenant mutation to one `purge_company_data(company_id, plan)` RPC. The function validates the manifest-derived plan before mutation, materializes parent scopes, breaks declared cycles, and executes inside one PostgreSQL transaction. Migration sources: `20260731161122_transactional_company_data_purge.sql` and `20260731170226_account_purge_immutable_event_exception.sql`.

Production rehearsal used disposable company `dddddddd-0000-4000-8000-000000000001`. The real export handler returned HTTP 200 in 9.235 seconds and included `project_tasks`, `line_items`, and `site_visits`. The first transactional deletion stopped at immutable ledger step 180/199 with SQLSTATE `55000`; every preceding mutation rolled back. The immutable-ledger migration then added an exact-company, DELETE-only, claim-free account-closure exception to three triggers. It never disables triggers or foreign keys and leaves every UPDATE immutable. The successful real-route rerun returned HTTP 200, completed 199/199 steps in one purge RPC, reported 51 affected rows, and finished in 1.184 seconds over five database requests.

The post-commit audit exposed two `user_permission_change_deliveries` created by later user tombstones. The manifest now purges that delivery table after permission overrides and users; regression coverage pins that order. After guarded fixture cleanup, all 220 manifest tables, every public UUID/text column, Auth users, and Storage objects had zero fixture residue. No non-fixture company/table pair disappeared and no non-fixture row count decreased across the saved before/after control. Focused manifest/delete/export coverage is 88/88 green. The new database functions are live; the web route remains local and cannot reach customers until Jackson separately authorizes the `ops-web` push/deploy.
