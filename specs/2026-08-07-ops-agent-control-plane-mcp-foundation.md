...
**Status:** Product direction approved by Jackson on 2026-08-07; the Claude-first read-only slice is production-live as of 2026-08-18 and its discovery expansion is production-live as of 2026-08-22 UTC. The remote endpoint is deployed at `https://app.opsapp.co/api/mcp`; Claude remains dynamically registered through an active OPS OAuth grant; and production audit records prove successful use of the read surface. Manifest `2026-08-20.capability-manifest.v7` externally exposes exactly eleven reads, including bounded customer and job discovery. Every write family and both site-visit control-plane capabilities remain disabled. The Phase C context switch remains a separate rollout decision and is not implied by MCP availability.
