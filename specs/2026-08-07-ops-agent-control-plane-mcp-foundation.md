...
**Status:** Product direction approved by Jackson on 2026-08-07; the Claude-first read-only slice is production-live as of 2026-08-18 and its discovery expansion is production-live as of 2026-08-22 UTC. The remote endpoint is deployed at `https://app.opsapp.co/api/mcp`; Claude remains dynamically registered through an active OPS OAuth grant; and production audit records prove successful use of the read surface. Production now runs capability manifest `2026-08-22.capability-manifest.v8` with thirty-four implemented reads, while immutable external exposure `2026-08-22.mcp-exposure.v1` still registers exactly the eleven v7-compatible reads and seven grantable scopes, including bounded customer and job discovery. The other twenty-three reads, every write family, and both site-visit control-plane capabilities remain externally unavailable. The Phase C context switch remains a separate rollout decision and is not implied by MCP availability.
...
**Status update (2026-08-29, dark-v8 production release):** All thirty-nine ordered P2 migrations are production-applied, and OPS-Web production HEAD `a567902f86946436385244acc1fd0154afe78a1e` is served by READY Vercel deployment `dpl_FGXsf3DT5rc3n1coooSyq3yuFLFc` at `app.opsapp.co`. The final deployment includes both live post-release auth repairs: preserving the sanitized MCP authorization return path after sign-in and using an on-page Firebase popup for MCP-specific Google/Apple login after the live full-page redirect returned no session. Manifest v8 composes all thirty-four reads in production. External registration remains immutable exposure v1 at eleven reads/seven scopes, all fourteen write shells remain unavailable, and frozen v7 bytes/cursors retain exact compatibility. The additional twenty-three reads are deployed dark, and no thirty-four-read exposure revision has been built. Scope and remaining external-release boundary: `specs/2026-08-29-mcp-read-catalogue-p2.md`.

**Status update (2026-08-29, Codex read-only host accepted):** The production authorization server admits only the captured Codex native callback family and keeps the complete URI byte-exact through consent, code, and token exchange. The additive registration-RPC migration, auth-continuation repair, and MCP-specific popup repair are production-live, and live OAuth metadata plus MCP bearer-challenge checks passed. The first live full-page Firebase provider return produced no session; MCP Google/Apple login now stays on-page and uses a popup. During final acceptance the Codex in-app browser returned Firebase `auth/network-request-failed`, but the system browser completed DCR/OAuth successfully. `codex mcp list --json` showed `ops_-_maverick_projects` enabled with `auth_status` `o_auth` at `https://app.opsapp.co/api/mcp`, and one exact `search_jobs` canary with `{"lifecycle_states":["active"],"limit":1}` completed successfully without printing business data. The constrained Codex canary report did not surface an explicit contract-version label. The accepted path remains read-only and did not change scopes, exposure, or grants. ChatGPT remains separately unproven.
...
- Shared domain methods beyond the eleven reads, including every write operation, remain unavailable externally.
...
- Claude and Codex have completed production acceptance for bounded read-only use. Codex proved DCR/OAuth plus one exact `search_jobs` call; the constrained Codex canary report did not surface an explicit contract-version label, and the accepted path remains inside the eleven-tool/seven-scope exposure-v1 boundary. ChatGPT and every other host require their own connector-compatibility, consent, and tool-call verification before being called supported.
...
**Current implementation note (2026-08-29):** The production server deliberately advertises DCR rather than CIMD or authorization-response issuer support. Claude uses its two exact hosted callbacks. The deployed Codex policy additionally accepts one complete explicit-port `127.0.0.1` callback and keeps exact equality afterward; it does not implement wildcard loopback ports or remote metadata retrieval. Codex completed production DCR/OAuth in the system browser and one exact read-only `search_jobs` canary after the in-app browser returned Firebase `auth/network-request-failed`, so the host is accepted for that bounded path. The constrained Codex canary report did not surface an explicit contract-version label. This is a documented deviation from the broader target-state list above, not a claim that those future mechanisms are live.
