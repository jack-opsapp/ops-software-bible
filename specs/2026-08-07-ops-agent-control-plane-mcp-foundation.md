...
**Status update (2026-08-29, Codex compatibility deployed; host proof pending):** The production authorization server now admits only the captured Codex native callback family and keeps the complete URI byte-exact through consent, code, and token exchange. The additive registration-RPC migration, auth-continuation repair, and MCP-specific popup repair are production-live, and live OAuth metadata plus MCP bearer-challenge checks passed. The first live full-page Firebase provider return produced no session; MCP Google/Apple login now stays on-page and uses a popup. Codex DCR discovery and registration work in production, but the full host canary is awaiting the user's final browser completion; no completed token exchange, authenticated tool call, or accepted Codex host is claimed yet. The deployment did not change scopes, exposure, or grants. ChatGPT remains separately unproven.
...
- No second external host has completed acceptance. Claude is the proven P1 host. Codex desktop DCR discovery and registration work in production, but its full canary is awaiting the user's final browser completion and still requires token/tool-list/tool-call, revocation, and cleanup proof; ChatGPT and every other host require their own connector-compatibility and consent verification before being called supported.
...
**Current implementation note (2026-08-29):** The production server deliberately advertises DCR rather than CIMD or authorization-response issuer support. Claude uses its two exact hosted callbacks. The deployed Codex policy additionally accepts one complete explicit-port `127.0.0.1` callback and keeps exact equality afterward; it does not implement wildcard loopback ports or remote metadata retrieval. Codex DCR discovery and registration work in production, but the full host canary is awaiting the user's final browser completion, so host acceptance is not yet claimed. This is a documented deviation from the broader target-state list above, not a claim that those future mechanisms are live.
