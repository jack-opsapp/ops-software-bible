...
A test = one ad group, one control ad, one challenger ad, both enabled. OPS computes the verdict; the routine never eyeballs significance. Verdict rule: ≥14 days, ≥2,000 impressions per ad, two-proportion z-test on CTR with p < 0.05, plus a conversion veto (if the control has ≥5 trial starts and the challenger has 0 on comparable clicks, no promotion). Max 8 weeks; then `no_verdict` and the challenger is retired or kept by judgment (proposal). Winner becomes control; the next challenger is proposed on the 4–8-week cadence. Asset labels (`BEST/LOW`) are shown and used for asset swaps inside an ad only as new-ad-plus-pause, never as in-place edits. **Amended 2026-09-14 (GOOGLE ADS ENGINE - P2-1-1-1):** a test is opened for every live pair, not only for a challenger the engine applied — the phase 2 blueprint built control/challenger pairs in all twelve groups with no test, so none would ever have been judged and the creative cadence would have added a third ad to each. OPS opens the test itself from the first day both ads served (never before they serve), cancels a test whose ad leaves the pair (retired, paused by hand, gone, or the challenger disapproved for its copy), and never tests a pair twice once it has a verdict. A group runs one challenger at a time: while one is in place the brief names no new challenger for the group, the validator refuses one, no replacement is promised, apply refuses a stale approval, and the blueprint must retire the live challenger in the same apply that adds its successor. See `04_API_AND_INTEGRATION.md`, Google Ads engine → ad pair tests.

### 5.5 Copy rules (deterministic, server-side; the routine also receives them)

Headlines ≤30 chars, descriptions ≤90; 8–12 headlines, 3–4 descriptions; no duplicates or near-duplicates; no exclamation marks; no emoji or repeated punctuation; no all-caps words other than `OPS`; sentence case; none of the banned words; never "contractor" for the audience; never lead with "AI"; every number from the brand-facts allowlist only (`$90`, `$140`, `$190`, "every feature, every tier", "no credit card", "free to start", crews of one to ten); competitor names only in the competitor campaign and only as `<Brand> alternative` / `Switching from <Brand>?`; final URL from the allowlist; 2–3 headlines pinned to position 1 and nothing else pinned; an independent editor subagent reviews every candidate against the OPS copywriter brief (the same bundled brief the Instagram routine uses) before submission. Google's `validateOnly` policy findings are returned to the routine as `GOOGLE_POLICY` with the topic so it can rewrite. **Amended 2026-09-14 (GOOGLE ADS ENGINE - P2-1-1-1-1):** "the competitor campaign" is now "an ad group that answers to competitor copy, landing on a compare page". The phase 2 account has no `COMPETITOR` campaign — competitor intent lives in `PRICING · US`, `SWITCH · US` and two groups of `CORE · CA` — and the engine had been reading copy rules off the old campaign-name prefixes, so it refused competitor copy in exactly the groups built for it. The blueprint is the source of truth for copy rules as it is for structure (§4.1): a campaign's kind is the blueprint's for that campaign name, an ad group's copy kind is the blueprint group's override or its campaign's kind, and competitor copy holds only on a `/compare/` page the ads actually land on. Anything the blueprint does not declare is never widened (an engine-built group inherits its campaign's kind; an undeclared campaign grants no competitor copy). The brief shows each ad group's copy kind to the routine. See `04_API_AND_INTEGRATION.md`, Google Ads engine → copy kinds.
