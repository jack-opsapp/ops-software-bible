...
- **try-ops:** middleware writes the same `ops_attribution` cookie on `.opsapp.co` that ops-site writes (identical JSON shape, 30 days, `SameSite=Lax`), capturing `gclid`, `gbraid`, `wbraid`, `fbclid`, the five UTM keys, landing path, referrer domain, first-touch time. The signup flow forwards the decoded first-touch payload to ops-web with the company-creation call so the `trial_attributions` row carries it.
