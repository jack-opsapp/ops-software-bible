...
WEB SHIPPED 2026-08-29 — ops-web asks on the same transitions through the same RPC/columns: evaluation + commits in src/lib/api/services/lead-won-prompt-service.ts, queue in src/stores/lead-won-prompt-store.ts, presenter src/components/ops/lead-won-prompt-host.tsx (house ConfirmDialog, mounted in dashboard-layout). Proposals fire from LifecycleMutationService.updateProjectStatus, ProjectTableService.changeProjectStatus, and ProjectTableService.bulkUpdateProjects — every human status door. Escape dismisses without recording; only KEEP OPEN writes won_prompt_declined_at/_by (first answer wins). Branch feat/lead-won-prompt-web, not merged/pushed at ship time.
