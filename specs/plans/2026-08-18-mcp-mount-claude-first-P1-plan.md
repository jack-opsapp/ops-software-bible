...
Files: `src/lib/agent-control-plane/mcp/oauth/tokens.ts` (entropy, sha256 hex hashing, constant-shape lookups), `pkce.ts` (S256 verify via `crypto`), `clients.ts` (DCR metadata validation + claude.ai redirect allowlist from research appendix), `grants.ts` (lifecycle over the RPCs), `config.ts` (issuer/resource/base-URL resolution — `NEXT_PUBLIC_APP_URL` in prod, request origin in dev), `scopes.ts` (the eight read scopes + human-readable consent strings), `index.ts`.
