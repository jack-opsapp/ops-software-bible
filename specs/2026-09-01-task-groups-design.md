...
**Recurrence.** Recurrence templates (`task_recurrences`) stay single-type — the server cron that materializes instances is untouched. A materialized instance is an ordinary task and may gain scopes after the fact (the cleaning-service add-on case). No template-level scope concept in this build.
...
## 11. Build ledger (2026-09-02)

| §10 line | Evidence |
|---|---|
| 1 grouped creation, one bar | iOS composer `ADD SCOPES` → `TaskScopePickerSheet` (snapshots `task-group-composer-editor.png`, `task-group-scope-picker.png`); web multi-select (`02-picker-accumulates-on-reopen.png`, `03-grouped-row-plus2-collapsed.png`); calendar cards render one bar + `+N` (`task_scopes_calendar_card.png`, web `01-month-grouped-task-plus2.png`) |
| 2 check-off law | `TaskScopeLawTests` (13), `TaskScopeOutboundTests` (20); web live round trip check → auto-complete → reopen on prod (`04-scopes-open-one-checked.png`, `05-after-complete-all.png`); DB rehearsal of every `set_task_scope_completion` branch |
| 3 split off | `splitScope` + `TaskScopeLawTests`; detail context menu `Move to its own day` |
| 4 single-type byte-identical | `LocalTask.normalizedScopes()` + `ProjectTaskComposerLogicTests`; web test asserts no checkbox chrome before the first pick; `task_scopes_single_type_row.png`, `01-picker-single-select-untouched.png` |
| 5 combo chip | `TaskSuggestionEngineTests` (15) incl. single keyHash stability; `task-group-quick-add-chips.png` |
| 6 estimate partition | prod rehearsal: 1 vinyl + 3 rail-variant + 1 untyped LABOR lines → 3 tasks (vinyl single, rail group of 3, untyped single) with gate on; 5 tasks with gate off |
| 7 agent clarifying question | carried by the OPS MCP buildout (agent write flow not in this initiative); read exposure staged (`agent_task_reads_scopes_v2`) |
