...
**Manual project choice is address-advisory (staged 2026-08-07, not applied).** `get_manual_project_link_candidates` returns every server-authorized, unclaimed same-company project and supplies `same_address` / `same_client` only to rank likely matches. An explicit operator-selected target may therefore be at any address and in any non-deleted status, provided the actor may view/link it and it is not owned by another lead. The guarded `convert_opportunity_to_project` patch removes address/status vetoes only for an explicit target; automatic nil-target creation retains every same-address duplicate guard. Migration `20260807123000_manual_project_link_any_address.sql`; OPS-Web commit `705d32dc`; iOS commit `a06fe97f`.
