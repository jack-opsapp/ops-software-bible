...
13. **Known gap — require_project_assignment is not enforced (as of 2026-07-05).** Like `require_receipt_photo` before this change, `require_project_assignment` renders a "REQUIRED FOR SUBMISSION" hint in `ExpenseFormSheet` but `validate()` does not block submission when it is on and no project is allocated. Pending a product decision (hard block vs. warn) before enforcement.
