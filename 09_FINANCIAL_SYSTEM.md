...
Updated test scope (2026-09-08 17:32 UTC): Jackson approved existing MAVERICK, native Codex and user-operated Claude instead of a new company/ChatGPT trial. Six fictional source/setup rows are now committed without changing pre-existing business records, company settings or taxes. The source is two hours at CAD100 with MAVERICK's existing 7.75% tax (CAD215.50); a future +8% output must total CAD232.74, not the disposable 5%-tax fixture's CAD226.80. Native Codex read the exact source successfully; no output draft was saved. A production fix for canonical PostgreSQL tax IDs makes the signed-in exact owner review readable. Enrollment remains unconfirmed and financial authority disabled. The explicitly authorized temporary QuickBooks sandbox sync pause was restored after setup, with zero fixture accounting queue rows. The Phase16 contract records exact IDs, hashes, release and cleanup receipts; a synthetic historical approved status is not evidence of customer approval or host financial acceptance.
