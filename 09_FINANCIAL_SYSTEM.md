...
11. **Receipt requirement enforcement + no-receipt escape hatch (2026-07-05).** When `expense_settings.require_receipt_photo = true`, iOS **blocks submission** of an expense that has no receipt photo. Enforcement is client-side in `ExpenseFormSheet` (`receiptGatePassed(submit:)`), gating **only submits** — saving a draft or an in-place edit never blocks. It is deliberately **not** a DB constraint/trigger: a server hard-block would reject writes from already-shipped iOS builds (which legitimately submit photo-less lines today), violating the additive-only iOS-sync contract. The crew can still file a genuinely receiptless purchase via the **"No receipt available"** fork: a reason (`lost` | `cash` | `digital` | `other`, modelled by `NoReceiptReason`) plus an optional note, persisted to two additive, nullable columns — `expenses.receipt_missing_reason` (CHECK-constrained to those four codes or NULL) and `expenses.receipt_missing_note`. A real photo always supersedes a reason (both columns are written NULL when an image is present). OPS-Web (approval-only) surfaces this to the approver in `expense-line-item-table.tsx`: a tan `ImageOff` indicator on the row (vs. the neutral placeholder that still marks legacy photo-less lines) and a `NO RECEIPT` block with the reason label + note in the expanded detail. The reason columns are submitter-written and consistent with the uploader-only edit rule; they are intentionally **not** added to `private.enforce_expense_edit_authority()`'s content-column list in this change (iOS already restricts edits to the submitter, and web is approval-only) — a candidate follow-up if direct-API hardening is later wanted.
12. **Silent-upload-failure fix (2026-07-05).** If a receipt photo **is** attached but its S3 upload throws, the expense is now **kept as a draft and not submitted** (previously the row was inserted before upload and still submitted, silently dropping the receipt). iOS surfaces `// RECEIPT DIDN'T UPLOAD — SAVED AS DRAFT` (warning toast + warning haptic) and the crew retries from the draft once back on signal.
13. **Project requirement enforcement + no-project escape hatch (2026-07-05).** `require_project_assignment` is now enforced the same way as `require_receipt_photo` (rule 11): iOS blocks submission when the setting is on and no project is allocated (`ExpenseFormSheet.projectGatePassed`, submit-only — drafts/in-place edits never block). The crew files a genuinely project-less purchase (overhead, shop supplies) via a "No project available" fork — a reason (`overhead` | `general` | `other`, modelled by `NoProjectReason`) plus an optional note, persisted to additive nullable `expenses.project_missing_reason` (CHECK-constrained to those three codes or NULL) + `project_missing_note`; a real project supersedes the reason (both NULL when a project is allocated). OPS-Web surfaces the reason in the expanded PROJECT field of `expense-line-item-table.tsx` (ops-amber). Client-side only, same shipped-build rationale as rule 11.
