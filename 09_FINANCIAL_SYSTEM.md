...
## Recurring reimbursements (2026-09-17; database and OPS-Web live in production, iOS unreleased)
...
- iOS (branch `feat/recurring-reimbursements`, code-complete, not yet built): batch review mirrors the web console (repeat mark, no flag, EDIT and SKIP, add action). My Expenses and the Books ledger show `Recurring · <MON YYYY>`, keep the line's real state pill, leave it out of the NO RECEIPT filter, and open a line sheet instead of the expense form. Expense Settings lists and adds setups. The `expense_recurring` notice has its icon and routing, and realtime subscribes the setups table. Reviewer corrections never target a recurring line.
...
**Proof.** OPS-Web `scripts/test-expense-recurring-reimbursements-postgres.sh` on disposable PostgreSQL 17: 96 recurring assertions, plus the existing 69 correction, 7 contention and 123 accounting assertions, plus two install-safety rejections.
