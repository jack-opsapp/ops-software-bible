...
**Manual project choice is address-advisory (applied to production 2026-08-17).** `get_manual_project_link_candidates` returns every server-authorized, unclaimed same-company project and supplies `same_address` / `same_client` only to rank likely matches. An explicit operator-selected target may therefore be at any address and in any non-deleted status, provided the actor may view/link it and it is not owned by another lead. The guarded `convert_opportunity_to_project` patch removes address/status vetoes only for an explicit target; automatic nil-target creation retains every same-address duplicate guard. Migration `20260807123000_manual_project_link_any_address.sql`; OPS-Web commit `705d32dc`; iOS commit `a06fe97f`.

**OPS-Web consumes the same RPC in the Won dialog (2026-09-05, branch `fix/w2-lead-project-link-web`, commits `390075b7e` + `6d66c3a7e`, awaiting push).** `GET /api/opportunities/[id]/link-candidates` (`src/app/api/opportunities/[id]/link-candidates/route.ts`) calls `get_manual_project_link_candidates` on the operator's **own verified Firebase idToken** via `getAccessTokenClient` — the RPC resolves its actor from `auth.jwt()->>'sub'` and takes no `p_actor_user_id`, so the service-role client cannot call it (unlike the sibling preflight route). Response `{ candidates: [{ projectId, title, address, status, sameAddress, sameClient }] }` in the server's ranked order, mapped one-to-one and never re-sorted; RPC SQLSTATE `42501`→403, `P0002`→404, `22023`/`22P02`→400, else 500. The dialog (`stage-transition-dialog.tsx`, hook `useManualProjectLinkCandidates`, query key `opportunities.linkCandidates`) renders **one** ranked list sourced solely from this route — every row selectable, `same_address`/`same_client` shown as an evidence badge (never a gate), a title/address filter past six rows, a retry on failure, and "Create a new project" as the default. Selecting a row confirms with `linkToProjectId` → the existing `link_opportunity_to_existing_project` path. The preflight's `duplicate_candidates` / `other_client_projects` are no longer rendered on web. Live-verified against production as pete/Maverick (`ops-web/docs/artifacts/lead-project-link-20260905/`).
...
9. **Attach lead photos** (added 2026-07-14, migration `convert_rpc_lead_deck_and_photo_carryover`; storage repair 2026-07-25) — the lead's own `opportunities.images[]` full public URLs → `project_photos` (`source='other'`, no `site_visit_id`, `uploaded_by = COALESCE(p_decided_by::text,'')`), **deduped by URL** against anything already copied, so a photo that arrived via a site visit is never duplicated. New iOS and web uploads use `projects/{companyId}/leads/{opportunityId}/…`; the conversion transaction copies the URL unchanged because the object is already in the project-public namespace. The former `opportunities/{companyId}/{opportunityId}/…` keys were writable but not publicly readable and must not be produced.
...
| `manualProjectLinkCandidates(for:)` | Complete server-authorized manual target list. Address/client flags rank likely matches but never filter the operator's choices. The conversion sheet falls back to the deployed same-address recheck while this staged RPC is unavailable and marks the complete-target check unavailable rather than fabricating authority. |
...
MCP scope checkpoint (2026-09-08): catalog capabilities currently expose bounded search and exact item/variant reads, including prices, inventory and separately authorized supplier costs. Conversational catalog/inventory setup, imports, creation, editing and stock mutations are not exposed MCP capabilities. Existing web/iOS editing does not imply MCP write authority. No catalog-write implementation was started by this status check.
...
- **Atomic approval.** iOS whole-envelope approval calls the `approve_expense_batch` RPC (the Phase-2a direct-write path is gone), followed by a best-effort `accounting-sync-expense` request per approved line. The current contract discrepancy below means provider delivery is not verified. Per-line early clear (`early_clear_expense_line`) is a quiet `CLEAR NOW` in a filling envelope's line detail.
- **Approval progress and readback (2026-09-10, local implementation; not released).** Person, swipe, ALL, and detail approval share a main-actor company/batch claim across console instances, with per-batch progress for bulk runs. `ExpenseViewModel` records successful atomic approval separately from readback, fetches only the affected batch and its lines concurrently, and replaces those canonical cache entries. Failed readback preserves the saved outcome and offers read-only refresh instead of another approval. Accepted receipts are observable across console instances and retain company/batch identity through missed refresh and account switching; only a later authoritative transition back to review can retire them. Bulk selections deduplicate, exclude flagged batches, and stop on the first failed write; completion counts retain the saved/requested count even when readback also fails. Accounting triggers remain serial and awaited. Realtime reloads coalesce during approval and debounced work rechecks activity when it wakes; stale console/detail reads cannot overwrite approved readback. Detail closes only after successful approval if it is still the same visible detail. Source fixes: `8db89579`, `dc0599e7`, integrated into local main `ce4a6e24`; regression suite: `ExpenseBatchApprovalTests`. The full iOS app build passed all 15 approval tests and 8 expense-notification regression tests with zero skips or failures, using mocked I/O. An additional exact-source macOS harness compiled the complete view model and passed the same 15 approval tests using real SwiftUI/Combine and controlled app/network boundaries. This verifies operation state, receipts, readback, debounce and failure outcomes; actual expense-screen interaction, repository/provider delivery, and customer latency remain separate. This removes the final whole-company refresh from completion and makes pending work visible; it does not establish customer latency improvement or reliable provider delivery.
- **Accounting contract discrepancy (live read-only inspection, 2026-09-10).** Active `accounting-sync-expense` v5 requires `expenseId` and `companyId`, while `ExpenseRepository.triggerAccountingSync` sends only `expense_id`. The inspected approval RPC and expense triggers do not enqueue expense accounting work, and the function directly calls providers with ad hoc retries. The progress repair leaves that existing effect path unchanged; expense-to-provider delivery must not be considered verified from these client changes. No live financial action was used for this inspection.
...
| `public.expense_envelope_sweep()` *(service_role)* | Daily pg_cron `expense_envelope_sweep_daily` (06:24 UTC through `private.run_expense_envelope_sweep_controlled()`; schedule verified in `cron.job` 2026-09-17): files due recurring reimbursement months first (2026-09-17), then auto-sends due `open` envelopes (one `expense_submitted` notification each), sweeps in completed drafts, adopts orphans (safety net), rolls stragglers forward. `migrations/20260601213757_expense_envelope_sweep_deep_link_expense.sql`. |
...
| `LineItemEditSheet.swift` | Bottom sheet for creating or editing a line item. Fields: description, type picker (LABOR/MATERIAL/OTHER), quantity, unit, unit price, optional toggle, taxable toggle. Shows computed line total. Delete button in edit mode. For a configurable product (new line, or an existing line whose `product_id` resolves locally) it shows the `// CONFIGURATION` panel: selects open on their default, integer counts open blank (`—`) and are entered with a 44pt −/+ pair (minus from blank = `0`), and a save with a blank required option is refused with a `// NOT ENTERED` prompt naming the options (rules in `LineItemConfigurationForm`; see `03_DATA_ARCHITECTURE.md` § `line_items.configured_options` shape). |
...
| `CreateEstimateDTO` | Write DTO. Fields: companyId, opportunityId, projectId, clientId, title, notes. `notes` maps to `estimates.notes`, a nullable text column added 2026-07-29 by `add_estimates_notes_for_ios_ar_decks` (SYSTEMS REPAIR W1-1..W1-6 wave; see `03_DATA_ARCHITECTURE.md` § SYSTEMS REPAIR Wave 1). **Before that column existed the DTO's `notes` key had no backing column**, so any non-nil value returned PGRST204 and the insert failed. `DeckBuilder/DeckBuilderViewModel.swift:4169-4177` always sends `EstimateGeneratorService.arAccuracyNote(...)` for an AR-measured deck, so *every* AR-measured deck failed estimate creation; manually-drawn decks and the standard path (`ViewModels/EstimateViewModel.swift:197`) passed only because a nil `notes` is omitted from the encoded payload. The fix is server-side and additive, so shipped iOS builds were unbroken the moment the column landed — no app release required. |
...
**Ledger** — `BooksLedger` + `BooksLedgerRows` + `BooksLedgerControls` (`OPS/Views/Books/Ledger/`): the segments render **flat hairline rows directly off the shared VMs** (`InvoiceViewModel` / `EstimateViewModel` / `ExpenseViewModel`) — the old `embedded: true` list-view mounting is gone. Books owns its own filter state so the chip sets differ from the standalone lists: invoices **ALL · UNPAID · OVERDUE · PAID**, estimates **ALL · OUT · WON**, expenses **ALL · NO RECEIPT** (tone-tinted counts). Expense rows use passive lifecycle pills, never a per-line approval state: `NO RECEIPT` overrides a missing receipt; otherwise the pill reports `UNFINISHED`, `FILLING`, `WITH OFFICE`, `APPROVED`, `PAID`, or `NEEDS FIX` from the line plus its server-authoritative envelope phase, with `PENDING` reserved for missing or unknown envelope state. Sort ranks urgency first for invoices (overdue → partial → out → draft → terminal) and estimates (approved → viewed → sent); expense rows place approved/reimbursed lines first, then order each group by expense date newest-first with updated/created timestamp and id fallbacks. Client/crew names resolve via `@Query [Client]` / `[User]` with legacy `[TeamMember]` fallback. Expense envelope rows show the resolved submitter beside the batch number in both TO REVIEW and TO PAY. Row tap → `InvoiceDetailView` / `EstimateDetailView` / `ExpenseFormSheet` (edit). Empty states carry create CTAs (`NEW ESTIMATE` → `EstimateFormSheet`, `LOG EXPENSE` → `ExpenseFormSheet`). iOS source commits `dd71de54`, `67405123`.
...
**Expense rows**: real receipt thumbnails — `AsyncImage(receipt_thumbnail_url ?? receipt_image_url)` at 34×42, rendered aspect-fit so no receipt content is clipped. Batch review intentionally reverses that source order (`receipt_image_url ?? receipt_thumbnail_url`) to bypass irreversibly cropped historical thumbnails and uses the same aspect-fit renderer; tapping opens that authoritative full image. The abstract receipt block is the loading/failure stand-in, and only a missing URL renders the rose dashed no-receipt state. Date metadata formats **date-only columns in UTC** (`BooksLedgerStatus.shortDate`) — parsing anchors midnight UTC, so local-time formatting read yesterday west of Greenwich (same class of bug the web ledger's unit suite caught; both surfaces now display the literal stored day).

**Batch-review entry**: approvers (`expenses.approve` — same gate as the Settings row) get a discreet state-aware `BooksReviewBatchesLink` in the expenses ledger marker: `BATCHES · n TO REVIEW` for `pending_review` / legacy `submitted` envelopes, then `BATCHES · n TO PAY` for approved envelopes awaiting payout, otherwise `BATCHES`. Filling `open` envelopes are explicitly excluded from review counts. The link pushes `ExpensesListView` inside the Books stack; the console is the only approval surface, and Settings retains its entry.
...
- Dynamic Type — hero numbers scale down via `minimumScaleFactor`; 44pt hit targets on the period pill, ledger rows, strip buttons, and the state-aware BATCHES link.
...
## Owner pricing-policy readiness (Phase 16, released and dormant)
...
The local historical-quote +8% proof creates a CAD 216.00 subtotal and 226.80 total from two fictional hours at 100.00 plus 5% tax, only after exact approval, with one durable draft/number under retry. At that historical checkpoint, Canpro had no saved estimates or tax rows and no usable products (two deleted catalog records existed). External Canpro source conflicts remain business decisions, not migration seeds. No company enrollment or real host acceptance occurred in that local proof. See [Phase16 contract](specs/2026-09-08-ops-mcp-financial-readiness.md).

Latest native-host checkpoint (2026-09-08 18:56 UTC): after separately approved effect review and fresh restricted Codex consent, native Codex prepared and replayed the same exact CAD232.74 MAVERICK proposal. This is a proposed private draft, not a saved estimate. Independent production readback confirms one pending action, zero quote-project estimates and zero fixture accounting queue rows, with sandbox sync enabled. The fictional history project's missing completion timestamp was corrected under the approved setup boundary without weakening source eligibility or changing real history. Exact owner save approval remains outstanding; Codex binding expires at 19:03:03 UTC and Claude's binding has not started. The subsequent 17:49 paragraph is historical, superseded only by this explicitly approved bounded trial.

Fresh Phase16 acceptance now proves that same arithmetic through actual loopback HTTP consent/token handlers, production MCP runtime composition, current OAuth SQL, company/source inspection, financial preparation, the named OPS approval service and independent durable readback. Nine fresh integrated scenarios cover timezone consistency plus retries, wrong/widened subjects, bad PKCE, stale rules/tax/owner/effect, real expiry, racing revocation and owner retirement of a pending approval. All business records are fictional and the disposable database is dropped. This is complete local protocol proof, not authenticated Claude or ChatGPT acceptance. The proposed first real trial uses a new explicitly fictional company and one held draft per host, under separately approved fixture, binding and save authority. The 2026-09-08 release applied both migrations and deployed the owner-review and exact-host plumbing; effect approval, financial enrollment and live host trials remain disabled. See the Phase16 contract for exact ledger hashes and production readback.

Updated test scope (2026-09-08 17:49 UTC): Jackson approved existing MAVERICK, native Codex and user-operated Claude instead of a new company/ChatGPT trial. Six fictional source/setup rows are committed without changing pre-existing business records, company settings or taxes. The source is two hours at CAD100 with MAVERICK's existing 7.75% tax (CAD215.50); a future +8% output must total CAD232.74, not the disposable 5%-tax fixture's CAD226.80. Native Codex read the exact source successfully; no output draft was saved. The production canonical-tax-ID fix enabled actual owner review, and Jackson enrolled `phase16-maverick-host-1` at 17:42 UTC. Independent policy/source hashes match its consumed preview and preparation-only receipt. Financial host authority remains disabled pending separate effect review/approval and exact temporary client consent; each output still requires its own OPS save approval. The authorized temporary QuickBooks sandbox sync pause was restored after setup, with zero fixture accounting queue rows. The Phase16 contract records exact IDs, hashes, release, enrollment and cleanup receipts; a synthetic historical approved status is not evidence of customer approval or host financial acceptance.

## Phase17 catalog prices, costs and stock (2026-09-08; not activated)

The conversational authoring candidate keeps billable `products` distinct from stock families and variants. It supports bounded unit/category/family/product/variant authoring and fixed, explicitly pinned material recipes. Omitted fields preserve current values. Service/material kind and existing pricing-unit changes require the dedicated editor; imports never infer conversions. Existing family, option, variant and recipe identities are preserved. SKU/name collisions, duplicate normalized option combinations and repeated product/variant recipes require resolution before any save. Existing selected/scaled/legacy inventory recipes are not silently represented as fixed recipes.

Price and supplier-cost writes have separate OAuth authority, and cost disclosure requires current financial permission. Products retain their existing `default_price`/`base_price` trigger contract. Catalog authoring does not create invoices, purchases, receipts, accounting records, provider changes or physical inventory movements. Newly created products are not published to the storefront. Newly created variants start at zero quantity.

A separate inventory operation accepts absolute counts for exact current variants and appends the canonical `manual_adjustment` inventory deduction (previous/new quantity, signed delta, actor and reason). Variants with any physical stock-unit history must use the existing physical-stock workflow. Stock approval seals the effective counting unit, including its family-default fallback. It changes no catalog prices or costs. Retries return the original receipt rather than applying stock twice.
...
Local source commits: expense ledger/authority `1d252530a`, payroll projection `0b74a9ad7`, provider relink/settings `f8701c301`, recovery controls `0b35d06b4`. Expense-owned project mappings and explicit Sage native-detail reads are complete at `1dd53ae66`; project settings UI is `bc0346799`. The final combined web vertical passed 284 tests across 22 files. PostgreSQL verification passed 123 ledger assertions plus seven real contention graphs, and seventeen payroll assertions plus two real rejection checks. Expense mappings freeze with source decisions, refresh only through authorized pre-write recovery, and are cleared on provider relink. Sage project choices require a transaction-level analysis category enabled for journals. All release/provider acceptance boundaries above remain unchanged.


## Correct and return a crew expense (2026-09-14; local, unreleased)

Bug `c381520d-84fa-40ea-ba66-67de14c717ce` adds an explicit CORRECT & RETURN action for eligible office review. A reviewer can correct merchant, notes, category, amount/tax/currency, date, payment method and project allocation/exception fields. The crew sees the original and corrected values with preserved human labels and an optional reviewer note. Existing flag-only review remains available for requests that need the crew to make the change themselves.

A correction returns the expense as rejected for crew review and commits the content, allocations, envelope recalculation, immutable feedback and one submitter notification together. It does not approve, pay or produce an accounting event. Approval/payment/export history prevents this correction route. Returned unbatched expenses remain held through background envelope sweeps until explicit crew resubmission; normal under-threshold approval policy resumes only through that resubmission. Exact correction markers clear, while history and any newer unrelated flag remain.

The iOS correction form preserves edits through stale reload and retains an exact command through an ambiguous response. A stale reload adopts fresh values only where the reviewer did not edit and identifies overlapping edits. Receipt images remain inspectable; reviewers cannot replace original receipt/OCR data. Account and read-generation checks protect asynchronous history and current-state refresh.

Server source `134be2424` passed an independent disposable-database run: 69 correction assertions, seven concurrent-session checks, 123 existing P5 accounting assertions with P7 installed, and two migration-safety rejection checks. Logs: `/private/tmp/ops-expense-correction-proof.K8C2GU`. This establishes local server behavior only; iOS runtime and distribution have separate evidence.
...
## Recurring reimbursements (2026-09-17; database and OPS-Web live in production; iOS on ops-ios main, not yet released)

A recurring reimbursement is a fixed monthly amount the office pays a crew member with their expenses — for example, a set amount each month for advertising on a personal vehicle. The office sets it up once; the database then files one pre-approved line a month into that person's envelope, including months with no receipts. Each line rides the ordinary envelope lifecycle: review, approval, Mark paid, payroll readiness and accounting capture are unchanged. Schema: `03_DATA_ARCHITECTURE.md § Recurring reimbursements`. Commands: `04_API_AND_INTEGRATION.md § Recurring reimbursement commands`.

**Placement.** A month's line joins the person's calendar envelope for the month's first day, chosen in this order: an `open`, `pending_review` or `submitted` envelope; else an approved envelope with `paid_at IS NULL` (the office already owes that period); else, when that period was already paid out and today is past it, today's envelope (the line keeps its own month in `recurring_period`); else a new `open` envelope. Per-job companies get a calendar-month envelope with `scope_project_id NULL`, because a reimbursement belongs to no job. Lines left in a returned (`rejected`) envelope are moved to where they will be paid; the crew cannot act on an office-owned line.

**Line shape.** `status = approved`, `approved_by` = the setup's last editor, `merchant_name` = the setup name, `description` = `Monthly · <Month YYYY>`, `expense_date` = the month's first day, `payment_method NULL`, `receipt_missing_reason = other` with note `Recurring reimbursement. No receipt needed.`, and `project_missing_reason = overhead` when the company requires a project. Shipped iOS builds read it as an ordinary approved expense with a no-receipt note.

**Office-owned lines.** Trigger `enforce_expense_recurring_line_authority` refuses any insert, update or delete of a recurring line outside a setup command ("Recurring reimbursements change from their setup"). Outside a command only the approval and payout stamps move (`status` approved ↔ reimbursed, `approved_by`, `approved_at`, `updated_at`, `accounting_sync_*`), so approve, Mark paid and its undo keep working. A command proves itself with a row in `private.expense_recurring_reimbursement_scope` keyed to the current transaction id, which clients cannot write or forge with a setting; `enforce_expense_edit_authority` honours the same scope. Internal maintenance also passes: requests carrying `service_role` claims and `postgres` logins. Since the 2026-09-17 authority repair (ledger `20260917165432_expense_authority_account_closure`), empty request claims also count as maintenance, so account closure (`purge_company_data`, which clears the request claims under the API login) can tombstone recurring lines; before it, closure was refused. Clients always carry claims, so no client write gains this path. See `03_DATA_ARCHITECTURE.md` § Company data export and account closure (2026-09-17).

**Commands.** Every command is `SECURITY DEFINER`, needs `expenses.approve` at scope all, takes the same company advisory lock as saves, corrections and decisions, and gives up after 250 ms with "Expenses are being updated. Try again." Every change passes the setup's `updated_at` and is refused if it moved.

| Command | Rule |
|---|---|
| `create_expense_recurring_reimbursement` | The person must be an active company member; only an admin may set one up for themselves. The first month must be within 12 months either side of this month on the company calendar. Currency is the company currency. A live setup with the same name for the same person is refused. Every due month is filed immediately. |
| `update_expense_recurring_reimbursement` | Name, amount, category. Unpaid months follow the change; paid months keep what was paid. The person is notified only when the amount changes. |
| `end_expense_recurring_reimbursement` | Sets the last month. `NULL` removes the end and resumes from this month without backfilling the gap. Refused before the first month, and before the latest month already on a batch (skip that month first). Ending never removes a line. |
| `delete_expense_recurring_reimbursement` | For a setup made in error: soft-deletes it and every unpaid line. Refused once any month has been paid (end it instead). |
| `skip_expense_recurring_reimbursement_line` | Soft-deletes one unpaid month. The tombstone stops that month being filed again. |
| `restore_expense_recurring_reimbursement_line` | Brings a skipped month back at the current amount; if its envelope was paid in the meantime the month rolls to today's envelope. Resolves the skip notice. |

**Daily sweep.** `expense_envelope_sweep()` now starts by filing every due month through `private.generate_expense_recurring_lines()` (one setup's failure never blocks another setup or the sweep) and by rescuing lines stranded in returned envelopes. The same replacement fixes a latent per-job gap: job-less `open` envelopes in per-job companies auto-send on `period_end + auto_submit_grace_days`; before, they never sent. pg_cron `expense_envelope_sweep_daily` runs at 06:24 UTC through `private.run_expense_envelope_sweep_controlled()`; schedule verified in `cron.job` 2026-09-17. Months follow the company time zone, so a month's line is filed by the first run after that month starts locally; for a company on America/Vancouver time, October 2026 files at the 2026-10-02 06:24 UTC run. The generator watermark `next_period` does not backfill months that passed while the person was inactive.

**Notifications.** The person is told on add, amount change, ending, resuming, skip and removal. See `07_SPECIALIZED_FEATURES.md §14.3.4c`.

**Clients.**

- OPS-Web (live in production 2026-09-17 05:09 UTC: PR #133, merge commit `7c3be79f4`, Vercel deployment `dpl_GR4mBUmucLtKyTwZASRdZTjGcn2r` on app.opsapp.co): console lines show a neutral repeat mark instead of the missing-receipt alarm and open to the arrangement with EDIT and SKIP (toast UNDO restores). A quiet `+ RECURRING REIMBURSEMENT` sits under a batch's lines. The dialog states which months file now and which were already paid out before anything commits. Settings → Expenses → RECURRING REIMBURSEMENTS lists every setup with this month's total. Dashboard receipt compliance and the batch popover leave recurring lines out of receipt and job counts and offer no flag. Realtime subscribes the setups table. The company data manifest classifies the table (exported, retained on purge).
- iOS (merged into ops-ios local main 2026-09-17 as merge commit `b488edbb`, branch `feat/recurring-reimbursements`; not pushed to origin and not in an App Store build yet — the founder's phone takes it at its next Xcode build from local main, crews with the next App Store release): batch review mirrors the web console (repeat mark, no flag, EDIT and SKIP, add action). My Expenses and the Books ledger show `Recurring · <MON YYYY>`, keep the line's real state pill, leave it out of the NO RECEIPT filter, and open a read-only line sheet (`RecurringLineSheet`) instead of the expense form; the sheet shows the month it pays for, its state and the arrangement, and gives approvers EDIT and SKIP. Expense Settings lists and adds setups. The `expense_recurring` notice has its icon and routing, and realtime subscribes the setups table. Reviewer corrections never target a recurring line, and a recurring line is never swiped away from a list. Batch review, the line sheet and the settings list accept an optional seeded `RecurringReimbursementViewModel` (production builds their own, as before) so their states render in tests without the network.
- Both clients: typed amounts read either decimal key (`35,50` is 35.50, never 3,550; ambiguous marks are refused), and neither offers a non-admin a reimbursement for themselves (the database refuses it). Edit forms send the setup's current `updated_at`, read when the form opens.

**Production.** Migration ledger `20260917023953_expense_recurring_reimbursements` (`migrations/20260917023953_expense_recurring_reimbursements.sql`, byte-identical to the ledger). The first live setup (one customer company, from August 2026) filed August into an approved, unpaid envelope and September into an open one — the placement rules above, observed in production. Real customer names, people and amounts are kept out of this public document.

**Proof.** OPS-Web `scripts/test-expense-recurring-reimbursements-postgres.sh` on disposable PostgreSQL 17: 96 recurring assertions, plus the existing 69 correction, 7 contention and 123 accounting assertions, plus two install-safety rejections. iOS (2026-09-17, on the merged tree): device build (`generic/platform=iOS`) clean with no new warnings; 61 recurring tests green on the iOS 26.5 simulator (`ExpenseRecurringTests`, `ExpenseRecurringReimbursementDTOTests`, `RecurringLineSurfaceTests`, `RecurringReimbursementViewModelTests`, and `RecurringReimbursementSnapshotTests`, which renders every surface to PNGs from invented fixtures and a stub repository); 339 tests green across the 22 test classes that touch any file the branch changed. Eight tests elsewhere (site visits, notification reschedule, sync status, bug-report pick) fail identically on main `5ec04899` without the branch, so they are not this feature's. The fixtures use invented people and amounts because ops-ios is public.
