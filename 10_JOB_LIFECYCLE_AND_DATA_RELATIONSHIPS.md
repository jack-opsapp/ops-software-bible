...
- Each applied archive/lost/reactivation runs through `public.execute_opportunity_lifecycle_guarded_action(...)`, which writes the audit row and the opportunity mutation in one database transaction. The RPC enforces company scope, action whitelist, approval key/idempotency, live opportunity guard fields, allowed mutation fields, and exact before/after audit payload keys server-side. The existing `opportunity_lifecycle_state` table is not sufficient for this audit because it has stale status fields but no action key, mode, guard reason, before/after snapshot, provenance/error fields, or applied-result record.
