...
**Trigger reliability repair (2026-08-20 — local only, not released).** Bug `4501a2dc-2b1a-427a-9c8c-04bd6fcdad74` traced recurring attachment-upsert 500s to a PL/pgSQL record/relation alias collision inside `private.reconcile_related_email_conversion_photo_sources`. OPS-Web commit `8c24e50d` and migration `20260820172857_fix_related_attachment_record_shadowing.sql` replace only that helper, explicitly no-op before a valid lowercase SHA-256 exists, and retain both opportunity and connection/thread sibling matching branches, deterministic order, eligibility semantics, signature, hardened `search_path`, and ACL. A PostgreSQL execution contract now runs in CI and covers the original exception, both match branches, exclusion boundaries, invalid hashes, and API-role revocation. This is merged to local OPS-Web `main`, not pushed/deployed/applied; production remains on the prior function until an explicitly approved migration release.
