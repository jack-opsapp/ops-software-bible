...
Server proof:34/34 local PostgreSQL17 tests, including real captured auto-advance/manual-boundary function bodies and unchanged legacy function hash. Focused client stage/continuity verification passed, with remaining adjacent failures closed in the final127/127 regression rerun; iOS source is integrated on local main622010a0. Physical-device behavior remains unmeasured. Production schema/readback and iOS install/release are separate explicit gates; the server contract remains prepared and unapplied. Full request/result contract is in chapter04 and private storage custody in chapter03.
