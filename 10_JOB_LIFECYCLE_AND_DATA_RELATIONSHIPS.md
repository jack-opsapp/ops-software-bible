...
- Every correspondence event insert must flip `opportunity_correspondence_events.opportunity_projection_applied` to true once its opportunity counter projection applies; projection runs under the same opportunity lock as insertion, so a healthy row is unprojected for seconds at most.
- `private.opportunity_has_pending_meaningful_email(company_id, opportunity_id)` is the shared guard over that invariant. The three guarded commercial RPCs — `convert_opportunity_to_project` (actorless path), `commit_lead_summary_snapshot`, and `apply_email_opportunity_deferred_disposition` — call it under the opportunity lock and raise `meaningful correspondence projection pending` with SQLSTATE 40001 while it returns true, so no commercial outcome can be decided against an incomplete conversation.
...
- Monitoring: `/api/cron/email/projection-stuck-check` runs every 5 minutes, 24/7, and fires a persistent operator-rail alert (deduped on `email-correspondence-projection-stuck`, auto-resolved on recovery) when any row matches `is_meaningful AND NOT opportunity_projection_applied AND created_at < now() - interval '5 minutes'`. That condition preceded the 2026-07-22 outage by 18 hours. A firing alert means a projection fault needs root-causing and the affected leads' evidence needs repair; it never blocks writes by itself.
