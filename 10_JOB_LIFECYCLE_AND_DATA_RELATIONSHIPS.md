...
Server proof:34/34 local PostgreSQL17 tests, including real captured auto-advance/manual-boundary function bodies and unchanged legacy function hash. Focused client stage/continuity verification passed, with remaining adjacent failures closed in the final127/127 regression rerun; iOS source is integrated on local main622010a0. Physical-device behavior remains unmeasured. The server migration is applied and its catalog, grants, RLS, trigger and unchanged legacy function are verified. Jackson approved a data-preserving optimized test installation and measurement on his iPhone; that device follow-through is in progress. An App Store release and git push remain unapproved. Full request/result contract is in chapter04 and private storage custody in chapter03.
...
| `20260817_STAGED_email_photo_source_attribution` — rewrites the completion function to write `source='email'` + provenance, and backfills the already-imported photos | `20260829233858` | **applied 2026-08-29** at the ops-web push GO (13 photos backfilled); archived as `migrations/20260829233858_email_photo_source_attribution.sql` |

The first two were pure-additive and invisible to every deployed client (web narrows unknown sources out of its gallery groups; iOS decodes `source` as a plain string). The third was held in `ops-web/supabase/migrations/staged/` and applied on 2026-08-29 as ledger `20260829233858` (header reworded at apply time, body byte-identical to the staged text). **The paired ops-web gallery half did not ship with it:** `feat/email-photo-source-attribution` (the `From Email` gallery group, `PhotoSource` widened to the full enum, lightbox provenance line) is still unmerged as of 2026-09-05, so the deployed gallery — whose `SOURCE_ORDER` is the four original sources — silently hides every `email`-sourced photo (37 live rows across 8 Canpro projects, 17 of them imported on 2026-09-04). That branch has been re-verified on current `main` and rides the next push block.
...
**Agent readiness widening (2026-09-05 — built, verified, STAGED behind the web deploy).** The three agent read implementations that partition project photos by source — `private.read_agent_job_readiness_issues_v4_impl`, `private.read_agent_job_participant_snapshot_v5_impl`, `private.read_agent_job_summary_as_system_v6_core` — enumerate the original six `photo_source` values and count anything outside them as `malformed_or_local_count`, and `deriveSitePhotoReadinessFact` (`ops-web/src/lib/agent-control-plane/services/readiness-rules.ts`) sums only the named buckets into `usable_photo_count`. Since the flip landed, every emailed photo is therefore "malformed" to the agent: on 2026-09-05 six Canpro projects hold nothing but emailed photos and read as having no usable site photos, so `SITE_PHOTOS_MISSING` fires and a `photo_request` would ask the customer for photos they already sent. This is customer-visible through the MCP mount.

Decision: emailed photos **are** usable site photos — they are photographs of the site supplied by the customer through the conversion pipeline, exactly what the rule and the `photo_request` purpose exist to detect. Only `deck_design` renders stay excluded (drawings, not photographs).

The fix ships in two halves that must land in this order:

1. **Web (ops-web `fix/agent-photo-source-email-20260905`, merged into `feat/email-photo-source-attribution`):** `ActiveRemotePhotosBySourceSchema` gains an *optional* `email` bucket and the usable sum adds it. Optional, never defaulted — the readiness, job-summary and communication-context repositories re-canonicalize the parsed raw sources against the projection hash the database computed, so parsing may not add a key the database did not emit (a `.default(0)` failed 18 repository tests for exactly that reason). The widened bundle therefore accepts both the six-bucket and the seven-bucket partition.
2. **Database (`ops-web/supabase/migrations/staged/20260905_STAGED_agent_photo_source_email_widening.sql`):** a guarded, idempotent transformation of the three live definitions — an `email_count` filter, `'email'` in the exhaustive list, and an `'email'` key in `active_remote_by_source`, each replacement asserted to match exactly once, readback-verified for content, owner, ACL, security and `search_path` — then `private.advance_agent_operational_read_revision` for every company holding live emailed photos so six-bucket-era cursors and fences go stale. **GO condition: apply only after the ops-web `main` deployment carrying half 1 is Ready on Vercel; the deployed bundle parses the partition with a strict six-key schema and would fail every readiness, summary and communication read the moment the seventh key appears.** Apply recipe and post-apply steps are in the staged README.

Rollback probe on prod, 2026-09-05 (one transaction: three real reads as Jackson at Canpro → apply → the same three reads → rollback), Canpro project `3661df4f` "1151 Reynolds Rd", whose six photos all arrived by email: before, `get_job_summary` / `get_job_communication_context` returned `structured_row_count: 6`, all six buckets `0`, `malformed_or_local_count: 6`; after, `email: 6`, `malformed_or_local_count: 0`; `list_job_readiness_issues` over an October window listed the same job and its entry flipped identically, with the `email` key on every candidate. Post-transform definition md5s matched the locally precomputed transform byte for byte (`6e9e109d…`, `a897b2a8…`, `95dc88a4…`) and the rollback restored the originals (`a3151ceb…`, `2cab37a1…`, `e8ddba2e…`). No capability or manifest revision is bumped: like the August read repairs, this is a corrective body change to the raw-fact partition, not a contract change — `rule_revision` stays `site-photos-missing:v1`.
...
- Financial table fields (`estimate_total`, `invoice_total`, `paid_total`, `value`, `project_cost`, `margin`) are gated at the SQL wire level. Without `projects.view_financials`, `project_table_rows` returns `null`, never zero; clients render the empty-state mark.
  - **Gate implementation (corrected 2026-09-08, ledger `20260908182719_project_table_rows_current_user_permission`):** the view is `security_invoker`, so its `perm` CTE evaluates as the calling API role. It now calls `private.current_user_has_permission_scoped('projects.view_financials', 'all')` — the client-safe wrapper that ledger `20260807204914_agent_control_plane_actor_authority` introduced when it made the actor-parameterised primitive `public.has_permission(uuid, text, text)` executable by `postgres`/`service_role` only. Between that hardening reaching production and 2026-09-08 the CTE still called `has_permission(...)` directly, so every browser read of `project_table_rows` failed `42501` (PostgREST 403) and the Projects table rendered "Couldn't load projects" for every company (bug `0ed95e91`). Rule: `security_invoker` views, RLS policy expressions and SECURITY INVOKER functions may only reference functions `anon` and `authenticated` can EXECUTE; use the `private.current_user_*` forms, never the actor-parameterised `public.has_permission` / `private.permission_user_is_admin` / `private.user_is_company_admin`.
...
## Approved task schedule/crew changes (2026-09-06; Phase 14 dormant)

A project remains the job anchor; an existing `project_tasks` occurrence is the change unit and every `task_scopes` row remains part of that same scheduled visit. Exact approval changes only occurrence dates and crew through the canonical actor mutation core. It advances `schedule_version` once, explicitly clears prior schedule confirmation, preserves task status/scopes/recurrence and recomputes the project's crew union. Unsupported paired, split or dependent work blocks preparation. Site visits remain separate booking records. The task mutation, internal events, reminders, project rollup and durable receipt are one transaction. [Phase 14 contract](specs/2026-09-06-ops-mcp-schedule-crew-approval.md) records the full contract and production gates.
