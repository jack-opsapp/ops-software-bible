...
**Prepared locally only.** OPS-Web commit `77ad6032d` is not merged, pushed, or deployed. It changes OPS-Web account creation and setup instrumentation; it does not release iOS, backfill old accounts, modify GA settings, or change trial/activation/revenue definitions. Verification: 171 targeted tests passed, 15 changed TypeScript files checked without diagnostics, and changed-file lint passed; repository-wide typechecking exhausted its default Node heap and is not claimed as passing.
