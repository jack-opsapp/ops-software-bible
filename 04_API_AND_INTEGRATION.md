...
#### Codex desktop OAuth compatibility (production-deployed; host proof pending, 2026-08-29)
...
Application tests cover the captured Codex payload plus adversarial redirect shapes. A disposable PostgreSQL 17 full-wave run proved exact storage, consent/code binding, wrong-port rejection without consuming the code, exact retry, migration replay, and all earlier P2 migrations. Migration `20260829192448_mcp_oauth_codex_dcr_callbacks.sql` is now production-applied, and OPS-Web production HEAD `a567902f86946436385244acc1fd0154afe78a1e` is served by READY deployment `dpl_FGXsf3DT5rc3n1coooSyq3yuFLFc`. That final deployment includes both live post-release auth repairs: it preserves the sanitized MCP authorization return path after sign-in and routes MCP-specific Google/Apple login through an on-page Firebase popup after the live full-page redirect returned no session. Ordinary production provider login retains the full-page redirect. OAuth metadata and the MCP bearer challenge passed live checks, and Codex DCR discovery and registration work in production. The full host canary is awaiting the user's final browser completion; no completed token exchange, authenticated `tools/list`/tool call, or accepted Codex host is claimed yet. Codex desktop compatibility does not by itself prove ChatGPT connector compatibility.
...
- **Codex:** the strict callback policy is deployed, and DCR discovery and registration work in production. The full host canary is awaiting the user's final browser completion. No completed token exchange, authenticated tool-list/call, revocation/cleanup proof, or accepted Codex host is claimed yet. ChatGPT remains separately unproven.
...
The database/application deployment contract is complete; the external deployment contract did not change. No thirty-four-read exposure revision has been built, so a separately authorized release must still create, review, and deploy a new immutable exposure before any P2 tool becomes visible. Claude's current production grant proves only the eleven-read exposure-v1 connector. Codex DCR discovery and registration work in production, but its full host canary is awaiting the user's final browser completion and remains unaccepted; ChatGPT remains unproven until it completes its own connector/OAuth/tool-call run. Full contract: `specs/2026-08-29-mcp-read-catalogue-p2.md`.
