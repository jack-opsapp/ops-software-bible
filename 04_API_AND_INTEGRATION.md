...
**Costs are recorded only as supplier cost profiles.** `prepare_set_catalog_pricing` is sale price and nothing else. OPS carries two cost models — the simple `catalog_variants.unit_cost_override` / `catalog_items.default_unit_cost` fields and the richer profile table — and `get_catalog_item` reads only the profiles, so they drift the moment one is written without the other. Whenever the profile that ends up as a variant's default changes, or that default's cost changes, `private.catalog_supplier_cost_profile_save` sets `unit_cost_override` to the same number in the same transaction and reports it as `variant_unit_cost_mirrored`; the compile predicts the mirror by the same rule the writer performs it, so the sealed proposal cannot be contradicted by the commit. That keeps the resolved cost from drifting for everything MCP writes — but it writes at variant level whatever level the family is costed at, which the precedence rule below does not allow; see **What remains**.
...
- **Four drifted variant cost mirrors are not yet aligned in production.** `20260916080000_align_variant_unit_cost_override_to_default_profile.sql` is authored and proved (4 rows, none from NULL) but unapplied and not on `main`, pending PR review; until it lands, four Glass Panel variants carry a `unit_cost_override` a fraction of a cent off their canonical default profile. The 35 variants with a NULL override are item-level costed and correct as they stand — they are not part of this work. See **Cost precedence** above.
- **Open: the supplier-cost writer has no level clause.** `private.catalog_supplier_cost_profile_save` mirrors a new default cost onto `catalog_variants.unit_cost_override` unconditionally, so the first MCP cost write on an item-level-costed family would convert that variant to variant-level costing as a side effect — the trap described in **Cost precedence**. The writer needs the level clause too. That fix is a follow-up, deliberately not slipped in on 2026-09-16: a writer change moves the catalogue write effect seal.
