...
## Weekly journal authoring (2026-09-10; live in production since 2026-09-10; generated photographs live 2026-09-15; topic funnel built 2026-09-16, not yet deployed)
...
| `GET /api/admin/journal/editorial` | Platform-admin read (settings, newsletter switch, newest six slots with rendered article, citations and the last six attempt events); no-store. Topic funnel (not yet deployed): adds `radar {scanned_at, ok, total, degraded}` and per slot `pitch` (the draft's `package.pitch`, or the live claim's pitch while `authoring`); claim tokens are never returned. |
