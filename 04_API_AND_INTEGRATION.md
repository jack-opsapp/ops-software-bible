...
### Supplier bill intake API (local/unreleased, 2026-09-04)
...
Extraction in `src/lib/accounting/supplier-bills/pdf-extraction.ts` is conservative and records provenance instead of inventing missing dates, purchase orders, job identities, quantities, or units. `src/lib/accounting/supplier-bills/canpro-reconciliation.ts` creates review suggestions only. The routes do not let extraction output bypass operator disposition. The service and repository live in `src/lib/accounting/supplier-bills/intake-service.ts` and `intake-repository.ts`; route implementation is under `src/app/api/internal/accounting/supplier-bills/intakes/` in local OPS-Web commit `ca7f46f3c`, with the review console completed in `bf3610e3e`.

The migration, endpoints, and clients are local/unreleased. Production has the earlier canonical AP tables and contract, but not this intake schema or these routes. No new external extraction vendor or usage-priced API is introduced; PDF parsing is local through `pdfjs-dist`. Existing S3, Vercel, and Supabase usage remains subject to the account's ordinary consumption costs after any future approved release. Exact contract and release gate: `specs/2026-09-03-canpro-supplier-bill-clearance.md`.
