...
- **The text repair is applied to production** as ledger version `20260916182941`, and found no double-encoded text there (below) — production had already been cleaned outside this branch.
...
### Double-encoded catalogue text repair (2026-09-16; applied to production, which was already clean)

**Production finding (2026-09-16, ledger version `20260916182941`).** The repair's text pass found **zero** double-encoded rows in production. Production text had already been cleaned before the apply, outside this branch: 131 supplier cost profiles carry a proper U+2014 em dash and **none** carries the mojibake bytes, and the `1¾"` option value and the `width × height` family description are both correct as stored. 133 profiles show `updated_at` inside the 2026-09-15 19:00–20:00 UTC window, so that cleanup happened after the local proving snapshot was taken. `private.catalog_text_repairs_20260916` therefore holds **4 rows, all `column_name = unit_cost`**: the four Canpro Glass Panel `vitrum-2026` supplier cost profiles, rounded half-up from 4.1992, 4.2804, 9.2684 and 9.7440 CAD to two decimals. No live supplier cost is finer than a cent now. The migration remains idempotent, and the runbook's "172 ledger rows" expectation was a rehearsal number, not a production one. Everything below describes the local production copy the migration was written against; its 168-row scan and its 52 → 88 read tally are local measurements and do not describe production.

The hand-written SQL that loaded Canpro's cost sheets before there was a write surface encoded its text as UTF-8 and then encoded those bytes again as if they were Latin-1. An em dash (U+2014, bytes `E2 80 94`) became three characters — U+00E2, U+0080, U+0094. U+0080 is a C1 control character, so `private.agent_p2_optional_canonical_text` nulls the value, the catalogue detail read marks the cost row invalid, and `private.agent_p2_catalog_detail_v1` raises `agent_catalog_source_data_invalid` for the whole family. The same bytes made the supplier-cost tool's whole-proposal safety gate refuse every cost change on an affected variant — the tool was unusable on the catalogue it was built for, which is how this was found.
