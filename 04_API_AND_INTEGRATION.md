...
#### Automatic inquiry creation boundary (2026-09-09; built, awaiting release)

Ops-web `20208b8a9` removes the remaining platform/pattern/form bypass after a production Wix author-assignment notice created an empty client and lead. This follow-up is local and has not replaced the released `d86d5664b` behavior above. Production records were not repaired or replayed.
...
Verification: 320 tests passed in 20 affected files, including the actual sync engine with isolated provider/database doubles, genuine inquiry/replay, non-inquiry forms, exact Wix notice, existing-job routing, deferred source retention, and recovered form isolation. Scoped TypeScript passed; ESLint had zero errors and six existing console warnings. Full-repository TypeScript exhausted the default Node heap and is not counted as passing. No production build or paid model evaluation was run for this follow-up; model accuracy and post-release runtime behavior remain separate from fixture proof. Sources: ops-web `src/lib/api/services/{sync-engine,ai-sync-reviewer,lead-feedback-prior-service}.ts`, `src/lib/email/{email-work-routing,contact-form-lead-gate}.ts`, and `docs/artifacts/email-work-correspondence/inquiry-boundary-verification.md`.
...
- `phase_c` — gates automatic drafting, AI enrichment, and unmatched-email classification. Does not gate the inbox screen or explicit operator-approved lead import. The local inquiry-boundary correction below removes automatic creation shortcuts that previously ran without this classifier.
...
**Automatic creation status (2026-09-09):** Production through `d86d5664b` still has sender/pattern/form shortcuts that can create clients and opportunities without `phase_c`. Local follow-up `20208b8a9` removes those shortcuts: every automatic new lead requires classified current-message inquiry evidence, so `phase_c`-disabled mail remains in the inbox or form review. This follow-up awaits explicit release approval; see “Automatic inquiry creation boundary” above. Mailbox synchronization and explicit operator-approved import remain independent of `inbox_ui`.
