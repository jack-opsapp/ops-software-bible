...
The deployed policy keeps Claude's two hosted callback strings exact and adds one raw Codex family: literal `http`, literal host `127.0.0.1`, one explicit decimal port from 1 through 65535, exact path `/callback/`, and a bounded base64url callback identifier. It rejects `localhost`, IPv6, alternate numeric/encoded loopback spellings, userinfo, query, fragment, path encoding, missing or padded ports, mixed callback families, and registrations containing multiple Codex callbacks. Consent preview, approval/denial redirect, authorization code, and token exchange continue to compare the complete registered URI byte-for-byte. CIMD and authorization-response issuer support remain unadvertised.

Application tests cover the captured Codex payload plus adversarial redirect shapes. A disposable PostgreSQL 17 full-wave run proved exact storage, consent/code binding, wrong-port rejection without consuming the code, exact retry, migration replay, and all earlier P2 migrations. Migration `20260829192448_mcp_oauth_codex_dcr_callbacks.sql` is now production-applied, and OPS-Web production HEAD `a567902f86946436385244acc1fd0154afe78a1e` is served by READY deployment `dpl_FGXsf3DT5rc3n1coooSyq3yuFLFc`. That final deployment includes both live post-release auth repairs: it preserves the sanitized MCP authorization return path after sign-in and routes MCP-specific Google/Apple login through an on-page Firebase popup after the live full-page redirect returned no session. Ordinary production provider login retains the full-page redirect. OAuth metadata and the MCP bearer challenge passed live checks. On 2026-08-29 the Codex in-app browser returned Firebase `auth/network-request-failed`, but the system-browser path completed DCR and OAuth successfully. `codex mcp list --json` then showed `ops_-_maverick_projects` enabled with `auth_status` `o_auth` and URL `https://app.opsapp.co/api/mcp`. One exact read-only `search_jobs` canary with `{"lifecycle_states":["active"],"limit":1}` completed successfully and printed no business data. The constrained Codex canary report did not surface an explicit contract-version label, so this proves the bounded host/OAuth/tool path but does not independently label the exposure contract. Codex desktop compatibility does not by itself prove ChatGPT connector compatibility.
...
The deployed transport runs capability manifest `2026-08-22.capability-manifest.v8` but registers only the immutable `2026-08-22.mcp-exposure.v1` catalogue: exactly eleven v7-compatible reads and seven grantable scopes. The original nine retain their v6/v7 database bridge. The remaining twenty-three v8 reads, every write family, and the two site-visit contracts are absent from external registration. The injected exposure object is the rollout control — manifest implementation alone cannot widen the transport. Results and error envelopes both pass through `serializeUntrustedPromptData` before entering any model context.
...
### Dark-v8 production deployment and Codex host proof (2026-08-29 UTC)

- **Database:** all thirty-nine ordered P2 migrations are production-applied, including consent-catalog versioning and strict Codex DCR callback registration.
- **Application:** OPS-Web production HEAD `a567902f86946436385244acc1fd0154afe78a1e` is served by READY Vercel deployment `dpl_FGXsf3DT5rc3n1coooSyq3yuFLFc` at `app.opsapp.co`. It includes both live post-release auth repairs: preserving the sanitized MCP authorization return path after sign-in and using an on-page Firebase popup for MCP-specific Google/Apple login after the live full-page redirect returned no session.
- **Live routes:** OAuth authorization-server metadata and protected-resource metadata passed live checks; the unauthenticated MCP endpoint returned the expected bearer challenge.
- **Policy versus exposure:** production runs manifest v8 with thirty-four implemented reads, while immutable exposure v1 remains exactly eleven tools and seven grantable scopes. The other twenty-three reads are deployed dark, no thirty-four-read exposure revision has been built, and no external write is available.
- **Codex:** accepted for the verified production read-only path on 2026-08-29. DCR/OAuth completed in the system browser after the in-app browser returned Firebase `auth/network-request-failed`; `codex mcp list --json` reported the connector enabled with OAuth, and the exact `search_jobs` canary succeeded without printing business data. The constrained Codex canary report did not surface an explicit contract-version label. ChatGPT remains separately unproven.
...
### MCP complete read catalogue P2 (production-deployed 2026-08-29; externally dark)

The deployed OPS-Web release composes the original eleven reads with twenty-three new domain methods under one nominal read-catalogue service. The MCP dispatch map contains all thirty-four fixed capability-to-method mappings, but `createOpsMcpServer` registers only the tools in the injected immutable exposure object. Exposure v1 remains exactly the original eleven reads and seven scopes, so P2 methods are not externally callable even though their implementation is production-present.
...
The intended deck request—“grab the deck design from Carly Hunter's site visit, total linear feet of railing and square footage, and show the geometry”—uses customer/job discovery, site-visit context, then the opaque design reference with `get_deck_design_geometry`. It never searches raw deck JSON by customer name and never treats geometry coordinates as measured dimensions. This flow remains unavailable to external hosts until a separately reviewed exposure revision includes its exact tools and scopes.

The database/application deployment contract is complete; the external deployment contract did not change. No thirty-four-read exposure revision has been built, so a separately authorized release must still create, review, and deploy a new immutable exposure before any P2 tool becomes visible. Claude's current production grant and Codex's accepted read-only canary remain within the eleven-tool/seven-scope exposure-v1 boundary; the constrained Codex canary report did not surface an explicit contract-version label and does not widen that boundary. ChatGPT remains unproven until it completes its own connector/OAuth/tool-call run. Full contract: `specs/2026-08-29-mcp-read-catalogue-p2.md`.
