...
## Catalogue recipe read and exposure V24 (2026-09-15; application and migration authored)
...
- **Activation.** The effect-policy seal row under `2026-09-15.catalog-setup-write.v1` is unseeded by design. Until Jackson explicitly approves activation and an operator inserts the reviewed hash, all five tools are registered, discoverable and inert.
- **The text repair is not applied to production.** `20260916060000_repair_double_encoded_catalog_text.sql` is authored and proved locally only (below).
- **Four Glass Panel variants** refuse a cost-authorised `get_catalog_item` with `agent_money_minor_units_not_exact`, before and after the repair alike: four `catalog_supplier_cost_profiles` rows carry a CAD unit cost with more than two decimal places, which the money projection refuses rather than rounding. Landed cost per square foot is a real sub-cent number; whether it should be stored and read at four decimal places or rounded is a product decision, not a code defect.
...
### Double-encoded catalogue text repair (2026-09-16; authored and proved locally, NOT applied to production)
