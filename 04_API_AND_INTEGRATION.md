...
## Phase17 catalog/inventory authoring candidate (2026-09-08)

Candidate manifest `2026-09-08.capability-manifest.v24`, exposure `2026-09-08.mcp-exposure.v19`, consent `2026-09-08.mcp-consent-catalog.v14`, schema `2026-09-08.v1` add `inspect_catalog_changes`, `prepare_catalog_changes`, and `prepare_inventory_adjustment`. The production exposure resolver/registration catalogue does not select v19. Existing clients and grants remain unchanged. `createCatalogAuthoringCandidateMcpServer` constructs the isolated acceptance surface only; the production server rejects that unregistered exposure. Commit capabilities are internal registry entries and never MCP tools.
...
Release boundaries: migration application, effect-policy activation, candidate OAuth registration/exposure enrollment, and any real catalog/stock canary each remain separately unapproved. Local protocol/SQL tests are not native ChatGPT/Claude acceptance or production write proof. Coordinate any shared release through the parent initiative; leave Phase16 actors, grants, trial policies, seals and synthetic records untouched.
