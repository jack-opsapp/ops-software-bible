...
**Status:** Locally committed in OPS-Web `237d151aba2a1ca28b349c5596b8e8ba2b56642c`, `9006d343d4dc10d3724b5627bfa26fa0d4a1eb7a`, and shared integration `1ecd310fda5110f8ef329e3229a4497ce23b39ed`. This is not deployed or enabled for any host. No database migration, production read repair, consent/grant change, or historical signed-manifest edit is included. Result v2 is distinct from the existing MCP exposure v2.
...
`registry/deck-geometry-exposure.ts` defines only the dormant, deck-only `2026-09-10.mcp-exposure.v23` candidate. `createDeckGeometryCandidateMcpServer` requires its exact pin and matching nominal MCP actor/token/grant/client/company/scopes; normal `createOpsMcpServer` rejects this revision. It is not registered in the active exposure catalog, consent, discovery, OAuth, or a grant transition. Candidate scope allowlist: `ops.customers.read`, `ops.files.read`, `ops.jobs.read`, `ops.schedule.read`, `ops.site_visits.read`. P19-2 reservations `2026-09-10.capability-manifest.v28` and `2026-09-10.mcp-consent-catalog.v18` remain unused. The candidate is a complete isolated allowlist, not an overlay or replacement for an operator's multipurpose connection.

**Integration and host acceptance:** Parent release ownership must compare the actual operator grant's complete tools/scopes before and after, preserve unrelated capabilities and existing authority, compose the approved compatible exposure without activating sibling work, and obtain explicit release/host-transition authorization. Do not repin a multipurpose grant directly to the deck-only candidate. The original native read was blocked by missing `ops.files.read`; do not manufacture that authority or treat local replay as a host success. After approved integration/deployment and grant/consent/host refresh as applicable, replay the original authenticated Claude request using original business arguments and independently verify current source custody, retained stairs, configured-versus-estimated coverage, tool discovery, and preserved existing grant capabilities. No host canary is claimed here.

**Local proof:** 222 relevant geometry, legacy transport and grant-pinned exposure tests pass. An extended 224-test run has 223 passes and one unchanged baseline dispatch expectation failure (the base test expects 50 methods but its base dispatcher already contains three additional catalog methods). Both baseline files are byte-identical to web base `23ffe646a77effba851fe063c68c15e93114b9cb`. Final test typing refinements pass 43/43; bounded server/service/new-test TypeScript and targeted lint pass. A whole-repository TypeScript attempt was stopped under resource pressure and is not claimed. Independent review has no unresolved findings. Parent independently reran 150/150 tests across twelve files, verified the baseline-file equality, and replayed the exact private source in memory with matching identity and unchanged source. Local and parent checks used Node v24.19.0; production declares Node 22.x, so this does not establish production-runtime acceptance. The original saved drawing was replayed through the pure local calculator without changing it; the optional lower edge and stair survive, configured flat remains zero, and perimeter remains partial because the landing opening is unlocated. No private drawing, identities, or quantities are committed. Evidence and exact commands: OPS-Web `docs/artifacts/deck-geometry-2026-09-10/HANDOFF.md`.
