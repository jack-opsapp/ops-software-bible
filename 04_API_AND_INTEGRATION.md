...
- **Anon execute/table grants.** The P6 function layer + the `accept_estimate_to_job_requests` table were granted to `anon` in `2026-05-29-03-ios-catalog-p6-anon-role-grants.sql` and `2026-05-30-01-ios-catalog-p6-accept-request-anon-grant.sql` (the latter is live but not yet in `schema_migrations` — pending tracking reconciliation).
