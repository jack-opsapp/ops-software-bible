...
**Status:** Full production integration is complete locally; no deployment, live migration or host acceptance is claimed here. The original repair (`237d151aba`, `9006d343d`, `1ecd310fd`) is integrated with production base `ab42b042613d487413f3d9f36f535cbe27a40504`. New web commits `7788200d8` and `6bc665097` add the complete successor and guarded migration. Result v2 is distinct from the older MCP exposure v2.
...
Migration `20260910225814_agent_deck_geometry_v23_exposure.sql` extends access resolution plus customer-update authority, queued reauthorization and rate limiting. New service-only `prepare_agent_customer_update_for_grant_as_system` derives the immutable grant exposure; it does not accept an exposure argument. The unchanged original preparation routine performs the full authority/evidence/no-change/policy checks and canonical company-first locking. The wrapper adds no early shared row lock. Old V14 proposals remain approvable after migration. Original preparation, refresh and immutable-consent routines stay frozen.
...
**Integration and host acceptance:** The release owner applies the additive SQL before deploying active-V23 code, verifies the READY production alias, and requests a fresh Claude authorization/new registration with the full existing tool/scope set. Refreshing an old token cannot promote its immutable exposure. The existing full V14 connection already has `ops.files.read`; no scope expansion is required. After rollout, replay the original authenticated request and verify source identity, retained null lower destination/stair, configured zero, partial measured perimeter, and full tool discovery. Local proof does not replace this host canary. Once V23 grants exist, rollback must keep V23 resolution/representation while separately stopping new V23 registrations, or repair forward; a preintegration deployment rejects V23.
