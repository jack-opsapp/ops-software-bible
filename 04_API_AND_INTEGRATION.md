...
The edge adapter accepts current `{expense_id}` and legacy `{expenseId,companyId}` requests, rejects conflicting aliases/extra keys and bodies over 4096 bytes, and forwards the original bearer JWT plus anon key to `request_expense_accounting_sync(p_expense_id uuid,p_company_id uuid default null)`. The RPC independently resolves the active Firebase actor and exact company under lock. The adapter performs no provider authentication, refresh or financial write. Its response is a durable queue receipt, never proof of provider acceptance.

`GET/POST /api/integrations/accounting/expense-settings` requires both `accounting.manage_connections` and `expenses.approve` at all scope. GET returns named native provider accounts/employees/tax choices plus OPS categories/crew and current mappings. POST validates choices against that exact connection's provider catalogue and calls service-only `save_expense_accounting_settings`. No account ID is guessed and no supplier is created for a crew member.
...
Deployment order remains separately approval-gated: prerequisite expense authority migration, accounting lifecycle migration, server/worker and edge adapter, then wait-free clients. No production expense migration, provider canary, deployment or App Store release occurred in this work.
