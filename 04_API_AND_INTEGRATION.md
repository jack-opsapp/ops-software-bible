...
**Current trial checkpoint, 04:40 UTC:** Jackson explicitly approved the one-hour exact MAVERICK catalog price/cost/stock connection and all test actions in that test company. The earlier activation-blocked checkpoint below is superseded. New client `15668aa4-20b4-487a-be4a-f993433e99cc`, binding `a9cb33b1-4ba2-4a97-a801-16d9e69b16f0`, grant `a22c86f6-eb6d-4577-bffd-24046c3dc6fa`, actor Pete Mitchell `8e811f98-9f2b-4f64-b409-ed56074b7dc8`, company `ddee107c-33cd-483e-8278-0f8d8a180181`, expiry 05:21:50 UTC. Installed reviewed effect hash `sha256:d199bce2ce7e02bde227df57edf167b5963edbd18885d866410645a84a6a33e0`; all 29 trial/OAuth/catalog function definitions and 66 captured canonical constraints/trigger definitions matched reviewed local evidence after unrelated ads migrations. Actual installed Codex app-server 0.153.4 completed PKCE OAuth with the exact seven scopes, read the MAVERICK company/catalog, inspected a seven-create fictional graph with zero stock/purchase/accounting effects, and prepared action `7b60c394-a830-4981-8c0c-3b25ddcfa843`, change set `389546d6-5789-481e-8f3a-a05f5377ea2d`. Identical native preparation replay returned the same action/change set with `replayed:true`. Independent SQL found zero test variants before approval. No business save is claimed at this checkpoint.

The native acceptance exposed a pre-existing queue request-isolation defect: concurrent routes used the module-global `setSupabaseOverride`, so a faster request's cleanup could make a slower authenticated request resolve the browser/anon client and show an empty queue. Seven deterministic route-interleaving tests reproduce that failure. The focused correction binds GET/POST/PATCH/DELETE/bulk queue service calls through existing request-local `runWithSupabase`; authentication, `agent.review`, current actor/company filtering, exact review and atomic commit remain unchanged. The database independently confirms the new proposal is readable by the exact operator. No RLS policy or company automation flag is relaxed.
...
Release authorization: Jackson approved application/database deployment and a temporary MAVERICK-only trial, with each production business save still requiring his exact approval inside OPS. Local tests are not native ChatGPT/Claude acceptance or production write proof. Existing Phase16 actors, grants, trial policies, seals and synthetic records remain untouched.
...
Catalog SQLSTATE `55P03` (lock contention), `57014` (statement cancellation) and `25P04` (transaction timeout/session termination) map to `TEMPORARILY_UNAVAILABLE`, `retryable: true`, with guidance to retry the same request. Approval retries preserve the exact action/change/seal/key; transport errors alone never establish success. Before activating this candidate, independently exercise the actual deployed PostgREST/pool error shape, same-key retry and persisted receipt readback. Direct SQL tests do not close that gate.
