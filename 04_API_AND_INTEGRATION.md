...
| `POST …/assignments/{id}/pitch` `{claim_token, pitch}` (topic funnel, 2026-09-16, not yet deployed) | Lease-checked like sources (404/409). OPS validates the pitch (`src/lib/journal/editorial/pitch.ts`): `topic`, `reader`, `why_now`, `signals` (0–12 radar ids), `chatter` (0–8 `{url https, shows}` from search), `ethos`, `angle`, `hook`, `headline`, `hooks_considered` (6–14 `{headline, hook, verdict}`), `runners_up` (2–4 `{topic, why_not}`). 422 `PITCH_REJECTED` `{code, issues}` with `code` ∈ `SCHEMA_INVALID, MARKUP_INVALID, TITLE_FORMAT, VOICE_REJECTED, STALE_FRAMING, DUPLICATE_TOPIC, PITCH_EVIDENCE` (≥1 radar signal whenever the radar holds any in 21 days; signals + chatter ≥3), `PITCH_SIGNAL_UNKNOWN` (not a radar signal from the last 21 days), `PITCH_HOOKS` (<6 distinct headlines), `PITCH_RUNNERS_UP`; rejections are logged `pitch_rejected`. 200 `{state:"pitched", lease_until, headline}` stores the pitch with each cited signal frozen as seen (`source` name, `kind`, `views`, `typical_views`, `momentum`, `replies`, `age_days`) plus `radar_scanned_at`; 429 `PITCH_LIMIT` after three. The claim (v3) adds `radar {scanned_at, spheres, feeds}`, `trend_signals` (≤150 from the last 14 days, six strongest per feed, ordered leadership → business → trades → industry → forum) and `pitch_limits`; `brief_version` becomes `ops-journal-2026-09-16-v3`. The draft route answers 422 `PITCH_MISSING` without this claim's pitch, copies the pitch into `package.pitch`, and the editor verdict adds `hooked` with reason `weak_hook`. |
