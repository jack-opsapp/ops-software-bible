...
Prepared, not applied: migration `20260914201338_finalize_exact_email_thread_recovery.sql` adds `private.email_thread_recovery_finalizations` and service-role-only `finalize_exact_email_thread_recovery_guarded`. The new RPC finalizes only the canonical thread link and inbox thread ownership after every exact-message recovery receipt is complete. It requires active actor permissions, a live mailbox lease owner, full current source/target/thread/link snapshots, the complete exact message set, and current attachment/inspection/materialization proof. It fences the seven dependent tables through commit and preserves both opportunity rows and message payloads. It does not alter the ordinary same-client reassignment contract. PostgreSQL 17 proof uses actual ownership triggers and attachment-state logic, with 23 assertions, 37 rejection cases, three concurrency checks, and two rollback readbacks. Production migration approval and live recovery verification remain separate. The private recovery also accounts for rotating Gmail attachment download tokens and older discovery records; provider re-enumeration alone is insufficient to finish stored-file custody for the audited history.
