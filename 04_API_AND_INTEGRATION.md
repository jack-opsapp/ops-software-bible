...
**Cost precedence (decided 2026-09-16).** The **default supplier cost profile** in `catalog_supplier_cost_profiles` is canonical. `catalog_variants.unit_cost_override` is a mirror of that profile's cost, kept in sync by `private.catalog_supplier_cost_profile_save` on every MCP write. The decision costs nothing today because nothing resolves between the two yet: a 2026-09-16 sweep of `public` and `private` found no estimate, takeoff, recipe or material function reading either field (estimate lines carry their own `unit_cost`), and the only two readers that exist — `private.agent_p2_catalog_detail_v1` and `private.agent_p2_purchase_order_cost_witness_v1` — already read the profile. The rule matters for what comes next, and it closes the precedence half of gap #17 that the mirror alone left open.

**The mirror drifted on day one.** The 2026-09-16 cost rounding moved four Glass Panel profile costs to the cent but left the mirror field alone, so four variants disagreed with themselves (4.1992 vs 4.20, 4.2804 vs 4.28, 9.2684 vs 9.27, 9.7440 vs 9.74). A further 35 Canpro variants carry a default profile with a null mirror; for all 35 the family default equals the profile cost, so aligning them changes no resolved number. Migration `20260916080000_align_variant_unit_cost_override_to_default_profile.sql` sets the mirror from the default profile for all 39 rows, refuses on any real precedence conflict, and ledgers every row into `private.catalog_text_repairs_20260916` under `column_name = 'unit_cost_override'` (a null before-value recorded as the text `'null'`). Proved locally inside a rolled-back transaction: 39 rows, 35 of them from null, postflight clean. It is authored on `feat/recipe-mcp-read` (commit `01f140982`), **not on `main` and not applied to production** — it is waiting on Jackson's go.
...
- **The variant cost mirror is not yet aligned in production.** `20260916080000_align_variant_unit_cost_override_to_default_profile.sql` is authored and proved (39 rows) but unapplied and not on `main`; until it lands, four Glass Panel variants carry a `unit_cost_override` a fraction of a cent off their canonical default profile, and 35 more carry a null mirror. See **Cost precedence** above.
