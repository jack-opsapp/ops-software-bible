...
**OPS-Web admin session recovery (prepared locally September 3, 2026; not production-live):** `AuthProvider` observes Firebase ID-token changes and mirrors the current token into the canonical `ops-auth-token` cookie only for the token's remaining lifetime (with a refresh safety margin and a one-hour ceiling), clearing the legacy `__session` cookie on every write or sign-out. Middleware decodes the token expiry only as an unverified routing hint: stale or malformed cookies are cleared, and protected requests are sent to `/login` with the exact same-origin path and query preserved. The `/admin` server layout remains the authorization boundary—it cryptographically verifies the Firebase token and checks the admin email allowlist before rendering; verified non-admins go to `/dashboard`. `/api/admin-debug` now uses the same admin-only gate before returning any diagnostics. This closes the stale-cookie loop where an expired token first lost `/admin`, landed on `/dashboard`, refreshed in the client, and worked only after the URL was typed a second time.
...
### Attachment-aware note persistence (2026-09-14; applied 2026-09-15 UTC)

Before notification dispatch, the live note-edit RPC accepts `p_note_id uuid, p_content text, p_mentioned_user_ids text[], p_event_id uuid, p_attachments jsonb DEFAULT NULL`. The first four named parameters preserve existing clients. Explicit attachment arrays may only detach current note attachments and return their immutable committed snapshot; omitted/SQL-NULL input preserves the existing response keys. Existing mention dispatch still uses the same persisted event UUID and recipient delta, with no extra notification path.

Migration `20260915021754_project_note_mentions_attachments.sql` is applied with explicit approval and mirrored on local OPS-Web main `c7098c283`. It is byte-identical to the prepared `20260914224026` artifact and independent of the pending accounting migrations. Production exposes one defaulted five-argument function (MD5 `2bbb8d6782b140843ab050fabe72431b`). Three anonymous no-change PostgREST probes confirmed both named request formats resolve and existing actor denial remains enforced. This is live API routing/security proof, not a successful signed-in edit or original-phone acceptance. See chapter03's note-attachment section and iOS `docs/artifacts/ios-bugs-release-20260914/photo-attachments.md` for verification and release limits.
...
| `ads_daily_ad_group/ad/asset/keyword`, `ads_click_map`, `ads_entities` | same `ads-sync` run (2026-09-09) | Entity snapshot (10 searchStream calls) + trailing 3 days — 30 on Mondays, Google restates inside its lookback window — of every grain (4 range calls + 1 `click_view` call per day). ~16 calls a weekday. `ads_funnel_by_keyword` joins the click map to trials, activations, and payments |
...
Three structural refusals, all throwing `PlannerError`: `BROAD_MATCH_REJECTED` on any
broad positive keyword; `NEGATIVE_BLOCKS_KEYWORD` when a negative would block a
keyword the same campaign bids on — money committed to a keyword that can never serve,
with nothing in Google's interface to say so; and `ROLE_ALREADY_LIVE` (2026-09-14) when
the file would create a control or challenger beside an enabled ad in the same role
that it does not also retire in `retire.adIds` — a second live challenger would never
be judged ("<campaign> › <group> already runs challenger ad N. To swap it, add N to
retire.adIds."). The file itself may list at most one control and one challenger per
group (`BlueprintError` `DUPLICATE_AD_ROLE`). A shared set is atomic in Google's model,
...
competitor names only in an ad group that answers to competitor copy and only as
`<Brand> alternative` / `Switching from <Brand>?` / `Tired of <Brand>?`, and a pin plan of
two or three headlines on position one. An ad group may override the campaign's copy kind
(`copyKind`) — intent lives at the ad group, not the campaign — and competitor copy,
claimed by the group or inherited from a competitor campaign, is allowed only on a group
whose landing page is a `/compare/` page (`COMPETITOR_COPY_WITHOUT_COMPARE_PAGE`; the
inherited case since 2026-09-14). The engine reads the same `kind` and `copyKind` from this
file (`src/lib/ads/engine/copy-kinds.ts`), so a challenger it writes answers to the rules the
blueprint's own ads were built under — see Google Ads engine → copy kinds.
...
**The guardrail stranded good ads (2026-09-10; fixed 2026-09-11, not yet deployed).** The engine
worker's `pauseDisapproved` paused any enabled, disapproved ad in an engine campaign at its daily tick.
At 14:59Z it paused all 22 non-brand ads while each carried the stale `DESTINATION_NOT_WORKING` verdict,
raised 22 persistent "AD DISAPPROVED" alerts promising a replacement the disabled routine never writes
(and that the brief would never have asked for), and had no path to restore an ad once Google approved
it again. The eleven controls were restored by hand on 2026-09-11
(`docs/artifacts/ads-engine/p2/restore-lead-ads-2026-09-11.mjs`, validateOnly first). GOOGLE ADS ENGINE -
P2-1-1 replaced it with the recorded, reversible guardrail described under "Google Ads engine →
Disapproved-ad guardrail" below (commits `ce58cf7c5`, `62ac27e31`, `258159da2` on `feat/ads-engine-p2`),
and its migration — applied to production — resolved the 22 alerts and their 20 rail rows. Replayed
against 2026-09-10, the new guardrail holds all 22, pauses none and alerts nothing. Record:
`ops-web/docs/artifacts/ads-engine/guardrail-restore/README.md`.
...
[2] STAGE  qbo_staging_* tables (per run_id) + computed customer matches
        ▼
[3] REVIEW /accounting → "QuickBooks Import" tab: reconciliation strip + match table; owner picks link/create/skip (Link may target ANY company client)
        ▼
...
0. **STEP 0 — Refuse unfinished decisions (2026-09-16).** Before any staged row is read or written, `findUnresolvedDecisions` (`src/lib/api/services/qbo-apply-decisions.ts` — the one pure rule shared by the review table, the Apply panel, the apply route and this engine) rejects a `needs_review` decision or a `link` whose `client_id` is missing/blank. A skipped customer's estimates, invoices, lines and payments are dropped, so an unfinished decision used to lose that customer's money silently; now the engine throws `Apply refused — N customer decision(s) unfinished (link with no OPS client: …; needs review: …). Nothing was written.`, `applyImport`'s guard marks the run `error`, and nothing is written. Both branches also hard-stop inside STEP 1 (a `needs_review` can never fall through to `create`).
1. **STEP 1 — Clients (link / create / skip).** Per the owner's `customer_qb_id` decision (a customer with NO decision in the payload defaults to `skip`): `skip` → null client id; `link` → verify the target client belongs to `companyId` (cross-tenant link rejected + warned), then write the QBO identity + lifecycle state (`qb_id`, `deleted_at` from `Active=false`) without overwriting name/email/phone/address; `create` → idempotent on `(company_id, qb_id)`, upsert `...clientFieldsFromCustomer(toShape(cust))` and tombstone inactive QBO customers with `deleted_at`. Builds `clientIdByCustomerQbId`.
...
| `import/apply/route.ts` | POST | **Background job (2026-07-04):** validate + scope; **(2026-09-16) after auth, permission and run scope, refuse `422 { error, code:"unresolved_decisions", unresolved:[{customer_qb_id, reason:"link_without_client"\|"needs_review"}] }` when any decision is unfinished — the run is not marked `applying`, no notification is opened, nothing runs**; otherwise mark the run `applying`, open a **persistent** `accounting_import_complete` rail notification, respond `202 { status:'applying', runId }`, then run the apply in Next `after()` and resolve that same notification to a completion (or failure) state. The review UI polls run status instead of blocking on the write, so the operator can navigate away. The apply engine/mappers are unchanged. |
...
- **LINK PICKER (2026-09-16)** → `ClientLinkPicker` (`src/components/accounting/qbo/client-link-picker.tsx`, on the house `EntityPicker`) searches EVERY live company client (`useClients`, deleted excluded) by name, email and phone — the importer's candidates lead with their `email match` / `exact match` / `NN%` qualifier, then everyone else A–Z (nameless clients render `—` and sort last). The option list is built only while a row's picker is open (hundreds of rows × hundreds of clients). Picking a client on a `needs_review` row resolves it to `link`; clearing never changes the action. Before this, the picker listed candidates only, so a customer filed under a business name while the OPS client carries the owner's name could not be linked and imported as a duplicate.
- **APPLY** → `useApplyImport` → `POST …/import/apply` with `{ runId, decisions }`. **Hard-blocked while any decision is unfinished** — a `needs_review` (inline `qbo.needsReviewBlock` hint) or a `link` with no client (row gets the rose blocking treatment + `qbo.linkMissingClientBlock` hint); both hints have `.one` singular keys (en + es). The apply route inserts the completion notification server-side and invalidates the Books queries.
...
### `public.reschedule_site_visit(p_site_visit_id uuid, p_scheduled_at timestamptz default null, p_duration_minutes int default null, p_assignee_ids text[] default null, p_reminder_lead_minutes int default null) → uuid`

Only a **booked** (`booked_at IS NOT NULL`), still-`scheduled` visit can move — a started visit is history, not a plan. NULL keeps a field unchanged; `p_reminder_lead_minutes = -1` is the explicit **clear** sentinel for the per-booking override (NULL means "leave as is", so a separate sentinel is required). Logs a `site_visit_scheduled` activity (`Site visit rescheduled`). The changed `scheduled_at` re-arms every prompt by construction because dedupe keys carry the epoch. An identical call writes nothing (idempotent). As of 2026-08-28, `p_scheduled_at` itself is NULL-keeps too (previously required), so crew/duration/reminder-only reschedules omit it; the past-time guard applies only when the time actually moves. Applied to prod 2026-08-28 as ledger `20260829022150` (mirror `migrations/20260829022150_reschedule_site_visit_scheduled_at_null_keeps.sql`), verified by object: `pg_get_function_arguments` shows `p_scheduled_at ... DEFAULT NULL`.
...
#### Stage moves — the `stage_transitions` write contract

Every `stage_transitions` row is written **server-side, inside the same transaction as the stage change**, with `transitioned_at = now()` and `duration_in_stage = now() - stage_entered_at` (a proper `interval`; NULL when the prior `stage_entered_at` was never set). Writers: `move_opportunity_stage` (direct moves — iOS, OPS-Web, `book_site_visit`'s new_lead→qualifying nudge, phase-C handoff consumption), `win_linked_opportunity` (linked-lead win), `convert_opportunity_to_project` (win+convert), and the email/AI disposition RPCs. No client constructs a transition row.

`move_opportunity_stage(p_opportunity_id uuid, p_to_stage text, p_user_id uuid, p_win_probability integer DEFAULT NULL)` — replaced 2026-08-29 (ledger `20260829183610`): reads the opportunity under a `FOR UPDATE` row lock (concurrent moves serialize, so each row records the true `from_stage`); raises `opportunity_not_found` (P0002) for missing/deleted rows; same-stage calls are a mutation-free no-op returning the row; the update sets `stage`, `stage_entered_at = now()`, `stage_manually_set = true`, `win_probability = COALESCE(p_win_probability, win_probability)` (web passes its stage-default; iOS and the internal 3-arg callers omit it → unchanged), and raises `access_denied` (42501) instead of returning a NULL row when RLS filters the update. SECURITY INVOKER; EXECUTE granted to anon/authenticated/service_role.

**History (why the contract is server-side-only):** until 2026-08-29, OPS-Web's `moveOpportunityStage` performed a client-side update + insert pair — it sent the duration as integer **milliseconds**, which Postgres parses as **seconds** (every web-recorded duration landed 1000× inflated: 106 whole-second rows, 78 of them > 1 year, max ≈ 352 years), stamped `stage_entered_at`/`transitioned_at` from the browser clock (skewed clocks planted future timestamps that later server-side moves subtracted against → 38 negative durations), and console-swallowed history-insert failures. These rows poison the pipeline metrics deck (avg velocity, per-stage duration averages in `metrics-service.ts`). Fix: web now calls the RPC (branch `fix/sweep0828-fweb-stage-transitions`); a one-time repair (÷1000 on the whole-second fingerprint rows, NULL the negatives — unrecoverable base) is staged at `OPS-Web docs/artifacts/web-bug-sweep-20260829-fweb/repair-stage-transition-durations.sql` and **must run only after the web fix deploys**.
...
#### Automatic inquiry creation boundary (2026-09-09)

**Status: released after explicit approval.** Ops-web `5b12d9823` removes the remaining platform/pattern/form bypass after a production Wix author-assignment notice created an empty client and lead. The customer alias `app.opsapp.co` independently resolved to deployment `dpl_Gv56toLk2MK1WCosoGYgS7sZeBVu`, READY at `2026-09-10T05:04:16.692Z` (September 9 in Edmonton). This release includes the runtime change developed as local commit `20208b8a9`; the published package omits customer-identifying audit details. Production records were not repaired or replayed. [Release evidence](docs/artifacts/email-inquiry-boundary-release.md).

`sync-engine.ts` now has one automatic client/opportunity creation path: unmatched inbound mail must receive an accepted classification with explicit `new_work` intent and a verbatim current-message request excerpt. `decideEmailWorkRouting` requires that evidence even for a first-time customer without a project; `createOpportunity` independently checks the excerpt before persistence. Sender domains, configured subject patterns, forwarder recognition, and parsed form identity cannot authorize creation. Outbound estimate-subject mail may link or remain review correspondence but creates no sales records. Existing ownership and authenticated external-intake correlation still link existing records before classification; explicit operator-approved import/manual creation is unchanged.

Unclassified parsed forms and message-scoped forwards retain full source bodies in Email Review. A deferred classification persists its audit first, then finalizes the readable correspondence receipt, preventing a retry from hiding an unpersisted audit behind a completed receipt. Exact-message replay creates no duplicate activity. Existing-job classifications continue through the released project/review RPC and produce no new client or lead.

`ai-sync-reviewer.ts` uses the effective customer sender for Stage A and feedback. Form/forward transport threads cannot supply other customers' full-thread Stage B context or exact-thread correction priors. Exact-message feedback remains authoritative. `reviewUnmatchedEmails` accepts restrictive `messageScopedSources` tuples from the sync engine, preserving verified recovery scope when reconstructed stored mail no longer has the original wrapper sender/authentication. Caller scope can only restrict inferred thread inheritance. Eligible project suggestions remain after actual accepted creation under the existing assignment, `phase_c`, and execution-policy guards.

The classifier's existing `phase_c` gate remains in force. With it disabled, automatic sync retains mail without bypassing classification to create a lead: ordinary unowned messages remain in the inbox, while recognized parsed forms/forwards remain in review. `inbox_ui` controls screen visibility only. No new database migration, endpoint, flag, or UI surface is introduced.

Verification: 320 tests passed again in 20 affected files after integration with then-current production main, including the actual sync engine with isolated provider/database doubles, genuine inquiry/replay, non-inquiry forms, exact Wix notice, existing-job routing, deferred source retention, and recovered form isolation. Scoped TypeScript passed; ESLint had zero errors and six existing console warnings. The earlier local full-repository TypeScript attempt exhausted the default Node heap; the Vercel production build subsequently compiled, passed full type validation, and deployed successfully. The signed-in Email Review page loaded after release in an existing account from another company, establishing page availability only. The read-only audited mailbox had no additional leads after the previously identified Wix record, and zero leads created after this release as of `2026-09-10T05:06:05Z`. Its last sync preceded deployment; there is no fresh inbound-message production canary yet. No paid model evaluation was run. Model accuracy remains separate from controlled fixture proof. Sources: ops-web `src/lib/api/services/{sync-engine,ai-sync-reviewer,lead-feedback-prior-service}.ts`, `src/lib/email/{email-work-routing,contact-form-lead-gate}.ts`, and `docs/artifacts/email-work-correspondence/inquiry-boundary-verification.md`.
...
Application correction released at 2026-09-14 20:17:52 UTC: ops-web `61cc787f9`, Vercel `dpl_5kZwgaDFAo6ZTTN3TjeAWaeji7LW`, independently verified on `app.opsapp.co`. The two-message recovery completed and replayed as a no-op. The separately approved cross-customer four-message recovery finalized at 21:21:34 UTC; its migration and live proof are recorded below.

The normal 20:44 UTC production worker completed both previously failing records at 20:44:54–55 UTC, clearing all component/error fields. Quote-only commercial evaluation did not change stage; the changed appointment evidence persisted as the correct participant-authority review outcome. This is live proof for those observed retry defects; it does not remove legitimate review requirements.
...
Applied with explicit approval: migration `20260914211750_finalize_exact_email_thread_recovery.sql` adds `private.email_thread_recovery_finalizations` and service-role-only `finalize_exact_email_thread_recovery_guarded`. The new RPC finalizes only the canonical thread link and inbox thread ownership after every exact-message recovery receipt is complete. It requires active actor permissions, a live mailbox lease owner, full current source/target/thread/link snapshots, the complete exact message set, and current attachment/inspection/materialization proof. It fences the seven dependent tables through commit and preserves both opportunity rows and message payloads. Both ordinary same-client reassignment definitions retain their exact pre-migration hashes. PostgreSQL 17 proof uses actual ownership triggers and attachment-state logic, with 23 assertions, 37 rejection cases, three concurrency checks, and two rollback readbacks.

Live recovery independently verified four original messages/events on the correct new lead, both corrected thread projections, four complete scan/recovery receipts, ten unchanged stored image copies (1,857,278 verified bytes), and ten completely unchanged older discovery rows. Five customer image inspections completed; five operator-sent copies have the expected operator_sent skip. The source completed job, its client/project/manual status/assignment/images and old thread are preserved; both summaries now reflect their respective histories. No extra client/project or duplicate target was created, and the mailbox was active and unlocked. Immediate finalization replay was a no-op. The private replay accounts for rotating Gmail download tokens; it uses existing stored files, canonical attribution/inspection, and the deployed filename-bearing attachment URL contract. The initial private URL check stopped safely, was reproduced with the real SQL fixture behavior, and was corrected with 19 passing tests before successful completion. Customer details remain outside public source.
...
**Direct inbound webhook (`POST /api/integrations/email-webhook`) — retired 2026-05-29; documentation corrected 2026-09-09:** The route returns HTTP 410 and does not create opportunities. Its former direct insertion bypassed lifecycle/source guards. Mailbox email flows through sync or explicit import; do not revive this raw-payload creation path. Source: ops-web `src/app/api/integrations/email-webhook/route.ts`.
...
- `phase_c` — gates automatic drafting, AI enrichment, and unmatched-email classification. Does not gate the inbox screen or explicit operator-approved lead import. The inquiry-boundary release removes automatic creation shortcuts that previously ran without this classifier.
...
**Automatic creation status (released 2026-09-09 local / 2026-09-10 UTC):** Ops-web `5b12d9823` removes the sender/pattern/form shortcuts previously present through `d86d5664b`. Every automatic new lead now requires classified current-message inquiry evidence, so `phase_c`-disabled mail remains in the inbox or form review. See “Automatic inquiry creation boundary” above. Mailbox synchronization and explicit operator-approved import remain independent of `inbox_ui`.
...
### Evidence normalization posture (prepared 2026-08-29, not deployed)

The `dir` repair recovered one message and no more. Re-measured live on 2026-08-28, `private.agent_provider_delivery_sources` still held **25 of 25 `text/html` sources rejected** since 08-20 (`text/plain`: 86 normalized, 4 rejected) — every HTML email reaching the agent boundary was still being replaced by `[CONTENT OMITTED: UNSAFE SOURCE]`. The remaining causes were guards that were right in intent and blunt in construction: they scanned declarations without their selectors, treated Outlook conditional markup as unreadable, and read a linked stylesheet as a live sheet. Bug `8db73af6`; repaired on OPS-Web branch `fix/bugsweep-email-20260828` (`src/lib/agent-control-plane/evidence/normalize-correspondence.ts`).

The boundary's job is unchanged: refuse any message whose reading it cannot prove matches what the recipient saw. What changed is which constructs it can prove.

| Construct | Treatment | Rationale |
|-----------|-----------|-----------|
| Outlook conditional markup (`<!--[if mso]> … <![endif]-->`) | No special handling; the HTML parse *is* the canonical reading | The canonical rendering is a non-Outlook client, and an HTML parser is that client: a downlevel-hidden block parses as a comment and drops out, and a downlevel-revealed block's content survives — exactly how Gmail renders each. The previous regex guard also deleted the visible half of the `<!--[if !mso]><!-->` pattern, which is the opposite of what the reader sees. |
| `<link rel="stylesheet">` | Node removed before analysis | Mail clients do not fetch external CSS and this DOM has no resource loader. A declaration that never arrives cannot conceal anything, so the node is dropped rather than treated as a live sheet. |
| Declaration scanning | Only declaration *bodies* are scanned; selectors are excluded first | `a:hover { … }` otherwise read as a declaration named `a`, which rejected essentially every styled email for a property nobody wrote. Unbalanced braces fall back to scanning the whole text, which can only reject more. |
| Rules that apply only on `:hover` / `:focus` / `:active` / `:visited` | Stripped from the sheet **before** any declaration guard runs | A rule that never applies to the static reading cannot conceal anything, but the guards judge declarations without knowing which selector carried them — so the rule has to be gone first, or the message is rejected for a style nobody ever renders. Evaluated per selector-list branch, so `.x:hover, .x { … }` still counts as static; `:not(:hover)` keeps its colon and `[title=":hover"]` keeps its static match, and both fail closed. |
| Style rules whose declarations are all visibility-neutral | Skippable regardless of selector resolvability | An explicit allowlist — borders, padding, list-style, table-layout, vertical-align, text-align, white-space, word-break and similar — none of which can hide, reveal, strike, or reorder text wherever they land. Everything in the concealment machinery is deliberately absent: colour and background, `display`/`visibility`/`opacity`, every sizing and offset property, `margin`, `text-indent`, the transform family, `filter`, `clip*`, `overflow`, `line-height`, the `font*` family, `text-decoration`, and `mso-hide`. Anything that *can* conceal must still name its elements in a selector the boundary can resolve. |
| `float` in a `<style>` rule | Still rejected | A rule-set float cannot be bound to its elements before the cascade runs, so the guard cannot prove the matched boxes carry no text of their own. |
| `float` in an inline `style` | Rejected only when the element carries text | A floated box leaves normal flow, so a floated box *with* text can render it ahead of content that precedes it in source order — the agent would read a different sentence than the recipient saw. A floated box with no text of its own (logos, spacers, image columns, the shape real mail uses) only reflows its neighbours. |
| `float: none` | Always passes | It restores normal flow; nothing can be reordered by it. |
| Client-prefixed mail boilerplate (`mso-line-height-rule`, `mso-table-lspace`, `mso-table-rspace`, `-ms-text-size-adjust`, `-webkit-text-size-adjust`, `-ms-interpolation-mode`) | Added to the supported declaration set | Present in essentially every commercially templated email; none can hide, reveal, strike, or reorder text. |

Float rejection is kept on purpose. Reordering the words a reader sees is a deception vector, not a cosmetic difference, and an agent that reads past a float is reading a different message than the recipient did.

The normalization revision advances to `ops.correspondence.normalized-text.v2`. Coverage: the existing `normalize-correspondence.test.ts` guard suite plus `src/lib/agent-control-plane/evidence/__tests__/normalize-real-mail.test.ts`, which reads real-world mail shapes end to end.

**Ledger contract for the repaired reading.** Migration `20260830113400_delivery_source_normalization_reprojection.sql` (written, not applied) makes the evidence ledger able to accept a repaired reading of bytes it already retains:

- The `normalization_revision` CHECK is widened from the single v1 literal to `{v1, v2}`. **It must be applied before any code carrying the v2 constant is deployed** — otherwise every delivery-source capture fails on a check violation, which is a full evidence-capture outage. The reverse order is safe: already-deployed code writing v1 stays valid.
- Source bytes (`content_value`) remain immutable forever, and the capture-time `source_sha256` is never rewritten — it is the tenant hash key `public.job_conversation_turns` references, so moving it would break immutable conversation turns.
- Only the derived text projection (`normalized_subject`, `normalized_plain_text`, `normalization_revision`, `normalization_status`) may be re-projected in place. `capture_agent_provider_delivery_source_as_system` stops treating projection drift as `agent_provider_delivery_source_idempotency_conflict` and updates those four columns instead; differing source bytes, subject, identities, media type, charset, selection revision, part ids, attachment descriptors, or evidence ids still conflict exactly as before.
...
### September 5 Maverick repairs — both repairs live; signed-in precision check complete

The approved scope-order, conversation, all-day task and read-input repairs are live on OPS-Web `2a349d8182e449af8891332f5b9636f3121df652`, with production migration `20260905191357_agent_maverick_read_repairs`. Authenticated reads pass and task detail agrees with project summary. The unchanged-title probe exposed lost timestamp precision and misleading no-change handling; the additional repair is live with production migration `20260905235844_agent_customer_update_source_precision` and web integration commit `e7c530015`. A fresh signed-in connector canary on 2026-09-06 copied the exact six-digit opportunity timestamp into `prepare_customer_update` and received the expected non-retryable `CUSTOMER_UPDATE_NO_CHANGE`. Independent database readback confirmed the title and timestamp remained exact, with zero customer-update proposals and zero Maverick conversation rows. Real changed-update approval/commit acceptance remains unclaimed. Exact release evidence, local verification and the separate unresolved B.C. timezone-data mismatch are recorded in [Maverick MCP repairs](specs/2026-09-05-maverick-mcp-read-repairs.md).

## Invisible Office Phase 13 exact customer-message follow-up

Candidate manifest v21 and exposure v15 add `prepare_customer_message` without changing active v14. The caller provides a job, exact job revision, exact inbound activity and provider-source digest, subject, body, and idempotency key. OPS derives the personal sender, one customer recipient, provider thread, internal thread, and reply message identity from current correspondence. No sender, recipient, CC, BCC, attachment, or outreach target is caller-controlled.

Approval remains inside OPS. The queue renders the exact sealed preview, accepts only its change-set UUID and SHA-256 digest, excludes the action from bulk and autonomous execution, and calls the established manual approved-email transport only after the named database approval. The existing durable intent, mailbox lease, provider acceptance, unknown-outcome quarantine, and reconciliation path remains authoritative. A new claim-time guard rechecks the entire message and rejects any newer correspondence. Receipts distinguish queued, attempted, provider accepted, reconciled sent, provider rejected, failed, cancelled-before-send, and unknown. Provider acceptance never populates `delivered_at`.

The implementation is locally complete but not yet production-applied or deployed. Consent v10 is deliberately absent, v15 is absent from the selectable exposure catalogue, and no client, grant, activation, host acceptance, or customer send exists. Canonical contract: [Phase 13](specs/2026-09-06-ops-mcp-customer-message-follow-up.md).
...
## Google Ads engine (LIVE on app.opsapp.co 2026-09-10; routine created but DISABLED)

Design: `specs/2026-09-08-google-ads-engine-design.md` §5–§7. Plan: `docs/plans/2026-09-08-google-ads-engine-p3-engine.md`. Source: `ops-web/src/lib/ads/engine/` (`handoff.ts` pure handlers, `handoff-runtime.ts` composition, `validate-proposal.ts`, `guardrails.ts`, `proposal-schemas.ts`, `brief.ts`, `apply.ts`, `worker.ts`, `pairs.ts`, `disapprovals.ts`, `copy-kinds.ts`, `admin.ts`, `repository.ts`), routes under `ops-web/src/app/api/internal/ads/engine/`, `ops-web/src/app/api/cron/ads-engine/`, `ops-web/src/app/api/admin/google-ads/engine/`. The routine that talks to these routes is `ops-web/docs/ads/engine-routine.md` (prompt `ads-routine-2026-09-14-v3`); the brief it receives is `ops-web/docs/ads/engine-brief-contract.md` (`ads-brief-2026-09-14-v2`). The routine never reaches Google; OPS applies after approval with `validateOnly` first.

**Rehearsal (2026-09-08, `ops-web/docs/artifacts/ads-engine/p3/local-e2e-2026-09-08/`).** Two full routine runs against a disposable database (real migrations, seeded phase 1 warehouse shapes) and the worktree dev server: claim → proposals → release, the server's `COPY_REJECTED` fix loop, the worker tick's `ADS PROPOSALS READY` notification and test scoring, the console at 1440×900, and approvals that reached Google's real account through `validateOnly` (refused with `RESOURCE_NOT_FOUND` on the fixture ids; the happy path needs phase 2's campaigns). It found and fixed: the editor subagent lacked the brand facts (prompt v2), the console fetched without the Firebase token (`authedFetch`), a decided card vanished with its verdict (kept on screen), and Google refusals read as raw JSON (one plain line, raw kept in `google_validation`). For phase 1 it found the daily sync failing on v25 (`campaign.start_date`/`end_date` unrecognized) and the entity snapshot missing `campaignSharedSet` rows; both were fixed on `feat/ads-engine-p1` on 2026-09-09 and proved against a disposable database with live Google reads (`ops-web/docs/artifacts/ads-engine/p1/v25-sync-fix-2026-09-09/`) — the sync now returns `synced` with 1678 entity rows. The account holds no `campaign_shared_set` attachments yet, so the first real attachment row waits on phase 2. The routine `OPS Google Ads engine` is `trig_01LroGoQJLg9GCPEAK3SD4dg`, disabled, connectors cleared.

**Shipped 2026-09-10.** PR [#120](https://github.com/jack-opsapp/ops-web/pull/120) merged phases 1 and 3 into `main` (`3bdc81e00`) and Vercel deployed it to production. Verified live by contract, not by dashboard: `/api/cron/ads-engine` answers 401 (was 404), `/api/cron/ads-briefing` answers 410 GONE, `/api/cron/ads-conversions` 401, `/api/internal/ads/engine/claim` 405 on GET and 401 on POST, and both admin engine routes 401 without a session. `ADS_ENGINE_TOKEN` is set in Vercel Production and equals `SOCIAL_AUTHORING_TOKEN`. **Nothing moves in the ad account**: the routine is disabled, and with no `engine`-labelled campaigns the worker tick has nothing to act on. CI could not vouch for this release — `lint-and-test` has failed on every run of `main` since 2026-09-05 in a step that precedes lint, type-check and the unit tests (see `docs/plans` and PR #121); the evidence is local instead: 433 tests, the engine SQL harness, and the rehearsal record.

**Shipped 2026-09-15 (PR [#123](https://github.com/jack-opsapp/ops-web/pull/123), merge `b5b0cb592`).** `feat/ads-engine-p2` — the phase 2 assets apply, the disapproved-ad guardrail, ad pair tests, blueprint copy kinds, and the asset mapper's type fix — merged into `main` on Jackson's go after an independent review (verdict: ship after the build fix, which was included). Proof before merge, on the tree merged with current `main`: `npx vitest run ads google-ads pmf analytics heavy-cron-schedule-isolation journal` 129 files / 1557 passed; `tsc --noEmit` only the 3 errors already on `main` (all in `__tests__`, skipped by `next build`); `NODE_OPTIONS=--max-old-space-size=8192 npm run build` passed (484 pages); Vercel's preview build of the branch passed. `lint-and-test` failed at `Test MCP v3 canary shutdown serialization`, the same pre-npm step that fails every `main` run (see Shipped 2026-09-10). Production deployment `dpl_138EdNzoYSP4peM1qg9Yk5H7rWdF` READY, target production, aliased to `app.opsapp.co`, 2026-09-15 00:01 UTC. Route contract unchanged and verified live: engine claim 401 (POST) / 405 (GET), `/api/cron/ads-engine` 401, setup `blueprint` and `enable` 401, admin engine health 401, `/api/cron/ads-sync` 401, `/api/cron/ads-briefing` 410. Read from Google at 00:02 UTC: all 5 engine campaigns `PAUSED`, 35 ads `APPROVED`. No cron schedule changed; the 14:59 UTC tick now runs the new guardrail, pair tracking and blueprint kinds, and with the campaigns paused it has nothing to act on. The routine stays disabled (prompt v3).
...
| Route | Contract |
|---|---|
| `POST /api/internal/ads/engine/claim` `{worker}` | Leases one run (40 minutes, `claim_ads_engine_run`) and builds the brief. `200 {run:{id, claim_token, lease_until, current_time, worker}, brief}` or `200 {run:null, reason}` with `reason` ∈ `engine_off` (every kind `off`), `run_active` (a lease is live), `idle`, `not_ready` (+`detail` `WAREHOUSE_UNAVAILABLE` or `NO_ENGINE_CAMPAIGNS`; the run is released with outcome `brief_unavailable`). An unexpected brief failure releases the run with outcome `error` and answers `500 BRIEF_FAILED`. The duties and brief version are checkpointed on the run. |
| `POST …/runs/{id}/proposals` `{claim_token, proposals:[…≤40]}` | 404 `RUN_NOT_FOUND`; 409 `CLAIM_NOT_OWNED` (token, lease or state); 422 `SCHEMA_INVALID` for a malformed envelope. Each item may carry its own `index` (default: position), and each index has a budget of three submissions (`record_ads_engine_submission`). Per item: `{index, accepted:true, id, kind, mode, target, replay?}` or `{index, accepted:false, code, issues:[{code, field, message, ref?}]}`. Codes: `SUBMISSIONS_EXHAUSTED`, `KIND_OFF`, and every validator code — `SCHEMA_INVALID, UNKNOWN_ENTITY, LEGACY_ENTITY, INSUFFICIENT_DATA, TERM_PRODUCED_TRIAL, TERM_NOT_IN_REPORT, BROAD_MATCH_REJECTED, THEME_MISMATCH, BUDGET_CAP, DAILY_CAP, COOLDOWN, CHANGE_TOO_LARGE, LADDER_NOT_MET, TEST_NOT_CONCLUDED, VERDICT_MISMATCH, NO_CONTROL_AD, URL_NOT_ALLOWED, STRUCTURAL_LIMIT, DUPLICATE_PROPOSAL, ALREADY_APPLIED, COPY_REJECTED` (the last carries the copy-rule issues, `ops-web/src/lib/ads/copy-rules.ts`). A duplicate of a proposal the same run already filed answers the original id with `replay:true`. Accepted proposals are stored `proposed` with `mode_at_submit` from `ads_engine_settings.modes`. HTTP 429 only when every item in the batch is exhausted. This endpoint has no import path into Google, the apply layer or the rail (proved by spies in `tests/unit/ads/engine/handoff.test.ts`). |
| `POST …/runs/{id}/release` `{claim_token, outcome:"done"|"nothing_to_do"|"error", summary?}` | Closes the run (`release_ads_engine_run`); the summary is the run's briefing. 409 when not owned. |

**Validation (server-side, deterministic, `validateProposal(proposal, ctx)`).** Zod strict per kind, then the spec's rules with every threshold read from `ads_engine_settings`: negatives only for terms in the 28-day search-term report that produced no trial; keyword pauses only at ≥30 clicks and 0 trials over ≥28 days or spend ≥3× `target_cost_per_trial`; no broad match; keywords must share a stem with their ad group's name or landing-page theme; challengers need an enabled `role-control` ad, no running test, no challenger already in place (`challengerInPlace`: an enabled `role-challenger` ad, or one the guardrail holds for a landing page; refused `TEST_NOT_CONCLUDED` "<group> already runs challenger ad N. A group tests one challenger at a time.", since 2026-09-14 — the validation context carries the guardrail's open episodes for it), the ad group's own landing URL, and a clean pass of the copy rules under the group's `copyKind` held to the page it lands on (paragraph "Copy kinds" below; `add_ad_group` under the kind the new group will have); promotions only for `challenger_won`/`no_verdict` tests; budget and CPC-cap changes ≤ `max_budget_change_pct`, ≥ `budget_cooldown_days` since the campaign's last money change, under `daily_cap` and the projected `monthly_cap`; bidding only to the next ladder rung on the trial-start trigger; at most `max_structural_per_run` structural proposals (`add_keywords`, `create_rsa_challenger`, `add_ad_group`); a normalised `target` key dedupes against open proposals; nothing under a `legacy`-labelled campaign. Accepted payloads are normalised from the snapshot (names, ids, control ad, budget resource), never from the routine's claims.

**Apply (`applyProposal`)** first refuses a `create_rsa_challenger` whose group has moved on since approval — its payload's control is no longer the group's enabled `role-control` ad, or the group already runs an enabled `role-challenger` — before anything reaches Google (`failed`, `google_validation {validateOnly:false, refused:"group_changed"}`, error "<group> already runs a challenger. Nothing changed in Google." / "<group> has a new control since you approved this. Nothing changed in Google."; 2026-09-14). It then turns the proposal into Google mutate operations with temporary ids wired parent→child, runs `validateOnly` first and re-sends only on a clean pass with `partialFailure:false`, records `ads_changes` (before/after, `measure_from = applied+1 day`, `measure_to = +14 days`), opens `ads_tests` for a challenger, and refreshes the entity snapshot. Labels `engine`, `gen-<run id>` (created in the same mutate when missing) and `role-*` go on ads, keywords and ad groups; budgets, bidding and shared negatives cannot carry a Google label, so the ledger holds the `gen-` label instead. `ADS_ENGINE_REHEARSAL=1` stops after validation (`google_validation` recorded, proposal stays `approved`). The Google door is `google-gateway.ts`, which resolves phase 1's `mutateGoogleAds`/`getServingCustomerIds` at call time and refuses with `ADS_CLIENT_MUTATE_UNAVAILABLE` until `feat/ads-engine-p1` is merged.

**Worker** — `GET /api/cron/ads-engine`, `59 14 * * *` UTC (the only free minute at hour 14; one minute before the routine claims), bearer `CRON_SECRET`, durable lease `ads-engine` (360 s), `maxDuration 300`. Per tick: `expire_ads_proposals`; apply approved proposals and `proposed` ones whose kind is still `auto` (never the human-only kinds `create_rsa_challenger, adjust_budget, adjust_cpc_cap, set_bidding_strategy, add_ad_group`); track ad pairs (paragraph "Ad pair tests" below: open a test for a live pair nobody is judging, cancel one that can no longer finish); conclude running tests by the two-proportion z-test on CTR (≥14 days, ≥2,000 impressions per arm, p<0.05, conversion veto when the control has ≥5 trial starts and the challenger 0 on ≥50% of its clicks; `no_verdict` after 56 days) and open the follow-up (`promote_challenger` in `propose` mode, never auto; `pause_ad` for the loser when the control wins) under a worker-authored run row (`worker='ops-worker'`); score `ads_changes` whose `measure_to` has passed (CTR delta ±10% → better/worse, else flat; `no_verdict` under 500 impressions on either side; account level for negatives, campaign level for money changes, ad-group level otherwise); guard against disapproved engine ads (next paragraph; live since 2026-09-15 — the 2026-09-10 build paused any enabled `DISAPPROVED` engine ad on the snapshot's word and could not restore); read `queryCampaignBudgetPacing` live and raise `ADS BUDGET PACING` when `search_budget_lost_impression_share > 0.3` three days running; `notify_ads_engine`; `check_ads_engine_stall(…, stall_hours, campaigns_live)` and clear the alarm once the heartbeat is fresh. Result `{expired, autoApplied, autoFailed, autoSkipped, testsOpened, testsCancelled, testsConcluded, followUps, verdicts, disapproved, held, restored, released, guardrail, pacing, notified, stalled, campaignsLive, rehearsal, google}` (`testsOpened`/`testsCancelled` since 2026-09-14; `held`/`restored`/`released`/`guardrail` since 2026-09-11; `disapproved` = ads the guardrail paused this tick; `guardrail` ∈ `idle|checked|unavailable`).

**Disapproved-ad guardrail (2026-09-11; LIVE 2026-09-15 via PR #123, merge `b5b0cb592` — commits `ce58cf7c5`, `62ac27e31`, `258159da2`; its migration is applied).** Rules in `ops-web/src/lib/ads/engine/disapprovals.ts` (pure), orchestration in `worker.ts` `guardDisapprovals`, live reads in `google-ads-client.ts` (`queryAdPolicyStates`, `queryAdGroupAdChangeEvents`), ledger `ads_guardrail_pauses` (`03_DATA_ARCHITECTURE.md`). The snapshot only nominates suspects (enabled, `DISAPPROVED`, in a non-legacy engine campaign); every decision is made on Google's live `ad_group_ad.status` and `policy_summary.approval_status`/`review_status`/`policy_topic_entries` (removed ads included), and each episode is recorded with its policy topics. **Hold:** a `DESTINATION_NOT_WORKING`-only verdict is held 20 h from first sighting, so the next daily check is the one that acts — a disapproved ad serves nothing, and that verdict is often one failed crawl (it stranded 22 good ads on 2026-09-10); any other verdict, a mixed one, or one with no topic is paused at once. **Pause:** validateOnly, then real, `partialFailure:false`, with the mutate window recorded. **Restore:** an ad the guardrail paused is switched back on (same write discipline) once Google's review is `REVIEWED` with `APPROVED`/`APPROVED_LIMITED`/`AREA_OF_INTEREST_ONLY`, and only if (1) its id is not in `config/ads/blueprint.json` `retire.adIds` — an unreadable blueprint restores nothing; (2) no `pause_ad` (payload `ad`), `promote_challenger` (payload `loser`) or `create_rsa_challenger` for its group is pending (wait) or was applied after the episode began (let go, `engine_decision`); (3) Google's `change_event` history shows the guardrail's own PAUSED commit, inside its recorded window ±60 s, as the last change to the ad. Every OPS actor shares one service account, so time — the UTC micros in the event's resource name — not identity separates them. Google refuses an open-ended range, a start older than 30 days and a `change_resource_name` filter (probe 2026-09-11), so the read is bounded (day before the earliest pause → tomorrow, Vancouver dates, ≤10,000 rows) and filtered in code. Unprovable (own commit missing, history cut short, read failed) → wait; older than the history → let go (`unverifiable`); removed or gone → `ad_gone`; switched on or changed by anyone else → `changed_elsewhere`; verdict cleared during the hold → `cleared_before_pause`. Closing an episode resolves every alert it raised (`resolve_ads_engine_alerts`). **Alerts** (copy through the ops-copywriter skill; `07_SPECIALIZED_FEATURES.md` §14): `AD DISAPPROVED` names Google's reason in plain words and says OPS turns the ad back on once Google approves it; it promises "a replacement for your review on its next run" only when the routine is active (heartbeat inside `stall_hours`, `create_rsa_challenger` not `off`) and `replacementDue` holds (a copy verdict, not a landing-page one; serving engine campaign and group; an enabled control other than the paused ad; no running test; no other challenger in place) — `computeDuties` names that group in the brief's creative duty by the same predicate (`<group> (Google disapproved an ad for <reason>; write its replacement)`), so promise and duty cannot disagree. A refused pause raises `AD DISAPPROVED` "… could not pause it (…) and tries again tomorrow"; a refused restore `AD STILL PAUSED`; restores report once per tick as `AD BACK ON`/`ADS BACK ON` (standard). The snapshot is refreshed once after any change; in rehearsal the guardrail validates and writes nothing; when Google cannot be read it waits (`guardrail: "unavailable"`). Record: `ops-web/docs/artifacts/ads-engine/guardrail-restore/README.md`. **Since 2026-09-14:** when the guardrail pauses the challenger of a running test for a non-landing-page verdict, it cancels that test in the same step (`stats.reason` "Google disapproved the challenger for <reason>.") before deciding the alert, so the replacement it promises is one the validator accepts; a landing-page pause, or a paused control, keeps the test running.

**Ad pair tests (2026-09-14; LIVE 2026-09-15 via PR #123, merge `b5b0cb592` — commits `4082cbe46`, `0dc872321`; no migration).** Spec §5.4 defines a test as one ad group, one control, one challenger, both enabled; before this, only an applied `create_rsa_challenger` opened one, so the phase 2 blueprint's pairs in all twelve engine groups had no `ads_tests` row, were never judged, and (28 days after launch) would have been named for a third ad that the validator and apply accepted — proved on production rows by the read-only dry runs in `ops-web/docs/artifacts/ads-engine/pair-tests/`. Rules in `ops-web/src/lib/ads/engine/pairs.ts` (pure), orchestration `worker.ts` `trackPairs` (after apply, before the verdicts). **Live pair:** a non-legacy engine campaign's ad group with exactly one enabled `role-control` and one enabled `role-challenger`, neither held paused by an open guardrail episode; two of either is no pair. **Open:** a live pair with no running test in its group and no test of its own with a verdict (`control_won`/`challenger_won`/`no_verdict` — that pair's fate is a proposal, never a second test) gets a row with `proposal_id` and `label` null and `started_at` = Vancouver midnight of the first account day both ads recorded impressions (`ads_daily_ad`, `impressions > 0`, on or after the day after the pair's last `cancelled` test when it has one), so the verdict window opens the next day like an applied challenger's. Nothing opens before a pair serves: a start before serving would count paused days toward the 56-day limit. The unique running-test index turns a lost race into "nothing opened". **Cancel** (`state` `cancelled`, `verdict_at` now, `stats` kept plus `reason`/`cancelled_at`, only while still `running`): an arm gone from the snapshot ("The challenger is no longer in the account."), an arm not enabled with no open guardrail episode — the blueprint retiring it, a hand edit ("The control/challenger was paused outside the test."), or the challenger held by the guardrail for its copy. A test whose ad group is missing from the snapshot is left alone. **One challenger at a time:** `challengerInPlace` (`disapprovals.ts`) is the single predicate behind the creative duty (a group with one is never named), the validator, `replacementDue`, and — snapshot-only — apply's last check. **Repository** (`repository.ts`): `listPairTests(adGroupIds)` (all states), `firstSharedServingDay(control, challenger, since)` (paged through every row), `openPairTest` (23505 → null), `cancelTest` (state-guarded). Proof: `tests/unit/ads/engine/pairs.test.ts`, `worker.test.ts`, `worker-guardrail.test.ts`, and the runtime harness `tests/sql/ads-engine-pair-tests-runtime.mts` (the real repository through supabase-js and PostgREST on a disposable PostgreSQL 17 cluster it creates and deletes). **Live state 2026-09-14:** 12 live pairs, 0 `ads_tests` rows, 0 served — the first tick after launch opens all twelve, dated to the launch day. Record: `ops-web/docs/artifacts/ads-engine/pair-tests/README.md`.

**Copy kinds (2026-09-14; LIVE 2026-09-15 via PR #123, merge `b5b0cb592` — commits `71e4372ca`, `fb75b80b4`; no migration).** Which copy rules an ad answers to used to come from the campaign **name**: `campaignKindOf` returned `brand`/`core`/`competitor` only for names starting `BRAND`/`CORE`/`COMPETITOR` (the old spec §4.2 names). Phase 2's `PRICING · US`, `SWITCH · US` and `TRADE · US` read as `other`, and the blueprint's per-group `copyKind` never reached the engine, so the validator refused the group's own approved competitor copy (`TRADEMARK_CAMPAIGN`) in all seven competitor-intent groups — proved on production rows, before and after, in `ops-web/docs/artifacts/ads-engine/copy-kinds/`. **Rule** (`ops-web/src/lib/ads/engine/copy-kinds.ts`, one resolver for the snapshot and the validator): a campaign's `kind` is the committed blueprint's for the campaign of that **exact** name (how the blueprint apply diffs the account); the `legacy` label wins; a campaign the blueprint does not declare is `other`, whatever its name. Every snapshot ad group carries `copyKind` (`brand|core|competitor`): the blueprint group's `copyKind`, else its campaign's kind, and `competitor` only when its ads land on a `/compare/` page (`isComparePage` in `copy-rules.ts`), else `core`. Nothing the blueprint did not declare is widened: a group the engine builds inherits its campaign's kind (a core campaign's new group stays core even on a compare page), a campaign with no kind (undeclared or legacy) gives its groups `core`, and an unreadable blueprint gives every non-legacy campaign `other` (a `legacy`-labelled one stays `legacy`) and every group `core`. **Validator:** `create_rsa_challenger` is judged under the group's `copyKind` held again to the challenger's landing page; `add_ad_group` under exactly the kind the new group will get once it exists (the same resolver with the blueprint in the validation context — `EngineValidationInputs.blueprint`); accepted payloads carry `copyKind` beside `campaignKind`. `TRADEMARK_CAMPAIGN` reads `"<Brand>" runs only in a competitor ad group that lands on a compare page. This group runs <kind> copy.` **Repository:** `createEngineRepository(client?, { loadBlueprint? })` (default: the committed `config/ads/blueprint.json`) is the one place the snapshot is built for the brief, handoff, worker, apply and console; `validationContext` reads the blueprint once and builds the snapshot with that same object. Legacy detection (`kind !== "legacy"` in the brief, worker, pairs, guardrail, admin, `dailyBudgetSum`) is unchanged. **Routine contract:** brief `ads-brief-2026-09-14-v2` carries `copyKind` on every snapshot ad group; prompt `ads-routine-2026-09-14-v3` names competitors only where `copyKind` is `competitor` (a new ad group: only in a competitor campaign, on a `/compare/` page) and passes `copy_kind` to its editor subagent. The disabled routine `trig_01LroGoQJLg9GCPEAK3SD4dg` carries v3 since 2026-09-14 22:43 UTC — safe against the build that preceded PR #123, whose brief had no `copyKind`. **Live state 2026-09-14 (read-only dry run):** kinds brand / competitor / competitor / core / core; the seven competitor-intent groups `competitor`, the other five brand or core; every group accepts its own approved copy (was 5 of 12), a planted competitor name is refused in all 5 groups that may not carry one, and a third ad is still refused in all 12. Record: `ops-web/docs/artifacts/ads-engine/copy-kinds/README.md`.

**Admin routes** (`withAdmin` + `requireAdmin`, `src/lib/ads/engine/admin.ts`): `GET /api/admin/google-ads/engine/proposals?state=proposed|…|all` → `{proposals, counts}`; `POST …/proposals/{id}` `{decision:"approve"|"reject", notes?}` → `review_ads_proposal` then, on approve, `applyProposal` synchronously inside a 25 s budget → `{proposal, outcome}` where `outcome.state` ∈ `applied|failed|validated|pending(reason google_unavailable|apply_timeout)|rejected` (a pending one is applied by the next worker tick); 409 `NOT_REVIEWABLE`, 404, 422. `GET|PATCH …/settings` (PATCH refuses `auto` for the human-only kinds: 422 `HUMAN_ONLY_KIND`; ranges validated). `GET …/tests` (both arms' first headlines joined from the snapshot), `GET …/changes` (90 days, with the proposal), `GET …/funnel` (`{rows, available}`; `available:false` until the phase 1 view exists), `GET …/health` (`last_run`, `recent_runs`, `next_due_at` = next 15:00 UTC, heartbeat, `stall`, `google`, `rehearsal`, `counts`, `modes`, `snapshot_at`).

**Console** (`/admin/google-ads`, `_components/engine/`): the proposals inbox (card per proposal — kind tag, OPS-authored summary, the routine's rationale in the agent palette, evidence table, a Google-style RSA preview with pins for challengers, APPROVE default / REJECT ghost with the shared reject dialog, one accent `APPROVE n` batch bar when several negative lists wait); once the account is live also the funnel by keyword (full-bleed table), tests (control vs challenger CTR bars, the verdict tag the only colour), the change ledger with outcomes, and engine health with per-kind mode segment controls and the caps. Hooks: `src/lib/hooks/use-ads-engine.ts`. Tests: `src/app/admin/google-ads/__tests__/engine-console.test.tsx`.

**Environment (production):** `ADS_ENGINE_TOKEN` (new, Jackson adds; ≥32 chars), `ADS_ENGINE_REHEARSAL` (unset in production; `1` only on a rehearsal stack), `CRON_SECRET`, `PMF_OPERATOR_USER_ID` / `PMF_OPERATOR_COMPANY_ID` (rail recipient), `TAVILY_API_KEY` (weekly market digest; optional), the Google Ads and Firebase admin credentials from phase 1.
...
| Route | Contract |
|---|---|
| `POST /api/internal/social/editorial/claim` `{worker}` | Returns `{assignment:null, reason:"idle"|"authoring_off"}` or one claimed assignment: `id, identity, kind, mode, attempt, attempts_remaining, claim_token, lease_until, brief_version ("ops-editorial-2026-09-08-v4"), current_time, source {id,title,slug,url,published_at,text,thumbnail_url}, recent_hooks (≤30), format {story_types, slides {min,max}, closing_slide}, limits, voice {path, sha256, content} (OPS copywriter brief, `docs/social/voice/ops-copywriter-brief.md`, the governing founder voice since 2026-09-08), guide {path, sha256, content}` (Sam Parr field guide, the pacing layer). Blog kind binds its own article (not live → `blocked/SOURCE_WITHDRAWN`, next row); recurring kinds pick the freshest live article unused by any non-blocked assignment, legacy run or post in 60 days (none → `blocked/NO_FRESH_SOURCE`). Up to three rows are tried per call. |
...
## Weekly journal authoring (2026-09-10; live in production since 2026-09-10; generated photographs live 2026-09-15; topic funnel live 2026-09-16)

Source: `ops-web/src/lib/journal/editorial/` (pure handlers `handoff.ts`, `policy.ts`, `article-html.ts`, `worker.ts`; composition `handoff-runtime.ts`, `runtime.ts`), branch `feat/journal-cloud-editorial`. Handoff routes under `ops-web/src/app/api/internal/journal/editorial/`: POST only, Node runtime, `cache-control: no-store`, bearer `JOURNAL_AUTHORING_TOKEN` (≥32 characters, constant-time; 503 `JOURNAL_AUTHORING_NOT_CONFIGURED`, 401 `JOURNAL_AUTHORING_INVALID`, 405 otherwise), 200 KB body limit (413). In the cloud the token is injected by an environment API credential for `app.opsapp.co` in the `OPS Journal` environment; the routine never sees it. Ledger: `03_DATA_ARCHITECTURE.md` § Weekly journal ledger. Feature: `07_SPECIALIZED_FEATURES.md` §21.

| Route | Contract |
|---|---|
| `POST /api/internal/journal/editorial/claim` `{worker}` | `{assignment:null, reason:"idle"\|"authoring_off"}` or the claimed slot: `id, identity, kind, mode, slot_date, publishes_at, attempt, attempts_remaining, claim_token, lease_until, current_time, brief_version ("ops-journal-2026-09-15-v2"), format, limits, max_sources, categories, industry_pages, recent_posts (≤40 live), backlog_topics (unused), recent_images (≤8: `{title, image_prompt}` of the newest live posts, so the writer varies its art direction), fetched_sources, brief, guide, product_facts` (each `{path, sha256, content}`: `docs/journal/voice/ops-journal-brief.md`, `blog-voice-sam-parr.md`, `ops-product-facts.md`). |
| `POST …/assignments/{id}/pitch` `{claim_token, pitch}` (topic funnel, live 2026-09-16, ops-web `2421f1358`) | Lease-checked like sources (404/409). OPS validates the pitch (`src/lib/journal/editorial/pitch.ts`): `topic`, `reader`, `why_now`, `signals` (0–12 radar ids), `chatter` (0–8 `{url https, shows}` from search), `ethos`, `angle`, `hook`, `headline`, `hooks_considered` (25–30 `{headline, hook, verdict, clicks 0–3}`: Sam Parr's 25 headlines with how many of three readers would click each), `runners_up` (2–4 `{topic, why_not}`). 422 `PITCH_REJECTED` `{code, issues}` with `code` ∈ `SCHEMA_INVALID, MARKUP_INVALID, TITLE_FORMAT, VOICE_REJECTED, STALE_FRAMING, DUPLICATE_TOPIC, PITCH_EVIDENCE` (≥1 radar signal whenever the radar holds any in 21 days; signals + chatter ≥3), `PITCH_SIGNAL_UNKNOWN` (not a radar signal published in the last 10 days), `PITCH_HOOKS` (<25 distinct headlines, or no reader clicks recorded); headlines and titles run 3–10 words, `PITCH_RUNNERS_UP`; rejections are logged `pitch_rejected`. 200 `{state:"pitched", lease_until, headline}` stores the pitch with each cited signal frozen as seen (`source` name, `kind`, `views`, `typical_views`, `momentum`, `replies`, `age_days`) plus `radar_scanned_at`; 429 `PITCH_LIMIT` after three. The claim (v3) adds `radar {scanned_at, spheres, feeds}`, `trend_signals` (≤150 from the last 7 days, six strongest per feed, ordered leadership → business → trades → industry → forum) and `pitch_limits`; `brief_version` becomes `ops-journal-2026-09-16-v3`. The draft route answers 422 `PITCH_MISSING` without this claim's pitch, copies the pitch into `package.pitch`, and the editor verdict adds `hooked` with reason `weak_hook`. |
| `POST …/assignments/{id}/sources` `{claim_token, url}` | OPS fetches the page through the shared guarded fetch (`fetchPublicResource` in `src/lib/social/public-media.ts`: public HTTPS only, DNS pinned, ≤3 redirects revalidated, ≤8 MB, 15 s; HTML via jsdom, PDF via pdfjs-dist, text) and keeps the snapshot. 200 `{source_id, existing, url, final_url, content_type, title, site_name, published_hint, modified_hint, fetched_at, truncated, text}`; 422 `SOURCE_*` with `status`; 429 `SOURCE_LIMIT`; 409 `CLAIM_NOT_OWNED`. |
| `POST …/assignments/{id}/draft` `{claim_token, candidate, editor, usage?}` | 404 / 409 as above; 422 `DRAFT_REJECTED` `{code, issues[{path,message}]}` with `code` ∈ `SCHEMA_INVALID, MARKUP_INVALID, TITLE_FORMAT, META_TITLE_LENGTH, CATEGORY_INVALID, TOPIC_INVALID, STRUCTURE, WORD_COUNT, FAQ_COUNT, FAQ_LENGTH, EMAIL_LENGTH, VOICE_REJECTED, STALE_FRAMING, SLUG_TAKEN, DUPLICATE_TOPIC, CITATION_INVALID, EVIDENCE_INVALID, LINK_REJECTED, INTERNAL_LINKS, WORKED_EXAMPLE_INVALID, UNSUPPORTED_NUMBER` (claim stays live; ≤3 submissions, then 429 `SUBMISSIONS_EXHAUSTED`); 200 `{state:"rejected"}` when the editor did not approve; 200 `{state:"drafted"}` otherwise, and an idempotent readback for the same token. Stores the validated package only; a boundary test proves no path to publication, storage or the rail. |
| `POST …/assignments/{id}/release` `{claim_token, outcome:"error"\|"unsupported", detail?}` | `error` → queued (+2 h; third attempt → `blocked/ATTEMPTS_EXHAUSTED`), `unsupported` → `blocked/UNSUPPORTED_TOPIC`. |
| `GET /api/cron/journal-editorial` | `9 * * * *`, bearer `CRON_SECRET`, durable lease `journal-editorial` (360 s), `maxDuration 300`. Recover → discover → promote ≤1 drafted slot (generate the header photograph from `package.article.image_prompt` + the fixed house style on OPS's `OPENAI_API_KEY` with `gpt-image-2.5-flare`, 1536 × 1024 high-quality JPEG, cropped by sharp to 1600 × 900, stored `blog/weekly/<slot>-<sha16>.jpg` on the product's blog image backend, require a public HEAD 200, schedule with `publish_at`) → fulfil at most one open photograph request when no draft was promoted this tick → publish due slots through the guarded RPC → newsletter lane (gated by `app_settings.blog_newsletter_enabled`) → notification outbox → stall check/resolve. Returns `{state, mode, created, missed, promoted, images:{replaced, failed}, published[], held[], newsletter, notified}`. Promotion failures record `IMAGE_FAILED` / `IMAGE_REFUSED` (moderation) / `IMAGE_NOT_AUTHORIZED` (401/403, e.g. an unverified OpenAI organization) / `IMAGE_NOT_CONFIGURED` / `IMAGE_INVALID` / `IMAGE_UNREADABLE`; the third blocks the slot with that code. |
| `GET /api/admin/journal/editorial` | Platform-admin read (settings, newsletter switch, newest six slots with rendered article, citations and the last six attempt events); no-store. Topic funnel (live 2026-09-16): adds `radar {scanned_at, ok, total, degraded}` and per slot `pitch` (the draft's `package.pitch`, or the live claim's pitch while `authoring`); claim tokens are never returned. |
| `POST /api/admin/journal/editorial/{id}` `{action}` | Platform admin; `stop`, `publish_now` (manual publish RPC; refusals return 409 with the code), `write_another` (409 `SLOT_PASSED`), `new_image` (opens a request through `request_journal_editorial_image` — 409 with `IMAGE_LIMIT` / `NOT_ELIGIBLE` / `NO_IMAGE_PROMPT` — then generates and swaps the photograph in the same request; 200 `{state, url}` or 502 `{code, retry}`, where `retry: true` means the hourly tick will try again; `maxDuration 300`), `send_test` (newsletter to the acting admin only, logged `blog_newsletter_test`). Actor recorded as `admin:<email>`. |

The existing Blog API (`/api/blog/posts`, `/api/blog/newsletter`, BLOG_API_KEY) is unchanged. `sendBlogNewsletter` now links `https://opsapp.co/journal/<slug>` (it previously pointed at the app's `/blog/<slug>`; no blog newsletter had ever been sent).
...
`inspect_financial_document` and `prepare_financial_document` are dormant v23/v17 definitions. Server-only inspect/prepare/commit/reject/filter and rate-limit RPCs share canonical actor identity; the OPS approval desk performs exact single-document saves with complete receipt readback. Active v20/v14/v9 and existing OAuth grants are unchanged. Consent v12 was absent at the Phase15 release; Phase16 adds its disabled, exact-trial path described below. See [Phase 15 contract](specs/2026-09-07-ops-mcp-financial-document-approval.md) for authority, arithmetic, locks, verification and release evidence and separate activation gates.

## Financial policy readiness (Phase 16, released and dormant)

Current host checkpoint (2026-09-08 18:56 UTC): Jackson approved the separate reviewed-effect installation and new 30-minute Codex/Claude test connections in MAVERICK. Native Codex completed fresh exact five-scope consent, source inspection, one CAD232.74 proposal and identical idempotent replay. Independent readback confirms one pending proposal and zero saved quote-project estimates or accounting queue rows. The exact Codex binding expires at 19:03:03 UTC; Claude is registered but its binding has not started. Public exposure and existing grants remain unchanged. Exact owner save approval and independent saved-document proof remain outstanding. The paragraphs below describing disabled activation are historical pre-trial boundaries; the final [Phase16 checkpoint](specs/2026-09-08-ops-mcp-financial-readiness.md#native-codex-financial-preparation--2026-09-08-1856-utc) records current IDs, hashes, authority and evidence.
...
Production repair (2026-09-08 17:29 UTC), web `e6fb07d7d6540db22c948fb1f8650684ca8e4af8`: `src/lib/agent-control-plane/contracts/financial-policy.ts` validates returned `preview.tax.id` with the existing canonical `PostgresUuidSchema`, not RFC-variant-strict `z.uuid()`. Existing valid database tax IDs must render without changing the tax row or narrowing PostgreSQL identity semantics. Owner/company/request identifiers and all source/hash/expiry/approval checks are unchanged. The real MAVERICK owner review now renders the exact default tax and proposed rules; 28 focused tests and full TypeScript validation pass. Rendering a review is not enrollment or financial host acceptance.

The released but disabled restricted v17/v12/v23 financial trial exposes only `get_company_context`, `inspect_financial_document` and `prepare_financial_document`, with exactly five scopes: `ops.company.read`, `ops.customers.read`, `ops.financial_documents.prepare`, `ops.financial_documents.read`, `ops.jobs.read`. Public active v20/v14/v9 and existing grants remain unchanged.

Service-only `provision_mcp_oauth_canary_as_system(uuid,uuid,uuid,text,text,timestamptz,uuid,text,text)` binds exact client/user/company, v17/v12, expiry (at most two hours), enrolled policy ID/hash and reviewed effect hash. The old six-argument v3 provisioner remains v3-only. No application route provisions financial bindings. Authorize preview/decision, code exchange, bearer, refresh, OAuth insert triggers and financial domain/approval/replay all recheck current state. Source/tax/owner/policy/effect drift, expiry and disabled bindings close access. `disable_mcp_oauth_canary_as_system(client,user,company)` disables the exact financial client and revokes grants/tokens; wrong-subject calls cannot disable it. Existing holds stay held. Both migrations and the application were approved and released on 2026-09-08. At 17:42 UTC Jackson separately enrolled MAVERICK's exact test policy through OPS; its consumed preview, policy/source hashes and preparation-only receipt were independently verified at 17:49 UTC. Effect resealing, financial bindings/grants and actual authenticated host trials remain unapproved and inactive. No financial document was created by enrollment. See [Phase16 contract](specs/2026-09-08-ops-mcp-financial-readiness.md).

## Phase17 catalog/inventory authoring and restricted trial (2026-09-09)

**Native Codex catalog acceptance passed, closed 05:16 UTC:** Jackson explicitly approved the one-hour MAVERICK-only catalog price/cost/stock connection and all fictional test-company actions, including the exact named OPS approval ceremony. Installed Codex app-server 0.153.4 completed real PKCE OAuth and all six native tools, with no model turn, paid API or service-minted token. Exact actor Pete Mitchell `8e811f98-9f2b-4f64-b409-ed56074b7dc8`, company `ddee107c-33cd-483e-8278-0f8d8a180181`, client `15668aa4-20b4-487a-be4a-f993433e99cc`, binding `a9cb33b1-4ba2-4a97-a801-16d9e69b16f0`, grant `a22c86f6-eb6d-4577-bffd-24046c3dc6fa`, expiry 05:21:50 UTC. Reviewed effect hash `sha256:d199bce2ce7e02bde227df57edf167b5963edbd18885d866410645a84a6a33e0`; all 29 trial/OAuth/catalog function definitions and 66 canonical guard definitions matched reviewed local evidence. The temporary client and binding were disabled at 05:14:09 UTC; independent 05:15 readback proves the grant revoked and all eight tokens inactive. The installed native Codex transport then returned `Auth required`; ordinary OPS company read still succeeded at 05:15:29. The owned probe process was stopped. No further trial authority remains.

Three real signed-in OPS approvals committed: seven-create catalog graph at 04:53:05 UTC (unit/category/family/service/two variants/recipe; change set `389546d6-5789-481e-8f3a-a05f5377ea2d`), separate zero-to-five black-pack stock count at 04:57:13 (`88edb927-d13d-4a39-a752-1cc075bc997f`), and two-variant price-only update at 05:08:16 (`c76c2ca1-2098-49f9-aa3d-367f7ceb9ad4`). All three identical native preparations replay as committed with the original action/change/seal. Independent readback confirms one stock audit row, three executed actions, three valid immutable receipt hashes, black CAD14/quantity5/cost7 and white CAD15/quantity0/cost7.50. Native catalog detail confirms both saved prices/quantities, units, options and the two-piece service recipe. The explicitly requested supplier-cost section is empty because these fictional variants have no supplier offers. Stock never resulted from a price edit; no purchases, accounting entries or provider changes were created. Stale stock and post-stock price fingerprints failed inspection without proposals. Other-company records, financial trials and Phase C flags were untouched. Public candidate activation and separate ChatGPT/Claude host acceptance are not claimed.

The native acceptance exposed a pre-existing queue request-isolation defect: concurrent routes used the module-global `setSupabaseOverride`, so a faster request's cleanup could make a slower authenticated request resolve the browser/anon client and show an empty queue. Seven deterministic route-interleaving tests reproduce that failure. The focused correction binds GET/POST/PATCH/DELETE/bulk queue service calls through existing request-local `runWithSupabase`; authentication, `agent.review`, current actor/company filtering, exact review and atomic commit remain unchanged. Release `c3e9f141d` restored the signed-in queue in production. No RLS policy or company automation flag is relaxed. The saved-review cleanup in `27ee6d8ad6ee2946a5b864976745abf39c8cf745` is READY on deployment `dpl_5KJ4AzicAygptVbc9TdkhmWFTsRJ` at `app.opsapp.co`. Actual signed-in history at 05:13 showed all three executed actions, readable Catalog labels, no raw sealed JSON and no saved-action approval controls. All 670 targeted tests across 25 files, expanded TypeScript, two installed Next client-boundary checks and the production build passed; independent Astra code review found no issues. Full redacted evidence and reproducible native probe are in web `docs/artifacts/phase17/native-acceptance-20260909.json` and `native-codex-catalog-probe.mjs`.

Manifest `2026-09-08.capability-manifest.v24`, exposure `2026-09-08.mcp-exposure.v19`, consent `2026-09-08.mcp-consent-catalog.v14`, schema `2026-09-08.v1` add `inspect_catalog_changes`, `prepare_catalog_changes`, and `prepare_inventory_adjustment`. The full candidate remains unselected. The approved release recognizes only a six-tool subject-bound trial: those three capabilities plus `get_company_context`, `search_catalog_items`, and `get_catalog_item`. Public exposure V14/consent V9 and existing clients/grants remain unchanged. Commit capabilities are internal registry entries and never MCP tools.

All hosts use the same domain service. Requests contain operation (`catalog` or `inventory`), CAD/USD company currency, source `{key,sha256,name,kind}`, 1–100 typed rows, explicit skipped rows with reasons, and an idempotency key. Each row carries stable row/source identity, entity, nullable existing UUID plus current fingerprint, and exact supported values. `row:<key>` references link records within one proposal. Money is a bounded two-decimal string. Missing information, identity collisions, incompatible units, unsupported recipe forms and unresolved references return `needs_input`; no partial subset is silently saved. File content and operator statements are untrusted business evidence. The source digest is supplied by the host, not a claim that OPS independently fetched or verified the source file bytes.

Base authority is `agent.review`, `catalog.view`, `ops.catalog.read`, and `ops.catalog.prepare`. Catalog changes require `catalog.manage`; file imports additionally require `catalog.import`; billable products and recipes require `catalog.products.manage/view`. Explicit price/minimum-charge changes require `ops.catalog_prices.write`. Explicit cost changes require `finances.view`, `ops.catalog_costs.read`, and `ops.catalog_costs.write`. Stock requires separate `catalog.stock.adjust` and `ops.inventory.adjust`. Cost values are excluded from ordinary price-only previews. Candidate consent labels are separate from Phase16 OAuth functions and are not registration authority.

System RPCs: `inspect_catalog_changes_as_system(context,request,request_id)` and `prepare_catalog_changes_as_system(...)`. Named review RPC: `commit_catalog_changes_as_actor(actor,company,action,change_set,preview_sha256,idempotency_key)`. Rejection uses `reject_catalog_changes_as_actor(actor,company,action)`; queue reads use `filter_catalog_actions_as_actor`. Public runtime execution of those mutations is service-role-only, with current actor/grant checks repeated inside the transaction. The durable candidate limiter provides actor/grant/company preparation quotas using the existing bucket infrastructure.

Release authorization: Jackson approved application/database deployment and explicitly authorized all MAVERICK test-company actions. Each save still requires the exact OPS review ceremony, which Codex may perform for these fictional fixtures under that authority. Existing Phase16 actors, grants, trial policies, seals and synthetic records remain untouched. Native Codex transport acceptance is not separate ChatGPT or Claude host acceptance.

`private.agent_catalog_trial_bindings` binds one immutable new OAuth client to an exact actor/company, reviewed catalog effect hash and at most two hours of validity. Identity, expiry and effect cannot change; disable is irreversible. Service-only `provision_catalog_oauth_trial_as_system(client,actor,company,expiry,reviewed_effect_sha256)` requires a fresh client, exact seven-scope ceiling and current company-wide permissions. The ceiling is `ops.catalog.prepare`, `ops.catalog.read`, `ops.catalog_costs.read`, `ops.catalog_costs.write`, `ops.catalog_prices.write`, `ops.company.read`, `ops.inventory.adjust`. Consent, code/grant/token insertion, bearer, refresh, domain prepare, named OPS approval and replay recheck current binding. During this restricted rollout, internal/API channels cannot bypass the original sealed MCP context. The table has forced RLS and no direct application privileges.

Approved production migrations applied on 2026-09-09: ledger `20260909021307_agent_catalog_authoring` (source `20260908221635`, SHA256 `0d9b3d9fe3864489d67ef668cecc5f3daf709d520b224bf605ff9812367d9efd`), `20260909021401_catalog_trial_oauth` (source `20260909015000`, SHA256 `525a23551e3f8130883c71d43d0a95b1c865e3326e1ac682566012fb1aa38d25`) and `20260909023431_catalog_trial_review_visibility` (source `20260909023500`, SHA256 `c2df0f2fdd62f5ea74784e728a25454f9c161f1ff50de604c1e7bf704b606cf5`). Independent post-migration readback: zero proposals/bindings/effect seals/trial clients; MAVERICK remains 35 families, 67 variants and 20 products. Web `1060d7deef5cab44df5f17bed4b82fa7264cebb1` is READY at `app.opsapp.co`; the initial `7b08d6571` build failed on a misplaced client directive and never replaced production. Two installed Next compiler regressions now pass. Final review-access release `73d2831934cb33012d7526e2923b77fc2ee505db` is READY at `app.opsapp.co` on deployment `dpl_9zpG3hV9RjUvXhUXehjqpLkCCcmY`. Public discovery remains the same 21 scopes; anonymous MCP access is HTTP401. Local integrated proof: 639 catalog/OAuth tests, 21 review-access/actor-binding tests, expanded TypeScript check, 50 additional PostgreSQL assertions, and real PostgREST16.2 HTTP503/PGRST001 disconnect rollback followed by same-key save/replay and immutable receipt readback.

Actual signed-in MAVERICK review initially returned an in-place 404 because Phase C gates `/agent`. Do not enable that company flag: it also enrolls scheduling/email automation. Service-only read `can_review_catalog_trial_as_actor(actor,company)` derives one boolean from an exact current consented catalog binding/grant. The authenticated feature-flags API exposes `mcp_catalog_review` with no routes/permissions and `Cache-Control: no-store`; the route store permits only exact `/agent/queue` under that positive hint. Existing `agent.review` RBAC, named queue filtering and every domain save/replay check remain independent. Adjacent automation routes, absent/failed/stale consent and other subjects remain closed. This is screen visibility, not save authority.

**Historical activation-blocked checkpoint, superseded by the approved native trial above:** Activation was blocked by tool safety review, which requires direct approval of the precise temporary catalog price/cost/stock access. No workaround was attempted. Independent readback proves zero catalog effect approvals, trial bindings, trial clients or proposals. The reviewed current effect hash was `sha256:5fc45fb89e47203d6fe9706cc9b17038fee2dd505bfa934f0f644c0252bd0eba`; all 66 captured canonical constraints and trigger-plus-function definitions remained byte-identical. This hash is evidence, not an installed approval; recheck it before any separately authorized trial. Native authoring, live review of a catalog proposal, production save and receipt acceptance remain unclaimed. The preflight paragraph below is a historical pre-migration checkpoint.

Catalog SQLSTATE `55P03` (lock contention), `57014` (statement cancellation) and `25P04` (transaction timeout/session termination), plus supported pool-disconnect codes, map to retryable unavailability. Production fault acceptance on 2026-09-09: an owned 35-second row lock on only fictional WHITE variant `f7b89e0f-eca6-db40-e1b0-cf05b5763945` overlapped the actual signed-in two-row price approval. At 05:07:14 the deployed PATCH returned HTTP500 and logged `The catalog is busy. Retry this approval shortly.` The UI restored the same approval. Independent 05:08:03 readback proved both prices unchanged, pending action and no receipt. After the owned lock released, that exact action/change/seal/key succeeded at 05:08:16; immutable receipt hash `sha256:96dbf110830d0b918d812c948a69964de19fb2154e183b6e6238323e04af41c4` independently validates. This proves deployed rollback and same-request retry through the actual application/PostgREST path, not direct-SQL business mutation. The underlying raw production PostgREST code was not exposed by the application log; the explicit HTTP503/PGRST001 observation remains local PostgREST16.2 evidence only.

Native Codex preflight (2026-09-09 01:23–01:25 UTC): Jackson authorized the ChatGPT/Codex test. Four actual authenticated OPS MCP reads confirmed MAVERICK company `ddee107c-33cd-483e-8278-0f8d8a180181`, two catalog pages with six distinct variants and stable snapshot pagination, and one exact variant's selling price, counting unit, recipe and physical-stock relationships. No supplier-cost section was requested. This proves the existing Codex read path only, not Phase17 authoring or ChatGPT/Claude acceptance. The three candidate authoring tools were absent from the native tool inventory, and a read-only production `pg_proc` query returned zero Phase17 inspect/prepare/commit/authority/deadline functions. Full catalog setup testing remains blocked on approved application/database release and a scoped MAVERICK trial. No production schema, business record, grant, binding or activation was changed. Redacted evidence is in web commit `37dcf515e`, `docs/artifacts/phase17/native-codex-preflight-20260909.json`; cursors and credentials are not retained.

## Phase 19 site-visit candidate API (2026-09-10; deployed, disabled)

**Restricted-trial update, 2026-09-14:** web source `75b9faf46` adds frozen V22/V17 to code-owned selection for an exact current trial only; public registration remains V23/V9 with its existing scope catalogue. Subject selection, refresh and bearer validation recheck the exact client/actor/company binding. Service-only `provision_site_visit_oauth_trial_as_system(client, actor, company, expires_at, reviewed_workflow_effect_sha256, reviewed_authorization_sha256)` requires a fresh pinned client and pre-existing company compatibility/effect authority. Exact retries return the original binding; changed, expired or disabled requests cannot renew it. The existing disable RPC revokes only that bound subject's client, grants and token families. No existing grant is repinned. The database lifecycle is installed under `20260914203418`; the scoped production app build is tracked in the [trial release evidence](docs/artifacts/2026-09-14-site-visit-trial-release.md). Native site-visit host acceptance and company activation are not claimed.

**Release update, 2026-09-12 UTC / September 11 local:** web source `fd8591a9bccac366656223ba3a84454856821bba` is production-live at `app.opsapp.co`, READY deployment `dpl_4WmqWxpmkKnT7Sn2d6Ls1swrdzdj`. The six database migrations are installed. Fresh exact-release verification passed 721 tests and focused production/protocol TypeScript; the hosted production build also passed. Anonymous MCP access still returns HTTP401/no-store with the unchanged 21-scope public catalogue; all three site-visit scope additions remain absent. A real signed-in Codex company-context read succeeds after release. That connection lacks schedule/site-visit scopes, so it cannot establish site-visit read or preparation acceptance; its grants were not expanded. Public V23/V9 and historical pins remain intact, and V22/V17 remain unselectable. [Exact release evidence](docs/artifacts/2026-09-12-site-visits-release.md).

The trusted site-visit workflow service exposes eleven bounded discovery/preparation operations through candidate manifest v27, exposure v22 and consent v17, all dated 2026-09-10. Existing active/default/selectable exposure remains unchanged. The service accepts validated OPS actor contexts and uses the same contracts for human-connected hosts and durable agents. New host tools do not expose raw CRUD, commits, capture start or capture complete.
...
Canonical source: [Phase 19 implementation](specs/2026-09-10-ops-mcp-site-visits-p19-implementation.md), web `contracts/site-visit-workflow.ts`, `services/site-visit-workflow/site-visit-workflow-service.ts`, and the six applied phase migrations mapped in chapter 03. Local MCP protocol fixtures and a successful unrelated company read do not establish site-visit native-host acceptance or activation.
...
## Catalogue recipe read and exposure V24 (2026-09-15; live in production 2026-09-16)

`2026-09-15.mcp-exposure.v24` is the full V23 successor, and it is larger than V23 in both directions. Tools: V23's ordered 35 plus five catalogue-setup prepare tools, appended in this order — `prepare_create_catalog_variant`, `prepare_set_variant_thresholds`, `prepare_set_catalog_pricing`, `prepare_set_supplier_cost`, `prepare_create_catalog_option` — for 40. Grantable scopes: V23's 21 plus `ops.catalog.prepare`, for 22. V24 binds to capability manifest `2026-09-15.capability-manifest.v28` (the v20 customer-update manifest reminted plus the catalogue-setup write family) and consent catalogue `2026-09-15.mcp-consent-catalog.v18` (the v9 label set plus one label, every other label byte-identical). V14 and V23 pins stay immutable on manifest v20 and consent v9 and keep resolving. Two things therefore change against V23: `get_catalog_item` reads the richer catalogue recipe projection documented below, and the catalogue setup writes exist at all — see the next section. V24 is the active revision (`ACTIVE_MCP_EXPOSURE_REVISION`), so new registrations select it. V24 also selects deck geometry result v2, exactly as V23 does.

`private.agent_p2_catalog_detail_v1` now takes `p_recipe_shape` (`'v1'` or `'v2'`) plus seven pinned shape bounds. The shape is chosen in `mcp/server-factory.ts` from the caller's exposure revision and flows through `DomainCallOptions.catalogRecipeShape` → `getCatalogItem` → the repository's RPC argument. It is never accepted from a tool argument; an unknown shape raises `invalid_agent_catalog_detail_request`.

| Shape | Recipe projection |
| --- | --- |
| v1 | Byte-identical to the pre-2026-09-15 projection: `product_ref`, `product_label`, `relationship`, `variant_ref`, `quantity_milliunits`, `unit`. Every pin through V23 reads this. |
| v2 | Adds `material_ref`, `family_ref`, `variant_selector`, `quantity_per_unit`, `quantity_basis` (`per_product_unit` / `per_option_count`) and `scaled_by` `{option_ref, option_name}` per line, plus a `recipe_products[]` section listing each referencing product's own options (`option_ref`, `name`, `kind`, `required`, `affects_recipe`, `default_value`, `values[]`). Only V24 reads this. |

`variant_selector` is an ordered array of `{catalog_option_label, value_expression}` entries, not an object keyed by option name. Catalogue option names are untrusted business data; an option called "Notes" or "Cost" would otherwise be read as a forbidden or cost-scoped field by `assertNoCatalogForbiddenFields`. Entries are canonically ordered by label bytes, bounded at 32 per line, and each side is sanitized through the same canonical-text discipline as other business text.

`quantity_per_unit` is a four-decimal string beside the existing three-decimal `quantity_milliunits`. Three of Canpro's twenty recipe lines are authored at four decimals (`0.0526`, `0.1667`); the milliunit integer cannot carry them, returns null, and makes shape v1 refuse the whole family with `agent_catalog_source_data_invalid`. Shape v2 states the quantity and leaves `quantity_milliunits` null, so those families become readable. A quantity finer than four decimals still fails the source check rather than being rounded into a wrong number.

Migration `20260915223000_agent_catalog_recipe_read_v24.sql` pins the base definition by md5 and sha256 before replacing it, drops the pre-shape overloads so PostgREST cannot resolve a stale projection, re-applies the exact service-role grants, and refuses to run while any unexpired catalogue, financial or site-visit trial binding is enabled. It then teaches `resolve_mcp_oauth_access_token_as_system`, `assert_agent_customer_update_authority`, `consume_agent_customer_update_prepare_rate_limit_as_system` and `prepare_agent_customer_update_for_grant_as_system` to accept V24 beside V23 by exact-anchor replacement; every V23 branch keeps its shape. No client, grant, consent row or effect-policy seal is created.

**Local proof:** applied inside one rolled-back transaction against a copy of production carrying Canpro's real catalogue. Shape v1 is byte-identical on three families; the EPL line scaled by "Left ends" reads `per_option_count` with its option named; Lag Screws' four lines come back in deterministic order with three different scaling options; `recipe_products` lists the product's nine options; an unknown shape raises; a V23 bearer still resolves under active revision V24 and a new V24 bearer resolves too. Transcript: web `docs/artifacts/mcp-recipe-read/local-proof.md`.

The five catalogue setup write tools ship in this same exposure revision, so an external client sees one consent change rather than two; they are documented in the next section.

## Catalogue setup write tools (2026-09-16; application and migrations live in production, override-level release applied, effect seal resealed)

**Release status (2026-09-16).** Branch `feat/recipe-mcp-read` merged to `main` as PR #127 (`https://github.com/jack-opsapp/ops-web/pull/127`), merge commit `8d0603b6a`, merged 19:58 UTC. Release runbook: web `docs/artifacts/mcp-catalog-setup-writes/release-runbook.md`. All eight migrations are applied to production (Supabase project `ijeekuhbatykdomumfjx`) and verified by object fingerprint — **47 of 47 expected function md5s match the runbook table**. Production ledger versions, in apply order (2026-09-16, roughly 18:00–18:45 UTC): `20260916165439 agent_catalog_recipe_read_v24`, `20260916172541 agent_catalog_setup_write_variant`, `20260916173055 agent_catalog_setup_write_thresholds`, then `agent_catalog_setup_write_pricing`, `agent_catalog_setup_write_supplier_cost`, `agent_catalog_setup_write_option`, `20260916182941 repair_double_encoded_catalog_text` and `agent_catalog_setup_write_money_precision`. Exposure V24 is live in the database: 40 tools, 22 scopes (including `ops.catalog.prepare`), consent catalogue v18, capability manifest v28. The effect seal was **activated on 2026-09-16 after the production code deploy reached READY** — the row `('2026-09-15.catalog-setup-write.v1', private.agent_catalog_setup_write_effect_revision())` was inserted into `private.agent_catalog_effect_policy`, so the five prepares are live rather than inert. At activation no proposal had been staged (`private.agent_catalog_setup_writes` and `private.agent_catalog_setup_writer_requests` were both empty); the first ones are the end-to-end production proof below. TypeScript on the merged tree shows only the six pre-existing test-file errors. The text-repair migration's production finding is recorded in its own section below and differs from the local rehearsal.

**Follow-up release (2026-09-16).** Two more migrations are applied to production. PR #128 (`https://github.com/jack-opsapp/ops-web/pull/128`, merge commit `5050e127d`) applied `20260916080000_align_variant_unit_cost_override_to_default_profile.sql` as ledger version `20260916203707` — see **The mirror drifted on day one** below. PR #130 (`https://github.com/jack-opsapp/ops-web/pull/130`, merge commit `f2f81fbd1`) applied `20260916090000_agent_catalog_setup_write_override_level.sql` (md5 `e04da739127844127c417c3515eb3c93`) as ledger version `20260916233332`, and **43 of 43 expected function md5s match** the runbook's follow-up section F — see **The override level rule** below. That migration replaces nine functions the effect seal hashes and adds three, so it moved the effect revision on purpose: from the moment it committed until the reseal, all five prepares refused every request with `CATALOG_SETUP_WRITE_ACTIVATION_REQUIRED` and every commit would have refused with `CATALOG_SETUP_WRITE_EFFECT_POLICY_CHANGED`, by design. The seal row `2026-09-15.catalog-setup-write.v1` was **resealed after the production deploy on 2026-09-16** — reissued once, after the code that reads the new preview shape reached READY — so the five prepares are live again.

Five `prepare_*` tools give the MCP surface the catalogue writes Canpro's setup needed — every one of which had been done by hand-written SQL. None of them writes anything. Each stages a proposal for a named OPS operator, returns `status: "approval_required"` with a sealed preview, and the write happens only when that operator approves the exact preview inside OPS. The whole vertical is dormant on merge: every prepare raises `CATALOG_SETUP_WRITE_ACTIVATION_REQUIRED` until an operator seeds a reviewed effect hash, and no migration seeds one — that seed was made by hand on 2026-09-16 after the deploy (release status above). Source plan: web `docs/plans/2026-09-15-mcp-catalog-setup-writes.md` (decisions W1–W10).

| Tool | What it prepares | Kind-specific refusals |
| --- | --- | --- |
| `prepare_create_catalog_variant` | One new variant on an existing family: an answer for every live option, an optional SKU, thresholds in whole units, a price override, and an optional opening quantity recorded as a stock unit plus a `receive` stock event. A supplied price or threshold equal to what the family (or, for thresholds, the category) already supplies is not written — the new variant inherits it. The preview reports the new variant's sale price, unit cost and both levels as value plus origin. | `CATALOG_SETUP_VARIANT_EXISTS` (the value set already exists — application-layer uniqueness; the database has no such constraint), `CATALOG_SETUP_PRICE_REQUIRED` (family has no `default_price`), `CATALOG_SETUP_OPTION_COVERAGE_INVALID`, `CATALOG_SETUP_FAMILY_HAS_NO_OPTIONS` |
| `prepare_set_variant_thresholds` | The warning and critical low-stock levels on one existing variant, whole units in and out, reporting the level in force with its origin — `variant`, `family`, `category` or `none`. A level equal to the one the variant inherits leaves it inheriting (an override it carried is cleared); each level is judged independently. An explicit null clears the variant's own level; an absent key leaves it. | `CATALOG_SETUP_THRESHOLDS_INVALID` (critical above warning, checked on the levels in force), `CATALOG_SETUP_THRESHOLDS_NOT_WHOLE`, `CATALOG_SETUP_NO_CHANGE`, `CATALOG_SETUP_VARIANT_NOT_FOUND` |
| `prepare_set_catalog_pricing` | The selling price of one family or one variant, as a decimal string of at most four fraction digits in the company's own currency. A family ref writes the family default only and lists every variant override that shadows it (`shadowing_variants`, each flagged `redundant` when it equals the new default — informational, never cleared). A variant ref writes the variant's own price only when it differs from the family default; an equal price leaves the variant inheriting. An explicit null clears at the level the ref names; clearing a family default lists every variant left with no price. Sale price only. | `CATALOG_SETUP_CURRENCY_INVALID`, `CATALOG_SETUP_NO_CHANGE`, `CATALOG_SETUP_AFFECTED_VARIANTS_TOO_MANY` (bounded at 128, refused rather than truncated), `CATALOG_SETUP_WRITE_AUTHORITY_DENIED` (missing `catalog.run_setup`) |
| `prepare_set_supplier_cost` | One supplier cost profile on one variant, keyed by `profile_key`: `label`, `unit_cost`, `is_default`, `activation_rule`, `source`. Promoting demotes the current default in the same write; a removed key is revived rather than re-inserted; the preview lists every profile the variant has, on both sides, and the variant's catalogue cost as `variant_unit_cost` `{amount, origin}`. | `CATALOG_SETUP_DEFAULT_REQUIRED` (a variant carrying profiles keeps exactly one default), `CATALOG_SETUP_CURRENCY_INVALID`, `CATALOG_SETUP_NO_CHANGE`, `CATALOG_SETUP_PROFILES_TOO_MANY`, `CATALOG_SETUP_PROFILE_PROVENANCE_MISSING` |
| `prepare_create_catalog_option` | One new option (a dimension such as `Height`) on an existing family with its values, plus `value_for_existing_variants`, which backfills every variant already on file in the same write. `sort_order` defaults to the family's highest plus ten. | `CATALOG_SETUP_OPTION_EXISTS` (case-insensitive, trimmed), `CATALOG_SETUP_OPTION_VALUES_DUPLICATE`, `CATALOG_SETUP_BACKFILL_VALUE_INVALID`, `CATALOG_SETUP_VARIANT_SET_AMBIGUOUS`, `CATALOG_SETUP_WRITE_INPUT_INVALID` (the backfill value is required when the family has variants and refused when it has none) |

Shared refusals across all five: `CATALOG_SETUP_WRITE_ACTIVATION_REQUIRED`, `CATALOG_SETUP_WRITE_AUTHORITY_DENIED`, `CATALOG_SETUP_WRITE_AUTHORITY_REVISION_INVALID` (a V14 or V23 pin can never prepare a catalogue write, whatever scopes it holds), `CATALOG_SETUP_FAMILY_NOT_FOUND`, `CATALOG_SETUP_SOURCE_UNSAFE_TEXT`, `CATALOG_SETUP_PREVIEW_TOO_LARGE`, `CATALOG_SETUP_WRITE_IDEMPOTENCY_CONFLICT`, and at commit `CATALOG_SETUP_WRITE_CONFIRMATION_STALE`, `CATALOG_SETUP_SOURCE_STALE`, `CATALOG_SETUP_SAVE_BLOCKED`, `CATALOG_SETUP_OVERRIDE_LEVEL_CHANGED`, `CATALOG_SETUP_READBACK_MISMATCH`, `CATALOG_SETUP_WRITE_EFFECT_POLICY_CHANGED`.

**Authority.** All five need `ops.catalog.read` + `ops.catalog.prepare` and the operator permissions `agent.review`, `catalog.manage`, `catalog.products.view`, `catalog.view`. A request carrying an opening quantity additionally needs `catalog.stock.adjust`. `prepare_set_catalog_pricing` and `prepare_set_supplier_cost` additionally need `catalog.run_setup`, because `catalog_items.default_price` and `catalog_supplier_cost_profiles` are written nowhere in OPS except the catalogue setup wizard, whose route gates on that key, and the supplier-cost row policy names it itself — a `SECURITY DEFINER` writer reached through MCP must not become a way around the authority guarding the field it writes. `prepare_set_supplier_cost` also requires the `ops.catalog_costs.read` scope and the `finances.view` permission, unconditionally rather than behind an input selector a caller could avoid. `prepare_create_catalog_option` asks for the shared base and nothing more: the three tables it writes carry company-isolation policies only, and a postflight refuses to install if that reasoning is ever quietly widened. The consent label for the one new scope, verbatim in consent catalogue v18: *"Prepare exact catalog changes for named operator approval in OPS; never change stock or prices without that approval"* — deliberately distinct from the dark V19 trial's catalogue label. Capability manifest is `2026-09-15.capability-manifest.v28`; the five prepares and the shared `commit_catalog_setup_write` are minted there beside the reminted v20 customer-update entries.

**One approval surface.** Five tools, one proposal table (`private.agent_catalog_setup_writes`, chapter 03), one `agent_actions.action_type` `approve_catalog_setup_write`, one preview component (`src/components/agent/catalog-setup-write-preview.tsx`, wired through `action-detail.tsx`, `queue-row.ts` and `approval-queue-service.ts`). An operator learns one review screen, not five. The row can only be created by the sealed database proposal, is visible only to the operator it was staged for and only while they still hold the permissions the prepare required, is approved one at a time against the exact preview seal, and is never swept into a bulk approve, an autonomous execution or a cancel. Restrictive RLS on `public.agent_actions` refuses every insert, update and delete of this action type from application roles outright and gates select on `public.can_read_catalog_setup_write_action(action, company)`.

**RPCs.** `public.prepare_catalog_setup_write_as_system(actor, company, grant, client, grant_revision, scope_ceiling, permission_revision, permission_keys, manifest_revision, exposure_revision, capability_id, capability_revision, kind, request_id, request, observed_at)` stages; `public.commit_catalog_setup_write_as_actor(actor, company, action_id, change_set_id, preview_sha256, idempotency_key)` applies; `public.reject_catalog_setup_write_as_actor(actor, company, action_id, review_notes)` declines. All three are service-role only. A new rate-limit policy id `mcp-catalog-setup-write-prepare:2026-09-15.v1` (`public.consume_catalog_setup_write_prepare_rate_limit_as_system`) carries the same 6 actor / 6 grant / 30 company units per minute as its siblings; the existing `mcp-catalog-prepare:2026-09-08.v1` policy is hard-pinned to exposure V19 and was not widened. Each prepare writes one persistent `agent_suggestion` notification with `action_url` `/agent/queue`, label `REVIEW`, a per-kind body line and dedupe key `catalog-setup-write:<action_id>`; commit and reject both clear it. Proposals expire in 30 minutes.

**The effect seal (W10).** `private.agent_catalog_setup_write_effect_revision()` hashes every non-internal trigger on the twelve catalogue, action and notification tables the writers touch, plus `public.catalog_setup_save`, the two narrow writers, this vertical's own prepare / commit / reject / compile / projection functions and everything they call transitively. `private.agent_catalog_setup_write_assert_seal(on_commit)` compares that against the row stored in `private.agent_catalog_effect_policy` under revision `2026-09-15.catalog-setup-write.v1`. No row, or a row that no longer matches: a prepare raises `CATALOG_SETUP_WRITE_ACTIVATION_REQUIRED` and a commit raises `CATALOG_SETUP_WRITE_EFFECT_POLICY_CHANGED`. **None of the five migrations seeds that row, and the first one's postflight refuses to install if it already exists.** Activation is a separate, explicit act: an operator reviews the installed effects and inserts the reviewed hash under that revision. Installing each later kind deliberately moves the hash, so a seal reviewed against a one-kind spine cannot silently cover a five-kind one.

**The commit lane.** `public.catalog_setup_save` is the writer for the variant, thresholds, variant-price-override and option kinds. It is `SECURITY INVOKER` and guards `p_company_id = private.get_user_company_id()`, so the commit installs the approving operator's own JWT claims (`sub` from `users.auth_id`, else `firebase_uid`), verifies `get_current_user_id()` and `get_user_company_id()` resolve to that operator and company, runs the save, and restores the claims — the write runs as the operator rather than bypassing the guard as `service_role`. The save payload is family-scoped and replacing, so every wrapper reads the family's current state and re-sends its **complete** document with one field moved; the family pre-image is hashed into the proposal and re-derived at commit, and a family that moved underneath the approval raises `CATALOG_SETUP_SOURCE_STALE`. Re-sending is exact, not merely equal in value: `catalog_variants.price_override` is an unconstrained `numeric` that keeps its stored display scale, so the family state carries `price_override_exact` and the payload re-sends that text — without it every sibling's `200` came back as `200.0000`, a row that moved under a preview promising one variant would change. After the save, an independent read through the same projection the preview predicted must equal the approved `after`, or the commit raises `CATALOG_SETUP_READBACK_MISMATCH`. Replaying a commit key returns the stored receipt with `replayed: true`.

**Two narrow writers, because the save function cannot reach these fields.** `public.catalog_setup_save`'s family section writes `category_id`, name, description, image URL, the two default thresholds, `default_unit_id` and notes — not `default_price` — and it has no supplier-cost section at all; no other Postgres function writes either. So `private.catalog_family_default_price_save(company, actor, family, default_price, idempotency_key, source)` and `private.catalog_supplier_cost_profile_save(company, actor, variant, profile, idempotency_key)` are installed beside it: both `private`, `SECURITY DEFINER`, `search_path = ''`, revoked from `public`, `anon`, `authenticated` **and** `service_role`, reachable only from the commit through `private.agent_catalog_setup_write_apply`, and idempotent through the private ledger `private.agent_catalog_setup_writer_requests` keyed `(company, writer, idempotency_key)`. The supplier-cost writer upserts on `(company, variant, profile_key)`, revives soft-deleted rows (that key is unique *without* a `deleted_at` predicate, so an insert would fail), keeps exactly one default per variant by demoting the old row first so the partial unique index never trips, refuses a write that would leave a variant carrying profiles with no default, and stamps server provenance under the reserved `ops` key that a caller can neither forge nor overwrite.

**Costs are recorded only as supplier cost profiles.** `prepare_set_catalog_pricing` is sale price and nothing else. OPS carries two cost models — the simple `catalog_variants.unit_cost_override` / `catalog_items.default_unit_cost` fields and the richer profile table — and `get_catalog_item` reads only the profiles, so they drift the moment one is written without the other. Whenever the profile that ends up as a variant's default changes, or that default's cost changes, `private.catalog_supplier_cost_profile_save` mirrors that cost into the catalogue in the same transaction **at the level the family already uses** and reports it as `variant_unit_cost_mirrored`: it sets `unit_cost_override := private.agent_catalog_setup_override_for(default cost, catalog_items.default_unit_cost)`, so a family whose `default_unit_cost` is NULL or different gets the cost on the variant's own term, and a family whose `default_unit_cost` equals it leaves the variant inheriting (an override it carried is cleared). The family column is read and never written — one variant's supplier cost does not get to move the cost of every other variant. The compile predicts the mirror by the same rule the writer performs it, so the sealed proposal cannot be contradicted by the commit. Until PR #130 (2026-09-16) the writer set the override whatever level the family was costed at; see **The override level rule** below.

**Three shape decisions worth knowing.** Opening quantity on a new variant is a stock unit plus a `receive` stock event, never a silent count — but `variants[].quantity` mirrors it in the same payload, because nothing in OPS projects stock events onto that scalar and `catalog_setup_save` replaces an omitted `quantity` with 0. Thresholds are whole units in and out, reported with the fallback origin actually in force, matching what `get_catalog_item` already reports through `warning_origin` / `critical_origin`. Option creation requires `value_for_existing_variants` whenever the family has variants, and lists every variant it will backfill by its current and resulting identity rather than counting them; the payload re-sends each variant's complete value set under client ids, because `catalog_setup_save` dedupes a draft's variant matrix on `option_value_client_ids` alone and a one-element signature per variant is refused with `matrix_signature_conflict` (fifteen blockers on Canpro's Vinyl). A family carrying two active variants with identical value sets cannot be backfilled unambiguously and is refused up front with `CATALOG_SETUP_VARIANT_SET_AMBIGUOUS` — Canpro's `Diverter` is one such family.

**Local proofs.** Every SQL function was proved inside a transaction that ends in `rollback`, on a local PostgreSQL 17 copy of production structure carrying Canpro Deck and Rail's real catalogue, with each run re-reading the database afterwards to show it untouched. Each proof exercises: the unseeded seal refusing the prepare; an operator seeding the hash; a prepare returning the sealed proposal with its `agent_actions` row and notification; the staged payload; a commit through `catalog_setup_save` as the operator with the read-back equal to the approved preview; a replay returning the stored receipt; a stale pre-image refused; a reject leaving the catalogue unchanged; and the rollback. Transcripts: web `docs/artifacts/mcp-catalog-setup-writes/{create-variant,set-thresholds,set-pricing,set-supplier-cost,create-option}-proof.md` with their runnable `.sql` beside them.

**First end-to-end production proof (2026-09-16, before PR #130).** `prepare_set_catalog_pricing` on Lag Screws 2" / Black moved the variant's price from 0.50 to 0.55 CAD and then back to 0.50, each an operator-approved proposal committed inside OPS. Both read-backs equalled the approved previews, the sibling variants were untouched, and each proposal's queue notification resolved itself on commit.

**Deploy ordering.** `20260915223000_agent_catalog_recipe_read_v24.sql` must be applied **before** the code that sets `ACTIVE_MCP_EXPOSURE_REVISION` to V24 is deployed. Every branch of `public.resolve_mcp_oauth_access_token_as_system` keys on `p_active_exposure_revision`; a database that does not yet name V24 matches no branch, and every MCP bearer — V14, V23 and V24 alike — fails to resolve. This is the same hazard V23 had, handled the same way (a preintegration deployment rejects the new revision). The five write migrations must then be applied in ledger order, each one's prerequisites naming the previous.

**Cost precedence (decided with Jackson 2026-09-16).** The **default supplier cost profile** in `catalog_supplier_cost_profiles` is canonical. The catalogue mirror of it is `coalesce(catalog_variants.unit_cost_override, catalog_items.default_unit_cost)` — the same coalesce the sale price uses — and it must be written **at whichever level the family already uses**. A family costed at item level keeps its variants' `unit_cost_override` NULL by design; that is inheritance working, not drift. **The trap:** "mirror the canonical field onto the variant" looks obviously right and is wrong. Writing the profile cost down onto an item-level family's variants silently converts the family to variant-level costing — a later family-level change on Corner Sleeve from $8.50 to $9 would stop reaching its six variants, because every one would now carry an override that wins. The decision costs nothing today because nothing resolves between the two yet: a 2026-09-16 sweep of `public` and `private` found no estimate, takeoff, recipe or material function reading either field (estimate lines carry their own `unit_cost`), and the only two readers that exist — `private.agent_p2_catalog_detail_v1` and `private.agent_p2_purchase_order_cost_witness_v1` — already read the profile. The rule matters for what comes next, and it closes the precedence half of gap #17 that the mirror alone left open.

**The override level rule (decided with Jackson 2026-09-16; live in production since PR #130).** Cost precedence is one case of a general rule. Five catalogue fields resolve through a variant term and a family term — sale price `coalesce(price_override, default_price)`, unit cost `coalesce(unit_cost_override, default_unit_cost)`, warning and critical thresholds `coalesce(variant, family default, category default)`, and unit `coalesce(unit_id, default_unit_id)`. A variant whose own term is NULL inherits, so the next family edit reaches it; a variant whose term is set is pinned. Which one a variant is, is a decision the company made about its catalogue, never a side effect of typing a number. The audit that led here found four writers breaking it: `set_pricing` on a variant ref (deliberately), `set_thresholds`, `create_variant`, and the supplier-cost mirror all wrote the variant term even when the value equalled what the variant already inherited. The rule now:

- **A family ref** writes the family default only. It never creates variant overrides, and its preview lists every override that shadows the new value.
- **A variant ref with nothing inherited** writes the override.
- **A variant ref with a value inherited** writes the override only when the value differs. When it is equal, the variant ends up inheriting — an existing override is cleared — and a request that would change nothing at all is refused `CATALOG_SETUP_NO_CHANGE`.

Equality is numeric (15 = 15.0000); thresholds are judged per level; an explicit null still clears the variant's own term. The rule lives in one function, `private.agent_catalog_setup_override_for(requested, inherited)`, and every writer that mirrors a value calls it — the `set_pricing` variant ref, `set_thresholds`, `create_variant` and the supplier-cost writer `private.catalog_supplier_cost_profile_save`. `private.agent_catalog_setup_amount_level(variant, family)` returns a resolved amount with its origin, shared by projections and compiles so a preview and its read-back cannot disagree.

**The commit guard.** A rule applied in four compiles is a rule four future edits can forget, so `public.commit_catalog_setup_write_as_actor` snapshots the family's live variants immediately before the approved write and, after it and before the read-back, calls `private.agent_catalog_setup_override_level_changes(company, family, before)`. It refuses with `CATALOG_SETUP_OVERRIDE_LEVEL_CHANGED` (SQLSTATE `55000`; the whole commit rolls back) any write that changed one of `price_override`, `unit_cost_override`, `warning_threshold`, `critical_threshold` or `unit_id` to a non-NULL value equal to what the variant inherits afterwards — thresholds inheriting family then category. A new variant counts as changed from NULL. The error detail names variant ids and fields only, never amounts. Redundant overrides the write did not touch never trip it: production carries 31 variants whose `unit_id` equals the family `default_unit_id`, created outside MCP, and writes elsewhere in those families still commit. `catalog_setup_save` is unchanged — it re-sends every sibling's stored values, so siblings keep their level on every commit. The guard covers all five kinds, including whatever a later kind gets wrong.

**Previews show the level, not just the number.** Wherever a proposal shows one of these fields, both sides carry the value and its origin — `variant`, `family`, `category` or `none`: `create_variant` reports the new variant's sale price, unit cost and both levels that way; `set_supplier_cost` reports `variant_unit_cost` as `{amount, origin}`; `set_thresholds` already did. A family `set_pricing` lists `shadowing_variants`, each with a `redundant` flag when its override equals the new default; they are shown, never cleared, because clearing one would be a level change nobody asked for. The same release fixed one latent defect: clearing a variant's price in a family carrying a default price could not commit before. Local proof: web `docs/artifacts/mcp-catalog-setup-writes/override-level-proof.md` (rolled back on a local copy of production with migrations 1–8), with its runnable `.sql` beside it.

**The mirror drifted on day one — on four rows.** Canpro on 2026-09-16, measured against each variant's default profile before PR #128: **4** variants carried a set `unit_cost_override` that disagrees with it — genuine drift, because the cost rounding moved four Glass Panel profile costs to the cent but left the mirror alone (4.1992 vs 4.20, 4.2804 vs 4.28, 9.2684 vs 9.27, 9.7440 vs 9.74). **35** carry a NULL override under a family `default_unit_cost` equal to the profile — Corner Sleeve, Picket Rail, Line Sleeve, the stair brackets, gate parts, torpedoes and others — which is item-level costing working correctly: not drift, and not to be touched. **58** agree outright. Migration `20260916080000_align_variant_unit_cost_override_to_default_profile.sql` is scoped to the 4 set-override rows only. It refuses to run (`align_unit_cost_override_precedence_conflict`) if any NULL-override variant's family default disagrees with its profile — that would be a precedence question for a human, not for the migration; sets each drifted override to its default profile's cost; ledgers every changed row into `private.catalog_text_repairs_20260916` under `column_name = 'unit_cost_override'` with the exact stored text on both sides; and its postflight refuses if any set override still diverges (`align_unit_cost_override_incomplete`) or if any ledger row came from NULL (`align_unit_cost_override_touched_inherited_rows`). It writes **4** ledger rows. It was proved locally in a rolled-back transaction (4 rows changed, 0 from NULL, 35 still NULL). An earlier draft that also wrote the profile cost down onto the 35 inherited variants was reverted before it was pushed or applied. **Merged as PR #128 (`https://github.com/jack-opsapp/ops-web/pull/128`, merge commit `5050e127d`) and applied to production 2026-09-16 as ledger version `20260916203707`.** It re-aligned exactly the 4 Glass Panel overrides to their default supplier profile — 4.1992 → 4.2, 4.2804 → 4.28, 9.2684 → 9.27, 9.7440 → 9.74 — with 0 ledger rows coming from NULL; the 35 item-level-costed variants were left inheriting.

**What remains.**

- **Activation — done 2026-09-16.** The effect-policy seal row under `2026-09-15.catalog-setup-write.v1` was unseeded by design on merge; an operator inserted the reviewed hash after the production deploy, so all five tools are now live rather than inert. The override-level migration moved the hash on purpose, and the row was resealed after that release's production deploy the same day.
- **The text repair is applied to production** as ledger version `20260916182941`, and its text pass found no double-encoded text there — correctly, because production never carried any (below). It stays installed and idempotent.
- **Four Glass Panel variants** refused a cost-authorised `get_catalog_item` with `agent_money_minor_units_not_exact`, because four `catalog_supplier_cost_profiles` rows carried a CAD unit cost with more than two decimal places and the money projection refuses rather than rounding. Those four costs were rounded half-up to the cent at apply time and logged in `private.catalog_text_repairs_20260916`; no live supplier cost is finer than a cent now. Whether landed cost per square foot — a real sub-cent number — should instead be stored and read at four decimal places is still a product decision, not a code defect.
- **Resolved 2026-09-16: the four drifted variant cost mirrors are aligned in production.** `20260916080000_align_variant_unit_cost_override_to_default_profile.sql` shipped as PR #128 (merge `5050e127d`, ledger version `20260916203707`): 4 overrides re-aligned, 0 ledger rows from NULL, the 35 item-level-costed variants left inheriting. See **The mirror drifted on day one** above.
- **Resolved 2026-09-16: every writer keeps the family's level.** The supplier-cost writer once mirrored a new default cost onto `catalog_variants.unit_cost_override` whatever level the family was costed at, and three other writers had the same flaw on their own fields. PR #130 (merge `f2f81fbd1`, ledger version `20260916233332`) put all four behind `private.agent_catalog_setup_override_for` and added the commit guard `CATALOG_SETUP_OVERRIDE_LEVEL_CHANGED`; the seal was resealed after the deploy. See **The override level rule** above.
- **Open: `create_variant` does not check critical ≤ warning when one side is inherited.** `set_thresholds` checks the two levels in force; `create_variant` does not, when only one of them is supplied and the other comes from the family or category. The preview shows both values with their origins, so the operator can see an inverted pair before approving, but nothing refuses it.
- **Recipe writes are not built.** Gaps #10 and #11 in the MCP gap log — creating or updating `product_materials` lines and `product_material_quantity_rules` — have no tool. `get_catalog_item` can now audit a recipe (shape v2); nothing can author one through MCP.

### Double-encoded catalogue text repair (2026-09-16; applied to production, which never carried the corruption)

**Production finding (2026-09-16, ledger version `20260916182941`).** The repair's text pass found **zero** double-encoded rows in production because production never carried any. Jackson typed those em dashes himself on 2026-09-15 — one item name (`Picket Rail — Level`) and 59 cost-profile labels — and the 133 cost profiles stamped 19:00–20:00 UTC that day are that edit; no row changed between it and the apply, so no third party cleaned anything. The garbling existed only in the local proving copy: its seed export was taken through the Supabase MCP `execute_sql` JSON path, which emitted each UTF-8 byte of an em dash as a separate escaped Latin-1 character (`â` ×176 in the export file, plus 5 en dashes as `â`), and loading that JSON into the UTF-8 local database produced the 168 “mojibake” rows. The local cluster is itself UTF-8/C — the fault is the export transport, not the database. The migration was therefore correct to find nothing; its text pass was a no-op and it remains installed and idempotent. This repair left `private.catalog_text_repairs_20260916` holding **4 rows, all `column_name = unit_cost`** (PR #128 later added 4 `unit_cost_override` rows beside them): the four Canpro Glass Panel `vitrum-2026` supplier cost profiles, rounded half-up from 4.1992, 4.2804, 9.2684 and 9.7440 CAD to two decimals. No live supplier cost is finer than a cent now, and the runbook's “172 ledger rows” was a rehearsal expectation. **Lesson:** any local copy seeded from Supabase MCP JSON output must be checked for `â` escape sequences before it is trusted on text questions — reseed through a UTF-8-clean path (`pg_dump`, CSV) for text-sensitive proofs. Everything below describes that local copy; its 168-row scan and its 52 → 88 read tally are local measurements and never described production.

In that local copy the text was double-encoded in transit: the MCP JSON export emitted each UTF-8 byte of a character as if it were a separate Latin-1 character, so an em dash (U+2014, bytes `E2 80 94`) arrived as three characters — U+00E2, U+0080, U+0094. U+0080 is a C1 control character, so `private.agent_p2_optional_canonical_text` nulls the value, the catalogue detail read marks the cost row invalid, and `private.agent_p2_catalog_detail_v1` raises `agent_catalog_source_data_invalid` for the whole family. The same bytes made the supplier-cost tool's whole-proposal safety gate refuse every cost change on an affected variant — the tool was unusable on the catalogue it was built for, which is how this was found.

The scan is the general double-encoding signature (a lead character U+00C2–U+00F4 followed by continuation characters U+0080–U+00BF) across a literal list of sixteen `(table, column)` pairs, so what the migration can reach is reviewable by reading it. On the local copy of production exactly four sequences exist and no others: `—` (176 occurrences, in `catalog_supplier_cost_profiles.label` and `.source`), `–` (5, in `.source`), `¾` (1, Torpedoes' `Length` option value, which should read `1¾"`) and `×` (1, the Glass Panel family description, which should read `width × height`). The last two are not control characters, so nothing ever failed on them — they were simply wrong on screen. **168 rows** in four columns: 59 labels, 107 `source` blocks, 1 `catalog_items.description`, 1 `catalog_option_values.value`.

The signature is a heuristic and decides nothing on its own: a row is repaired only when it survives the exact inverse of the corruption (`convert_from(convert_to(value, 'LATIN1'), 'UTF8')`) **and** the result carries no control character. Anything else is left alone and counted in a `NOTICE`; all 168 local rows pass both tests. Every changed value's table, column, id, before and after go into `private.catalog_text_repairs_20260916` (RLS enabled and forced, revoked from every app role), so reversing the repair is an `UPDATE`. A repaired value no longer matches the signature, which is what makes the migration idempotent; its own postflight counts the rows still repairable and refuses to commit unless that count is zero. It writes data only — no trigger, no function the catalogue write seal hashes, and no statement naming the effect policy.

Measured effect on a cost-authorised `get_catalog_item` across Canpro's 103 live variants: **52 readable before, 88 after**. The 4 `agent_money_minor_units_not_exact` refusals are unchanged (the Glass Panel sub-cent costs above). The remaining 11 refusals — 8 on `Line`, 2 on `Picket Rail 19'6`, 1 on `Line Sleeve` — come from the recipe section of `agent_p2_catalog_detail_v1`, not the cost section, and are nothing to do with encoding. Both tallies were taken against the pre-V24 shape-v1 definition. A direct check of the rows (2026-09-16, local production copy) settles what those 11 are: the three families' recipe lines carry four-decimal quantities (`0.1667`, `0.0526`, `0.0526`) and no unit at all (`unit_id` null on every line), so the refusal is the four-decimal milliunit case the recipe-read proof (§7b) shows shape v1 refusing and shape v2 reading — not a raw unit without a label, which none of these lines has. The 11 variants are exactly those families' live variants (8 + 1 + 2). Under exposure V24, after the repair, 99 of Canpro's 103 variants read; the 4 Glass Panel sub-cent cost cases are the only refusals left. V23 and older pins keep shape v1 and keep refusing those three families until the connection re-registers. Proof: web `docs/artifacts/mcp-catalog-setup-writes/text-repair-proof.md`.
...
## Phone choice-write protocol (2026-09-11; local, unapplied)

**Approved production release, reverified 2026-09-14 UTC:** the phone v2 endpoints and SQL-only MCP compatibility repair are installed, with unchanged MCP public schemas/scopes/consent. All three v2 phone endpoints rejected unauthenticated calls with SQLSTATE `42501` in a read-only transaction; execute grants preserve the existing Firebase-bridge actor-check pattern, not anonymous business-write authority. Company/host MCP activation remains disabled. Earlier local/unapplied text records pre-release status. Signed-in device/host acceptance is separate. [Exact verification](docs/artifacts/2026-09-14-site-visit-choice-release.md).

**MCP coexistence follow-up, 2026-09-12:** local OPS-Web `babe8ec1d` adds an unapplied SQL-only repair after the phone extension. Existing answer forms may set an exact immutable choice label or explicitly clear it through the existing named-operator approval/commit path; unsupported labels fail before approval. Creating or editing an ordinary default checklist preserves choice metadata on the previous default. Frozen MCP inputs still cannot author or select choice-bearing templates; editing such a template is now explicitly rejected before approval. Public signatures, scope/exposure/consent pins and the shared human/agent capability layer are unchanged. No live host acceptance is claimed. [Exact proof](docs/artifacts/2026-09-12-site-visit-choice-coexistence.md).

`apply_site_visit_write_v2`, `review_site_visit_write_v2` and `resolve_site_visit_write_v2` retain their v1 parameter shapes. New commands use protocol `site-visit-writes:2026-09-11.v2`; answer snapshots travel as `rows[].values.choice_snapshot`, beside pure `answer_value`, never inside answer evidence. Exact actor/company identity, immutable command receipts, locked current revisions and explicit conflict review are preserved.

New command construction selects v2 when current, base, remote or referenced-template metadata requires choices, including conversion away from a choice. Already queued requests retain their bytes and identity. Explicit v2 review/resolution can supersede an unchanged original v1 request, including an offline plain answer whose template gained choices before delivery. First-attempt v2 apply still requires a v2 command; missing v2 endpoints do not fall back to destructive v1 writes.

The existing MCP read result accepts raw template/answer metadata and continues seeing ordinary selected text. Its pinned eight-kind preparation contracts are not broadened: unsupported choice-template selection or edits remain rejected. This task adds phone behavior only and does not change MCP exposure, consent or effect enrollment. Source: new migration `20260912004121_site_visit_single_choice_v2.sql`, iOS `SiteVisitWriteCommand.swift`, `SiteVisitWriteModels.swift`, `SiteVisitVersionedSync.swift` and `SiteVisitServerMerge.swift`.

### Phone base revision and self-conflict settlement (2026-09-15; ops-ios local, unreleased)

Server contract unchanged. This records how the phone chooses `rows[].base_revision` and what it does with a `stale_edit` receipt, after bug `0e110106` (2026-09-15, visit `96090555`): the phone saved a checklist answer as revision 2 while the operator kept typing, every later command shipped base 1, `apply_site_visit_rows` answered `stale_edit`, the queue parked it as "changed on another device", and RETRY re-sent the frozen command forever. A second route to the same state was a successor built from a stale in-memory write state (base 0 after the phone's own create had landed as revision 1).

Rules now enforced by `SiteVisitVersionedSync` (tests: `SiteVisitVersionedSyncTests`, `SiteVisitWriteCommandTests`):

- **A `saved` receipt row with the model's identity always advances the local write state** (`revision`, `baseRevision`, `baseRow`) even when the local row has drifted and no `dependsOn` successor is queued yet. Previously only a queued descendant advanced it.
- **An unattempted command is rebased before it leaves the phone** onto the highest revision this queue has already seen for each row: receipts of its own operations with outcome `saved`, `resolved` or `superseded`, plus the persisted write state read from a fresh context. Conflict receipts are never a source (their rows may be another writer's). A frozen command (`siteVisitWriteAttemptedAt` set) is never rewritten; the server keys idempotency on its bytes.
- **A `stale_edit` whose current rows are, byte for byte, rows this queue itself delivered is a self-conflict.** The phone persists a `pending` resolution with `current = receipt.rows` and calls `resolve_site_visit_write` in the same pass; the operation completes without parking. A current row the phone never saved still parks for review with the existing copy.
- The capture screen's first-keystroke base snapshot no longer overwrites a revision that a receipt advanced meanwhile (`SiteVisitWriteState.restoringCapturedBase`).

Deck attachment ordering (bug `6271078d`): `SiteVisitSyncOperation.Payload` for artifact writes now carries `deck_design_id`; the queue holds a `deck_design` artifact behind an unresolved `deckDesign` create, and behind a local deck that has never reached the server (`lastSyncedAt == nil`, `needsSync == true`, no completed create) while the deck editor still holds that create back. Sending earlier was a guaranteed `23503 site_visit_artifacts_deck_design_id_fkey`.

## Expense decision RPC authority (2026-09-11; local, unapplied)

The additive expense authority migration keeps `approve_expense_batch(p_batch_id uuid)`, `early_clear_expense_line(p_expense_id uuid)`, `mark_expense_batch_paid(p_batch_id uuid)` and `unmark_expense_batch_paid(p_batch_id uuid)` returning `void`. Existing public execution grants remain; actor and exact target-company authorization are enforced inside their owner-only private dispatcher. Complete-attempt contention recovery preserves transactional revision, placement and notification effects. SQLSTATE `40001` indicates bounded retry exhaustion. No client payload or provider delivery contract changes. See chapter 09 for the demonstrated baseline gaps, 66 repaired authorization cases, four contention graphs and unreleased migration reference.

## Expense accounting delivery and settings (2026-09-14; local, unreleased)

This pending implementation supersedes the historical direct-provider `accounting-sync-expense` description above only after deployment. Source: OPS-Web `supabase/functions/accounting-sync-expense/{handler,index}.ts`, `src/lib/api/services/expense-accounting-{provider-service,queue-processor}.ts`, and pending migration `20260912203328_expense_accounting_lifecycle.sql`.

The edge adapter accepts current `{expense_id}` and legacy `{expenseId,companyId}` requests, rejects conflicting aliases/extra keys and bodies over 4096 bytes, and forwards the original bearer JWT plus anon key to `request_expense_accounting_sync(p_expense_id uuid,p_company_id uuid default null)`. The RPC independently resolves the active Firebase actor and exact company under lock. The adapter performs no provider authentication, refresh or financial write. Its response is a durable queue receipt, never proof of provider acceptance. Preserve the existing edge gateway `verify_jwt:false` setting (version 5 metadata read back September 14); authentication is performed by the Firebase-aware REST RPC, not the legacy Supabase JWT gateway.

`GET/POST /api/integrations/accounting/expense-settings` requires both `accounting.manage_connections` and `expenses.approve` at all scope. GET returns named native provider accounts/employees/tax/project choices plus expense-relevant OPS projects, categories/crew and current mappings. POST validates choices against that exact connection's provider catalogue and calls service-only `save_expense_accounting_settings`. No account ID is guessed and no supplier is created for a crew member. POST echoes the GET `catalogueBinding`; the route and SQL compare the exact provider/environment/business identity before replacing settings. The service RPC appends optional tenth argument `p_project_mappings jsonb`: omission preserves mappings and `[]` clears them. Project assignments require exact OPS company ownership and native provider eligibility; unavailable saved assignments are removal-only choices.

Service-only `prepare_expense_accounting_write` requires the exact claimed worker, event, company, connection and provider identity/environment before freezing the payload. `finalize_expense_accounting_sync` records the provider identity and queue receipt atomically. `retry_expense_accounting_before_write` requires the same two operator permissions and permits corrected settings only before any frozen posting or provider evidence exists; it does not reset uncertain payments.

September15 release preparation supersedes sequential schema installation: the four expense prerequisites must install together in one transaction after retiring/draining the legacy direct-provider edge and placing expense-aware holding workers in service. The new queue-only adapter fails closed if its RPC is absent; it never falls back to a provider write. The exact staged release sequence is retained with the iOS release evidence. **Executed 2026-09-15:** hold build live 06:03:57Z, `accounting-sync-expense` replaced by the queue-only adapter as version 6 at 06:05:21Z (bundle SHA256 `2f86ac7575ea84520d2522b958e3ffbf44d75a20996618b29be5cd0a33f580ec`, `verify_jwt` false), atomic bundle applied 06:24:03Z, release web build `3fc2f6d76` live 06:28:37Z. The legacy direct-provider writer no longer exists in production and must not be redeployed. No provider canary or App Store release occurred.

Expense provider delivery additionally requires `EXPENSE_ACCOUNTING_WRITE_ENABLED` to equal exactly `true`; omission and every other value keep it paused. This gate precedes connection/token/session/payload preparation. Rows with provider evidence retain reconciliation priority; other expense rows enter a guarded blocked state. Failure to persist either state propagates without claiming a saved pause or scheduling a retry. The existing invoice/customer/supplier paths and their global/provider gates remain unchanged. GET classifies otherwise safe expense issues as paused; POST retry returns HTTP409 before calling the retry RPC while paused. Existing frozen/accepted evidence still forbids unsafe resend. The UI refreshes a stale retry into its current paused state without announcing that it queued successfully. This source is live in production since 2026-09-15 06:28Z with the gate absent; a later activation approval sets it for an exact company and provider.
...
The September15 local correction re-resolves actor/company after lock waits before mutation or replay, preventing a removed login binding from using the earlier resolved actor. The current deployed lifecycle worker is `lead-assignment-deliveries`, workload `lead-outbox`, scheduled at `2-59/10 * * * *`; it already calls `ProjectStatusLifecycleOutboxService.processBatch`. This read-only deployment inspection establishes worker compatibility, not a customer reopen or phone acceptance result.
...
## Try OPS demo and canonical signup handoff (2026-09-14; released 2026-09-15)

**Release status (2026-09-15 UTC):** schema ledger `20260915032402` is live and independently verified. OPS-Web source `e26310f4c836868d19e4539965abb58172dd9a85` is live at `app.opsapp.co` in READY deployment `dpl_ARcDA9dFLFEndpgyDo8vQa6dUhrt`; the full production typecheck and 484 generated pages passed. Its registration shell and exact missing-credential validation responses passed excluded live probes. TryOps source `29679e5a818e35b69fcfa4cef23040c32b024cac` is live at `try.opsapp.co` in READY deployment `dpl_AMSkhLy4Ya4y9tNoHKMqyAWbQdrX`. All 31 excluded live route checks passed; the live phone demo completes and its native trial action reaches canonical registration. The existing signed-in session then redirects to the dashboard. No production account or outbound-email canary has been performed. [Release evidence](docs/artifacts/2026-09-15-tryops-production-release.md).
...
| Route | Released behavior |
|---|---|
| `GET /demo` | Sample UI; an exact allowlist accepts an originating paid landing route in `from`, otherwise Exit returns to `/`. |
| `GET /demo/start-trial` | Unconditional native 303 to `https://app.opsapp.co/register`, with no-store and no-referrer. It ignores caller-selected destinations and cannot create a user, trial or email claim. |
| `POST /api/demo/session` | Same-origin, production-only optional session establishment; 200 ready, 204 excluded, 409 rejected identity, 503 unavailable. |
| `POST /api/demo/events` | Strict bounded diagnostics with token verification and replay/milestone dedupe; 201 recorded, 200 duplicate, 204 excluded, 400 invalid event, 409 identity/conflict, 429 session cap, 503 unavailable. |
| `/tutorial-interactive`, `/tutorial-intro`, `/tutorial/:step`, `/tutorial/complete` | Redirect to the supported demo with only allowlisted navigation context. Old variant cookies no longer select a broken tutorial. |
| `/signup` and existing `/signup/:step` pages | Redirect to canonical registration or login; a valid bounded invitation code goes to canonical `/join?code=...`. No legacy setup success screen remains reachable. |
| `/download` | Redirect to the existing OPS App Store listing. It makes no signup/trial-success claim. |

The duplicate acquisition-host auth endpoints (`apple`, `google`, `join-company`, `login`, `signup`, `sync-user`, `validate-code`), company update/invite, invite send, SMS send-link and user-detail endpoints return 410 with a fixed canonical destination. Their handlers do not parse credentials, write account/company data or send messages. The globally loaded legacy Google identity script is removed after its reachable consumers are retired. Canonical OPS-Web authentication remains in place.

OPS-Web adds optional `stageSignupDemo` after verified account synchronization and setup checkpoints, then `retrySignupDemo` after canonical company creation. The server binds the opaque cookie to a verified subject, keeps durable staging outside editable setup progress, and rechecks original timing plus company-owner eligibility before recording the association. Tracking failures never turn a valid signup into a product error; exhausted measurement staging is explicitly logged. Existing first-touch, experiment attribution, company/trial RPC and welcome-email rules remain authoritative.

`POST /api/setup/complete` additionally requires a verified active user, an existing nondeleted company and a successful guarded onboarding-completion update with row readback. A failed/empty save returns an error instead of success. Existing/free/joined company membership remains legitimate; completion does not add a paid-plan or trial requirement.

The canonical registration page also enlarges its primary submit target to the documented 52px phone tier and its password/provider/sign-in/invitation controls to at least 44px. Scoped input sizing removes native intrinsic-width pressure on narrow phones; the shared invitation prompt can wrap its input/action row and reuses existing localized accessible names for the input and close control. Authentication and routing behavior are unchanged. Sources: OPS-Web `src/app/(auth)/register/page.tsx` and `src/components/auth/join-team-prompt.tsx`, integrated commit `e26310f4c836868d19e4539965abb58172dd9a85`; 18 focused registration/button checks pass.

The existing authenticated `/api/ab-cron` invokes bounded demo recovery independently of experiment enablement. No new cron schedule, production invocation or flag change is part of this implementation. Missing optional schema is reported as unavailable while native signup remains usable. See the matching analytics and growth-contract sections for diagnostic versus business-truth definitions. Source: TryOps `app/demo/`, `app/api/demo/`, `lib/demo/`, compatibility routes and middleware; OPS-Web `src/lib/pmf/demo-attribution.ts` and auth/setup routes. The additive migration and both application deployments were explicitly approved and completed on September 15 UTC; this does not authorize future production changes.

## Recurring reimbursement commands (2026-09-17; live in production)

Six `public` functions, all `SECURITY DEFINER` with an empty search path and `lock_timeout 250ms`, executable by `authenticated` only (revoked from `public`, `anon` and `service_role`). Each requires `expenses.approve` at scope all and returns the setup as JSON: every table column plus `lines`, an array of `{expense_id, period, batch_id, status, amount, deleted}` ordered by period and including skipped months. Behaviour: `09_FINANCIAL_SYSTEM.md § Recurring reimbursements`.

| Function | Parameters |
|---|---|
| `create_expense_recurring_reimbursement` | `p_user_id uuid, p_name text, p_amount numeric, p_first_period date, p_category_id uuid DEFAULT NULL` |
| `update_expense_recurring_reimbursement` | `p_id uuid, p_name text, p_amount numeric, p_category_id uuid, p_expected_updated_at timestamptz` — all required; send an explicit JSON `null` category. |
| `end_expense_recurring_reimbursement` | `p_id uuid, p_last_period date, p_expected_updated_at timestamptz` — an explicit `null` last month removes the end. |
| `delete_expense_recurring_reimbursement` | `p_id uuid, p_expected_updated_at timestamptz` |
| `skip_expense_recurring_reimbursement_line` | `p_expense_id uuid` |
| `restore_expense_recurring_reimbursement_line` | `p_expense_id uuid` |

PostgREST resolves each function by its parameter names, so a required parameter must be present even when null. `p_expected_updated_at` is compared exactly, to the microsecond: pass the `updated_at` string back as received rather than round-tripping it through a date type.

Refusals (clients map these messages to their own copy): "You do not have permission to manage recurring reimbursements." (42501); "Your access changed. Reload and try again." (40001); "This recurring reimbursement is no longer available." (42501); "This recurring reimbursement changed. Reload and try again." (P0001); "Expenses are being updated. Try again." (40001); "Name it in 80 characters or fewer." / "Enter an amount from 0.01 to 10,000.00." / "Choose who receives this reimbursement." / "Choose the first month." / "Start within 12 months of this month." / "Your company currency is not set correctly." / "End on or after the first month, or delete it instead." / "<Mon YYYY> is already on a batch. Skip that month first, or end after it." (22023); "That category is unavailable." (23503); "That person is not an active member of your company." / "Only an admin can set up a recurring reimbursement for themselves." / "That line is not a recurring reimbursement." (42501); "This person already has a recurring reimbursement with that name." (23505); "That month has already been paid." / "A month has already been paid. End it instead." / "This recurring reimbursement was removed." / "That month is after this reimbursement ends." (55000).

Clients: OPS-Web `src/lib/api/services/expense-recurring-reimbursement-service.ts` with hooks in `use-expense-recurring-reimbursements.ts`; iOS `ExpenseRecurringReimbursementRepository` behind `RecurringReimbursementViewModel`. Both subscribe `expense_recurring_reimbursements` postgres_changes filtered by `company_id` (web `useExpenseRealtime`, iOS `RealtimeProcessor`, which posts the existing `.expenseUpdated` / `.opsExpensesDidChange` signals).
