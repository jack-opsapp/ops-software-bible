...
Application tests cover the captured Codex payload plus adversarial redirect shapes. A disposable PostgreSQL 17 full-wave run proved exact storage, consent/code binding, wrong-port rejection without consuming the code, exact retry, migration replay, and all earlier P2 migrations. Migration `20260829192448_mcp_oauth_codex_dcr_callbacks.sql` is now production-applied, and OPS-Web production HEAD `87abc02ac3205254c1446bb50f7eda3607000140` is served by READY deployment `dpl_6oUh3jTry9DWRAuTD1pnPSqKLRaA`. That final deployment includes the live post-release auth continuation repair, preserving the sanitized MCP authorization return path after sign-in. OAuth metadata and the MCP bearer challenge passed live checks, and Codex DCR discovery and registration work in production. The full host canary is awaiting the user's final browser consent; no completed token exchange, authenticated `tools/list`/tool call, or accepted Codex host is claimed yet. Codex desktop compatibility does not by itself prove ChatGPT connector compatibility.
...
- **Application:** OPS-Web production HEAD `87abc02ac3205254c1446bb50f7eda3607000140` is served by READY Vercel deployment `dpl_6oUh3jTry9DWRAuTD1pnPSqKLRaA` at `app.opsapp.co`. It includes the live post-release auth continuation repair that preserves the sanitized MCP authorization return path after sign-in.
...
- **Codex:** the strict callback policy is deployed, and DCR discovery and registration work in production. The full host canary is awaiting the user's final browser consent. No completed token exchange, authenticated tool-list/call, revocation/cleanup proof, or accepted Codex host is claimed yet. ChatGPT remains separately unproven.
...
The database/application deployment contract is complete; the external deployment contract did not change. No thirty-four-read exposure revision has been built, so a separately authorized release must still create, review, and deploy a new immutable exposure before any P2 tool becomes visible. Claude's current production grant proves only the eleven-read exposure-v1 connector. Codex DCR discovery and registration work in production, but its full host canary is awaiting the user's final browser consent and remains unaccepted; ChatGPT remains unproven until it completes its own connector/OAuth/tool-call run. Full contract: `specs/2026-08-29-mcp-read-catalogue-p2.md`.
