...
## Weekly journal authoring (2026-09-10; built and tested locally, NOT deployed)
...
| Route | Contract |
|---|---|
| `POST /api/internal/journal/editorial/claim` `{worker}` | `{assignment:null, reason:"idle"\|"authoring_off"}` or the claimed slot: `id, identity, kind, mode, slot_date, publishes_at, attempt, attempts_remaining, claim_token, lease_until, current_time, brief_version ("ops-journal-2026-09-10-v1"), format, limits, max_sources, categories, industry_pages, recent_posts (≤40 live), backlog_topics (unused), fetched_sources, brief, guide, product_facts` (each `{path, sha256, content}`: `docs/journal/voice/ops-journal-brief.md`, `blog-voice-sam-parr.md`, `ops-product-facts.md`). |
...
| `GET /api/cron/journal-editorial` | `9 * * * *`, bearer `CRON_SECRET`, durable lease `journal-editorial` (360 s), `maxDuration 300`. Recover → discover → promote ≤2 drafted slots (render the 1200 × 630 hero plate, store `blog/journal/<slot>-<sha16>.jpg` on the product's blog image backend, require a public HEAD 200, schedule with `publish_at`) → publish due slots through the guarded RPC → newsletter lane (gated by `app_settings.blog_newsletter_enabled`) → notification outbox → stall check/resolve. Returns `{state, mode, created, missed, promoted, published[], held[], newsletter, notified}`. |
...
| `POST /api/admin/journal/editorial/{id}` `{action}` | Platform admin; `stop`, `publish_now` (manual publish RPC; refusals return 409 with the code), `write_another` (409 `SLOT_PASSED`), `send_test` (newsletter to the acting admin only, logged `blog_newsletter_test`). Actor recorded as `admin:<email>`. |
...
Deployment order remains separately approval-gated: prerequisite expense authority migration, accounting lifecycle migration, payroll reimbursement projection migration, server/worker and edge adapter, then wait-free clients. No production expense migration, provider canary, deployment or App Store release occurred in this work.
