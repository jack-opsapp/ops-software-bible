...
The dormant foundation is production-released, but the capability is **not customer-live**. Ledger versions `20260831042518_agent_day_closeout_foundation_zero`, `20260831042631_agent_day_closeout_routine_worker`, and `20260831042924_agent_day_closeout_fk_indexes` are applied and mirrored byte-exact. OPS-Web commit `1742861a` is served by Ready production deployment `dpl_GEwkEiT9AwjQSrZXyoYQtWAXoJ46` at `app.opsapp.co`. Live readback proves all six tables have RLS enabled, no policies, no direct app-role grants, and zero rows; public RPCs are service-role-only. The follow-up index migration clears every scoped missing-foreign-key-index advisor result. V3/v9/v2-consent remain inactive and no connector has received the new authority. The cron route requires both `CRON_SECRET` and `OPS_DAY_CLOSEOUT_ROUTINES_ENABLED=true`, is not registered in `vercel.json`, and routine rows default disabled. No configuration RPC or product interface can enable one. The worker makes no model/provider call; with no schedule or rows, this release adds no scheduled invocation or model/provider cost. Scheduler invocation and database-read cost must be measured before registration. Representative production correspondence is also still incomplete: normalization v2 has recovered some HTML mail, but rejected/unreadable delivery-source rows remain, so the closeout correctly reports correspondence as not evaluated rather than claiming a complete inbox sweep.

Activation still requires Jackson's explicit permission plus separate authenticated host acceptance for consent, discovery, the v3 prepare tool, refresh/revocation, exact OPS approval, receipt replay, tenant isolation, and routine handoff. Existing Codex acceptance proves a bounded read-only v1 path only; production v2 discovery is live but authenticated v2 host acceptance remains pending, and neither fact proves v3 write-path acceptance. The live unauthenticated MCP challenge still advertises exactly the twenty read-only v2 scopes, while an unauthenticated cron probe returns 401. The release passed the focused closeout/authority/MCP suite, a full 419-route Node 22 production build, and post-release error/fatal log checks. Full design and remaining gates: `specs/2026-08-30-ops-mcp-day-closeout-foundation-zero.md`.
