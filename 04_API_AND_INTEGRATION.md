...
Measured effect on a cost-authorised `get_catalog_item` across Canpro's 103 live variants: **52 readable before, 88 after**. The 4 `agent_money_minor_units_not_exact` refusals are unchanged (the Glass Panel sub-cent costs above). The remaining 11 refusals — 8 on `Line`, 2 on `Picket Rail 19'6`, 1 on `Line Sleeve` — come from the recipe section of `agent_p2_catalog_detail_v1`, not the cost section, and are nothing to do with encoding. Both tallies were taken against the pre-V24 shape-v1 definition, so how many of those 11 clear under exposure V24 is not measured: the text-repair proof attributes them to a recipe row carrying a raw unit with no resolved label or abbreviation, which shape v2 refuses exactly as v1 does, while the recipe-read proof (§7b) shows `Line Sleeve` refusing shape v1 on a four-decimal quantity that shape v2 reads. Re-running the tally with `p_recipe_shape = 'v2'` would settle it. Proof: web `docs/artifacts/mcp-catalog-setup-writes/text-repair-proof.md`.
