...
**Creation (2026-08-17):** all iOS rail-row creation goes through the repository's "Narrow creation RPCs" extension — one typed method per RPC of § "iOS notification-surface RPCs" (plus `syncReviewStack` for the review stacks). The legacy `createNotification(_:)` direct INSERT fails `42501` for app roles under the 2026-07-15 hardening and remains only as the shape used by not-yet-rewired legacy call sites.
