...
`2026-09-15.mcp-exposure.v24` is the full V23 successor: the same ordered 35 tools and 21 grantable scopes, the same customer-update capability manifest v20 and V9 consent. Only one thing changes — `get_catalog_item` reads the richer catalogue recipe projection. V24 is the active revision, so new registrations select it; V14 and V23 client/grant snapshots stay immutable and keep resolving. V24 also selects deck geometry result v2, exactly as V23 does.
