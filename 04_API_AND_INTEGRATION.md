...
Application tests cover the captured Codex payload plus adversarial redirect shapes. A disposable PostgreSQL 17 full-wave run proved exact storage, consent/code binding, wrong-port rejection without consuming the code, exact retry, migration replay, and all earlier P2 migrations. Migration `20260829192448_mcp_oauth_codex_dcr_callbacks.sql` is now production-applied, and OPS-Web commit `5e6ac2fb4db2b3f07b76b673d05ca7a92ae02584` is served by READY deployment `dpl_CWAAobozwoKHAYJfqATC1FN7bABL`. OAuth metadata and the MCP bearer challenge passed live checks. The real Codex canary is awaiting user browser consent; no completed token exchange, authenticated `tools/list`/tool call, or accepted Codex host is claimed yet. Codex desktop compatibility does not by itself prove ChatGPT connector compatibility.
...
- **Application:** OPS-Web commit `5e6ac2fb4db2b3f07b76b673d05ca7a92ae02584` is served by READY Vercel deployment `dpl_CWAAobozwoKHAYJfqATC1FN7bABL` at `app.opsapp.co`.
...
- **Codex:** the strict callback policy is deployed, but the real host canary is awaiting user browser consent. No completed token exchange, authenticated tool-list/call, revocation/cleanup proof, or accepted Codex host is claimed yet. ChatGPT remains separately unproven.
...
The database/application deployment contract is complete; the external deployment contract did not change. No thirty-four-read exposure revision has been built, so a separately authorized release must still create, review, and deploy a new immutable exposure before any P2 tool becomes visible. Claude's current production grant proves only the eleven-read exposure-v1 connector. Codex OAuth compatibility is production-deployed, but its host canary is awaiting user browser consent and remains unaccepted; ChatGPT remains unproven until it completes its own connector/OAuth/tool-call run. Full contract: `specs/2026-08-29-mcp-read-catalogue-p2.md`.
