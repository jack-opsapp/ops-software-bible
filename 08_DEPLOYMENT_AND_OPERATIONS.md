...
**Project-health failure contract (2026-08-06).** `/api/cron/project-health` reads overdue work directly from active, non-deleted `project_tasks.end_date`; it must not reference the removed `project_tasks.calendar_event_id` or join `calendar_events`. After each bounded company fan-out, any per-company error must reject the guarded work so `runWithCronWorkloadControl` completes the lease with `p_succeeded = false`; the route returns HTTP 500 with bounded error results instead of reporting `ok: true`. Sources: OPS-Web repair commit `6a2c1d8b6b4393e4b8649427b4f9345d98e6f480`, `ops-web/src/app/api/cron/project-health/route.ts`, `ops-web/src/lib/api/services/project-lifecycle-service.ts`, and `ops-web/src/lib/api/services/cron-workload-control-service.ts`.

**Schema-cache contract drift (2026-08-22).** PostgREST `PGRST202` means the requested RPC signature is absent from, or stale in, the API schema cache; it is not database resource pressure. A guarded workload must still fail and retain normal retry semantics, but this error must not open the shared database-pressure circuit. Production had skipped historical notification-identity migration `20260813172000`; forward ledger `20260822041423_email_anomaly_notification_identity_forward_repair_20260813172000` restored both service-only anomaly-notification RPCs, their exact unique indexes, and issued `NOTIFY pgrst, 'reload schema'`. The first scheduled anomaly-check after repair returned HTTP 200 at `2026-08-22T04:18:25Z`; its workload row completed at `04:18:42.952369Z` with no lease, zero consecutive failures, and a closed circuit. True connection and gateway failures (`PGRST000`–`PGRST003`) remain database-pressure signals.

**Empty approved-action reconciliation claim (2026-08-22).** `claim_next_approved_action_email_reconciliation` legitimately returns an all-null table composite when no eligible row exists. Before OPS-Web `c7f0bae4`, that shape was mapped as an intent and caused a false `APPROVED_ACTION_EMAIL_RECONCILIATION_LEASE_MISSING` failure. The code-only repair recognizes only the complete all-null shape as no work and still rejects partial malformed payloads. No migration or data repair was required. Commit `c7f0bae4` is live in READY deployment `dpl_8xzscNt3GEtCnGu7obNezdAdyCa4`. The first post-deploy scheduled run returned HTTP 200 at `2026-08-22T05:08:14Z`; the `send-reconciliation` workload completed at `05:08:16.502846Z` with no lease, zero consecutive failures, and a closed circuit. Production had zero approved-action intents and zero reconciliation leases, and the prior lease-missing error was absent from the run window.

### Provider and derived continuation contract

`/api/cron/email-sync` persists an opaque `ops-email-sync:v1` continuation before derived notifications or model-backed follow-up work. The nested provider cursor is `gmail:v1` for bounded Gmail history/message work or `m365:v2` for the Microsoft folder-delta plus pending-folder walk. One Gmail invocation is limited to 10 provider pages and 25 messages; M365 shares a 50-page folder/message budget. The targeted lead-summary drain takes at most five opportunity IDs per cycle, with at most two model attempts per opportunity. A model contract/refusal then uses only the deterministic trusted-fact fallback; if no authoritative current fact exists, that ID receives a terminal deferred disposition instead of pinning the same continuation forever. Provider unavailability requeues the derived ID while allowing provider progress; genuine persistence failures hold cursor publication for idempotent replay.

Production migration record `20260805190312_email_provider_snapshot_health`, from repository source `20260805151056_email_provider_snapshot_health.sql`, adds nullable `provider_snapshot_at` so provider catch-up can be healthy while an `ops-email-sync:v1` derived-summary queue still keeps end-to-end `last_synced_at` nonterminal. Gmail structured cursors are provider-pending. M365 is provider-terminal only with a Graph folder `$deltatoken` and an empty pending-folder queue; folder `$skiptoken`, legacy v1/raw URLs, and pending message-folder work fail closed as pending. Production readback confirmed the column and the five-argument checkpoint signature; the fourth positional argument remains `p_clear_recovery` and the new fifth argument defaults false.

**Provider-health rollout record (2026-08-05/06):** the database-first migration was applied and read back as production record `20260805190312_email_provider_snapshot_health` before byte-identical repair commit `146f6d69c4d52d985bc516c0453db6590f92cf97` was promoted as READY deployment `dpl_6gxJ4P8dv7ohpXpi1wbbdPuyR7FY`. An authenticated live sync on that deployment drained the derived continuation from seven pending opportunity IDs to zero and advanced both health timestamps. The durable release is OPS-Web main commit `17e70559e5db5d51861e4d8bc8d57b2cd490d10b`, served by READY deployment `dpl_AiDK9ZkSfv6BvbP9pSkZiRwnVgSh` at `2026-08-06T06:29:26.442Z`. `app.opsapp.co` resolves to that exact SHA with `aliasError = null`; root and login returned HTTP 200, the unauthenticated heartbeat returned the expected HTTP 401, and deployment-scoped logs contained no error/fatal entry or HTTP 5xx. The migration remains live. Current database readback has zero wrapped continuations, sync locks, or recovery state; `last_synced_at` is `2026-08-06T03:57:31.91Z` and `provider_snapshot_at` is `2026-08-06T03:57:32.135712Z`. The exact final deployment was not made to perform another authenticated sync or provider/email mutation, so its proof is current alias, route, log, and database state; behavioral mutation proof remains the earlier byte-identical deployment. Do not reapply the migration.
