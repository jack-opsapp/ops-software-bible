...
Prepared migration `20260805151056_email_provider_snapshot_health.sql` adds nullable `provider_snapshot_at` so provider catch-up can be healthy while an `ops-email-sync:v1` derived-summary queue still keeps end-to-end `last_synced_at` nonterminal. Gmail structured cursors are provider-pending. M365 is provider-terminal only with a Graph folder `$deltatoken` and an empty pending-folder queue; folder `$skiptoken`, legacy v1/raw URLs, and pending message-folder work fail closed as pending.

**Provider-health rollout order (source-only as of 2026-08-05):** apply and read back the migration first, including the five-argument checkpoint signature whose fourth positional argument remains `p_clear_recovery`; then deploy OPS-Web commit `7a74e1cd`; then verify a real owner-fenced provider checkpoint, terminal completion, and heartbeat classification. Database-first is backward compatible because the new fifth argument defaults false. Code-first is not compatible because the route selects the new column and sends the new named RPC argument. The nightly repair did not apply the migration or deploy the web code.
