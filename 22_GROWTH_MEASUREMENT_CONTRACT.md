...
**Deployment state:** Web/site source is pushed and deployed; production Supabase is migrated and read back; all three GA properties and `sc-domain:opsapp.co` are directly readable by the production identity; GA4 and Search Console retained-history backfills are complete with zero duplicate grains. Google-managed default app key events are verified and require no operator cleanup. The iOS App Store release, Apple commerce availability, and seven-day certification remain open.
...
## 9. Optional Try OPS sample demo (2026-09-14; released 2026-09-15)
...
**Evidence boundary:** the final refined two-action UI passes 234 local checks and a TryOps production build. Both applications and the service-only schema are deployed; 31 TryOps and four Web live HTTP checks pass, with the live demo and native signup handoff inspected in the browser. Eleven connected local HTTP/PostgREST/PostgreSQL groups independently verify canonical trial association, recovery and welcome-claim uniqueness; the harness does not perform production signup or send email. Browser emulation verifies all three demo states at eight screen sizes. No conversion lift, ordinary-user time-to-value, physical-phone performance, production signup or welcome delivery is established. Primary source: `try-ops/components/demo/`, `lib/demo/`, `app/demo/`, and `components/landing/ProductProof.tsx`. [Exact production release evidence and limits](docs/artifacts/2026-09-15-tryops-production-release.md).
